		...
		struct dpu_iomem *base_mem)
{
	...
	if (blk_end > base_end) {
		dpu_pr_err("wrong postpq pre proc hw info 0x%x, %d, 0x%x, %d\n",
		...
		struct dpu_iomem *base_mem)
{
	...
	hw_post_pre_proc = dpu_mem_alloc(sizeof(*hw_post_pre_proc));
	dpu_check_and_return(!hw_post_pre_proc, NULL, "hw_post_pre_proc is null\n");

	ret = dpu_postpre_proc_cap_init(hw_post_pre_proc, cap, base_mem);
	if (ret) {
		dpu_pr_err("failed to init post pre proc hw blk %d\n",
				cap->base.id);
		dpu_mem_free(hw_post_pre_proc);
		return NULL;
	}
...
{
	if (hw)
		dpu_mem_free(to_dpu_hw_postpre_proc(hw));
}
