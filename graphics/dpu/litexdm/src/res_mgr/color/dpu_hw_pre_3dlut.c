		...
		struct dpu_iomem *base_mem)
{
	...
	if (blk_end > base_end) {
		DPU_COLOR_ERROR("wrong 3dlut hw info 0x%x, %d, 0x%x, %d\n",
		...
		struct dpu_iomem *base_mem)
{
	...
	dpu_check_and_return(!hw_3dlut, NULL, "hw_3dlut is null\n");

	ret = dpu_hw_pre_3dlut_cap_init(hw_3dlut, cap, base_mem);
	if (ret) {
		DPU_COLOR_ERROR("failed to init pre 3dlut hw blk %d\n",
				cap->base.id);
...
{
	if (hw)
		dpu_mem_free(to_dpu_hw_pre_3dlut(hw));
}
