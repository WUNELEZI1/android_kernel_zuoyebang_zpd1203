...
#define min_t(a, b, c) (sizeof(a) ? min(b, c) : 0)
		...
		struct dpu_dsc_config *dsc_config)
{
	...
	dpu_pr_debug("pps parameters:\n");
	dpu_pr_debug("dsc_version_minor: %d\n", dsc_config->dsc_version_minor);
	dpu_pr_debug("bits_per_component: %d\n", dsc_config->bits_per_component);
	dpu_pr_debug("pps_identifier: %d\n", dsc_config->pps_identifier);
	dpu_pr_debug("line_buf_depth: %d\n", dsc_config->line_buf_depth);
	dpu_pr_debug("block_pred_enable: %d\n", dsc_config->block_pred_enable);
	dpu_pr_debug("bits_per_pixel: %d\n", dsc_config->bits_per_pixel);
	dpu_pr_debug("pic_height: %d, pic_width:%d\n",
			dsc_config->pic_height, dsc_config->pic_width);
	dpu_pr_debug("slice height:%d, slice width:%d\n",
			dsc_config->slice_height, dsc_config->slice_width);
	dpu_pr_debug("slice_chunk_size: %d\n", dsc_config->slice_chunk_size);
	dpu_pr_debug("initial_xmit_delay: %d\n", dsc_config->initial_xmit_delay);
	dpu_pr_debug("initial_decoding_delay: %d\n", dsc_config->initial_dec_delay);
	dpu_pr_debug("initial_scale_value:%d\n", dsc_config->initial_scale_value);
	dpu_pr_debug("scale_increment_interval: %d\n", dsc_config->scale_increment_interval);
	dpu_pr_debug("scale_decrement_interval: %d\n", dsc_config->scale_decrement_interval);
	dpu_pr_debug("first_line_bpg_offset: %d\n", dsc_config->first_line_bpg_offset);
	dpu_pr_debug("nfl_bpg_offset: %d\n", dsc_config->nfl_bpg_offset);
	dpu_pr_debug("slice_bpg_offset:%d\n", dsc_config->slice_bpg_offset);
	dpu_pr_debug("initial_offset:%d\n", dsc_config->initial_offset);
	dpu_pr_debug("final_offset:%d\n", dsc_config->final_offset);
	dpu_pr_debug("flatness_min_qp %d\n", dsc_config->flatness_min_qp);
	dpu_pr_debug("flatness_max_qp %d\n", dsc_config->flatness_max_qp);
	dpu_pr_debug("convert_rgb:%d, simple_422:%d, vbr_enable:%d\n",
			dsc_config->convert_rgb, dsc_config->simple_422, dsc_config->vbr_enable);
	dpu_pr_debug("nsl_bpg_offset:%d\n", dsc_config->nsl_bpg_offset);
	dpu_pr_debug("rc_model_size:%d\n", dsc_config->rc_model_size);
	dpu_pr_debug("rc_quant_incr_limit0:%d\n", dsc_config->rc_quant_incr_limit0);
	dpu_pr_debug("rc_quant_incr_limit1:%d\n", dsc_config->rc_quant_incr_limit1);
	dpu_pr_debug("rc_edge_factor:%d, second_line_bpg_offset:%d\n",
			dsc_config->rc_edge_factor, dsc_config->second_line_bpg_offset);
	dpu_pr_debug("native_420:%d, native_422:%d\n",
		...
		struct dsc_caps_sink *sink_cap, struct dsc_caps_src *src_cap)
{
	...
	if (sink_cap == NULL) {
		...
		if (in_params->dsc_version_major != DSC_VERSION_MAJOR_V1) {
			dpu_pr_err("unsupported dsc major version %d\n",
		...
		if (in_params->dsc_version_minor != DSC_VERSION_MINOR_V1 &&
			...
			dpu_pr_err("unsupported dsc minor version %d\n",
		...
		if (in_params->pic_height > src_cap->max_pic_height) {
			dpu_pr_debug("error: unsupported pic height %d, caps:%d\n",
					in_params->pic_height, src_cap->max_pic_height);
		}

		/* picture width */
		if (in_params->pic_width > src_cap->max_pic_width) {
			dpu_pr_err("unsupported pic width %d, caps:%d\n",
		...
		if (in_params->slice_height > src_cap->max_slice_height) {
			dpu_pr_warn("unsupported slice_height %d, reset to:%d\n",
		...
		if (in_params->slice_width > src_cap->max_slice_width) {
			dpu_pr_err("unsupported slice width %d, caps:%d\n",
		...
		if (i == src_cap->bpcs_size) {
			dpu_pr_err("unsupported bits_per_component %d\n",
		...
		if (in_params->bits_per_pixel != 0x8) {
			dpu_pr_warn("unsupported compressed bpp: %d, reset to %d",
		...
		if (line_buffer_depth > src_cap->max_line_buffer_depth) {
			dpu_pr_warn("unsupported line buffer depth (bpc + 1): %d, reset to:%d\n",
		...
		if (slice_cnt > src_cap->max_slice_cnt) {
			dpu_pr_warn("unsupported slice count: %d, reset to %d\n",
					slice_cnt, src_cap->max_slice_cnt);
			slice_cnt = src_cap->max_slice_cnt;
			slice_width = DIV_ROUND_UP(in_params->pic_width, slice_cnt);
			dpu_pr_warn("reset slice width to:%d", slice_width);
			if (slice_width > src_cap->max_slice_width) {
				dpu_pr_err("unsupported slice width, caps:%d\n",
		...
		if (!sink_cap->dsc_supported) {
			dpu_pr_err("sink doesn`t support dsc\n");
			return -1;
		}
		...
		/* picture height */
		if (in_params->pic_height > src_cap->max_pic_height) {
			dpu_pr_err("unsupported pic height %d, caps:%d\n",
					in_params->pic_height, src_cap->max_pic_height);
			return -1;
		}
		...
		if (in_params->slice_width > sink_cap->max_slice_width ||
				in_params->slice_width > src_cap->max_slice_width) {
			dpu_pr_err("unsupported slice width %d, src caps:%d, sink caps:%d\n",
		...
		/* slice height */
		in_params->slice_height = min_t(u32, in_params->pic_height,
		...
		if (line_buffer_depth > sink_cap->max_line_buffer_depth ||
			...
			dpu_pr_warn("unsupported line buffer depth %d, reset to:%d\n",
		...
		struct dpu_dsc_config *dsc_config)
{
	...
	dpu_pr_debug("dsc_config->slice_count: %d\n", dsc_config->slice_count);
	dpu_pr_debug("dsc_config->rc_model_size: %d\n",
			dsc_config->rc_model_size);
	dpu_pr_debug("dsc_config->initial_offset: %d\n",
			dsc_config->initial_offset);
	dpu_pr_debug("dsc_config->initial_xmit_delay: %d\n",
			dsc_config->initial_xmit_delay);
	dpu_pr_debug("dsc_config->bits_per_pixel: %d\n",
			dsc_config->bits_per_pixel);
	dpu_pr_debug("dsc_config->first_line_bpg_offset: %d\n",
			dsc_config->first_line_bpg_offset);
		...
		struct dpu_dsc_config *dsc_config)
{
	...
	if (dpu_dsc_params_correct(&parms_priv, sink_cap, src_cap)) {
		dpu_pr_err("invalid value for dsc caps\n");
		goto error;
	}
	...
	do {
		...
		if (dsc_config->slice_height == 1) {
			dpu_pr_err("unsupported rate buffer size: %d Byte\n",
					rate_buffer_size);
			goto error;
		}

		dsc_config->slice_height /= 2;
		dpu_pr_warn("unsupported rate buffer size: %d Byte, reset slice height to %d\n",
