...
void dpu_hw_post_hist_enable_update(struct dpu_hw_blk *hw)
{
	if (!hw) {
...
void dpu_hw_post_hist_enable(struct dpu_hw_blk *hw, bool enable)
{
	if (!hw) {
		DPU_ERROR("invalid parameter\n");
		return;
...
void dpu_hw_post_hist_set(struct dpu_hw_blk *hw,
		struct postpq_hw_cfg *hw_cfg)
{
	...
	/* use left should always be true in post hist */
	DPU_REG_WRITE(hw, HIST_USE_LEFT_OFFSET, 1, CMDLIST_WRITE);
}

void dpu_hw_post_hist_flush(struct dpu_hw_blk *hw)
{
	if (!hw) {
		DPU_ERROR("invalid parameter\n");
		return;
...
void dpu_hw_post_hist_get(struct dpu_hw_blk *hw, int base_idx, int end_idx, void *data)
{
