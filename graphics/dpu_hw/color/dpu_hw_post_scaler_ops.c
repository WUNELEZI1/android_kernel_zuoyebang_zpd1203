...
void dpu_hw_post_scaler_dump_enable(struct dpu_hw_blk *hw, bool enable)
{
	if (!hw) {
		DPU_ERROR("Invalid parameter!\n");
		return;
	...
	DPU_REG_WRITE(hw, DEBUG_CNT_EN_OFFSET, enable, CMDLIST_WRITE);
}

static void dpu_hw_post_scaler_stirpe_size_set(struct dpu_hw_blk *hw,
...
static void dpu_hw_post_scaler_gauss_kernel_set(struct dpu_hw_blk *hw,
...
static void dpu_hw_post_scaler_detail_enhance_set(struct dpu_hw_blk *hw,
		struct dpu_2d_scaler_cfg *cfg)
{
	...
	dpu_hw_post_scaler_gauss_kernel_set(hw, &(cfg->gauss_kernel));
}

static void dpu_hw_post_scaler_outlier_remove_set(struct dpu_hw_blk *hw,
		struct dpu_2d_scaler_cfg *cfg)
{
	u32 val = 0;
...
static void dpu_hw_post_scaler_bicubic_kernel_set(struct dpu_hw_blk *hw,
...
static void dpu_hw_post_scaler_lanczos3_kernel_set(struct dpu_hw_blk *hw,
...
static void dpu_hw_post_scaler_stripe_set(struct dpu_hw_blk *hw,
		struct dpu_2d_scaler_cfg *scaler)
{
	DPU_REG_WRITE(hw, STRIPE_INIT_PHASE_OFFSET,
			scaler->stripe_init_phase, CMDLIST_WRITE);
	DPU_REG_WRITE(hw, CROP_WIDTH_OUT_PHASE_OFFSET,
			scaler->crop_width_out_phase, CMDLIST_WRITE);
	DPU_REG_WRITE(hw, CROP_HEIGHT_OUT_PHASE_OFFSET,
			scaler->crop_height_out_phase, CMDLIST_WRITE);
}
		...
		struct postpq_hw_cfg *hw_cfg)
{
	...
	dpu_hw_post_scaler_stirpe_size_set(hw, scaler);
	DPU_REG_WRITE(hw, ALPHA_DISABLE_OFFSET, scaler->alpha_disable,
			CMDLIST_WRITE);
	DPU_REG_WRITE(hw, GRADIENT_THRESHOLD_OFFSET, scaler->gradient_threshold,
			CMDLIST_WRITE);

	DPU_REG_WRITE(hw, INTERP_METHOD_OFFSET, scaler->interp_method,
			CMDLIST_WRITE);
	if (scaler->interp_method == SCALER_2D_LANCZOS3)
		dpu_hw_post_scaler_lanczos3_kernel_set(hw, &(scaler->lanczos3_kernel));
	else
		dpu_hw_post_scaler_bicubic_kernel_set(hw, &(scaler->bicubic_kernel));

	DPU_REG_WRITE(hw, DETAIL_ENHANCE_ENABLE_OFFSET,
			scaler->detail_enhance_enable, CMDLIST_WRITE);
	if (scaler->detail_enhance_enable) {
		dpu_hw_post_scaler_detail_enhance_set(hw, scaler);

		DPU_REG_WRITE(hw, OUTLIER_REMOVE_ENABLE_OFFSET,
				scaler->outlier_remove_enable, CMDLIST_WRITE);
		if (scaler->outlier_remove_enable)
			dpu_hw_post_scaler_outlier_remove_set(hw, scaler);
	}

	dpu_hw_post_scaler_stripe_set(hw, scaler);
}
...
void dpu_hw_post_scaler_flush(struct dpu_hw_blk *hw)
{
	...
	DPU_REG_WRITE(hw, DPU_2DSCL_FORCE_UPDATE_EN_OFFSET, 0x02,
			CMDLIST_WRITE);
...
{
	DPU_COLOR_INFO("post_scaler: 0x%x, 0x%x, 0x%x, 0x%x\n",
			DPU_REG_READ(hw, DEBUG_SRC_IMG_ROW_COL_ID_OFFSET),
			DPU_REG_READ(hw, DEBUG_SRC_IMG_FRM_ID_OFFSET),
			DPU_REG_READ(hw, DEBUG_DST_IMG_ROW_COL_ID_OFFSET),
			DPU_REG_READ(hw, DEBUG_DST_IMG_FRM_ID_OFFSET));
}
