...
extern int minet_nl_notify(void *data, int len);
	...
	list_for_each_safe(p, n, &app->head) {
		...
		list_for_each_safe(p1, n1, &flow->head) {
			...
			if (nf) {
				...
				fi->stat.in_pkt, fi->stat.in_len, fi->stat.out_pkt, fi->stat.out_len);
#endif
				nf->cnt ++;
			}
...
static unsigned int hook4_udp_in(void *priv, struct sk_buff *skb,
	const struct nf_hook_state *state)
{
	...
	/* match addr */
	if (is_local_or_lan_addr(iph->saddr))
		return NF_ACCEPT;
	...
	spin_lock_bh(&app->lock);
	if (!list_empty(&app->head)) {
		ai =  list_entry(app->head.next, app_info, head);
	} else {
		spin_unlock_bh(&app->lock);
		goto put_cpu;
	}
...
static unsigned int hook6_udp_in(void *priv, struct sk_buff *skb,
	const struct nf_hook_state *state)
{
	unsigned int cpu;
	app_head *app = NULL;
	...
	const struct nf_hook_state *state)
{
	...
	if (state->hook == NF_INET_LOCAL_IN && iph->protocol == IPPROTO_UDP)
		return hook4_udp_in(priv, skb, state);
	...
	/*
	minet_dbg("%s hook:%d, pf:%d, af:%d, proto:%d, uid:%d", __func__,
		state->hook, state->pf, sk->sk_family, iph->protocol, get_sock_uid(sk));
	*/
	...
	/* match addr */
	//if (is_local_or_lan_addr(sk->sk_daddr))
	...
	/* match app */
	spin_lock_bh(&app->lock);
	ai = match_app(app, get_sock_uid(sk));
	if (NULL == ai) {
		spin_unlock_bh(&app->lock);
		goto put_cpu;
	}
	...
	const struct nf_hook_state *state)
{
	...
	if (state->hook == NF_INET_LOCAL_IN && iph->nexthdr == IPPROTO_UDP)
		return hook6_udp_in(priv, skb, state);
	...
	/*
	minet_dbg("%s hook:%d, pf:%d, af:%d, proto:%d, uid:%d", __func__,
		state->hook, state->pf, sk->sk_family, iph->nexthdr, get_sock_uid(sk));
	*/
...
static int minet_init_module(void)
{
	int cpu, ret = 0;
	app_head *app = NULL;
...
static void minet_cleanup_module(void)
{
	minet_dbg("%s", __func__);
...
int minet_init(void)
{
	return minet_init_module();
}

void minet_cleanup(void)
{
	minet_cleanup_module();
}
