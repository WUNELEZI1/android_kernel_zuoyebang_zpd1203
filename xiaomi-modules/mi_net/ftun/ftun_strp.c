...
#else
static int ftun_strp_read_copy(struct tls_strparser *strp, bool qshort)
{
	/* If the rbuf is small or rcv window has collapsed to 0 we need
	 * to read the data out. Otherwise the connection will stall.
	 * Without pressure threshold of INT_MAX will never be ready.
	...
	ftun_err("%s qshort:%d", __func__, qshort);
	return 0;
}
#endif
...
static int tls_strp_read_sock(struct tls_strparser *strp)
{
	...
	if (inq < strp->stm.full_len)
		return ftun_strp_read_copy(strp, true);

	if (!strp->stm.full_len) {
		tls_strp_load_anchor_with_queue(strp, inq);

		sz = ftun_rx_msg_size(strp, strp->anchor);
		if (sz < 0) {
	...
	if (!tls_strp_check_queue_ok(strp))
		return ftun_strp_read_copy(strp, false);
