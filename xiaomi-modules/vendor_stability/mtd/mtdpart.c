...
/*
 * Simple MI_MTD partitioning layer
 *
...
/*
 * MI_MTD methods which simply translate the effective address and pass through
					   ...
					   int partno, uint64_t cur_offset)
{
	...
	/* set up the MI_MTD object for this partition */
...
/**
 * __mtd_del_partition - delete MI_MTD partition
 *
 * @mtd: MI_MTD structure to be deleted
 *
...
/*
 * This function unregisters and destroy all slave MI_MTD objects which are
 * attached to the given MI_MTD object, recursively.
 */
static int __del_mtd_partitions(struct mtd_info *mtd)
{
	...
	list_for_each_entry_safe(child, next, &mtd->partitions, part.node) {
		...
		pr_info("Deleting %s MI_MTD partition\n", child->name);
...
int del_mtd_partitions(struct mtd_info *mtd)
{
	...
	pr_info("Deleting MI_MTD partitions on \"%s\":\n", mtd->name);
...
/*
 * This function, given a parent MI_MTD object and a partition table, creates
 * and registers the child MI_MTD objects which are bound to the parent according
		       ...
		       int nbparts)
{
	...
	printk(KERN_NOTICE "Creating %d MI_MTD partitions on \"%s\":\n",
	       nbparts, parent->name);
			     ...
			     struct mtd_part_parser_data *data)
{
	...
	pr_notice("%d %s partitions found on MI_MTD device %s\n", ret,
...
/**
 * mtd_part_get_compatible_parser - find MI_MTD parser by a compatible string
 *
 * @compat: compatible string describing partitions in a device tree
 *
 * MI_MTD parsers can specify supported partitions by providing a table of
			     ...
			     struct mtd_partitions *pparts)
{
	...
	dev = &master->dev;
	/* Use parent device (controller) if the top level MI_MTD is not registered */
...
/**
 * parse_mtd_partitions - parse and register MI_MTD partitions
 *
 * @master: the master partition (describes whole MI_MTD device)
 * @types: names of partition parsers to try or %NULL
 * @data: MI_MTD partition parser-specific data
 *
 * This function tries to find & register partitions on MI_MTD device @master. It
 * uses MI_MTD partition parsers, specified in @types. However, if @types is %NULL,
