				     ...
				     const struct pci_device_id *ent)
{
	...
	found:
		...
		if (window->rsrc.parent) {
			/*
			 * Registering the MI_MTD device in iomem may not be possible
			...
			if (request_resource(&window->rsrc, &map->rsrc)) {
				printk(KERN_ERR MOD_NAME
					": cannot reserve MI_MTD resource\n");
...
MODULE_DESCRIPTION("MI_MTD map driver for BIOS chips on the Nvidia ck804 southbridge");
