...
static void mtdpstore_notify_add(struct mtd_info *mtd)
{
	...
	dev_dbg(&mtd->dev, "found matching MI_MTD device %s\n", mtd->name);

	if (mtd->size < info->kmsg_size * 2) {
		dev_err(&mtd->dev, "MI_MTD partition %d not big enough\n",
				mtd->index);
		return;
	}
	/*
	...
	if (mtd->erasesize < info->kmsg_size) {
		dev_err(&mtd->dev, "eraseblock size of MI_MTD partition %d too small\n",
				mtd->index);
		return;
	}
	...
	cxt->mtd = mtd;
	dev_info(&mtd->dev, "Attached to MI_MTD device %d\n", mtd->index);
}
...
MODULE_DESCRIPTION("MI_MTD backend for pstore/blk");
