 ...
 * @mtd: MI_MTD info for each NOR-flash.
 ...
 * @mtd: MI_MTD information of the memory bank
				 ...
				 u32 bank, struct device_node *np)
{
	...
	if (ret) {
		dev_err(&dev->pdev->dev, "Err MI_MTD partition=%d\n", ret);
		return ret;
	}
...
MODULE_DESCRIPTION("MI_MTD SMI driver for serial nor flash chips");
