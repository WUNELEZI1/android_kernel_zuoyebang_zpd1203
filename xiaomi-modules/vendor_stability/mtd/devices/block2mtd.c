...
/* Static info about the MI_MTD, used in cleanup_module */
		...
		size_t label, int timeout)
{
	...
	if (IS_ERR(bdev)) {
		dump_stack();
		err = PTR_ERR(bdev);
		pr_err("error: cannot open device %s, err is %d\n", devname, err);
...
MODULE_DESCRIPTION("Emulate an MI_MTD using a block device");
