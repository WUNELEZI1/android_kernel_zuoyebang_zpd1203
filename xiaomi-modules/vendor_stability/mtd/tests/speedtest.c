 ...
 *
 * Test read and write speed of a MI_MTD device.
 *
...
MODULE_PARM_DESC(dev, "MI_MTD device number to use");
...
static int __init mtd_speedtest_init(void)
{
	...
	if (dev < 0) {
		...
		pr_crit("CAREFUL: This test wipes all data on the specified MI_MTD device!\n");
		return -EINVAL;
	}

	if (count)
		pr_info("MI_MTD device: %d    count: %d\n", dev, count);
	else
		pr_info("MI_MTD device: %d\n", dev);

	mtd = get_mtd_device(NULL, dev);
	if (IS_ERR(mtd)) {
		err = PTR_ERR(mtd);
		pr_err("error: cannot get MI_MTD device\n");
		return err;
	}
	...
	pr_info("MI_MTD device size %llu, eraseblock size %u, "
