/* This version ported to the Linux-MI_MTD system by dwmw2@infradead.org
 *
...
static int scan_header(partition_t *part)
{
	...
	printk(KERN_NOTICE "ftl: FTL EraseUnitSize %x != MI_MTD erasesize %x\n",
		      ...
		      uint16_t xfernum)
{
    ...
    /* Is there a free erase slot? Always in MI_MTD. */
		    ...
		    u_long sector, u_long nblocks)
{
	...
	if (log_addr == 0xffffffff)
	    ...
	    if (ret) {
		printk(KERN_WARNING "Error reading MI_MTD device in ftl_read()\n");
		return ret;
	    }
	}
	buffer += SECTOR_SIZE;
