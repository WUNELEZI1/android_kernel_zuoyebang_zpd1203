...
/*
 * Core registration and callback routines for MI_MTD
 * drivers and users.
 *
...
/* REVISIT once MI_MTD uses the driver model better, whoever allocates
 ...
 * @mtd: pointer to new MI_MTD device info structure
 ...
 * to @wunit.
 *
 * From the pairing info the MI_MTD user can find all the wunits paired with
...
static void mtd_check_of_node(struct mtd_info *mtd)
{
	...
	/* Check if MI_MTD already has a device node */
...
/**
 *	add_mtd_device - register an MI_MTD device
 *	@mtd: pointer to new MI_MTD device info structure
 *
 *	Add a device to the list of MI_MTD devices present in the system, and
 *	notify each currently active MI_MTD 'user' of its arrival. Returns
...
int add_mtd_device(struct mtd_info *mtd)
{
	 ...
	 * mtd_device_parse_register() multiple times on the same master MI_MTD,
	 * especially with CONFIG_MTD_PARTITIONED_MASTER=y.
	 */
	if (WARN_ONCE(mtd->dev.type, "MI_MTD already registered\n"))
		return -EEXIST;

	BUG_ON(mtd->writesize == 0);

	/*
	 * MI_MTD drivers should implement ->_{write,read}() or
...
/**
 *	del_mtd_device - unregister an MI_MTD device
 *	@mtd: pointer to MI_MTD device info structure
 *
 *	Remove a device from the list of MI_MTD devices present in the system,
 *	and notify each currently active MI_MTD 'user' of its departure.
...
/**
 * mtd_device_parse_register - parse partitions and register an MI_MTD device.
 *
 * @mtd: the MI_MTD device to register
 * @types: the list of MI_MTD partition probes to try, see
 *         'parse_mtd_partitions()' for more information
 * @parser_data: MI_MTD partition parser-specific data
 * @parts: fallback partition information to register, if parsing fails;
 *         only valid if %nr_parts > %0
 * @nr_parts: the number of partitions in parts, if zero then the full
 *            MI_MTD device is registered if no partition info is found
 *
 * This function aggregates MI_MTD partitions parsing (done by
 * 'parse_mtd_partitions()') and MI_MTD device and partitions registering. It
 * basically follows the most common pattern found in many MI_MTD drivers:
 *
 * * If the MTD_PARTITIONED_MASTER option is set, then the device as a whole is
 *   registered first.
 * * Then It tries to probe partitions on MI_MTD device @mtd using parsers
 *   specified in @types (if @types is %NULL, then the default list of parsers
 *   is used, see 'parse_mtd_partitions()' for more information). If none are
 *   found this functions tries to fallback to information specified in
 *   @parts/@nr_parts.
 * * If no partitions were found this function just registers the MI_MTD device
 *   @mtd and exits.
 *
...
/**
 * mtd_device_unregister - unregister an existing MI_MTD device.
 *
 * @master: the MI_MTD device to unregister.  This will unregister both the master
...
/**
 *	register_mtd_user - register a 'user' of MI_MTD devices.
 *	@new: pointer to notifier info structure
 *
 *	Registers a pair of callbacks function to be called upon addition
 *	or removal of MI_MTD devices. Causes the 'add' callback to be immediately
 *	invoked for each MI_MTD device currently present in the system.
 */
...
/**
 *	unregister_mtd_user - unregister a 'user' of MI_MTD devices.
 *	@old: pointer to notifier info structure
 *
 *	Removes a callback function pair from the list of 'users' to be
 *	notified upon addition or removal of MI_MTD devices. Causes the
 *	'remove' callback to be immediately invoked for each MI_MTD device
...
/**
 *	get_mtd_device - obtain a validated handle for an MI_MTD device
 *	@mtd: last known address of the required MI_MTD device
 *	@num: internal device number of the required MI_MTD device
 *
...
/**
 * of_get_mtd_device_by_node - obtain an MI_MTD device associated with a given node
 *
 * @np: device tree node
 */
...
/**
 *	get_mtd_device_nm - obtain a validated handle for an MI_MTD device by
 *	device name
 *	@name: MI_MTD device name to open
 *
 * 	This function returns MI_MTD device description structure in case of
 ...
 * @to: the MI_MTD device offset to write to
 * @retlen: on exit contains the count of bytes written to the MI_MTD device.
 *
...
/*
 * mtd_writev - the vector-based MI_MTD write method
 * @mtd: mtd device description object pointer
 * @vecs: the vectors to write
 * @count: count of vectors in @vecs
 * @to: the MI_MTD device offset to write to
 ...
 * the MI_MTD device's min. I/O unit, i.e. the "mtd->writesize" value.
 *
...
MODULE_DESCRIPTION("Core MI_MTD registration and access routines");
