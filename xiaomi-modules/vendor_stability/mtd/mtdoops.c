...
#include "../../../xring-modules/include/soc/xring/xr_pmic_key_event.h"

/* Maximum MI_MTD partition size */
#define MTDOOPS_MAX_MTD_SIZE (16 * 1024 * 1024)
static unsigned long record_size = 4096;
module_param(record_size, ulong, 0400);
MODULE_PARM_DESC(record_size,
		"record size for MI_MTD OOPS pages in bytes (default 4096)");

static char mtddev[80];
module_param_string(mtddev, mtddev, 80, 0400);
MODULE_PARM_DESC(mtddev,
		"name or index number of the MI_MTD device to use");
...
struct work_struct pwrkey_work;
struct workqueue_struct *pwrkey_wq;
...
static void pwrkey_long_press_mtdoops_do_dump(struct work_struct *work) {
	struct mtdoops_context *cxt = &oops_cxt;
...
static int pwrkey_long_press_irq_event(struct notifier_block *this, unsigned long event,
										void *ptr) {
	pr_err("[%s] event = %lu\n", __func__, event);
	if (event == XR_POWERKEY_PRESS_HOLD_6S) {
		pr_err("[%s] start\n", __func__);
...
static void mtdoops_notify_add(struct mtd_info *mtd)
{
	...
	if (mtd->size < mtd->erasesize * 2) {
		pr_err("MI_MTD partition %d not big enough for mtdoops\n",
		       mtd->index);
		return;
	}
	if (mtd->erasesize < record_size) {
		pr_err("eraseblock size of MI_MTD partition %d too small\n",
		       mtd->index);
		return;
	}
	...
	xr_pmic_key_register_notifier(&cxt->pwrkey_long_press_nb);

	cxt->mtd = mtd;
	cxt->oops_pages = (int)mtd->size / record_size;
	find_next_position(cxt);
	pr_info("Attached to MI_MTD device %d\n", mtd->index);
}
...
#define parse_u32(name, field, default_value) {				\
	...
	parse_u32("record-size", cxt->pmsg_data.buffer_size, 0);

	cxt->pmsg_data.mem_address = res->start + cxt->pmsg_data.buffer_size;
	cxt->pmsg_data.mem_size = cxt->pmsg_data.buffer_size;
#undef parse_u32
...
static int __init mtdoops_init(void)
{
	...
	/* Setup the MI_MTD device to use */
	cxt->mtd_index = -1;
	...
	if (pwrkey_wq == NULL) {
		pr_err("pwrkey_wq create wq failed\n");
		return -ENOMEM;
	}
	INIT_WORK(&pwrkey_work, pwrkey_long_press_mtdoops_do_dump);
...
static void __exit mtdoops_exit(void)
{
	...
	cancel_work_sync(&pwrkey_work);
	destroy_workqueue(pwrkey_wq);
}
...
MODULE_DESCRIPTION("MI_MTD Oops/Panic console logger/driver");
