...
struct mtd_info *cfi_cmdset_0002(struct map_info *map, int primary)
{
	...
	pr_debug("MI_MTD %s(): write buffer size %d\n", __func__,
			mtd->writebufsize);
	...
	case FL_ERASING:
		...
		for (;;) {
			...
			if (time_after(jiffies, timeo)) {
				...
				printk(KERN_ERR "MI_MTD %s(): chip not ready after erase suspend\n", __func__);
				return -EIO;
			}
...
static void put_chip(struct map_info *map, struct flchip *chip, unsigned long adr)
{
	...
	case FL_STATUS:
		break;
	default:
		printk(KERN_ERR "MI_MTD: put_chip() called with oldstate %d!!\n", chip->oldstate);
	}
	wake_up(&chip->wq);
}
 ...
 * didn't emit calls to its own support functions). Also configuring MI_MTD CFI
					  ...
					  int mode, struct cfi_private *cfi)
{
	...
	for (;;) {
		...
		if (time_after(jiffies, timeo) &&
			...
			printk(KERN_WARNING "MI_MTD %s(): software timeout\n", __func__);
				     ...
				     int mode)
{
	...
	pr_debug("MI_MTD %s(): WRITE 0x%.8lx(0x%.8lx)\n", __func__, adr,
		 datum.x[0]);
					 ...
					 map_word datum)
{
	...
	for (;;) {
		...
		if (time_after(jiffies, timeo) &&
		    !chip_good(map, chip, adr, &datum)) {
			pr_err("MI_MTD %s(): software timeout, address:0x%.8lx.\n",
			       __func__, adr);
			ret = -EIO;
			break;
		}
				  ...
				  unsigned long adr, map_word datum)
{
	...
	pr_debug("MI_MTD %s(): PANIC WRITE 0x%.8lx(0x%.8lx)\n",
			__func__, adr, datum.x[0]);
...
static int __xipram do_erase_chip(struct map_info *map, struct flchip *chip)
{
	...
	pr_debug("MI_MTD %s(): ERASE 0x%.8lx\n",
	       __func__, chip->start);
			 ...
			 unsigned long adr, int len, void *thunk)
{
	...
	pr_debug("MI_MTD %s(): LOCK 0x%08lx len %d\n", __func__, adr, len);
					...
					unsigned long adr, int len, void *thunk)
{
	...
	pr_debug("MI_MTD %s(): XXLOCK 0x%08lx len %d\n", __func__, adr, len);
...
MODULE_DESCRIPTION("MI_MTD chip driver for AMD/Fujitsu flash chips");
