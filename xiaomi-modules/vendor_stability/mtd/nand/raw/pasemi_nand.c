...
static int pasemi_nand_probe(struct platform_device *ofdev)
{
	...
	/* Allocate memory for MI_MTD device structure and private data */
	...
	/* Link the private data with the MI_MTD structure */
	...
	if (mtd_device_register(pasemi_nand_mtd, NULL, 0)) {
		dev_err(dev, "Unable to register MI_MTD device\n");
		err = -ENODEV;
...
static void pasemi_nand_remove(struct platform_device *ofdev)
{
	...
	/* Free the MI_MTD device structure */
	kfree(ddata);
}
