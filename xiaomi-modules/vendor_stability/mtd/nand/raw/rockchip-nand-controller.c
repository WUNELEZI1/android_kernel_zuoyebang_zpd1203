				 ...
				 int oob_on, int page)
{
	...
	if ((chip->options & NAND_IS_BOOT_MEDIUM) &&
		...
		/*
		 * There's currently no method to notify the MI_MTD framework that
				   ...
				   int oob_on, int page)
{
	 ...
	 * OOB data from the MI_MTD framework, because this page address
	 * conflicts for example with the bad block marker (BBM),
	 * so we shift all OOB data including the BBM with 4 byte positions.
	 * As a consequence the OOB size available to the MI_MTD framework is
				 ...
				 struct device_node *np)
{
	...
	if (ret) {
		dev_err(dev, "MI_MTD parse partition error\n");
		nand_cleanup(chip);
		return ret;
	}
