			    ...
			    struct atmel_nand *nand)
{
	...
	/*
	 * Use a bounce buffer when the buffer passed by the MI_MTD user is not
	 * suitable for DMA.
	 */
	if (nc->dmac)
...
static int atmel_nand_attach_chip(struct nand_chip *chip)
{
	...
	if (nc->caps->legacy_of_bindings || !nc->dev->of_node) {
		/*
		 * We keep the MI_MTD name unchanged to avoid breaking platforms
		 * where the MI_MTD cmdline parser is used and the bootloader
