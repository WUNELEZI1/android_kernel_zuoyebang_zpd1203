...
/*
 * This is a small driver which implements fake MI_MTD devices on top of UBI
 * volumes. This sounds strange, but it is in fact quite useful to make
 * MI_MTD-oriented software (including all the legacy software) work on top of
 * UBI.
 *
 * Gluebi emulates MI_MTD devices of "MTD_UBIVOLUME" type. Their minimal I/O unit
 ...
 * @mtd: emulated MI_MTD device description object
...
/**
 * gluebi_get_device - get MI_MTD device reference.
 * @mtd: the MI_MTD device description object
 *
 * This function is called every time the MI_MTD device is being opened and
 * implements the MI_MTD get_device() operation. Returns zero in case of success
...
static int gluebi_get_device(struct mtd_info *mtd)
{
	...
	if (gluebi->refcnt > 0) {
		/*
		 * The MI_MTD device is already referenced and this is just one
		 * more reference. MI_MTD allows many users to open the same
	...
	/*
	 * This is the first reference to this UBI volume via the MI_MTD device
...
/**
 * gluebi_put_device - put MI_MTD device reference.
 * @mtd: the MI_MTD device description object
 *
 * This function is called every time the MI_MTD device is being put. Returns
...
/**
 * gluebi_read - read operation of emulated MI_MTD devices.
...
/**
 * gluebi_write - write operation of emulated MI_MTD devices.
 * @mtd: MI_MTD device description object
...
/**
 * gluebi_erase - erase operation of emulated MI_MTD devices.
 * @mtd: the MI_MTD device description object
...
static int gluebi_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	/*
	 * MI_MTD erase operations are synchronous, so we have to make sure the
 ...
 * corresponding fake MI_MTD device. Returns zero in case of success and a
			 ...
			 struct ubi_volume_info *vi)
{
	...
	/*
	 * In case of dynamic a volume, MI_MTD device size is just volume size. In
	...
	if (g)
		err_msg("gluebi MI_MTD device %d form UBI device %d volume %d already exists",
			g->mtd.index, vi->ubi_num, vi->vol_id);
	mutex_unlock(&devices_mutex);

	if (mtd_device_register(mtd, NULL, 0)) {
		err_msg("cannot add MI_MTD device");
		kfree(mtd->name);
		kfree(gluebi);
		return -ENFILE;
	}
...
static int gluebi_remove(struct ubi_volume_info *vi)
{
	...
	if (err) {
		err_msg("cannot remove fake MI_MTD device %d, UBI device %d, volume %d, error %d",
 ...
 * if te volume @vol is dynamic, and changes MI_MTD device size if the
 ...
 * corresponding fake MI_MTD device size. This function returns zero in case of
...
static void __exit ubi_gluebi_exit(void)
{
	...
	list_for_each_entry_safe(gluebi, g, &gluebi_devices, list) {
		...
		err = mtd_device_unregister(mtd);
		if (err)
			err_msg("error %d while removing gluebi MI_MTD device %d, UBI device %d, volume %d - ignoring",
...
MODULE_DESCRIPTION("MI_MTD emulation layer over UBI volumes");
