 ...
 *
 * When UBI is initialized, it attaches all the MI_MTD devices specified as the
 * module load parameters or the kernel boot parameters. If MI_MTD devices were
 * specified, UBI does not attach any MI_MTD device, but it is possible to do
...
/**
 * struct mtd_dev_param - MI_MTD device parameter description data structure.
 * @name: MI_MTD character device node path, MI_MTD device name, or MI_MTD device number
 *        string
 * @ubi_num: UBI number
...
/* MI_MTD devices specification parameters */
...
static int get_bad_peb_limit(const struct ubi_device *ubi, int max_beb_per1024)
{
	 ...
	 * not just the MI_MTD partition size because the maximum
	 * number of bad eraseblocks is a percentage of the
	 * whole device and bad eraseblocks are not fairly
	 * distributed over the flash chip. So the worst case
	 * is that all the bad eraseblocks of the chip are in
	 * the MI_MTD partition we are attaching (ubi->mtd).
	 */
...
static int io_init(struct ubi_device *ubi, int max_beb_per1024)
{
	...
	/*
	 * Note, in this implementation we support MI_MTD devices with 0x7FFFFFFF
	...
	if (!(ubi->mtd->flags & MTD_WRITEABLE)) {
		ubi_msg(ubi, "MI_MTD device %d is write-protected, attach in read-only mode",
			ubi->mtd->index);
		ubi->ro_mode = 1;
	}
	 ...
	 * unfortunately, MI_MTD does not provide this information. We should loop
...
/**
 * ubi_attach_mtd_dev - attach an MI_MTD device.
 * @mtd: MI_MTD device description object
 * @ubi_num: number to assign to the new UBI device
 * @vid_hdr_offset: VID header offset
 * @max_beb_per1024: maximum expected number of bad PEB per 1024 PEBs
 * @disable_fm: whether disable fastmap
 *
 * This function attaches MI_MTD device @mtd_dev to UBI and assign @ubi_num number
		       ...
		       int vid_hdr_offset, int max_beb_per1024, bool disable_fm)
{
	...
	/*
	 * Check if we already have the same MI_MTD device attached.
	 *
	...
	/*
	 * Make sure this MI_MTD device is not emulated on top of an UBI volume
	 * already. Well, generally this recursion works fine, but there are
	 * different problems like the UBI module takes a reference to itself
	 * by attaching (and thus, opening) the emulated MI_MTD device. This
	 * results in inability to unload the module. And in general it makes
	 * no sense to attach emulated MI_MTD devices, so we prohibit this.
	 */
...
/**
 * ubi_detach_mtd_dev - detach an MI_MTD device.
 * @ubi_num: UBI device number to detach from
 * @anyway: detach MI_MTD even if device reference count is not zero
 *
 * This function destroys an UBI device number @ubi_num and detaches the
 * underlying MI_MTD device. Returns zero in case of success and %-EBUSY if the
...
/**
 * open_mtd_by_chdev - open an MI_MTD device by its character device node path.
 * @mtd_dev: MI_MTD character device node path
 *
 * This helper function opens an MI_MTD device by its character node device path.
 * Returns MI_MTD device description object in case of success and a negative
...
static struct mtd_info * __init open_mtd_by_chdev(const char *mtd_dev)
{
	...
	/* Probably this is an MI_MTD character device node path */
	...
	/* MI_MTD device number is defined by the major / minor numbers */
...
/**
 * open_mtd_device - open MI_MTD device by name, character device path, or number.
 * @mtd_dev: name, character device node path, or MI_MTD device device number
 *
 * This function tries to open and MI_MTD device described by @mtd_dev string,
 * which is first treated as ASCII MI_MTD device number, and if it is not true, it
 * is treated as MI_MTD device name, and if that is also not true, it is treated
 * as MI_MTD character device node path. Returns MI_MTD device description object in
...
static int __init ubi_init(void)
{
	...
	if (mtd_devs > UBI_MAX_DEVICES) {
		pr_err("UBI error: too many MI_MTD devices, maximum is %d\n",
		       UBI_MAX_DEVICES);
		return -EINVAL;
	}
	...
	for (i = 0; i < mtd_devs; i++) {
		...
		if (err < 0) {
			 ...
			 * the kernel and the MI_MTD devices to attach are passed
...
MODULE_PARM_DESC(mtd, "MI_MTD devices to attach. Parameter format: mtd=<name|num|path>[,<vid_hdr_offs>[,max_beb_per1024[,ubi_num]]].\n"
		      "Multiple \"mtd\" parameters may be specified.\n"
		      "MI_MTD devices may be specified by their number, name, or path to the MI_MTD character device node.\n"
		      ...
		      "\n"
		      "Example 1: mtd=/dev/mtd0 - attach MI_MTD device /dev/mtd0.\n"
		      "Example 2: mtd=content,1984 mtd=4 - attach MI_MTD device with name \"content\" using VID header offset 1984, and MI_MTD device number 4 with default VID header offset.\n"
		      "Example 3: mtd=/dev/mtd1,0,25 - attach MI_MTD device /dev/mtd1 using default VID header offset and reserve 25*nand_size_in_blocks/1024 erase blocks for bad block handling.\n"
		      "Example 4: mtd=/dev/mtd1,0,0,5 - attach MI_MTD device /dev/mtd1 to UBI 5 and using default values for the other fields.\n"
		      "example 5: mtd=1,0,0,5 mtd=2,0,0,6,1 - attach MI_MTD device /dev/mtd1 to UBI 5 and disable fastmap; attach MI_MTD device /dev/mtd2 to UBI 6 and enable fastmap.(only works when fastmap is enabled and fm_autoconvert=Y).\n"
