 ...
 *
 * This sub-system is responsible for attaching MI_MTD devices and it also
 ...
 * about such PEBs every time the MI_MTD device is attached.
 *
		    ...
		    int pnum, bool fast)
{
	...
	case UBI_IO_BAD_HDR:
			 ...
			 * from an old one. On a fine MI_MTD stack a PEB must not render
 ...
 * corrupted PEBs and we should not attach this MI_MTD device. Returns zero if we
 * should proceed with attaching the MI_MTD device, and %-EINVAL if we should not.
 */
static int late_analysis(struct ubi_device *ubi, struct ubi_attach_info *ai)
{
	...
	if (ai->empty_peb_count + ai->maybe_bad_peb_count == peb_count) {
		...
		if (ai->maybe_bad_peb_count <= 2) {
			ai->is_empty = 1;
			ubi_msg(ubi, "empty MI_MTD device detected");
			get_random_bytes(&ubi->image_seq,
					 sizeof(ubi->image_seq));
		} else {
			ubi_err(ubi, "MI_MTD device is not UBI-formatted and possibly contains non-UBI data - refusing it");
			return -EINVAL;
		}
...
/**
 * scan_all - scan entire MI_MTD device.
 * @ubi: UBI device description object
 * @ai: attach info object
 * @start: start scanning at this PEB
 *
 * This function does full scanning of an MI_MTD device and returns complete
...
/**
 * ubi_attach - attach an MI_MTD device.
