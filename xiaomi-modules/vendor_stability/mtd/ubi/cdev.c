 ...
 * delete them. In other words, it is used for attaching and detaching MI_MTD
 * devices.
 */
			    ...
			    unsigned long arg)
{
	...
	switch (cmd) {
	/* Attach an MI_MTD device command */
	case UBI_IOCATT:
	{
		struct ubi_attach_req req;
		struct mtd_info *mtd;

		dbg_gen("attach MI_MTD device");
	...
	/* Detach an MI_MTD device command */
	case UBI_IOCDET:
	{
		int ubi_num;

		dbg_gen("detach MI_MTD device");
