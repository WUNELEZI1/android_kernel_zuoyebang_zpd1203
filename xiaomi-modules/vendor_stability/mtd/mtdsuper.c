...
/* MI_MTD-based superblock management
 *
...
/*
 * get a superblock on an MI_MTD-backed filesystem
 */
...
/*
 * get a superblock on an MI_MTD-backed filesystem by MI_MTD device number
 */
...
/**
 * get_tree_mtd - Get a superblock based on a single MI_MTD device
				...
				struct fs_context *fc))
{
	 ...
	 * CONFIG_BLOCK=n - we specify the underlying MI_MTD device by number or
	...
	if (fc->source[0] == 'm' &&
		...
		if (fc->source[3] == ':') {
			...
			errorf(fc, "MI_MTD: MI_MTD device with name \"%s\" not found",
			       fc->source + 4);

		} else if (isdigit(fc->source[3])) {
			/* mount by MI_MTD device number name */
			char *endptr;
	...
	if (ret) {
		errorf(fc, "MI_MTD: Couldn't look up '%s': %d", fc->source, ret);
		return ret;
	}
	...
	if (!(fc->sb_flags & SB_SILENT))
		errorf(fc, "MI_MTD: Attempt to mount non-MI_MTD device \"%s\"",
		       fc->source);
	return -EINVAL;
}
...
/*
 * destroy an MI_MTD-based superblock
 */
