 ...
 * Turns an MI_MTD device into a swap device with block wear leveling
 *
...
MODULE_PARM_DESC(partitions, "MI_MTD partition numbers to use as swap "
		"partitions=\"1,3,5\"");
...
retry:
	...
	if (ret < 0) {
		dev_err(d->dev, "Write to MI_MTD device failed: %d (%zd written)",
			ret, retlen);
		goto err;
	}

	if (retlen != PAGE_SIZE) {
		dev_err(d->dev, "Short write to MI_MTD device: %zd written",
			retlen);
		ret = -EIO;
		goto err;
	}
...
static void mtdswap_add_mtd(struct mtd_blktrans_ops *tr, struct mtd_info *mtd)
{
	...
	printk(KERN_INFO "%s: Enabling MI_MTD swap on device %lu, size %llu KB, "
...
MODULE_DESCRIPTION("Block device access to an MI_MTD suitable for using as "
		"swap space");
