...
{
	struct usb_redriver *redriver = NULL;
...
void usb_redriver_notify_hpd(struct usb_redriver *ur, unsigned int hpd)
{
	if (ur == NULL) {
		pr_err("redriver: ur is null.\n");
		return;
	} else if (ur->notify_hpd == NULL) {
		pr_err("redriver: ur->notify_hpd is null.\n");
		return;
	}

	if (ur && ur->notify_hpd) {
		pr_info("redriver: notify_hpd.\n");
		ur->notify_hpd(ur, hpd);
	}
}
EXPORT_SYMBOL(usb_redriver_notify_hpd);
