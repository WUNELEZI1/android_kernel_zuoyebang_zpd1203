...
static irqreturn_t subpmic_irq_thread(int irq, void *data)
{
	...
	/* hold wake lock for about 500ms to handle irq thread handler */
	pm_wakeup_dev_event(sc->dev, 500, true);

	ret = regmap_bulk_read(sc->rmap, SUBPMIC_REG_HK_IRQ, &evt, 1);
	if (ret) {
		mca_log_err("failed to read irq event\n");
		return IRQ_HANDLED;
	}
...
static int subpmic_add_irq_chip(struct subpmic_device *sc)
{
    ...
    if (!sc->domain) {
        mca_log_err("failed to create irq domain\n");
        return -ENOMEM;
    }
    ...
    if (ret) {
        mca_log_info("failed to request irq %d for %s\n",
			sc->irqn, dev_name(sc->dev));
		irq_domain_remove(sc->domain);
		return ret;
    }

    mca_log_info("subpmic irq = %d\n", sc->irqn);
...
static int subpmic_check_chip(struct subpmic_device *sc)
{
    ...
    if (ret < 0 || (did != SC6601_DEVICE_ID &&
                    did != SC6601_1P1_DEVICE_ID &&
                    did != SC6601A_DEVICE_ID &&
                    did != SC6607_1P1_DEVICE_ID)) {
        return -ENODEV;
    }
    return 0;
}

static int subpmic_sc6601a_probe(struct i2c_client *client)
{
	...
	client->irq = of_get_named_gpio(node, "sc6601-gpio", 0);

	ret = gpio_direction_input(client->irq);
	gpio_set_value(client->irq, 0);

	mca_log_err("get sc6601 irq: %d\n", client->irq);
	if (!client->irq) {
		mca_log_err("get sc6601 irq: failed %d\n", client->irq);
		return 0;
	}
    ...
    if (IS_ERR(sc->rmap)) {
        mca_log_err("failed to init regmap\n");
        ret = PTR_ERR(sc->rmap);
        goto err;
    }
    // check id
	...
	if (ret < 0) {
		mca_log_err("failed to check device id\n");
		goto err;
	}
	gpio_set_value(client->irq, 1);
	ret = gpio_direction_input(client->irq);
	sc->irqn = gpio_to_irq(client->irq);
    // clear flag
    ret = regmap_bulk_read(sc->rmap, SUBPMIC_REG_HK_IRQ, &evt, 1);
    // add irq
    ret = subpmic_add_irq_chip(sc);
	if (ret < 0) {
		mca_log_err("failed to add irq chip\n");
		goto err;
	}
...
static void subpmic_remove(struct i2c_client *client)
{
...
static int subpmic_resume(struct device *dev)
{
	...
	/* hold wake lock for about 100ms for resume */
	pm_wakeup_dev_event(sc->dev, 100, true);
	/* enable_irq(sc->irqn); */
	if (device_may_wakeup(dev))
...
static const struct of_device_id subpmic_of_match[] = {
	{ .compatible = "xm_subpmic_sc6601a,subpmic_core",},
	{ },
};

static struct i2c_driver subpmic_driver = {
	.probe = subpmic_sc6601a_probe,
