...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_evt_vdm"
#endif
	...
	const struct vdm_state_transition *state_transition)
{
	...
	if (!pd_check_pe_state_ready(pd_port)) {
		mca_log_debug("670 : invalid, current status\n");
		return false;
	}
	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	mca_log_debug("659 : Invalid UVDM\n");
	return false;
}
		...
		struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (pd_event->msg == PD_HW_TX_DISCARD &&
		...
		mca_log_info("vdm_discard_retry\n");
		...
		struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (!pd_check_pe_state_ready(pd_port)) {
		...
		mca_log_debug("skip vdm_request, not ready_state (%d)\n",
	...
	if (!pd_check_tcp_msg_valid(pd_port, new_state)) {
		...
		mca_log_debug("skip vdm_request, WRONG DATA ROLE\n");
		return false;
	}
