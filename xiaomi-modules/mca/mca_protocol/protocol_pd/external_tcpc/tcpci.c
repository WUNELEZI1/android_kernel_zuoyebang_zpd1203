...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_tcpci"
#endif
...
int tcpci_check_vbus_valid_from_ic(struct tcpc_device *tcpc)
{
	...
	if (tcpci_get_power_status(tcpc, &power_status) == 0) {
		if (vbus_level != tcpc->vbus_level) {
			...
			mca_log_err("[Warning] ps_changed %d -> %d\n", vbus_level, tcpc->vbus_level);
		}
	}
...
int tcpci_ops_get_antiburn_status(struct tcpc_device *tcpc, bool en)
{
	if (tcpc->ops->get_antiburn_status) {
		return tcpc->ops->get_antiburn_status(tcpc, en);
	}
	return 0;
}
EXPORT_SYMBOL(tcpci_ops_get_antiburn_status);
...
int tcpci_notify_recv_src_cap(struct tcpc_device *tcpc)
{
	struct tcp_notify tcp_noti;

	memset(&tcp_noti, 0, sizeof(tcp_noti));
	return tcpc_check_notify_time(tcpc, &tcp_noti,
	TCP_NOTIFY_IDX_MISC, TCP_NOTIFY_RECV_SRC_CAP);
}
EXPORT_SYMBOL(tcpci_notify_recv_src_cap);
...
int tcpci_dp_configure(struct tcpc_device *tcpc, uint32_t dp_config)
{
	...
	mca_log_info("pin assignment: 0x%x\n",
...
{
	/* DFP_U : Not call this function during internal flow */
	mca_log_info("Status1: 0x%x, ack=%d\n", dp_status, ack);
	return 0;
}
	...
	uint32_t local_cfg, uint32_t remote_cfg, bool ack)
{
	 ...
	 */
	mca_log_info("ConfigDone, L:0x%x, R:0x%x, ack=%d\n",
...
int tcpci_dc_notify_en_unlock(struct tcpc_device *tcpc)
{
	...
	mca_log_info("DirectCharge en_unlock\n");
	ret = tcpc_check_notify_time(tcpc, &tcp_noti,
