...
static inline int tcpci_force_cc_toggle(struct tcpc_device *tcpc)
{
	return tcpc->tcpc_flags & TCPC_FLAGS_FORCE_TOGGLE;
}

static inline int tcpci_skip_check_vbus(struct tcpc_device *tcpc)
{
	return tcpc->tcpc_flags & TCPC_FLAGS_SKIP_CHECK_VBUS;
}

static inline int tcpci_check_vbus_valid(struct tcpc_device *tcpc)
{
	return tcpci_skip_check_vbus(tcpc) ||
