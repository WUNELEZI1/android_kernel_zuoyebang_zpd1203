...
extern int tcpci_ops_get_antiburn_status(struct tcpc_device *tcpc, bool en);
...
extern int tcpm_set_pd_charging_policy_and_request(struct tcpc_device *tcpc,
	uint8_t policy, int mv, int ma, const struct tcp_dpm_event_cb_data *cb_data);
...
static inline int tcpm_set_pd_charging_policy_and_request(struct tcpc_device *tcpc,
	uint8_t policy, int mv, int ma, const struct tcp_dpm_event_cb_data *cb_data)
{
	return TCPM_ERROR_NO_IMPLEMENT;
}
