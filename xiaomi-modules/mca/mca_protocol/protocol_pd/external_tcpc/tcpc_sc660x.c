...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_tcpc"
#endif
...
struct sc660x {
    ...
    uint16_t chip_pid;
    uint16_t chip_vid;
	unsigned int xr_quirk_suspend_keep_irq : 1;
};
...
static int sc660x_read_device(void *client, u32 reg, int len, void *dst)
{
    ...
    while (1) {
        ...
        t2 = local_clock();
        mca_log_info("%s del = %lluus, reg = 0x%02X, len = %d, val = 0x%08X\n",
...
static int sc660x_write_device(void *client, u32 reg, int len, const void *src)
{
    ...
    while (1) {
        ...
        t2 = local_clock();
        mca_log_info("%s del = %lluus, reg = %02X, len = %d, val = 0x%08X\n",
                __func__, (t2 - t1) / NSEC_PER_USEC, reg, len, *(u16 *)src);
        if (ret < 0 && count > 1)
            count--;
        else
            break;
...
#endif /* CONFIG_RT_REGMAP */
    if (ret < 0) {
        mca_log_err("sc660x reg read fail\n");
        return ret;
    }
    return val;
}
...
#endif /* CONFIG_RT_REGMAP */
    if (ret < 0)
        mca_log_err("sc660x reg write fail\n");
    return ret;
}
            ...
            u8 reg, int len, void *dst)
{
    ...
    if (ret < 0)
        mca_log_err("sc660x block read fail\n");
    return ret;
}
...
#endif /* #ifdef CONFIG_RT_REGMAP */
    if (ret < 0)
        mca_log_err("sc660x block write fail\n");
    return ret;
}
...
static int sc660x_regmap_init(struct sc660x *sc)
{
    ...
    if (!sc->m_dev) {
        mca_log_err("sc660x rt_regmap register fail\n");
        return -EINVAL;
    }
#endif
    return 0;
...
static int sc660x_init_alert(struct tcpc_device *tcpc)
{
    ...
    if (ret < 0) {
        mca_log_err("%s set GPIO fail(%d)\n", __func__, ret);
        return ret;
    }

    ret = gpio_to_irq(sc->irq_gpio);
    if (ret < 0) {
        mca_log_err("%s gpio to irq fail(%d)",
                    __func__, ret);
        return ret;
    }
    sc->irq = ret;

    mca_log_info("%s IRQ number = %d\n", __func__, sc->irq);
    ...
    ret = devm_request_threaded_irq(sc->dev, sc->irq, NULL,
                    ...
                    name, sc);
    if (ret < 0) {
        mca_log_err("%s request irq fail(%d)\n",
                    __func__, ret);
        return ret;
    }
...
static int sc660x_get_typec_antiburn_status(struct tcpc_device *tcpc, bool en)
{
	int ret = 0;
	tcpc->anti_is_trigger = en;
	return ret;
}
...
#endif	/* CONFIG_USB_POWER_DELIVERY */
	.get_antiburn_status = sc660x_get_typec_antiburn_status,
};
...
static int sc660x_parse_dt(struct sc660x *sc, struct device *dev)
{
	...
	sc->xr_quirk_suspend_keep_irq =
		of_property_present(np, "sc-tcpc,xr,quirk-suspend-keep-irq");
	if (sc->xr_quirk_suspend_keep_irq)
		mca_log_info("%s [QUIRK]:keep irq enable while suspend\n", __func__);

	sc->irq_gpio = of_get_named_gpio(np, "sc660x,intr_gpio_num", 0);
	if (!gpio_is_valid(sc->irq_gpio)) {
		mca_log_err("no intr_gpio info\n");
		return -EINVAL;
	}
	return ret;
}
...
static int sc660x_tcpcdev_init(struct sc660x *sc, struct device *dev)
{
    ...
    if (of_property_read_u32(np, "sc-tcpc,role_def", &val) >= 0) {
        ...
        mca_log_info("use default Role DRP\n");
        desc->role_def = TYPEC_ROLE_DRP;
    }
    ...
    if (of_property_read_u32(np, "sc-tcpc,vconn_supply", &val) >= 0) {
        ...
        mca_log_info("use default VconnSupply\n");
        desc->vconn_supply = TCPC_VCONN_SUPPLY_ALWAYS;
    }
    ...
    mca_log_info("PD_REV30\n");
#endif	/* CONFIG_USB_PD_REV30 */
    sc->tcpc->tcpc_flags |= TCPC_FLAGS_ALERT_V10;
    sc->tcpc->tcpc_flags |= TCPC_FLAGS_DISABLE_LEGACY;

	if (of_property_present(np, "sc-tcpc,xr,quirk-skip-vbus-check")) {
		mca_log_info("quirk skip check vbus\n");
		sc->tcpc->tcpc_flags |= TCPC_FLAGS_SKIP_CHECK_VBUS;
	}
	if (of_property_present(np, "sc-tcpc,xr,quirk-force-cc-toggle")) {
		mca_log_info("quirk force cc toggle\n");
		sc->tcpc->tcpc_flags |= TCPC_FLAGS_FORCE_TOGGLE;
	}
...
static inline int sc660x_check_revision(struct i2c_client *client)
{
    ...
    if (ret < 0) {
        mca_log_err("read chip ID fail(%d)\n", ret);
        return -EIO;
    }

    if (vid != SOUTHCHIP_PD_VID) { 
        mca_log_err("%s failed, VID=0x%04x\n", __func__, vid);
        return -ENODEV;
    }
    sc->chip_vid = vid;
    ...
    if (ret < 0) {
        mca_log_err("read product ID fail(%d)\n", ret);
        return -EIO;
    }

    if (pid != SC660X_PID) {
        mca_log_err("%s failed, PID=0x%04x\n", __func__, pid);
        return -ENODEV;
    }
    sc->chip_pid = pid;
    ...
    if (ret < 0) {
        mca_log_err("read device ID fail(%d)\n", ret);
        return -EIO;
    }
...
#endif
{
    ...
    if (i2c_check_functionality(client->adapter,
        ...
        mca_log_info("I2C functionality : OK...\n");
    else
        mca_log_info("I2C functionality check : failuare...\n");
    ...
    sc->chip_id = chip_id;
    mca_log_info("chip info [0x%0x,0x%0x,0x%0x]\n", sc->chip_vid, 
    ...
    if (use_dt) {
        ...
        mca_log_err("no dts node\n");
        return -ENODEV;
    }

    ret = sc660x_regmap_init(sc);
    if (ret < 0) {
        mca_log_err("sc660x regmap init fail\n");
        goto err_regmap_init;
    }

    ret = sc660x_tcpcdev_init(sc, &client->dev);
    if (ret < 0) {
        mca_log_err("sc660x tcpc dev init fail\n");
        goto err_tcpc_reg;
    }

    ret = sc660x_init_alert(sc->tcpc);
    if (ret < 0) {
        mca_log_err("sc660x init alert fail\n");
        goto err_irq_init;
    }
    mca_log_info("%s probe OK!\n", __func__);
    return 0;
...
static int sc660x_i2c_suspend(struct device *dev)
{
	...
	if (likely(!sc->xr_quirk_suspend_keep_irq))
		disable_irq(sc->irq);
