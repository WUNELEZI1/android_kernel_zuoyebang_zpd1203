...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_tcpci_core"
#endif
					...
					const char *buf, size_t count)
{
	...
	case TCPC_DESC_ROLE_DEF:
		...
		if (ret < 0) {
			mca_log_err("get parameters fail\n");
			return -EINVAL;
		}
...
static void tcpc_device_release(struct device *dev)
{
	...
	mca_log_info("%s : %s device release\n", __func__, dev_name(dev));
	PD_BUG_ON(tcpc == NULL);
	/* Un-init pe thread */
	...
	struct tcpc_desc *tcpc_desc, struct tcpc_ops *ops, void *drv_data)
{
	...
	mca_log_info("%s register tcpc device (%s)\n", __func__, tcpc_desc->name);
	tcpc = devm_kzalloc(parent, sizeof(*tcpc), GFP_KERNEL);
	if (!tcpc) {
		mca_log_err("%s : allocate tcpc memory failed\n", __func__);
		return NULL;
	}
	...
	if (ret < 0)
		mca_log_err(&tcpc->dev, "dual role usb init fail\n");
...
int tcpc_device_irq_enable(struct tcpc_device *tcpc)
{
	...
	if (!tcpc->ops->init) {
		mca_log_err("%s Please implment tcpc ops init function\n",
			  __func__);
		return -EINVAL;
	}
	...
	tcpci_unlock_typec(tcpc);
	if (ret < 0) {
		mca_log_err("%s : tcpc typec init fail\n", __func__);
		return ret;
	}

	schedule_delayed_work(&tcpc->event_init_work, 0);

	mca_log_info("%s : tcpc irq enable OK!\n", __func__);
	return 0;
}
...
static void tcpc_event_init_work(struct work_struct *work)
{
	...
	if (!tcpc->usb_psy) {
		tcpci_unlock_typec(tcpc);
		mca_log_err("%s get usb psy fail\n", __func__);
		return;
	}
#endif /* CONFIG_USB_PD_WAIT_BC12 */
	tcpc->pd_inited_flag = 1;
	mca_log_info("%s typec attach new = %d\n",
	...
	if (!tcpc->bat_psy) {
		mca_log_err("%s get battery psy fail\n", __func__);
		return;
	}
	tcpc->charging_status = BSDO_BAT_INFO_IDLE;
	tcpc->bat_soc = 0;
	...
	if (retval < 0)
		mca_log_err("%s register power supply notifier fail\n", __func__);
#endif /* CONFIG_USB_PD_REV30 */
	...
	struct notifier_block *nb, uint8_t idx)
{
	...
	if (retval < 0)
		mca_log_err("Failed to add resource to manager(%d)\n", retval);
...
static int __init tcpc_class_init(void)
{
	...
	if (IS_ERR(tcpc_class)) {
		mca_log_info("Unable to create tcpc class; errno = %ld\n",
				PTR_ERR(tcpc_class));
		return PTR_ERR(tcpc_class);
	}
	tcpc_init_attrs(&tcpc_dev_type);
	regmap_plat_init();
	mca_log_info("TCPC class init OK\n");
	return 0;
}
...
static void __exit tcpc_class_exit(void)
{
	...
	mca_log_info("TCPC class un-init OK\n");
}
