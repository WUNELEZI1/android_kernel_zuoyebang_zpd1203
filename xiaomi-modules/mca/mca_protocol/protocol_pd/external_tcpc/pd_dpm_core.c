...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_dpm"
#endif
...
int dpm_check_supported_modes(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(svdm_svid_ops) - 1; i++) {
		if (svdm_svid_ops[i + 1].svid <= svdm_svid_ops[i].svid) {
			is_disorder = true;
			break;
		}
		mca_log_info("SVDM supported mode [%zu]: name = %s, svid = 0x%x\n",
			i, svdm_svid_ops[i].name,
			svdm_svid_ops[i].svid);
	}
	mca_log_info("%s : found \"disorder\"...\n", __func__);
...
void pd_dpm_inform_cable_id(struct pd_port *pd_port, bool src_startup)
{
	...
	if (payload) {
		...
		mca_log_debug("InformCable, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
		...
		struct pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	for (i = 0; i < snk_cap->nr; i++) {
		...
		if (find_cap) {
			...
			mca_log_debug("Find SrcCap%d(%s):%d mw\n",
		...
		struct pd_port *pd_port, struct dpm_rdo_info_t *req_info)
{
	...
	mca_log_info("Policy=0x%X\n", charging_policy);

	for (i = 0; i < src_cap->nr; i++)
		mca_log_info("SrcCap%d: 0x%08x\n", i+1, src_cap->pdos[i]);
	...
	struct pd_port *pd_port, struct dpm_rdo_info_t *req_info)
{
	...
	if (req_info->mismatch) {
		...
		mca_log_info("cap miss match case\n");
	}
		...
		struct tcp_dpm_pd_request *pd_req)
{
	...
	if (!find_cap) {
		mca_log_info("Can't find match_cap\n");
	...
	struct tcp_dpm_pd_request_ex *pd_req)
{
	...
	if ((pd_port->dpm_charging_policy & DPM_CHARGING_POLICY_MASK)
		== DPM_CHARGING_POLICY_PPS) {
		mca_log_info("Reject tcp_rqeuest_ex if charging_policy=pps\n");
		return TCP_DPM_RET_DENIED_INVALID_REQUEST;
    }
#endif	/* CONFIG_USB_PD_REV30_PPS_SINK */
...
static uint8_t pd_dpm_build_rdo(struct pd_port *pd_port)
{
	...
	if (!find_cap) {
		mca_log_info("Can't find any SrcCap\n");
		dpm_build_default_request_info(pd_port, &req_info);
	} else
		mca_log_info("Select SrcCap%d\n", req_info.pos);
	...
	struct pd_port *pd_port, uint32_t rdo, uint8_t rdo_pos)
{
	...
	if ((rdo_pos == 0) || (rdo_pos > src_cap->nr)) {
		mca_log_info("RequestPos Wrong (%d)\n", rdo_pos);
		return false;
	}
	...
	if (src_info.ma < op_curr) {
		mca_log_info("src_i (%d) < op_i (%d)\n", src_info.ma, op_curr);
		return false;
	}

	if (rdo & RDO_CAP_MISMATCH) {
		/* TODO: handle it later */
		mca_log_info("CAP_MISMATCH\n");
		pd_port->pe_data.dpm_flags |= DPM_FLAGS_PARTNER_MISMATCH;
	} else if (src_info.ma < max_curr) {
		mca_log_info("src_i (%d) < max_i (%d)\n", src_info.ma, max_curr);
		return false;
	}
	...
	if ((pdo & PDO_TYPE_MASK) == PDO_TYPE_APDO) {
		...
		if ((sink_v < src_info.vmin) || (sink_v > src_info.vmax)) {
			mca_log_info("sink_v (%d) not in src_v (%d~%d)\n",
	...
	struct pd_port *pd_port, uint16_t svid, uint8_t ops)
{
	...
	mca_log_debug("EnterMode (svid0x%04x, ops:%d)\n", svid, ops);
	...
	struct pd_port *pd_port, uint16_t svid, uint8_t ops)
{
	...
	mca_log_debug("ExitMode (svid0x%04x, mode:%d)\n", svid, ops);
	...
	uint16_t svid, uint32_t *mode_list, uint8_t count)
{
	...
	mca_log_debug("InformMode (0x%04x:%d):\n", svid, count);
	for (i = 0; i < count; i++)
		mca_log_debug("Mode[%d]: 0x%08x\n", i, mode_list[i]);
	...
	struct pd_port *pd_port, uint16_t svid, uint8_t ops)
{
	...
	mca_log_debug("EnterMode (svid0x%04x, mode:%d)\n", svid, ops);
...
void pd_dpm_dfp_inform_id(struct pd_port *pd_port, bool ack)
{
	...
	if (ack) {
		mca_log_debug("InformID, 0x%02x, 0x%02x, 0x%02x, 0x%02x\n",
				payload[0], payload[1], payload[2], payload[3]);
	...
	struct pd_port *pd_port, uint32_t *svid_list, uint8_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		mca_log_debug("svid[%d]: 0x%04x 0x%04x\n", i, svid[0], svid[1]);
	...
	if (discover_again) {
		mca_log_debug("DiscoverSVID Again\n");
...
void pd_dpm_dfp_inform_modes(struct pd_port *pd_port, bool ack)
{
	...
	if (ack) {
		...
		if (svid != expected_svid) {
			ack = false;
			mca_log_info("Not expected SVID (0x%04x, 0x%04x)\n",
				svid, expected_svid);
		} else {
			count = pd_get_msg_vdm_data_count(pd_port);
...
void pd_dpm_dfp_inform_exit_mode(struct pd_port *pd_port)
{
	...
	if ((expected_svid != svid) || (expected_ops != ops))
		mca_log_debug("expected_svid & ops wrong\n");
...
void pd_dpm_dfp_inform_attention(struct pd_port *pd_port)
{
	...
	mca_log_debug("Attention (svid0x%04x, mode:%d)\n", svid, ops);
...
void pd_dpm_inform_source_cap_ext(struct pd_port *pd_port)
{
	...
	if (dpm_check_ext_msg_event(pd_port, PD_EXT_SOURCE_CAP_EXT)) {
		...
		mca_log_info("vid=0x%04x, pid=0x%04x\n", scedb->vid, scedb->pid);
		mca_log_info("fw_ver=0x%02x, hw_ver=0x%02x\n",
...
void pd_dpm_inform_mfrs_info(struct pd_port *pd_port)
{
	...
	if (dpm_check_ext_msg_event(pd_port, PD_EXT_MFR_INFO)) {
		...
		mca_log_info("vid=0x%x, pid=0x%x\n", midb->vid, midb->pid);
	}
}
...
void pd_dpm_inform_country_codes(struct pd_port *pd_port)
{
	...
	if (dpm_check_ext_msg_event(pd_port, PD_EXT_COUNTRY_CODES)) {
		...
		mca_log_info("len=%d, country_code[0]=0x%04x\n",
...
void pd_dpm_inform_alert(struct pd_port *pd_port)
{
	...
	mca_log_info("inform_alert:0x%08x\n", data[0]);
...
int pd_dpm_send_alert(struct pd_port *pd_port)
{
	...
	mca_log_info("send_alert:0x%08x\n", ado);
...
void pd_dpm_inform_status(struct pd_port *pd_port)
{
	...
	if (dpm_check_ext_msg_event(pd_port, PD_EXT_STATUS)) {
		...
		mca_log_info("Temp=%d, IN=0x%x, BAT_IN=0x%x, EVT=0x%x, PTF=0x%x\n",
	...
	struct svdm_svid_data *svid_data, const struct svdm_svid_ops *ops)
{
	...
	if (ret) {
		...
		mca_log_debug("register_svdm: 0x%x\n", ops->svid);
	}
