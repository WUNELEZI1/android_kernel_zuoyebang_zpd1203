...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_pd_core"
#endif
	...
	struct pd_battery_info *bat_info)
{
	...
	if (ret < 0) {
		mca_log_err("%s get bat,mfrs fail\n", __func__);
	...
	struct pd_port *pd_port, struct device_node *np)
{
	...
	if (ret < 0) {
		mca_log_err("%s get pd bat NR fail\n", __func__);
		pd_port->bat_nr = 0;
		return 0;
	}

	pd_port->bat_nr = val;
	mca_log_info("%s Battery NR = %d\n", __func__, pd_port->bat_nr);
	...
	if (!pd_port->fix_bat_info) {
		mca_log_err("%s get fix_bat_info memory fail\n", __func__);
		return -ENOMEM;
	}
	...
	for (i = 0; i < pd_port->bat_nr; i++) {
		...
		if (!sub) {
			mca_log_err("%s get sub bat node fail\n", __func__);
			return -ENODEV;
		}
	...
	for (i = 0; i < pd_port->bat_nr; i++) {
		mca_log_info("%s fix_bat_info[%d].mfrs_info.vid = 0x%x, .mfrs_info.pid = 0x%x, .mfrs_string = %s, .bat_design_cap = %d\n",
			__func__, i,
	...
	struct pd_country_authority *country_info)
{
	...
	if (ret < 0) {
		mca_log_err("%s get country code fail\n", __func__);
		return -ENODEV;
	}
	...
	if (ret < 0) {
		mca_log_err("%s get country len fail\n", __func__);
		return -ENODEV;
	}
	...
	if (!country_info->data) {
		mca_log_err("%s get country info data mem fail\n",
			__func__);
		return -ENOMEM;
	}
	...
	ret = of_property_read_u32_array(sub, "pd,country_data",
		...
		country_info->len);
	if (ret < 0)
		mca_log_err("%s get country data fail\n", __func__);
	...
	struct pd_port *pd_port, struct device_node *np)
{
	...
	if (ret < 0) {
		mca_log_err("%s get country nr fail\n", __func__);
		pd_port->country_nr = 0;
		return 0;
	}

	mca_log_info("%s Country NR = %d\n", __func__, pd_port->country_nr);
	...
	if (!pd_port->country_info) {
		mca_log_err("%s get country info memory fail\n", __func__);
		return -ENOMEM;
	}
	...
	for (i = 0; i < pd_port->country_nr; i++) {
		...
		sub = of_find_node_by_name(np, temp_string);
		if (!sub) {
			mca_log_err("%s get sub country node fail\n",
				__func__);
			return -ENODEV;
		}
...
{
	mca_log_info("%s vid = 0x%x, pid = 0x%x, xid = 0x%x, fw_ver = 0x%x, hw_ver = 0x%0x\n",
		__func__,
		cap->vid, cap->pid, cap->xid,
		cap->fw_ver, cap->hw_ver);

	mca_log_info("%s voltage_regulation = %d, hold_time_ms = %d, compliance = 0x%x, touch_current = 0x%x, peak_current = %d %d %d\n",
		__func__,
	...
	mca_log_info("%s touch_temp = %d, source_inputs = 0x%x, batteries = 0x%x, source_pdp = 0x%x\n",
		__func__,
		cap->touch_temp,
		cap->source_inputs,
		cap->batteries,
	...
	struct pd_port *pd_port, struct device_node *np)
{
	...
	if (ret < 0)
		mca_log_err("%s get source-cap-ext fail\n", __func__);
	else
	...
	if (ret < 0)
		mca_log_err("%s get sink-cap-ext fail\n", __func__);
	...
	struct pd_port *pd_port, struct device_node *np)
{
	...
	mca_log_info("%s VID = 0x%x, PID = 0x%x\n", __func__, vid, pid);
	...
	if (ret < 0) {
		mca_log_err("%s get pd mfrs fail\n", __func__);
	...
	mca_log_info("%s PD mfrs_string = %s\n",
...
static int pd_parse_pdata(struct pd_port *pd_port)
{
	...
	if (np) {
		...
		if (ret < 0)
			mca_log_err("%s get source pdo size fail\n", __func__);
		...
		if (ret < 0)
			mca_log_err("%s get id vdo size fail\n", __func__);
		ret = of_property_read_u32_array(np, "pd,id-vdo-data",
			(u32 *)pd_port->id_vdos, pd_port->id_vdo_nr);
		if (ret < 0)
			mca_log_err("%s get id vdo data fail\n", __func__);

		mca_log_info("%s id vdos data =\n", __func__);
		...
		if (of_property_read_u32(np, "pd,charging_policy", &val) < 0)
			mca_log_info("%s get charging policy fail\n", __func__);

		pd_port->dpm_charging_policy = val;
		pd_port->dpm_charging_policy_default = val;
		mca_log_info("%s charging_policy = %d\n", __func__, val);
...
static void fg_bat_absent_work(struct work_struct *work)
{
	...
	if (ret < 0)
		mca_log_err("%s: tcpm shutdown fail\n", __func__);
}
...
int pd_enable_bist_test_mode(struct pd_port *pd_port, bool en)
{
	...
	mca_log_debug("bist_test_mode=%d\n", en);
		...
		uint8_t msg, bool ext, uint16_t count, const uint32_t *data)
{
	...
	if (tcpc->typec_attach_old == 0) {
		mca_log_debug("[SendMsg] Unattached\n");
		return 0;
	}

	if (tcpc->pd_hard_reset_event_pending) {
		mca_log_debug("[SendMsg] HardReset Pending");
		return 0;
	}
		...
		uint32_t timer_id)
{
	...
	if (alert_status & TCPC_REG_ALERT_RX_STATUS) {
		mca_log_debug("RX Busy, stop send VDM\n");
		return 0;
	}
#endif	/* CONFIG_USB_PD_STOP_SEND_VDM_IF_RX_BUSY */
	...
	uint8_t reply, uint8_t cnt, uint32_t *data_obj)
{
	...
	if (alert_status & TCPC_REG_ALERT_RX_STATUS) {
		mca_log_debug("RX Busy, stop reply VDM\n");
		return 0;
	}
#endif	/* CONFIG_USB_PD_STOP_REPLY_VDM_IF_RX_BUSY */

	if (reply != CMDT_RSP_ACK)
		mca_log_info("VDM_NAK_BUSY\n");
	else
		mca_log_info("VDM_ACK\n");
...
bool pd_is_multi_chunk_msg(struct pd_port *pd_port)
{
	...
	if (pd_get_msg_hdr_ext(pd_port)) {
		...
		if (size > MAX_EXTENDED_MSG_CHUNK_LEN) {
			mca_log_info("multi_chunk_msg = TRUE (%d)\n", size);
			return true;
		}
	}
