...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_evt"
#endif
	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (pd_port->pe_data.pe_state_flags &
		...
		mca_log_info("Ignore Unknown Event\n");
		goto out;
	}

	if (pd_check_pe_during_hard_reset(pd_port)) {
		mca_log_info("Ignore Event during HReset\n");
		goto out;
	}
...
bool pd_process_tx_failed(struct pd_port *pd_port)
{
	...
	if (pd_check_pe_state_ready(pd_port) ||
		...
		mca_log_debug("Ignore tx_failed\n");
		return false;
	}
	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	if (!ret)
		mca_log_debug("Ignore not SOP Ctrl Msg\n");
	...
	struct pd_port *pd_port, struct pd_event *pd_event)
{
	...
	mca_log_debug("Trap in idle state, Ignore All MSG (%d:%d)\n",
