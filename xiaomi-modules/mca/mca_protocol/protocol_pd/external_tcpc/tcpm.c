...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_tcpm"
#endif
...
int tcpm_set_pd_charging_policy_and_request(struct tcpc_device *tcpc,
...
EXPORT_SYMBOL(tcpm_set_pd_charging_policy_and_request);
	...
	struct tcpc_device *tcpc, int ret, struct tcp_dpm_event *event)
{
	...
	if (pd_port->tcpm_bk_event_id != event->event_id) {
		mca_log_debug("bk_event_cb_dummy: expect:%d real:%d\n",
