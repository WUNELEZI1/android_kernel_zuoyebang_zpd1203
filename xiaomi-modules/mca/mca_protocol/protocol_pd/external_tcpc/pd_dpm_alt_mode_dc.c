...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc6601a_dpm_dc"
#endif
...
bool dc_dfp_start_en_unlock1(struct pd_port *pd_port)
{
	...
	mca_log_debug("en_unlock1: 0x%x, 0x%x\n", rn_code[0], rn_code[1]);
...
bool dc_dfp_start_en_unlock2(struct pd_port *pd_port)
{
	...
	mca_log_debug("en_unlock2: 0x%x, 0x%x\n", pd_port->dc_pass_code, rn_code);
	...
	case DC_UFP_T0:
		{
			...
			mca_log_info("T0: recv_code: 0x%x, 0x%x\n",
			...
			mca_log_info("T0: reply: 0x%x, 0x%x\n",
	...
	case DC_UFP_T1:
		{
			recv_code[0] = pd_port->uvdm_data[1];
			recv_code[1] = pd_port->uvdm_data[2];
			mca_log_info("T1: recv_code: 0x%x, 0x%x\n",
				recv_code[0], recv_code[1]);

			if (recv_code[0] != pd_port->dc_pass_code) {
				mca_log_info("T1: pass_code error\n");
				reply_cmd[1] = 0;
			} else {
				mca_log_info("T1: pass_code success\n");
				reply_cmd[1] = 1;
			}
