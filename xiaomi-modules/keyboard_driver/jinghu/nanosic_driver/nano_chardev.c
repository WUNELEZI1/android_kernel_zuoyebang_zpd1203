...
static DEFINE_MUTEX(client_registry_mutex);
...
static int Nanosic_chardev_client_register(nanosic_client_t *client)
{
    ...
    mutex_lock(&client_registry_mutex);
...
_exit:
    mutex_unlock(&client_registry_mutex);
 ...
 ** */
int Nanosic_chardev_client_write(char *data, int datalen)
{
...
{
    nanosic_client_t *client = kzalloc(sizeof(nanosic_client_t), GFP_KERNEL);
...
static unsigned int Nanosic_chardev_fops_poll(struct file *file, struct poll_table_struct *wait)
{
    return -ENOSYS;
}
 ...
 ** */
static ssize_t Nanosic_chardev_fops_read(struct file *file, char __user *user, size_t size, loff_t *loff)
{
    Q_buffer_t *buf = NULL;
    int retval = 0;
    uint8_t read_pos;
 ...
 ** */
static ssize_t Nanosic_chardev_fops_write(struct file *file, const char __user *user, size_t size, loff_t *loff)
{
 ...
 *
 ** */
static long Nanosic_chardev_fops_ioctl(struct file *file, unsigned int cmd, unsigned long args)
{
    return -ENOSYS;
}
 ...
 ** */
static char *Nanosic_chardev_devnode(const struct device *dev, umode_t *mode)
{
...
int Nanosic_chardev_register(void)
{
    ...
    if (NULL != (nanosic_cdev_class = class_create(DRV_TOKEN)))
    {
