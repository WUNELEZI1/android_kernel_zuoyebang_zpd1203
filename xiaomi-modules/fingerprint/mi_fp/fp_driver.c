...
static struct device_node *prim_panel;
static void *cookie = NULL;
#if IS_ENABLED(CONFIG_FP_HAVE_MULTI_SCREEN)
static struct device_node *second_panel;
...
static void fp_screen_state_for_fingerprint_callback(enum xring_panel_event_tag notifier_tag,
            struct xring_panel_event_notification *notification, void *client_data)
{
	struct fp_device *fp_dev = client_data;
	if (!fp_dev || !notification || notification->data.early_trigger) {
		pr_err("%s:invalid argument\n", __func__);
		return;
	}

	if (notifier_tag == XRING_PANEL_EVENT_TAG_PRIMARY || notifier_tag == XRING_PANEL_EVENT_TAG_SECONDARY) {
...
static void fp_register_panel_notifier_work(struct work_struct *work)
{
	...
	if (prim_panel) {
		...
		if (!cookie) {
			cookie = xring_panel_event_notifier_register(XRING_PANEL_EVENT_TAG_PRIMARY,
					XRING_PANEL_EVENT_CLIENT_PRIMARY_FINGERPRINT, prim_panel,
	...
	if (sec_panel) {
		...
		if (!cookie_sec) {
			cookie_sec = xring_panel_event_notifier_register(XRING_PANEL_EVENT_TAG_SECONDARY,
					XRING_PANEL_EVENT_CLIENT_SECONDARY_FINGERPRINT, sec_panel,
					fp_screen_state_for_fingerprint_callback, (void*)fp_dev);
...
static long fp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case FP_IOC_REGULATOR_ENABLE_PWM:
			pr_debug( "FP_IOC_REGULATOR_ENABLE_PWM ======\n" );
#ifdef FP_REGULATOR_PWM_CONTROL
			fp_enable_regulator_mode_pwm(fp_dev);
#endif
			pr_debug( "FP_IOC_REGULATOR_ENABLE_PWM exit ======\n" );
			break;

		case FP_IOC_REGULATOR_DISABLE_PWM:
			pr_debug( "FP_IOC_REGULATOR_DISABLE_PWM ======\n" );
#ifdef FP_REGULATOR_PWM_CONTROL
			fp_disable_regulator_mode_pwm(fp_dev);
#endif
			pr_debug( "FP_IOC_REGULATOR_DISABLE_PWM exit ======\n" );
			break;

		case FP_IOC_ACQUIRE_WAKELOCK:
			pr_debug( "FP_IOC_ACQUIRE_WAKELOCK ======\n" );
		...
		case FP_IOC_RELEASE_WAKELOCK:
			pr_debug( "FP_IOC_RELEASE_WAKELOCK ======\n" );
			if (atomic_read(&fp_dev->wakelock_acquired) == 0)
				break;
		...
		case FP_IOC_WAKEUP_SYSTEM:
			pr_debug( "FP_IOC_WAKEUP_SYSTEM ======\n" );
			if(copy_from_user(&wakeup_time, (u_int32_t *)arg, sizeof(u_int32_t))) {
				pr_info("%s: FP_IOC_WAKEUP_SYSTEM failed.\n", __func__);
				retval = -EFAULT;
				break;
			}
			if (wakeup_time == 0) {
				pr_err("FP_IOC_WAKEUP_SYSTEM: wakeup_time is 0.\n");
				retval = -EFAULT;
			} else {
				if (fp_wakesrc != NULL) {
				    __pm_wakeup_event(fp_wakesrc, wakeup_time);
				} else {
					pr_err("FP_IOC_WAKEUP_SYSTEM: fp_wakesrc is NULL.\n");
					retval = -EFAULT;
				}
			}
			break;
		...
		case FP_IOC_INTR2_OPERATION:
			if(copy_from_user((char *)vendor_name, (char *)arg, 10*sizeof(char))) {
				pr_info("%s: FP_IOC_INTR2_OPERATION failed.\n", __func__);
				retval = -EFAULT;
				break;
			}
			pr_debug( "FP_IOC_INTR2_OPERATION %s ======\n", vendor_name);

			if (!strcmp(vendor_name, "down")) {
				pinctrl_select_state(fp_dev->pinctrl, fp_dev->pins_reset_high);
			} else if (!strcmp(vendor_name, "up")) {
				pinctrl_select_state(fp_dev->pinctrl, fp_dev->pins_reset_low);
			} else {
				pr_err("%s is unknown operation name", vendor_name);
				retval = -EPERM;
			}
			break;
...
#ifdef FP_ULTRA_QCOM
	fp_dev->input->phys = NULL;
	fp_dev->input->id.bustype = BUS_HOST;
	fp_dev->input->id.version = QBT_INPUT_DEV_VERSION;

	fp_dev->input->evbit[0] = BIT_MASK(EV_KEY) |  BIT_MASK(EV_ABS);
	fp_dev->input->keybit[BIT_WORD(BTN_TOUCH)] = BIT_MASK(BTN_TOUCH);

	fp_dev->input->keybit[BIT_WORD(KEY_HOMEPAGE)] |=
		BIT_MASK(KEY_HOMEPAGE);
	fp_dev->input->keybit[BIT_WORD(KEY_VOLUMEDOWN)] |=
		BIT_MASK(KEY_VOLUMEDOWN);
	fp_dev->input->keybit[BIT_WORD(KEY_POWER)] |=
		BIT_MASK(KEY_POWER);

	input_set_abs_params(fp_dev->input, ABS_X,
			     0,
			     1000,
			     0, 0);
	input_set_abs_params(fp_dev->input, ABS_Y,
			     0,
			     1000,
			     0, 0);
#else
...
#endif
{
	...
	if (prim_panel && cookie) {
		of_node_put(prim_panel);
		prim_panel = NULL;
		xring_panel_event_notifier_unregister(cookie);
	} else {
		pr_err("%s:prim_panel_event_notifier_unregister falt\n", __func__);
	}
#if IS_ENABLED(CONFIG_FP_HAVE_MULTI_SCREEN)
	if (sec_panel && cookie_sec) {
		of_node_put(sec_panel);
		sec_panel = NULL;
		xring_panel_event_notifier_unregister(cookie_sec);
	} else {
...
#ifdef FP_REGULATOR_PWM_CONTROL
	regulator_put(fp_dev->extension_regulator1); //释放   mldo7 regulator
	regulator_put(fp_dev->extension_regulator2); //释放   mldo8 regulator
#endif
...
{
	int rc = 0;
	struct fp_device *fp_dev = platform_get_drvdata(pdev);
