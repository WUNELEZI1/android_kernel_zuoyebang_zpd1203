...
#define DTS_PINCTL_LS_LOW			    "ls_low"  //level shift low
#define DTS_PINCTL_SPI_DEFAULT          "spi_default"
...
int fp_parse_dts(struct fp_device *fp_dev)
{
	...
	if (node) {
		...
		fp_dev->extension_regulator1 = regulator_get(&fp_dev->driver_device->dev,"extension1_vreg");
		fp_dev->extension_regulator2 = regulator_get(&fp_dev->driver_device->dev,"extension2_vreg");
		fp_dev->extension_regulator3 = regulator_get(&fp_dev->driver_device->dev,"extension3_vreg");
#endif
	...
	if (fp_dev->driver_device) {
		...
		if (IS_ERR(fp_dev->pins_reset_low)) {
			...
			pr_info("reset pins output low\n");
		}

		fp_dev->pins_ls_low =
			pinctrl_lookup_state(fp_dev->pinctrl, DTS_PINCTL_LS_LOW);
		if (IS_ERR(fp_dev->pins_ls_low)) {
			ret = PTR_ERR(fp_dev->pins_ls_low);
			pr_debug("can't find  pinctrl ls_low\n");
			// return ret;
		} else {
			pinctrl_select_state(fp_dev->pinctrl, fp_dev->pins_ls_low);
			pr_info("level shift pins output low\n");
		}
		fp_dev->pins_spiio_spi_mode =
			pinctrl_lookup_state(fp_dev->pinctrl, DTS_PINCTL_SPI_DEFAULT);
		if (IS_ERR(fp_dev->pins_spiio_spi_mode)) {
			ret = PTR_ERR(fp_dev->pins_spiio_spi_mode);
			pr_debug("can't find  pinctrl spi_default\n");
			// return ret;
		} else {
			pinctrl_select_state(fp_dev->pinctrl, fp_dev->pins_spiio_spi_mode);
			pr_info("spi_default select\n");
		}
		fp_dev->pins_irq_low =
			pinctrl_lookup_state(fp_dev->pinctrl, DTS_PINCTL_IRQ_DEFAULT);
		if (IS_ERR(fp_dev->pins_irq_low)) {
			ret = PTR_ERR(fp_dev->pins_irq_low);
			pr_debug("can't find  pinctrl irq_default\n");
			// return ret;
		} else {
			pinctrl_select_state(fp_dev->pinctrl, fp_dev->pins_irq_low);
			pr_info("irq_default select\n");
		}
...
#ifdef FP_REGULATOR_PWM_CONTROL
void fp_enable_regulator_mode_pwm(struct fp_device *fp_dev) {
	int ret;

	ret = regulator_enable(fp_dev->extension_regulator1);
	if(ret) {
		printk("mldo7 regulator enable error!");
	}

	ret = regulator_enable(fp_dev->extension_regulator2);
	if(ret) {
		printk("mldo8 regulator enable error!");
	}
    
	ret = xr_bob_fpwm_set_enable(fp_dev->extension_regulator3, 1);
	if(ret) {
		printk("fpwm set error!");
	}

	ret = regulator_disable(fp_dev->extension_regulator1);
	if(ret) {
		printk("mldo7 regulator disable error!");
	}

	ret = regulator_disable(fp_dev->extension_regulator2);
	if(ret) {
		printk("mldo8 regulator disable error!");
	} 
}

void fp_disable_regulator_mode_pwm(struct fp_device *fp_dev) {
	int ret;
	...
	ret = xr_bob_fpwm_set_enable(fp_dev->extension_regulator3, 0);
	if(ret) {
		printk("fpwm set error!");
	}
