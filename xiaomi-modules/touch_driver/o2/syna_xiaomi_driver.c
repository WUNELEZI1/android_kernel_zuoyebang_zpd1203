...
#ifdef TOUCH_SEC_DP_PANEL
	for (i = 0; i < xiaomi_bdata.config_array_size; i++) {
		if (xiaomi_driver_data.lockdown_info[1] == xiaomi_bdata.config_array[i].dp_vendor)
			break;
	}
#else
	for (i = 0; i < xiaomi_bdata.config_array_size; i++) {
...
int syna_set_thermal_temp(int temp, bool force)
{
	...
	LOGI("temp: %d", temp);
	syna_set_ic_mode(SET_THP_TEMPERATURE, restemp);
...
static void syna_tcm_set_cur_value(int mode, int *value)
{
	...
	if (mode == DATA_MODE_67) {
		//TODO
		return;
	}
	if (mode == DATA_MODE_122) {
		syna_set_ic_mode(UPDATE_IDLE_BASELINE, value);
		return;
	}
	if (mode == DATA_MODE_137) {
