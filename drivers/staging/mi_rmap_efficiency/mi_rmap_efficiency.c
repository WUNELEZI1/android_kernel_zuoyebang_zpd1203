...
static void mi_folio_check_bypass(void *data, struct folio *folio, s8 priority,
				 unsigned long nr_to_scan, int lru, bool *bypass)
{
	int mapcount = 0;
	int inactivate_thres = 0;
	...
	inactivate_thres = get_dynamic_mapcount_thres(priority);

	mapcount = folio_mapcount(folio);

	if (priority < DEF_PRIORITY - 2 && mapcount >= inactivate_thres) {
		*bypass = true;
		return;
	}
...
static int __init mi_rmap_efficiency_init(void)
{
	...
	register_trace_android_vh_folio_referenced_check_bypass(mi_folio_check_bypass, NULL);
...
static void __exit mi_rmap_efficiency_exit(void)
{
	...
	unregister_trace_android_vh_folio_referenced_check_bypass(mi_folio_check_bypass, NULL);
