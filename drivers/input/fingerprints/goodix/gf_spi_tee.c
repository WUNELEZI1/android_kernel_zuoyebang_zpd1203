...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
    ...
    if (node) {
        /*
    virq_num = of_get_named_gpio(node, "int-gpios", 0);
    virq = gpio_to_irq(virq_num);
    gf_debug(ERR_LOG, "[gf][goodix_test] %s virq = %d  virq_num = %d\n", __func__,
         virq,virq_num);
    ...
    */
    ...
    gf_dev->pins_spi_cs_high = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fpsensor_fpc_cs_high");
    ...
    gf_dev->pins_spi_cs_low = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fpsensor_fpc_cs_low");
    ...
    */
    ...
    if (status < 0) {
        gf_debug(ERR_LOG, "%s can't regulator_set_voltage\n", __func__);
        return -EPERM;
    }
    ret = regulator_get_voltage(gf_dev->vreg);
    gf_debug(ERR_LOG, "regulator_value %d!!\n", ret);
    /*
    ...
    */
    gf_debug(DEBUG_LOG, "%s, get pinctrl and regulator success!\n", __func__);
#endif
    return 0;
}
...
static int gf_hw_power_disable(struct gf_device *gf_dev)
{
    ...
    if (regulator_is_enabled(gf_dev->vreg) <= 0) {
        ...
        pr_info("%s, regulator_disable return rc valuie = :%d !\n", __func__, rc);
        pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_reset_low);
    }
    // voltage = regulator_get_voltage(gf_dev->vreg);
    enabled = regulator_is_enabled(gf_dev->vreg);
    pr_info("%s, power enabled:%d \n", __func__, enabled);
    return rc;
}
                                   ...
                                   unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && event == DRM_EVENT_BLANK && gf_dev) {
        ...
        case DRM_BLANK_UNBLANK:
            gf_dev->fb_black = 0;
            pr_err("[XMFP]: [%s] : lcd on notify\n", __func__);
            gf_netlink_send(gf_dev, GF_NETLINK_SCREEN_ON);
            break;
        case DRM_BLANK_POWERDOWN:
            gf_dev->fb_black = 1;
            gf_dev->wait_finger_down = true;
            pr_err("[XMFP]: [%s] : lcd off notify\n", __func__);
            gf_netlink_send(gf_dev, GF_NETLINK_SCREEN_OFF);
            break;
        default:
            pr_err("[XMFP]: [%s] : other notifier, ignore\n", __func__);
            break;
        }
    }
#else
...
#else
        ret = mtk_disp_notifier_register("gf_fb_notifier", &gf_dev->notifier);
        //ret = fb_register_client(&gf_dev->notifier);
        if (ret)
            gf_debug(ERR_LOG, "gf_fb_register_client fail.\n");
        else
            gf_debug(INFO_LOG, "gf_fb_register_client success.\n");
#endif
#endif
        gf_dev->sig_count = 0;
...
int gf_spi_read_bytes(struct gf_device *gf_dev, u16 addr, u32 data_len, u8 *rx_buf)
{
    ...
    *(tmp_buf + 2) = (u8)(addr & 0xFF);
    ...
    xfer[0].delay.value = 5;
    spi_message_add_tail(&xfer[0], &msg);
    err = spi_sync(gf_dev->spi, &msg);
    if (err)
        dev_err(&gf_dev->spi->dev, "%s : spi_sync failed.\n", __func__);
...
static int gf_probe(struct platform_device *pdev)
{
...
#endif
#ifndef CONFIG_SPI_MT65XX_MODULE
    gf_debug(INFO_LOG, "%s, CONFIG_SPI_MT65XX enter==========\n", __func__);
    ...
    gf_dev->spi->bits_per_word = 8;
    gf_dev->spi->max_speed_hz = 1 * 1000 * 1000;
    gf_dev->spi->cs_setup.value = 1;
    gf_dev->spi->cs_setup.unit = 0;
    gf_dev->spi->cs_hold.value = 1;
    gf_dev->spi->cs_hold.unit = 0;
    gf_dev->spi->cs_inactive.value = 1;
    gf_dev->spi->cs_inactive.unit = 0;
    ret = spi_setup(gf_dev->spi);
    if (ret < 0) {
        gf_debug(INFO_LOG, "%s, Failed to perform SPI setup\n", __func__);
        //goto err_spi_setup;
    }
    */
...
static int check_hwid(struct spi_device *spi)
{
    ...
    do {
        ...
        gf_spi_read_bytes(gf_dev, 0x0000, 4, rx_test);
        gf_debug(INFO_LOG, "%s rx_test chip id:0x%x 0x%x 0x%x 0x%x \n", __func__, rx_test[0], rx_test[1], rx_test[2], rx_test[3]);
        if ((rx_test[0] != 0x06) || (rx_test[1] != 0x01) || (rx_test[2] != 0x02) || (rx_test[3] != 0x01)) {
...
err_buf:
    ...
    devm_pinctrl_put(gf_dev->pinctrl_gpios);
    //gf_hw_power_disable(gf_dev);
    status = regulator_disable(gf_dev->vreg);
    pr_info("%s, regulator_disable return status valuie = :%d !\n", __func__, status);
    gf_spi_clk_enable(gf_dev, 0);
    kfree(gf_dev->spi_buffer);
    mutex_destroy(&gf_dev->buf_lock);
    mutex_destroy(&gf_dev->release_lock);
