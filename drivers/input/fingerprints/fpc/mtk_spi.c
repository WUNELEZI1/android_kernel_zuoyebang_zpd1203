...
EXPORT_SYMBOL_GPL(goodix_fp_exist);
EXPORT_SYMBOL_GPL(global_spi);
EXPORT_SYMBOL_GPL(spi_fingerprint);
EXPORT_SYMBOL_GPL(fpc1022_fp_exist);
                          ...
                          char *buffer)
{
    ...
    mutex_lock(&fpc->fb_lock);
    value = fpc->fb_black ? 1 : 0;
    mutex_unlock(&fpc->fb_lock);
                 ...
                 unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && event == DRM_EVENT_BLANK) {
        ...
        pr_info("[XMFP]: [info] %s blank = 0x%x\n", __func__, blank);
        switch (blank) {
        case DRM_BLANK_POWERDOWN:
            fpc->fb_black = true;
            sysfs_notify(&fpc->dev->kobj, NULL, dev_attr_screen.attr.name);
            pr_info("[XMFP]: [info] %s lcd off notify\n", __func__);
            break;
        case DRM_BLANK_UNBLANK:
            fpc->fb_black = false;
            pr_info("[XMFP]: [info] %s lcd on notify\n", __func__);
            break;
        default:
...
static int mtk6765_probe(struct spi_device *spidev)
{
    ...
    */
    ...
    fpc->spidev->cs_setup.value = 100;
    fpc->spidev->cs_setup.unit = 1;
    fpc->spidev->cs_hold.value = 100;
    fpc->spidev->cs_hold.unit = 1;
    fpc->spidev->cs_inactive.value = 2;
    fpc->spidev->cs_inactive.unit = 0;
    ret = spi_setup(fpc->spidev);
    if (ret < 0) {
        dev_err(dev, "Failed to perform SPI setup\n");
        // goto err_spi_setup;
    }
    ...
    */
    ...
    // power on
    buck = devm_regulator_get(dev, "VFP");

    if (buck == NULL) {
        dev_err(dev, "failed to get VFP-supply!!\n");
        goto error_regulator_enable;
    } else {
        ret = regulator_set_voltage(buck, 3000000, 3000000);
        if (ret < 0) {
    ...
    irq_num = gpio_to_irq(fpc->irq_gpio); /*get irq num*/
    if (!irq_num) {
        rc = -EINVAL;
...
early_initcall(fpc_sensor_init);
