...
static int fts_enter_proximity_mode(int mode)
{
    ...
    if (mode) {
        buf_value = 0x01;
        set_prox_lcd_reset_gpio_keep_high(true, 1);
        tp_proximity_event = 1;
    } else {
        buf_value = 0x00;
        set_prox_lcd_reset_gpio_keep_high(false, 1);
...
int fts_proximity_readdata(struct fts_ts_data *ts_data)
{
    ...
    if (regvalue == 0x01 ||regvalue == 0x02 || regvalue == 0x03) {
        ...
        FTS_DEBUG("regvalue = %x,but fatory_bulid ipi send 3",regvalue);
...
static int fts_inside_insert(struct fts_ts_data *ts_data, struct ts_event *events)
{
    ...
    //FTS_DEBUG("fts lcdrate_flag(%02x)", ts_data->lcdrate_flag);
...
void fts_usb_online_work(struct work_struct *work)
{
    ...
    //FTS_INFO("last_bat_status=%d, pval.intval=%d\n", last_bat_status, pval.intval);
    if (new_usb_online != last_bat_status) {
        ...
        if(!ts_data->fw_loading){
           fts_ts_usb_plugin_handler(current_bat_status);
        }
...
static int fts_ts_resume(struct device *dev)
{
    ...
    fts_ts_set_earphone_mode(ts_data->earphone_mode);
#endif
    FTS_FUNC_EXIT();
    return 0;
}
