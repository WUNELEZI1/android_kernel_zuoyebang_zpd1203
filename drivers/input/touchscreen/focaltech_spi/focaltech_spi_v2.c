...
static int fts_check_lcm_compatibility(const char *expected_lcm)
{
    ...
    if (strstr(videolfb_tag->lcmname, expected_lcm)) {
        ...
        FTS_ERROR("Unsupported LCM: %s", videolfb_tag->lcmname);
    }
...
static int __init fts_ts_spi_init(void)
{
    ...
    FTS_INFO("fts_ts_spi_init start");

/* compare LCM ID  start */
    if (fts_check_lcm_compatibility("dsi_panel_p15_46_03_0b_hd_vdo")) {
        FTS_ERROR("LCM compatibility check failed");
        return ret;
    } else {
