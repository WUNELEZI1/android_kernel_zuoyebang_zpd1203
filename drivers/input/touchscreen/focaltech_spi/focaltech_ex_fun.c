...
int lct_fts_tp_earphone_callback(bool en)
{
    int ret = 0;
    struct fts_ts_data *ts_data = fts_data;
...
int lct_create_procfs(struct fts_ts_data *ts_data)
{
    ...
    ret = init_lct_tp_info("[Vendor]HKC,[FW]unknow,[IC]ft8057m\n");
    if(ret) {
...
#if LCT_TP_EARPHONE_PLUGIN
    ret = tp_earphone_init(lct_fts_tp_earphone_callback);
    if (ret < 0) {
        FTS_ERROR("tp_earphone_init Failed!");
        goto err_init_lct_tp_earphone_fail;

    } else {
        FTS_INFO("tp_earphone_init Succeeded!");
