 ...
 *
 * $Revision: 157588 $
 * $Date: 2025-01-10 15:05:51 +0800 (週五, 10 一月 2025) $
 *
...
int lct_nvt_tp_gesture_callback(bool flag)
{
	...
	if (flag) {
		...
		set_lcd_reset_gpio_keep_high(true, 0);
		NVT_LOG("enable gesture mode\n");
	} else {
		ts->is_gesture_mode = false;
		set_lcd_reset_gpio_keep_high(false, 0);
...
static int last_bat_status = -1;
static int current_bat_status = 0;
static void nvt_usb_online_work(struct work_struct *work)
{
	...
	if (!bat_psy) {
		pr_err("get battery psy failed\n");
		return;
	}
	ret = power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_STATUS, &pval);
	if (ret) {
		pr_err("get battery psy online state failed\n");
		return;
	}
	...
	//NVT_LOG("last_bat_status=%d, pval.intval=%d\n", last_bat_status, pval.intval);
	if (new_usb_online != last_bat_status) {
		...
		nvt_ts_usb_plugin_handler(current_bat_status);
...
info_retry:
	...
	NVT_LOG("tp_info: [Vendor]truly,[FW]0x%02x,[IC]nt36528a\n",ts->fw_ver);
	sprintf(tp_info_buf, "[Vendor]truly,[FW]0x%02x,[IC]nt36528a\n", ts->fw_ver);
...
int32_t nvt_check_proximity(uint8_t input_id, uint8_t *data)
{
	...
	if ((input_id == DATA_PROTOCOL) && (func_type == FUNCPAGE_PROXIMITY)) {
		...
		if (proximity_state == PROXIMITY_ON) {
			...
			tp_ps_data = 0;
			rc = mtk_ipi_send(&scp_ipidev, IPI_OUT_TP_PS, 0, &tp_ps_data, 1, 0);
			NVT_LOG("get proximity off event, level=%d.\n", proximity_level);
		} else {
			// should never go here
...
static int lcm_compatibility_check(const char *expected_lcm)
{
...
/* compare LCM ID  start */
	if (lcm_compatibility_check("dsi_panel_p15_43_02_0a_hd_vdo")) {
		NVT_ERR("LCM compatibility check failed");
		return ret;
	} else {
		NVT_LOG("[Vendor]NVT,[IC]nt36528a");
	}
/* compare LCM ID  end */
