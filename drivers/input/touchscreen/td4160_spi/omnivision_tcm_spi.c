...
static int ovt_tcm_check_dt(struct device *dev, struct device_node *np)
{
	int i;
	int count;
	...
	for (i = 0; i < count; i++) {
		...
		of_node_put(node);
		if (!IS_ERR(panel)) {
			LOGN(dev,"%s, active_tcm_panel find ok\n", __func__);
			active_tcm_panel = panel;
			return 0;
		}
	}
	LOGN(dev,"%s, find panel error exit\n", __func__);
...
static int parse_dt(struct device *dev, struct ovt_tcm_board_data *bdata)
{
	...
	const char *name;
#ifdef CONFIG_DRM_IN_TP_DRIVER
	retval = ovt_tcm_check_dt(dev, np);
...
*/
	...
	spi->max_speed_hz=6000000; //spi 6M
