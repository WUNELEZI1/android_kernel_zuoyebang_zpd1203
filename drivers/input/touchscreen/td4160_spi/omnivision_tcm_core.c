...
char ovt_tp_fw_name[30] = {0};
char ovt_tp_mp_name[30] = {0};

#if WAKEUP_GESTURE
extern void set_lcd_reset_gpio_keep_high(bool en, int module);
#endif
...
extern void set_prox_lcd_reset_gpio_keep_high(bool en, int module);

static int ovt_tcm_mtk_notifier(struct notifier_block *nb,
...
#if WAKEUP_GESTURE
int lct_ovt_tp_gesture_switch_callback(bool flag)
{
	struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
	if (tcm_hcd->in_suspend) {
		tcm_hcd->delay_gesture = true;
		LOGW(tcm_hcd->pdev->dev.parent,"The gesture mode will be %s the next time you wakes up.\n", flag?"enabled":"disbaled");
		return 0;
	}
	set_lct_tp_gesture_status(flag);
	if (flag) {
		tcm_hcd->wakeup_gesture_enabled = true;
		set_lcd_reset_gpio_keep_high(true, 2);
		LOGI(tcm_hcd->pdev->dev.parent,
				"enable gesture mode\n");
	} else {
		tcm_hcd->wakeup_gesture_enabled = false;
		set_lcd_reset_gpio_keep_high(false, 2);
		LOGI(tcm_hcd->pdev->dev.parent,
				"disable gesture mode\n");
	}
	return 0;
}
EXPORT_SYMBOL(lct_ovt_tp_gesture_switch_callback);
#endif

#if TP_EARPHONE_PLUGIN
static int lct_ovt_tp_earphone_callback(bool flag)
{
	struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;

	tcm_hcd->earphone_mode = flag;
	if (tcm_hcd->in_suspend) {
		LOGW(tcm_hcd->pdev->dev.parent,"Earphone mode will be %s the next time you wakes up.\n", tcm_hcd->earphone_mode?"enabled":"disbaled");
		return 0;
	}
	ovt_tcm_set_dynamic_config_host_interface(DC_ENABLE_EAR_PHONE,tcm_hcd->earphone_mode);
	LOGI(tcm_hcd->pdev->dev.parent,
				"%s earphone mode\n", tcm_hcd->earphone_mode?"Enable":"Disable");
	return 0;
}
#endif
...
{
        g_tcm_hcd->enable_irq(g_tcm_hcd, en, true);
        if (!en) touch_free_objects();
        set_lct_tp_work_status(en);
        LOGE(g_tcm_hcd->pdev->dev.parent,"lct_ovt_tp_work_callback Touchpad en:%d\n", en);
        return 0;
}
#ifdef SUPPORT_FACE_DETECT
int lct_ovt_tp_proximity_switch_callback(bool flag)
{
	int rc = 0;
	struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;

	if (tcm_hcd->sensor_tp_fw_status == false && tcm_hcd->sensor_tp_probe_status == false) {
		LOGE(tcm_hcd->pdev->dev.parent,"tcm not allowed to set proximity mode, flag = %d\n", flag);
		return -1;
	}

	rc = ovt_tcm_set_dynamic_config_host_interface(DC_ENABLE_FACE, flag);
	LOGN(tcm_hcd->pdev->dev.parent,"tcm set proximity mode, flag = %d, rc = %d\n", flag, rc);
	if (flag) {
		set_prox_lcd_reset_gpio_keep_high(true,2);
		tcm_hcd->proximity_event = 1;
	}else{
		set_prox_lcd_reset_gpio_keep_high(false,2);
		tcm_hcd->proximity_event = 0;
	}

	mtk_ipi_send(&scp_ipidev, IPI_OUT_TP_PS, 0, 0, 1, 0);
	return 0;
}
#endif
...
int ovt_tcm_set_dynamic_config_host_interface(unsigned char id, unsigned short value)
{
	...
	if ((!tcm_hcd) || (tcm_hcd->ovt_tcm_driver_removing)) {
		LOGN(tcm_hcd->pdev->dev.parent,"tcm shutdown, return\n");
		return 0;
	}
...
static void ovt_tcm_resume_work(struct work_struct *work)
{
	struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
	LOGI(tcm_hcd->pdev->dev.parent,"++\n");
...
static void ovt_tcm_usb_online_work(struct work_struct *work)
{
    int ret;
    struct power_supply *bat_psy;
    struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
    union power_supply_propval pval = {0, };
    int new_usb_online = 0;
    bat_psy = power_supply_get_by_name("battery");
    if (IS_ERR_OR_NULL(bat_psy)) {
        LOGE(tcm_hcd->pdev->dev.parent,"get battery psy failed\n");
        return;
    }
    ret = power_supply_get_property(bat_psy, POWER_SUPPLY_PROP_STATUS, &pval);
    if (ret) {
        LOGE(tcm_hcd->pdev->dev.parent,"get battery psy online state failed\n");
        return;
    }
    ...
    //LOGN(tcm_hcd->pdev->dev.parent,"last_bat_status=%d, pval.intval=%d\n", last_bat_status, pval.intval);
    if (new_usb_online != last_bat_status) {
        current_bat_status = new_usb_online;
        LOGI(tcm_hcd->pdev->dev.parent,"usb_online state has changed, current_online(%d), last_online(%d) \n", current_bat_status, last_bat_status);
        if (tcm_hcd->sensor_tp_fw_status == false) {
			LOGE(tcm_hcd->pdev->dev.parent,"tcm firmware is not ready\n");
			return;
		}
        ovt_tcm_set_dynamic_config_host_interface(DC_CHARGER_CONNECTED, current_bat_status);
        last_bat_status = current_bat_status;
    }
}

static int ovt_tcm_power_usb_notifier_callback(struct notifier_block *nb, unsigned long ev, void *v)
{
    struct ovt_tcm_hcd *tcm_hcd = container_of(nb, struct ovt_tcm_hcd, power_usb_notifier);
    if (ev != PSY_EVENT_PROP_CHANGED)
        return -1;
    queue_work(tcm_hcd->event_wq, &tcm_hcd->online_work);
...
static int ovt_palm_sensor_cmd(int on)
{
	...
	struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
	if (on) {
		ret = ovt_tcm_set_dynamic_config_host_interface(DC_ENABLE_PALM,1);
	} else {
		ret = ovt_tcm_set_dynamic_config_host_interface(DC_ENABLE_PALM,0);
	}
	if (ret < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "Enter %s(%d): set ovt_tcm_set_func_palm_detect_en_state failed, on=%d\n", __func__, __LINE__, on);
		return -EINVAL;
	}
	LOGI(tcm_hcd->pdev->dev.parent, "Enter %s(%d): set ovt_tcm_set_func_palm_detect_en_state success, on=%d\n", __func__, __LINE__, on);
	return 0;
}
static int ovt_palm_sensor_write(int value)
{
	...
	if(tcm_hcd->in_suspend){
		LOGI(tcm_hcd->pdev->dev.parent,"in_suspend is %s\n",__func__);
		return 0;
	}
	if(!value){
		ret = ovt_palm_sensor_cmd(value);
		if (!ret) {
		LOGE(tcm_hcd->pdev->dev.parent, "Enter %s(%d): set ovt_palm_sensor_cmd succeed\n", __func__, __LINE__);
		}
	}
	return ret;
}
		...
		unsigned char *response_code, unsigned int polling_delay_ms)
{
	...
	if (is_romboot_hdl) {
		if (HDL_WR_CHUNK_SIZE) { //force to remaining_length when romboot hdl
...
#if OVT_PM_WAIT_BUS_RESUME_COMPLETE
	if ((tcm_hcd->wakeup_gesture_enabled > 0) && tcm_hcd->in_suspend) {
		pm_wakeup_event(&tcm_hcd->pdev->dev, 5000);
	}
	if (tcm_hcd->dev_pm_suspend) {
		retval = wait_for_completion_timeout(&tcm_hcd->dev_pm_resume_completion, msecs_to_jiffies(500));
		if (!retval) {
			LOGI(tcm_hcd->pdev->dev.parent, "system(bus) can't finished resuming procedure, skip it!\n");
			goto exit;
		}
	}
#endif /* OVT_PM_WAIT_BUS_RESUME_COMPLETE */
		...
		if (irq_freed) {
			...
			if (node) {
				...
				if (retval < 0) {
					...
					LOGE(tcm_hcd->pdev->dev.parent,
						"ok to  create interrupt thread\n");
				}
			} else {
				LOGE(tcm_hcd->pdev->dev.parent,
					"can not find tp isr node\n");
				retval = -1;				
			}
#else
			retval = request_threaded_irq(tcm_hcd->irq, NULL,
...
get_app_info:
	...
	fw_length = sizeof(tcm_hcd->app_info.customer_config_id)/sizeof(tcm_hcd->app_info.customer_config_id[0]);
	fw_version =(uint64_t) (tcm_hcd->app_info.customer_config_id[fw_length-1] << 8) | tcm_hcd->app_info.customer_config_id[0];

	if (0 == strcmp(ovt_tp_fw_name, FW_IMAGE_NAME)) {
		LOGI(tcm_hcd->pdev->dev.parent,
				"tp_info: [Vendor]BOE,[FW]0x%02x,[IC]td4160\n", fw_version);
		sprintf(tp_info_buf, "[Vendor]BOE,[FW]0x%02x,[IC]td4160\n",fw_version);
	} else {
		LOGE(tcm_hcd->pdev->dev.parent,
				"unkown firmware name!!");
...
static int ovt_tcm_do_resume(struct ovt_tcm_hcd *tcm_hcd)
{
	...
	if (tcm_hcd->in_hdl_mode) {
		...
		if (!tcm_hcd->wakeup_gesture_enabled && get_lct_tp_work_status())
			tcm_hcd->enable_irq(tcm_hcd, true, NULL);
#ifdef SUPPORT_FACE_DETECT
	if (tcm_hcd->proximity_event) {
		if (!tcm_hcd->enable_irq(tcm_hcd, false, true)) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to disable irq irq:%d\n", tcm_hcd->irq);
		}
		ovt_tcm_set_dynamic_config_host_interface(DC_ENABLE_FACE, true);
	}
#endif

#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
	if (ovt_palm_sensor_switch) {
		LOGI(tcm_hcd->pdev->dev.parent, "Enter %s(%d): palm sensor OFF, switch to ON\n", __func__, __LINE__);
		update_palm_sensor_value(0);
...
#ifdef MTK_PLATFORM
static void ovt_mtk_tcm_suspend(struct device *dev)
{
	struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
	ovt_tcm_do_suspend(tcm_hcd);
}
static void ovt_mtk_tcm_resume(struct device *dev)
{
	struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
...
#if defined(CONFIG_PM_IN_TP_DRIVER) || defined(CONFIG_DRM_IN_TP_DRIVER) || defined(CONFIG_FB_IN_TP_DRIVER)
...
static int ovt_tcm_suspend(struct device *dev)
{
	int retval;
	struct ovt_tcm_hcd *tcm_hcd = dev_get_drvdata(dev);
	LOGI(tcm_hcd->pdev->dev.parent,"START\n");
	retval = ovt_tcm_do_suspend(tcm_hcd);
	LOGI(tcm_hcd->pdev->dev.parent,"END\n");
	return 0;
}
static int ovt_tcm_resume(struct device *dev)
{
...
#else
	LOGE(tcm_hcd->pdev->dev.parent,"START\n");
	ovt_tcm_do_resume(tcm_hcd);
	LOGE(tcm_hcd->pdev->dev.parent,"END\n");
#endif
	return 0;
}
#ifdef SPEED_UP_RESUME
...
#elif defined(CONFIG_FB_IN_TP_DRIVER)
static int ovt_tcm_fb_notifier_cb(struct notifier_block *nb,
		unsigned long action, void *data)
{
	int retval;
		...
		unsigned long action, void *data)
{
	...
	flush_workqueue(tcm_hcd->resume_wq);
	if (*blank == MTK_DISP_BLANK_POWERDOWN) {
		if (action == MTK_DISP_EARLY_EVENT_BLANK) {
			...
			if(!tcm_hcd->proximity_event){
				...
				LOGI(tcm_hcd->pdev->dev.parent,"proximity_event = %d\n", tcm_hcd->proximity_event);
				if (tcm_hcd->enable_irq(tcm_hcd, true, NULL)) {
					LOGE(tcm_hcd->pdev->dev.parent,
							"Failed to enable irq irq:%d\n", tcm_hcd->irq);
				}
				touch_free_objects();
				LOGI(tcm_hcd->pdev->dev.parent,"proximity suspend\n");
			}
#else
			ovt_tcm_suspend(&tcm_hcd->pdev->dev);
#endif
			} else {
				LOGI(tcm_hcd->pdev->dev.parent,"suspend notification post commit\n");
				if (!tcm_hcd->wakeup_gesture_enabled && tcm_hcd->in_suspend) {
					gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
				}
			}
		...
		if (action == MTK_DISP_EARLY_EVENT_BLANK) {
			LOGE(tcm_hcd->pdev->dev.parent,"resume notification pre commit\n");
				if (!tcm_hcd->wakeup_gesture_enabled && tcm_hcd->in_suspend)
					gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);

		} else {
			LOGE(tcm_hcd->pdev->dev.parent,"action=%lu, *blank=%d\n", action, *blank);
			//ovt_tcm_resume(&tcm_hcd->pdev->dev); 
			queue_work(tcm_hcd->resume_wq, &tcm_hcd->resume_work);
		}
	}
    return 0;
}
...
int lct_ovt_tp_set_screen_angle_callback(int angle)
{
    ...
    int ret = -EIO;
    struct ovt_tcm_hcd *tcm_hcd = g_tcm_hcd;
    LOGI(tcm_hcd->pdev->dev.parent,"set angle start!");
    ret = ovt_tcm_set_dynamic_config_host_interface(DC_GRIP_AREA,1);
    LOGI(tcm_hcd->pdev->dev.parent,"open grip start!");
    if (ret < 0) {
    ...
    ret = ovt_tcm_set_dynamic_config_host_interface(DC_SCREEN_ANGLE,val);
    if (ret < 0) {
...
/*create /sys/touchscreen */
...
#if LCT_TP_DATA_DUMP
	retval = lct_tp_data_dump_proc_init();
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent, "lct_tp_data_dump_proc_init Failed. retval = %d\n", retval);
	} else {
		LOGI(tcm_hcd->pdev->dev.parent,"lct_tp_data_dump_proc_init Succeeded!");
	}
#endif

	retval = init_lct_tp_info("[Vendor]unkown,[FW]unkown,[IC]unkown\n");
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"init_lct_tp_info Failed!\n");
		goto err_init_lct_tp_info_failed;
	} else {
		LOGI(tcm_hcd->pdev->dev.parent,
				"init_lct_tp_info Succeeded!\n");
	}

	retval = init_lct_tp_work(lct_ovt_tp_work_callback);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"init_lct_tp_work Failed!\n");
		goto err_init_lct_tp_work_failed;
	} else {
		LOGI(tcm_hcd->pdev->dev.parent,
				"init_lct_tp_work Succeeded!\n");
	}


#if LCT_TP_GRIP_AREA_EN
	retval = init_lct_tp_grip_area(lct_ovt_tp_set_screen_angle_callback, lct_ovt_tp_get_screen_angle_callback);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,"init_lct_tp_grip_area Failed!\n");
		goto err_init_lct_tp_grip_area_failed;

	} else {
		LOGI(tcm_hcd->pdev->dev.parent,"init_lct_tp_grip_area Succeeded!\n");
	}
#endif

	retval = init_lct_tp_selftest(lct_ovt_tp_selftest_callback);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,"init_lct_tp_selftest Failed!\n");
		goto err_init_lct_tp_selftest_fail;
	} else {
		LOGI(tcm_hcd->pdev->dev.parent,"init_lct_tp_selftest Succeeded!\n");
	}

#ifdef SUPPORT_FACE_DETECT
	retval = init_lct_tp_proximity_switch(lct_ovt_tp_proximity_switch_callback);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"init_lct_tp_proximity_switch Failed!\n");
		goto err_init_lct_tp_proximity_switch_failed;
	} else {
		LOGI(tcm_hcd->pdev->dev.parent,
				"init_lct_tp_proximity_switch Succeeded!\n");
	}
#endif

	tcm_hcd->event_wq = alloc_workqueue("tcm_hcd-event-queue", WQ_UNBOUND | WQ_HIGHPRI, 1);
#if LCT_TP_USB_PLUGIN
	INIT_WORK(&tcm_hcd->online_work, ovt_tcm_usb_online_work);
	tcm_hcd->power_usb_notifier.notifier_call = ovt_tcm_power_usb_notifier_callback;
	retval = power_supply_reg_notifier(&tcm_hcd->power_usb_notifier);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,"Couldn't register psy notifier ret = %d\n", retval);
		goto err_register_power_usb_notifier_failed;
	}
#endif

#if WAKEUP_GESTURE
	retval = init_lct_tp_gesture(lct_ovt_tp_gesture_switch_callback);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"init_lct_tp_gesture Failed!\n");
		goto err_init_lct_tp_gesture_failed;
	} else {
		LOGI(tcm_hcd->pdev->dev.parent,
				"init_lct_tp_gesture Succeeded!\n");
        }
#endif

#if TP_EARPHONE_PLUGIN
	retval = tp_earphone_init(lct_ovt_tp_earphone_callback);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"tp_earphone_init Succeeded! Failed!\n");
		goto err_init_lct_tp_earphone_fail;
	} else {
		LOGI(tcm_hcd->pdev->dev.parent,
				"tp_earphone_init Succeeded!\n");
        }
#endif

	tcm_hcd->resume_wq = alloc_workqueue("ovt_tcm_resume_wq", WQ_UNBOUND | WQ_MEM_RECLAIM | WQ_HIGHPRI, 1);
	if (!tcm_hcd->resume_wq) {
		LOGE(tcm_hcd->pdev->dev.parent,
		"ERROR: Cannot create work thread\n");
		goto err_register_resume_work;
	}
	INIT_WORK(&tcm_hcd->resume_work, ovt_tcm_resume_work);

#if OVT_PM_WAIT_BUS_RESUME_COMPLETE
	tcm_hcd->dev_pm_suspend = false;
	init_completion(&tcm_hcd->dev_pm_resume_completion);
#endif /* OVT_PM_WAIT_BUS_RESUME_COMPLETE */

#ifdef CONFIG_TOUCHSCREEN_XIAOMI_TOUCHFEATURE
	memset(&xiaomi_touch_interfaces, 0x00, sizeof(struct xiaomi_touch_interface));
	xiaomi_touch_interfaces.palm_sensor_write = ovt_palm_sensor_write;
	...
	tcm_hcd->disp_notifier.notifier_call = ovt_tcm_mtk_notifier;
	retval = mtk_disp_notifier_register("SPI_MODULE_NAME",&tcm_hcd->disp_notifier);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
				"Failed to register FB notifier client\n");
	}
...
prepare_modules:
	...
	mutex_unlock(&tcm_hcd->suspend_resume_mutex);
#ifdef SUPPORT_FACE_DETECT
	tcm_hcd->sensor_tp_probe_status = true;
#endif
	return 0;
...
err_register_resume_work:
	cancel_work_sync(&tcm_hcd->resume_work);

#if LCT_TP_USB_PLUGIN
err_register_power_usb_notifier_failed:
	power_supply_unreg_notifier(&tcm_hcd->power_usb_notifier);
	tcm_hcd->power_usb_notifier.notifier_call = NULL;
	cancel_work_sync(&tcm_hcd->online_work);
	if (tcm_hcd->event_wq) {
		destroy_workqueue(tcm_hcd->event_wq);
		tcm_hcd->event_wq = NULL;
	}
#endif

	if (tcm_hcd->resume_wq) {
		destroy_workqueue(tcm_hcd->resume_wq);
		tcm_hcd->resume_wq = NULL;
	}

#if TP_EARPHONE_PLUGIN
err_init_lct_tp_earphone_fail:
	tp_earphone_uninit();
#endif
...
#endif
#endif
	LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_remove enter\n");
...
/*
#ifdef CONFIG_PM_IN_TP_DRIVER
static const struct dev_pm_ops ovt_tcm_dev_pm_ops = {
#if !defined(CONFIG_DRM_IN_TP_DRIVER) && !defined(CONFIG_FB_IN_TP_DRIVER)
#ifndef MTK_PLATFORM
...
#if OVT_PM_WAIT_BUS_RESUME_COMPLETE
/**
 * ovt_tcm_pm_suspend - PM suspend function
...
static int ovt_tcm_pm_suspend(struct device *dev)
{
	...
	tcm_hcd->dev_pm_suspend = true;
	reinit_completion(&tcm_hcd->dev_pm_resume_completion);
	return 0;
}
/**
 * ovt_tcm_pm_resume - PM resume function
...
static int ovt_tcm_pm_resume(struct device *dev)
{
	...
	tcm_hcd->dev_pm_suspend = false;
	complete(&tcm_hcd->dev_pm_resume_completion);
	return 0;
}

static const struct dev_pm_ops ovt_dev_pm_ops = {
...
static struct platform_driver ovt_tcm_driver = {
	.driver = {
		...
		.owner = THIS_MODULE,
#ifdef OVT_PM_WAIT_BUS_RESUME_COMPLETE
		.pm = &ovt_dev_pm_ops,
#endif
	},
	.probe = ovt_tcm_probe,
...
static int ovt_tcm_check_lcm_compatibility(const char *expected_lcm)
{
    struct device_node *chosen_node;
    struct tag_videolfb *videolfb_tag;
    ...
    if (!chosen_node) {
        pr_err("%s:ovt_tcm Missing /chosen node in device tree\n",__func__);
        return ret;
    }

    videolfb_tag = (struct tag_videolfb *)of_get_property(chosen_node, "atag,videolfb", (int *)&size);
    if (!videolfb_tag) {
        pr_err("%s:ovt_tcm videolfb tag not found\n",__func__);
        goto release_node;
    }
    memset(ovt_tp_fw_name, 0, sizeof(ovt_tp_fw_name));
    memset(ovt_tp_mp_name, 0, sizeof(ovt_tp_mp_name));
    if (strstr(videolfb_tag->lcmname, expected_lcm)) {
        ret = 0;
    } else {
        pr_err("%s:ovt_tcm Unsupported LCM: %s\n" ,__func__,videolfb_tag->lcmname);
    }

    strcpy(ovt_tp_fw_name, FW_IMAGE_NAME);
    strcpy(ovt_tp_mp_name, FW_IMAGE_NAME);
    strcpy(ovt_tp_fw_name, FW_IMAGE_NAME);
    strcpy(ovt_tp_mp_name, FW_IMAGE_NAME);
...
static int tpd_local_init(void)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,"ovt tcm driver %s: entry\n", __func__);
...
struct tpd_driver_t ovt_mtk_driver = {
	.tpd_device_name = "omnivision_tcm",
	.tpd_local_init = tpd_local_init,
	.suspend = ovt_mtk_tcm_suspend,
	.resume = ovt_mtk_tcm_resume,
	.tpd_have_button = 0,
};
...
static int __init ovt_tcm_module_init(void)
{
   ...
   pr_err("%s:ovt_tcm ovt_tcm_spi_init start",__func__);

/* compare LCM ID  start */
    if (!ovt_tcm_check_lcm_compatibility("dsi_panel_p15_35_10_0c_hd_vdo")) {
        sprintf("%s:ovt_tcm [Vendor]BOE,[IC]TD4160",__func__);
    } else {
        pr_err("ovt_tcm LCM compatibility check failed");
        return ret;
    }
/* compare LCM ID  end */
