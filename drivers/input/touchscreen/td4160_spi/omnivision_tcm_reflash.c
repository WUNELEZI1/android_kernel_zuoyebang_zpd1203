...
#define REFLASH_FW_IMAGE_NAME "omnivision/reflash_firmware.img"

#define REFLASH_FW_IMAGE_NAME_MANUAL "omnivision/reflash_firmware_manual.img"
		...
		while(retry_cnt--) {
			if (reflash_hcd->reflash_by_manual == false) {
				retval = request_firmware(&reflash_hcd->fw_entry,
					REFLASH_FW_IMAGE_NAME, tcm_hcd->pdev->dev.parent);
			} else {
				retval = request_firmware(&reflash_hcd->fw_entry,
					REFLASH_FW_IMAGE_NAME_MANUAL,  tcm_hcd->pdev->dev.parent);
			}

			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
						"Failed to request %s, retry_cnt:%d\n",
						reflash_hcd->reflash_by_manual ? REFLASH_FW_IMAGE_NAME_MANUAL : REFLASH_FW_IMAGE_NAME, retry_cnt);
				if (retry_cnt == 0) {
					return retval;
				}
				msleep(1000);
