...
#define TEST_FW_IMAGE_NAME "hdl_firmware_test.img"
#define LPWG_FW_IMAGE_NAME "omnivision/lpwg_hdl_firmware.img"
#endif
...
static int zeroflash_parse_fw_image(void)
{
	...
	for (idx = 0; idx < num_of_areas; idx++) {
		...
		if (0 == strncmp((char *)descriptor->id_string,
			...
			tcm_hcd->image_packrat_number = image_info->packrat_number;
#endif
			LOGD(tcm_hcd->pdev->dev.parent,
...
exit:
	...
	tcm_hcd->sensor_tp_fw_status = true;
#endif
	return;
