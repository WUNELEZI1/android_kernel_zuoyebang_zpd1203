...
#if LCT_TP_DATA_DUMP
#define testing_raw_delta_read(t_name) \
static ssize_t testing_proc_##t_name##_read(struct seq_file *file, char* buf) \
{ \
	int retval; \
	int count = 0; \
	int rows, cols, i, j; \
...
\
	...
	for (i = 0; i < rows; i++) { \
		for (j = 0; j < cols; j++) { \
			data_value = (short)le2_to_uint(&report_data_buf[(i * cols + j) * 2]); \
			retval = snprintf(buf, PAGE_SIZE - count,"%4d, ", data_value); \
			seq_printf(file, buf);\
			buf += retval; \
			count += retval; \
		} \
...
testing_raw_delta_read(raw_data)
testing_raw_delta_read(delta_data)

static struct proc_dir_entry *lct_tp_data_dump_entry;
static char ovt_dump_data[4096] = {0};

int lct_tp_data_dump_proc_read(struct seq_file *file, void* data)
{
	//print raw data
	seq_printf(file,"\nRAW DATA\n");
	testing_proc_raw_data_read(file, ovt_dump_data);

	memset(ovt_dump_data, 0, sizeof(ovt_dump_data));

	//print diff data
	seq_printf(file,"\nDIFF DATA\n");
	testing_proc_delta_data_read(file, ovt_dump_data);

	return 0;
}

static int lct_tp_data_dump_open(struct inode* inode, struct file* file)
{
	return single_open(file, lct_tp_data_dump_proc_read, NULL);
}
...
int lct_tp_data_dump_proc_init(void)
{
	lct_tp_data_dump_entry = proc_create(TP_DATA_DUMP, 0444, NULL, &lct_tp_data_dump_fops);
	if (lct_tp_data_dump_entry == NULL) {
		return -ENOMEM;
	}
    return 0;
}

void lct_tp_data_dump_proc_exit(void)
{
	if (lct_tp_data_dump_entry != NULL) {
		remove_proc_entry(TP_DATA_DUMP, NULL);
		lct_tp_data_dump_entry = NULL;
	}
}
#endif
...
static int ovt_tcm_get_thr_from_csvfile(void)
{
	...
	LOGN(tcm_hcd->pdev->dev.parent,"ovt tcm csv parser: rows: %d, cols: %d file_path:%s", rows, cols, file_path);
	ret = ovt_tcm_parse_csvfile(file_path, CSV_RAW_DATA_MIN_ARRAY,
			threshold->raw_data_min_limits, rows, cols);
	if (ret) {
		LOGN(tcm_hcd->pdev->dev.parent,"ovt tcm csv parser: %s: Failed get %s \n", __func__, CSV_RAW_DATA_MIN_ARRAY);
		return ret;
	}
	...
	LOGN(tcm_hcd->pdev->dev.parent,"ovt tcm csv parser: %s: success \n", __func__);
	return 0;
}
#endif

char *g_testing_output_buf;
...
int lct_ovt_tp_selftest_callback(unsigned char cmd)
{
	int ret = 0;
	int test_result = 0;
	struct ovt_tcm_hcd *tcm_hcd = testing_hcd->tcm_hcd;
	...
	case TP_SELFTEST_CMD_XIAOMI_SHORT:
		...
		break;
	default:
		LOGN(tcm_hcd->pdev->dev.parent,"lct : touch selftest command: unknow\n");
		break;
	}

	if (!ret) {
		LOGN(tcm_hcd->pdev->dev.parent,"lct: tp selftest Passed\n");
		test_result =2;
	} else {
		LOGN(tcm_hcd->pdev->dev.parent,"lct : tp selftest Failed\n");
		test_result = 1;
	}
#ifdef SUPPORT_TEST_IMG_HDL
	tcm_hcd->download_firmware_image_id(tcm_hcd, NORMAL_APP_FIRMWARE);
#endif
	mutex_unlock(&tcm_hcd->extif_mutex);
	return test_result;
}
...
static int testing_do_testing(void)
{
...
#ifdef SUPPORT_TEST_IMG_HDL
	retval = tcm_hcd->download_firmware_image_id(tcm_hcd, TEST_FIRMWARE);
	if (retval < 0) {
		LOGE(tcm_hcd->pdev->dev.parent,
  ...
  mm_segment_t old_fs;
	...
	if (retval < 0) {
		LOGN(tcm_hcd->pdev->dev.parent,"ovt_tcm_get_thr_from_csvfile error, return\n");
		snprintf(g_testing_output_buf + strlen(g_testing_output_buf), OUTPUT_TO_CSV_STRING_LEN - strlen(g_testing_output_buf),
	...
	if (IS_ERR_OR_NULL(fp)) {
			LOGN(tcm_hcd->pdev->dev.parent,"ovt tcm Open log file '%s' failed.\n", file_path);
	...
	LOGN(tcm_hcd->pdev->dev.parent,"g_testing_output_buf , strlen is %d\n", (int)strlen(g_testing_output_buf));
	if (!IS_ERR_OR_NULL(fp)) {
		LOGN(tcm_hcd->pdev->dev.parent,"ovt tcm csv parser: filp close\n");
		filp_close(fp, NULL);
		fp = NULL;
	}
  set_fs(old_fs);
