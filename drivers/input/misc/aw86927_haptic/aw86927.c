...
{
	aw_info("%s enter! flag: %d\n", __func__, flag);
	if (flag) {
		if (!atomic_read(&aw86927->is_in_pm_awake)) {
			pm_stay_awake(aw86927->dev);
			atomic_set(&aw86927->is_in_pm_awake, 1);
		}
		...
		if (atomic_read(&aw86927->is_in_pm_awake)) {
			...
			atomic_set(&aw86927->is_in_pm_awake, 0);
		}
		aw86927_i2c_write_bits(aw86927, AW86927_REG_SYSINTM,
				       AW86927_BIT_SYSINTM_FF_AEM_MASK,
...
static int aw86927_haptic_is_within_cali_range(struct aw86927 *aw86927)
{
	...
	if ((aw86927->f0 < f0_cali_min) || aw86927->f0 > f0_cali_max) {
		aw_err("%s f0 calibration out of range = %d!\n",
			__func__, aw86927->f0);
		XM_HAP_F0_CAL_EXCEPTION(aw86927->f0, aw86927->info.f0_pre, "F0 Calibration failed, use default val");
		ret = -1;
	}
	return ret;
}
