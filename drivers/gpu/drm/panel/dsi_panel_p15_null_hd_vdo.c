...
static const char *panel_name = "panel_name=dsi_panel_p15_null_hd_vdo";
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	int ret;
	pr_info("%s dsi_panel_p15_null_hd_vdo +\n", __func__);
	...
	if (dsi_node) {
		...
		if (endpoint) {
			...
			if (!remote_node) {
                ...
                pr_info("%s,MI_EVENT_PANEL_RECOGNIZE_ERR\n", __func__);
#endif
				return -ENODEV;
			}
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "dsi_panel_p15_null_hd_vdo,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "dsi_panel_p15_null_hd_vdo",
		.owner = THIS_MODULE,
		.of_match_table = lcm_of_match,
	},
};
module_mipi_dsi_driver(lcm_driver);
MODULE_AUTHOR("Ma shuaishuai <mashuaishuai1@longcheer.com>");
MODULE_DESCRIPTION("dsi_panel_p15_null_hd_vdo panel driver");
