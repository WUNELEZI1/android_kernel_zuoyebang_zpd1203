...
#define DATA_RATE1      1122
#define MODE2_FPS       120
#define MODE2_HFP       32
#define MODE2_HSA       4
#define MODE2_HBP       20
#define MODE2_VFP       180
#define MODE2_VSA       4
#define MODE2_VBP       30
#define MODE1_FPS       90
#define MODE1_HFP       32
...
extern bool lcd_reset_keep_high_zeroc;
extern bool prox_lcd_reset_keep_high_zeroc;
...
static const char *panel_name = "panel_name=dsi_panel_p15_35_10_0c_hd_vdo";
...
static struct LCM_setting_table init_setting[] = {
	...
	//CABC code
	{0xB8, 06, {0x02,0x60,0x6C,0x1D,0x27,0x23}},
	{0xB9, 06, {0x02,0x80,0x4A,0x08,0x3F,0x69}},
	{0xBA, 06, {0x02,0xA2,0x35,0x00,0x22,0xd2}},
	//ESD code
	{0xBE, 03, {0x06,0x00,0x85}},
	{0xCE, 24, {0x7D,0x40,0x43,0x49,0x55,0x62,0x71,0x82,0x94,0xA8,0xB9,0xCB,0xDB,0xE9,0xF5,0xFC,0xFF,0x04,0x00,0x00,0x00,0x00,0x04,0x8C}},
	{0xB0, 01, {0x03}},
...
static struct LCM_setting_table TP[] = {
	{0xB0, 01, {0x00}},
	{0xD1, 24, {0x4C,0x58,0x03,0x33,0x33,0x14,0x07,0xBB,0x55,0x45,0x55,0x45,0x00,0x3B,0x07,0x07,0x3B,0x30,0x06,0x72,0x33,0x13,0x00,0x97}},
	{0xC4, 51, {0x00,0x02,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01,0x00,0x02,0x00,0x00,0x00,0x00,0x00,0x00}},
	{0xB2, 01, {0x04}},
	{0xB0, 01, {0x03}},
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	ctx->error = 0;
	ctx->prepared = false;
	if (prox_lcd_reset_keep_high_zeroc && esd_panel_dead == false) {
		push_table(ctx, TP,
			sizeof(TP) / sizeof(struct LCM_setting_table));
		pr_info("[LCM] prox_lcd_reset_keep_high_zeroc = %d\n",prox_lcd_reset_keep_high_zeroc);
		pr_info("[LCM]%s:tp_promixity_en\n",__func__);
		return 0;
	}
	if (lcd_reset_keep_high_zeroc && esd_panel_dead == false) {
		pr_info("[LCM]%s:lcd_reset_keep_high_zeroc = %d\n",lcd_reset_keep_high_zeroc);
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	if (ret < 0)
		pr_info("TD4160--kernel--bias----cmd=%0x--i2c write error----\n", cmd);
	else
		pr_info("TD4160--kernel--bias----cmd=%0x--i2c write success----\n", cmd);
	/* set AVEE -6.0V*/
	cmd = 0x01;
	data = 0x14;
	ret = lcd_bias_write_bytes(cmd, data);
...
#define MIN_BL			13	//最低背光值
#define NORMAL_MAX_BL		2047	//normal段最高背光值
#define	MAX_BL			4095	//最高背光值

#define	MIN_REG_VALUE_BOE	12	//BOE最低寄存器亮度值
#define NORMAL_MAX_REG_BOE	2233	//BOEnoraml段最高寄存器亮度值08B9
#define MAX_REG			2799	//12bit背光寄存器最大值 0C00

static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb,
	void *handle, unsigned int level)
{
	char bl_tb[] = {0x51, 0x07, 0xff};
	pr_info("%s:level BOE = %d\n",__func__,level);
	panel_inited = (level > 0);
	if (level == 0) {
		pr_info("%s:level= %d\n",__func__,level);
	} else if (level > 0 && level<= MIN_REG_VALUE_BOE) {
		level = MIN_REG_VALUE_BOE;
	} else if (level <= NORMAL_MAX_BL)  {
		level = (level - MIN_BL) * (NORMAL_MAX_REG_BOE - MIN_REG_VALUE_BOE) / (NORMAL_MAX_BL - MIN_BL) + MIN_REG_VALUE_BOE;
		pr_info("%s:BOE_hbm_off,level= %d\n",__func__,level);
	} else if (level <= MAX_BL) {
		level = (level - NORMAL_MAX_BL) * (MAX_REG - NORMAL_MAX_REG_BOE) / (MAX_BL - NORMAL_MAX_BL) + NORMAL_MAX_REG_BOE;
		pr_info("%s:BOE_hbm_on,level = %d\n",__func__,level);
	} else {
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	int ret;
	pr_info("%s dsi_panel_p15_35_10_0c_hd_vdo +\n", __func__);
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "dsi_panel_p15_35_10_0c_hd_vdo,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "dsi_panel_p15_35_10_0c_hd_vdo",
		.owner = THIS_MODULE,
...
MODULE_AUTHOR("p15");
MODULE_DESCRIPTION("dsi_panel_p15_35_10_0c_hd_vdo panel driver");
