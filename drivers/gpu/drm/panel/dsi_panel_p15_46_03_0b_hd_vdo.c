...
#define DATA_RATE1		1272
#define MODE2_FPS		120
#define MODE2_HFP		44
#define MODE2_HSA		6
#define MODE2_HBP		18
#define MODE2_VFP		362
#define MODE2_VSA		4
#define MODE2_VBP		28
#define MODE1_FPS		90
#define MODE1_HFP		44
...
#define MODE0_HSA		6
#define MODE0_HBP		18
#define MODE0_VFP		2352
#define MODE0_VSA		4
#define MODE0_VBP		28
#define FRAME_WIDTH (720)
#define	FRAME_HEIGHT (1600)

extern bool lcd_reset_keep_high_zerob;
extern bool prox_lcd_reset_keep_high_zerob;
...
static const char *panel_name = "panel_name=dsi_panel_p15_46_03_0b_hd_vdo";
...
static struct LCM_setting_table init_setting[] = {
	...
	//CABC
	{0x00,1,{0x80}},
	{0xCA,12,{0xDF,0xD7,0xC7,0xBA,0xAF,0xA6,0x9E,0x98,0x92,0x8D,0x89,0x85}},
	{0x00,1,{0x90}},
	{0xCA,9,{0xFE,0xFF,0x3C,0xF5,0xFF,0x1E,0xFA,0xFF,0x66}},
	//27kHZ
	{0x00,1,{0xB0}},
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	ctx->error = 0;
	ctx->prepared = false;
	if (prox_lcd_reset_keep_high_zerob) {
		pr_info("[LCM]%s:prox_lcd_reset_keep_high_zerob = %d\n",prox_lcd_reset_keep_high_zerob);
		pr_info("[LCM]%s:tp_promixity_en\n",__func__);
		return 0;
	}
	if (lcd_reset_keep_high_zerob) {
		pr_info("[LCM]%s:lcd_reset_keep_high_zerob = %d\n",lcd_reset_keep_high_zerob);
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	if (ret < 0)
		pr_info("FT8057m--kernel--bias----cmd=%0x--i2c write error----\n", cmd);
	else
		pr_info("FT8057m--kernel--bias----cmd=%0x--i2c write success----\n", cmd);
	/* set AVEE -6.0V*/
	cmd = 0x01;
	data = 0x14;
	ret = lcd_bias_write_bytes(cmd, data);
...
#define	MIN_REG_VALUE_HK	12	//hk最低寄存器亮度值
#define NORMAL_MAX_REG_HK	2669	//hk noraml段最高寄存器亮度值
#define MAX_REG			3328	//12bit背光寄存器最大值 0D00

static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb,
	void *handle, unsigned int level)
{
	char bl_tb[] = {0x51, 0x07, 0xff};
	pr_info("%s:level hk = %d\n",__func__,level);
	panel_inited = (level > 0);
	if (level == 0) {
		pr_info("%s:level= %d\n",__func__,level);
	} else if (level > 0 && level<= MIN_REG_VALUE_HK) {
		level = MIN_REG_VALUE_HK;
	} else if (level <= NORMAL_MAX_BL)  {
		level = (level - MIN_BL) * (NORMAL_MAX_REG_HK - MIN_REG_VALUE_HK) / (NORMAL_MAX_BL - MIN_BL) + MIN_REG_VALUE_HK;
		pr_info("%s:HK_hbm_off,level= %d\n",__func__,level);
	} else if (level <= MAX_BL) {
		level = (level - NORMAL_MAX_BL) * (MAX_REG - NORMAL_MAX_REG_HK) / (MAX_BL - NORMAL_MAX_BL) + NORMAL_MAX_REG_HK;
		pr_info("%s:HK_hbm_on,level = %d\n",__func__,level);
	} else {
		...
		unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
    ...
    pr_info("dsi_panel_p15,dst_fps=%d\n",dst_fps);
	if (isFpsChange) {
		if (dst_fps == MODE0_FPS)
            ...
            mode_switch_to_90(panel, stage);
            pr_info("dsi_panel_p15,force to 90Hz=%d\n",dst_fps);
        }
	}
	return ret;
}
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	int ret;
	pr_info("%s dsi_panel_p15_46_03_0b_hd_vdo +\n", __func__);
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "dsi_panel_p15_46_03_0b_hd_vdo,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "dsi_panel_p15_46_03_0b_hd_vdo",
		.owner = THIS_MODULE,
...
MODULE_AUTHOR("p15");
MODULE_DESCRIPTION("dsi_panel_p15_46_03_0b_hd_vdo panel driver");
