...
* sp0821 marco
******************************************************************/
#define SP0821_DRIVER_VERSION	"V9.0"
...
int sp0821_GetSensorID(struct sp0821 *sp0821)
{
	...
	do {
		...
		if (reg_data == SP0821_SENSOR_ID) {
			qvga_dev_err(sp0821->dev, "scw-drv-%s: Read Sensor ID sucess = 0x%02x\n", __func__, reg_data);
			driver_flag = 1;
		return 0;
		} else {
			qvga_dev_err(sp0821->dev, "scw-drv-%s: Read Sensor ID Fail = 0x%02x\n", __func__, reg_data);
			driver_flag = 0;
		}
		mdelay(10);
		retry--;
...
static void sp0821_vcama_control(struct sp0821 *sp0821, bool flag)
{
	int value;
	int count = 0;
	int ret;
...
static void sp0821_vcam_control(struct sp0821 *sp0821, bool flag)
{
        ...
        if (IS_ERR(vcamd)) {
                qvga_dev_err(sp0821->dev, "%s get regulator failed\n", __func__);
                regulator_put(vcamd);
                return;
        }
        if (flag) {
        ...
        if (flag) {
                ...
                ret_vaule = regulator_enable(sp0821->vcamio);
        } else {
                regulator_disable(sp0821->vcamio);
        }
        return;
}
/*
...
{
	//sp0821_hw_off_reset1(g_sp0821);
	//mdelay(5);
	// sp0821_vcam_control(g_sp0821, true);
	//mdelay(1);
	sp0821_vcama_control(g_sp0821, true);
	mdelay(5);
	sp0821_hw_on_reset1(g_sp0821);
...
static void sp0821_hw_off(struct sp0821 *sp0821)
{
	...
	sp0821_vcama_control(sp0821, false);
			       ...
			       size_t len)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		length = sp0821_i2c_read(g_sp0821, databuf[0], &reg_data);
...
static const struct attribute_group sp0821_attribute_group = {
			     ...
			     const struct i2c_device_id *id)
{
	...
	mdelay(10);
	sp0821_vcama_control(sp0821, true);
	mdelay(1);
	...
	if (ret < 0) {
		qvga_dev_err(&client->dev,
			   "%s: sp0821 read_sensorid failed ret=%d\n", __func__,
			   ret);
...
void sp0821_yuv_init_robe(void)
{
	printk("robe_debug: %s run begin.\n", __func__);
	sp0821_yuv_init();
	printk("robe_debug: %s run end.\n", __func__);
...
void sp0821_yuv_exit_robe(void)
{
	printk("robe_debug: %s run begin.\n", __func__);
	sp0821_yuv_exit();
	printk("robe_debug: %s run end.\n", __func__);
...
MODULE_SOFTDEP("pre: imgsensor_isp6s post: sp_sp0821_iii");
*/
