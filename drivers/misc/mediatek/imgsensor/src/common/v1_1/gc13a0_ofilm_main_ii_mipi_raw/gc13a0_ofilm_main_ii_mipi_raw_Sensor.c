 ...
 * Filename:
 * ---------
 *     gc13a0_ofilm_main_ii_mipi_raw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "gc13a0_ofilm_main_ii_mipi_raw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC13A0_OFILM_MAIN_II_SENSOR_ID,
...
#if 1
static kal_uint16 read_cmos_sensor_gc13a0_ofilm_otp(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
static void gc13a0_ofilm_fusion_id_read(void)
{
	int i;
	for (i=0; i<12; i++) {
		fusion_id_main[i] = read_cmos_sensor_gc13a0_ofilm_otp(0x10+i);
	pr_debug("%s %d zengx fusion_id_main[%d]=0x%2x\n",__func__, __LINE__, i, fusion_id_main[i]);
	}
}
static void gc13a0_ofilm_sn_id_read(void)
{
	int i;
	for (i=0; i<25; i++) {
	    sn_main[i] = read_cmos_sensor_gc13a0_ofilm_otp(0x197E + i);
		pr_debug("%s %d zengx sn_main[%d]=0x%2x\n",__func__, __LINE__, i, sn_main[i]);
	}
}
#endif
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[gc13a0_mipi_raw]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
#if 1
					gc13a0_ofilm_fusion_id_read();
					gc13a0_ofilm_sn_id_read();
#endif
				return ERROR_NONE;
			}
			pr_debug("[gc13a0_mipi_raw]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[gc13a0_mipi_raw]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_debug("[gc13a0_mipi_raw]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
UINT32 GC13A0_OFILM_MAIN_II_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
