...
wl2866_ldo_pin_map_t wl2866_ldo_pin_map[] = {
	...
	{ IMGSENSOR_SENSOR_IDX_MAIN3, IMGSENSOR_HW_PIN_AVDD, WL2866_LDO_AVDD2}, //2.8V, Macro
};
typedef struct {
...
wl2866_ldo_votage_map_t wl2866_ldo_avdd_votage_map[] = {
	...
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_2900, 0x88},
};
...
wl2866_ldo_votage_map_t sgm38121_ldo_avdd_votage_map[] = {
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_2800, 0xB1},
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_2900, 0xBE},
};
wl2866_ldo_votage_map_t sgm38121_ldo_dvdd_votage_map[] = {
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_1000, 0x3E},
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_1050, 0x44},
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_1100, 0x4B},
	{ IMGSENSOR_HW_PIN_STATE_LEVEL_1200, 0x57},
};
...
static int32_t smartldo_get_ldo_value(enum IMGSENSOR_HW_PIN_STATE pin_state, cam_cell_id_t cell_id)
{
	...
	if(chipid == 0x80){
		printk("[%s] this is sgm38121_ldo\n", __func__);
		if (cell_id == WL2866_LDO_DVDD1 ||
			cell_id == WL2866_LDO_DVDD2) {
				votage_map = sgm38121_ldo_dvdd_votage_map;
				size = ARRAY_SIZE(sgm38121_ldo_dvdd_votage_map);
		} else if (cell_id == WL2866_LDO_AVDD1 ||
			cell_id == WL2866_LDO_AVDD2) {
				votage_map = sgm38121_ldo_avdd_votage_map;
				size = ARRAY_SIZE(sgm38121_ldo_avdd_votage_map);
		} else {
			printk("%s:error cell_id = %d", __func__, cell_id);
			return -1;
		}
	}else{
		printk("[%s] this is not sgm38121_ldo\n", __func__);
		if (cell_id == WL2866_LDO_DVDD1 ||
			cell_id == WL2866_LDO_DVDD2) {
...
static int wl2866_ldo_set_value(cam_cell_id_t ldo_id, uint32_t value)
{
	...
	if (ret == 0) {
		...
		printk("%s: read ldo_id =%d register value = %d success \n", __func__, value);
	}
	return ret;
}
static int cam_smart_ldo_wl2866_control(enum IMGSENSOR_SENSOR_IDX sensor_idx, cam_cell_id_t ldo_id, uint32_t value, bool enable)
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		...
		printk("[%s] read chip rev success,chipid = 0x%02x\n", __func__,ret);
	}
	/*
