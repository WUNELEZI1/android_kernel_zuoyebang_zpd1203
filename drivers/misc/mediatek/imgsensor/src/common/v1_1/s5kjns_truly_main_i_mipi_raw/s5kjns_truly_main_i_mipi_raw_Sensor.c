...
#include "s5kjns_truly_main_i_mipi_raw_Sensor.h"
...
static void s5kjns_fusion_id_read(void)
{
	...
	for (i=0; i<6; i++) {
		...
		//pr_info("%s addr = 0x%4x fusion_id_main[%d]=0x%2x\n",__func__, 0x10 + i, i, fusion_id_main[i]);
	}
}
static void s5kjns_sn_read(void)
{
	...
	for (i=0; i<14; i++) {
		...
		//pr_info("%s addr = 0x%4x sn_main[%d]=0x%2x\n",__func__, 0x3fe3 + i, i, sn_main[i]);
	}
}
#endif
...
//		mdelay(5);
		...
		for (i = 0; i < 5; i++) {
			LOG_INF("%s streaming check is %d", __func__,
					 read_cmos_sensor_8(0x0005));
			LOG_INF("%s streaming check 0x0100 is %d", __func__,
					 read_cmos_sensor_8(0x0100));
			LOG_INF("%s streaming check 0x6028 is %d", __func__,
...
{
	LOG_INF("s5kjns_truly_main_i_mipi_raw_SensorInit in\n");
