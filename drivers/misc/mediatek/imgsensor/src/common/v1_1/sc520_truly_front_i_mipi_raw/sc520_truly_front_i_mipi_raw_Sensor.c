...
LOG_INF("[%s] sensor_init\n", __func__);
	...
	write_cmos_sensor8(0x481d,0x1d);
	...
	write_cmos_sensor8(0x3650,0x31);
...
static void sc520cs_fusion_id_read(void)
{
	...
	LOG_INF("%s %d lss fusion_id_front[%d]=0x%2x\n",__func__, __LINE__, i, fusion_id_front[i]);
	}
}
static void sc520cs_sn_id_read(void)
{
	int i;
	for (i=0; i<25; i++) {
	        sn_front[i] = read_cmos_sensor_sc520cs(0x1F87+i);
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor8(0x5000,sc520_vaule|0x06);
		write_cmos_sensor8(0x391f,0xc9);
		write_cmos_sensor8(0x0100,0x01);
	}
	LOG_INF("sc520 sensor 0x5000 reg id1:0x%x,id2:0x%x,id3:0x%x,id4:0x%x\n",
				 (read_cmos_sensor(0x5000)),sc520_vaule, (sc520_vaule & 0xf9),(sc520_vaule | 0x06));
	spin_lock(&imgsensor_drv_lock);
