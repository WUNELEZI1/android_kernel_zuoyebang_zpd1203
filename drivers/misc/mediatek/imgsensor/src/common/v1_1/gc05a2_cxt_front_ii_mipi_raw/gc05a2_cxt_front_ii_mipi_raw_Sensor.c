...
#include "gc05a2_cxt_front_ii_mipi_raw_Sensor.h"
#include "gc05a2otp.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC05A2_CXT_FRONT_II_SENSOR_ID,
...
#if 1
static kal_uint16 read_cmos_sensor_gc05a2_cxt_otp(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
static void gc05a2_fusion_id_read(void)
{
	int i;
	for (i=0; i<14; i++) {
		fusion_id_front[i] = read_cmos_sensor_gc05a2_cxt_otp(0x10+i);
	pr_debug("%s %d lss fusion_id_front[%d]=0x%2x\n",__func__, __LINE__, i, fusion_id_front[i]);
	}
}
static void gc05a2_sn_id_read(void)
{
	int i;
	for (i=0; i<25; i++) {
	        sn_front[i] = read_cmos_sensor_gc05a2_cxt_otp(0x0EDC+i);
...
UINT32 GC05A2_CXT_FRONT_II_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
