...
static struct pm_qos_request i2c_qos_request;
...
static int upm7610_get_chip_state_from_upm6922(struct tcpc_device *tcpc);
...
static int upm7610_read_device(void *client, u32 reg, int len, void *dst)
{
	...
	/* tmp use for system suspend */
	cpu_latency_qos_update_request(&i2c_qos_request, 50);
	/* tmp use for system suspend  end */
...
#define UPM6922P_CHG_REGS_0B 0x0B
#define UPM6922P_PN_YARD_MASK 0x78
#define UPM6922P_PN_1_VAL 0x02
...
static int upm7610_get_chip_state_from_upm6922(struct tcpc_device *tcpc)
{
	...
	int ret = 0;
	int val = 0;
	int upm7610_chip = CHIP_ID_DEFUALT;
	int i_tmp;

	chip->client->addr = UPM6922P_CHG_ADDR;

	for (i_tmp = 0; i_tmp < 3;  i_tmp ++) {
		ret = upm7610_read_device(chip->client, UPM6922P_CHG_REGS_0B, 1, &val);
		if (ret < 0) {
			pr_err("upm6922p block read fail\n");
	...
	pr_info("upm7610: read upm6922p Reg[0x0B] = 0x%2x\n", val);
	pr_info("upm7610: read upm7610_chip = 0x%2x\n", upm7610_chip);

	val = (val & UPM6922P_PN_YARD_MASK) >> 3;
	if (val == UPM6922P_PN_1_VAL) {
		pr_info("It is upm6922p cfg !\n");
		upm7610_chip = CHIP_ID_UPM6922P;
	} else {
		pr_info("It is others buck chg + upm7610 cfg !\n");
		upm7610_chip = CHIP_ID_UPM7610;
	}

	pr_info("upm7610: read upm7610_chip = 0x%2x\n", upm7610_chip);
	pr_info("upm7610: read new val = 0x%2x\n", val);

	chip->client->addr = UPM7610_ADDR;
...
static int upm7610_init_alert(struct tcpc_device *tcpc)
{
	...
	if (chip->chip_func_sw == CHIP_ID_DEFUALT) {
		chip->chip_func_sw = upm7610_get_chip_state_from_upm6922(tcpc);
		pr_info("upm7610: read upm7610_chip_int = 0x%2x\n", chip->chip_func_sw);
	}

	if (chip->chip_func_sw == CHIP_ID_UPM6922P) {
		pr_info("pd chip is 6922\n");
	} else {
		pr_info("pd chip is 7610\n");
		ret = of_get_named_gpio(np, "upm7610,intr-gpio", 0);
...
static inline int upm7610_init_cfg_adjust(struct tcpc_device *tcpc)
{
	int data = 0;
	uint8_t tmp1 = 0;
	...
	pr_err("upm7610: default cfg data [0xC5]:0x%x\n", data);
...
{
    if (enable) {
        upm7610_i2c_set_bits(tcpc, UPM7610_REG_VDR_DEF_ALERT_MASK, UPM7610_REG_VSAFE0V_STATUS_MASK);
        upm7610_i2c_set_bits(tcpc, TCPC_V10_REG_ALERT_MASK+1, TCPC_V10_REG_EXTENDED_STATUS>>8);
    } else {
        upm7610_i2c_clr_bits(tcpc, UPM7610_REG_VDR_DEF_ALERT_MASK, UPM7610_REG_VSAFE0V_STATUS_MASK);
        upm7610_i2c_clr_bits(tcpc, TCPC_V10_REG_ALERT_MASK+1, TCPC_V10_REG_EXTENDED_STATUS>>8);
    }
				...
				const struct i2c_device_id *id)
{
	...
	/* tmp use for system suspend */
	cpu_latency_qos_add_request(&i2c_qos_request, PM_QOS_DEFAULT_VALUE);
	/* tmp use for system suspend  end */
	...
	chip->vconn_en = false;
	chip->lpm_en = false;
	chip->chip_func_sw =  CHIP_ID_DEFUALT;
