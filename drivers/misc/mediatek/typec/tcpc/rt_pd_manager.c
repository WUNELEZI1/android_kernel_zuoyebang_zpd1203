...
struct rt_pd_manager_data {
	...
	struct delayed_work upm_power_work;
	struct charger_device *chg_dev;
	struct charger_device *dvchg_dev;

};

int upm_power_pd_mode_hiz(struct rt_pd_manager_data *rpmd, bool en);
				...
				unsigned long event, void *data)
{
	...
	static bool is_otg = false;
        bool is_charging_done = false;
	mt_dbg(rpmd->dev, "event = %lu, idx = %d ++\n", event, idx);

        if (IS_ERR_OR_NULL(rpmd->dvchg_dev)) {
                rpmd->dvchg_dev = get_charger_by_name("primary_dvchg");
                dev_err(rpmd->dev, "%s get chg dev fail\n", __func__);
        }
	...
	case TCP_NOTIFY_SINK_VBUS:
		dev_info(rpmd->dev, "%s sink vbus %dmV %dmA type(0x%02X)\n",
                ...
                if(noti->vbus_state.ma == 0 && ( is_otg == false )) {
                	charger_dev_is_charging_done(rpmd->chg_dev, &is_charging_done);
			dev_info(rpmd->dev, "%s is_charging_done: %s\n", __func__, is_charging_done ?"true" :"false");
			if(!is_charging_done) {
				dev_info(rpmd->dev, "%s set hiz\n", __func__);
				charger_dev_enable(rpmd->dvchg_dev, false);
				val1.intval = true;
				charger_dev_set_property(rpmd->chg_dev, CHARGER_PROP_ERP_FORCE_HIZ, &val1);
				upm_power_pd_mode_hiz(rpmd, true);
			}
                } else {
                        dev_info(rpmd->dev, "%s cancel hiz\n", __func__);
                        val1.intval = false;
                        charger_dev_set_property(rpmd->chg_dev, CHARGER_PROP_ERP_FORCE_HIZ, &val1);
			upm_power_pd_mode_hiz(rpmd, false);
                        charger_dev_set_mivr(rpmd->chg_dev, 4600000);
                        charger_dev_enable_hz(rpmd->chg_dev, 0);
                }
		break;
	case TCP_NOTIFY_SOURCE_VBUS:
		...
		val1.intval = false;
                charger_dev_set_property(rpmd->chg_dev, CHARGER_PROP_ERP_FORCE_HIZ, &val1);
		cancel_delayed_work(&rpmd->upm_power_work);
		dev_info(rpmd->dev, "%s cancel hiz\n", __func__);
		upm_power_pd_mode_hiz(rpmd, false);
		charger_dev_set_mivr(rpmd->chg_dev, 4600000);
		charger_dev_enable_hz(rpmd->chg_dev, 0);
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if ((old_state != TYPEC_UNATTACHED) && (new_state == TYPEC_UNATTACHED)){
			if(rpmd->tcpc[idx]->screen_and_audio_off == true){
				dev_info(rpmd->dev, "%s set SNK while tcpc plugout with screen and audio off\n", __func__);
				tcpm_typec_change_role_postpone(rpmd->tcpc[idx], TYPEC_ROLE_SNK, true);
			}else{
				dev_info(rpmd->dev, "%s set TRY_SNK while tcpc plugout with screen and audio on\n", __func__);
				tcpm_typec_change_role_postpone(rpmd->tcpc[idx], TYPEC_ROLE_TRY_SNK, true);
			}
			/* disabe HIZ */
			cancel_delayed_work(&rpmd->upm_power_work);
			dev_info(rpmd->dev, "%s cancel hiz\n", __func__);
...
int upm_power_pd_mode_hiz(struct rt_pd_manager_data *rpmd, bool en)
{
	if (IS_ERR_OR_NULL(rpmd->chg_dev))
		rpmd->chg_dev = get_charger_by_name("primary_chg");
	cancel_delayed_work(&rpmd->upm_power_work);
	if (en) {
		pr_info("upm7610: enter charger_dev_enable_hz\n");
		charger_dev_set_mivr(rpmd->chg_dev, 12000000);
		charger_dev_enable_hz(rpmd->chg_dev, 1);
		schedule_delayed_work(
				&rpmd->upm_power_work, msecs_to_jiffies(5000));
	}
	return 0;
}
static void upm_power_delay_work(struct work_struct *work)
{
	struct rt_pd_manager_data *rpmd = container_of(work, struct rt_pd_manager_data,
					       upm_power_work.work);
	pr_info("upm7610: exit charger_dev_enable_hz\n");
	charger_dev_set_mivr(rpmd->chg_dev, 4600000);
	charger_dev_enable_hz(rpmd->chg_dev, 0);
	return;
}
...
static int typec_port_init(struct rt_pd_manager_data *rpmd, int idx)
{
	...
	rpmd->chg_dev = get_charger_by_name("primary_chg");
        ...
        INIT_DELAYED_WORK(&rpmd->upm_power_work, upm_power_delay_work);
out:
	return ret;
}
