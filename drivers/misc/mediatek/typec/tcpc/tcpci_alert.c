...
static void tcpci_alert_recv_msg(struct tcpc_device *tcpc)
{
	...
	mutex_lock(&pd_port->upm_rxbuf_lock);
	...
	tcpci_alert_status_clear(tcpc, TCPC_REG_ALERT_RX_STATUS);
	mutex_unlock(&pd_port->upm_rxbuf_lock);

/* add quick read another rx packet */
	rv = tcpci_get_alert_status_and_mask(tcpc, &alert_status, &alert_mask);
	if (rv < 0)
		return;

	TCPC_INFO("upm7610 read again, 20250601, Alert:0x%04x, Mask:0x%04x\n", alert_status, alert_mask);
	...
	if (alert_status & TCPC_REG_ALERT_RX_STATUS) {
		...
		//tcpc->curr_pd_msg = pd_msg;
		/* add for 1-6-7 */
		TCPC_INFO("upm7610 %s, msg_hdr=0x%x\n", __func__, pd_msg->msg_hdr);
		if ((pd_msg->msg_hdr & 0xFF) == 0x86) { //ps_ready
			if (pd_port->pe_state_curr == PE_SNK_SELECT_CAPABILITY) {
				pd_port->pe_state_curr = PE_SNK_READY;
				PE_TRANSIT_STATE(pd_port, PE_SNK_READY);
				TCPC_INFO("upm7610 %s, pe_state_curr=%d\n", __func__, pd_port->pe_state_curr);

			}
		}
		if ((pd_msg->msg_hdr & 0xFF) == 0x68) { //get_sink_cap
			TCPC_INFO("upm7610 %s, pe_state_curr=%d\n", __func__, pd_port->pe_state_curr);
			//if (pd_port->pe_state_curr == PE_SNK_SELECT_CAPABILITY) {
			//	pd_port->pe_state_curr = PE_SNK_READY;
				PE_TRANSIT_STATE(pd_port, PE_SNK_READY);
...
int tcpci_report_usb_port_attached(struct tcpc_device *tcpc)
{
	...
	TCPC_INFO("usb_port_attached\n");
        pd_port->pd_src_state = TCPC_SRC_INIT;
