...
int tcpm_typec_src_cap_changed(struct tcpc_device *tcpc, enum tcpc_src_state state)
{
	int ret = 0;
        struct pd_port *pd_port = &tcpc->pd_port;

        if (pd_port->pe_state_curr == PE_SRC_READY && pd_port->pd_src_state != state) {
                pd_dpm_send_source_caps(pd_port);
                pd_port->pd_src_state = state;
        } else {
                ret = -EAGAIN;
        }
	return ret;
}
EXPORT_SYMBOL(tcpm_typec_src_cap_changed);
