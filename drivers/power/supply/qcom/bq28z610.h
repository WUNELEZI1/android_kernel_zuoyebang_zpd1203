...
#define IRREGULATOR_BATT1 1
#define IRREGULATOR_BATT2 2
#define IRREGULATOR_BATT3 3
...
enum bq_fg_mac_cmd {
	...
	FG_MAC_CMD_FG1_DF_CHECK = 0X0005,
	FG_MAC_CMD_CHEM_ID	= 0x0006,
	FG_MAC_CMD_FG1_CHEMID = 0X0008,
...
struct bq_fg_chip {
	...
	int termv;
	int iterm;
	int connector_protect;
	int cv;
	struct poweroff_voltage_config poweroff_conf;
	...
	int (*pfg_read_soh)(struct bq_fg_chip *bq);
	int (*pfg_read_cyclecount)(struct bq_fg_chip *bq);
	int (*pfg_mac_read_block)(struct bq_fg_chip *bq, u16 cmd, u8 *buf, u8 len);
	int (*pfg_mac_write_block)(struct bq_fg_chip *bq, u16 cmd, u8 *data, u8 len);
	int (*pfg_write_block)(struct bq_fg_chip *bq, u8 reg, u8 *data, u8 len);
	int is_eu_mode;
