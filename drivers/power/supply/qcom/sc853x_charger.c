...
static const char* sc853x_psy_name[] = {
    [SC853X_STANDALONE] = "sc-cp-standalone",
...
enum sc853x_fields{
    ...
    PMID2OUT_UVP_DG_SET, PMID2OUT_OVP_DG_SET, IBAT_OCP_DG_SET, VBUS_OCP_DG_SET, VOUT_OVP_DG_SET,/*reg0Fh*/
                ...
                enum sc853x_fields field_id, int val)
{
    ...
    if (ret < 0) {
        dev_err(sc->dev, "sc853x write field %d fail: %d\n", field_id, ret);
    }
...
__maybe_unused static int sc853x_get_status(struct sc853x_chip *sc, uint32_t *status)
{
    int ret, val;
    *status = 0;
...
__maybe_unused static int sc853x_dump_reg(struct sc853x_chip *sc)
{
    int ret;
    int i;
    int val;
...
static int sc853x_set_otg(struct sc853x_chip *sc, bool en)
{
    int ret = 0;
    int value = 0;

    ret = sc853x_field_read(sc, CP_EN, &value);
    if (value)
        ret |= sc853x_field_write(sc, CP_EN, false);
    ...
    if (en) {
        ...
        ret |= sc853x_field_write(sc, PMID2OUT_UVP, 0x07);
        ret |= sc853x_field_write(sc, SS_TIMEOUT, 0);
        ret |= sc853x_field_write(sc, PMID2OUT_UVP_DIS, true);
        ret |= sc853x_field_write(sc, CP_EN, true);
    } else {
        sc->otg_flag = false;
        ret |= sc853x_field_write(sc, CP_EN, false);
        ret |= sc853x_field_write(sc, MODE, sc->cfg.work_mode);
    }
    mdelay(200);
    ret |= sc853x_field_read(sc, CP_SWITCHING_STAT, &value);
    if (en && value == 0) {
        ...
        ret |= sc853x_field_write(sc, PMID2OUT_UVP_DIS, false);
    }
    dev_err(sc->dev, "%s %d value:%d\n", __func__, en, value);
    return ret;
}

static int sc853x_set_ovp_gate(struct sc853x_chip *sc, bool en)
{
    int ret;
    int retry = 0;
...
static int mtk_sc853x_enable_cp_adc(struct charger_device *chg_dev, bool enable)
{
    struct sc853x_chip *sc = charger_get_data(chg_dev);

    return sc853x_enable_adc(sc, enable);
}

static int mtk_sc853x_set_otg(struct charger_device *charger_pump, bool en)
{
    struct sc853x_chip *sc = charger_get_data(charger_pump);
    int ret = 0, value = 0;
    ...
    if(en) {
        ret |= sc853x_field_write(sc, MODE, 2);
        ret |= sc853x_field_write(sc, OTG_EN, true);
        ret |= sc853x_field_write(sc, ACDRV_MANUAL_EN, true);
        ret |= sc853x_field_write(sc, ACDRV_EN, true);
        ret |= sc853x_field_write(sc, PMID2OUT_UVP, 0x07);
...
static const struct charger_ops sc853x_chg_ops = {
    ...
    .enable_cp_adc = mtk_sc853x_enable_cp_adc,
    .enable_otg = mtk_sc853x_set_otg,
};
		...
		int val2, long mask)
{
    ...
    case PSY_IIO_SC_ADC_CONTROL:
        sc853x_enable_adc(sc, val1);
        break;

    case PSY_IIO_SC_OTG_CONTROL:
        sc853x_set_otg(sc, val1);
        break;

    case PSY_IIO_SC_OVP_GATE_CONTROL:
        sc853x_set_ovp_gate(sc, val1);
        break;

    default:
        pr_err("Unsupported sc853x IIO chan %d\n", chan->channel);
        rc = -EINVAL;
        break;
    }
				...
				const struct fwnode_reference_args *iiospec)
{
	...
	for (i = 0; i < ARRAY_SIZE(sc853x_iio_psy_channels); i++, iio_chan++)
...
static const struct iio_info sc853x_iio_info = {
	.read_raw	= sc853x_iio_read_raw,
	.write_raw	= sc853x_iio_write_raw,
	.fwnode_xlate	= sc853x_iio_of_xlate,
};

static int sc853x_init_iio_psy(struct sc853x_chip *sc)
{
    ...
    indio_dev->info = &sc853x_iio_info;
    ...
    if (sc->role == SC853X_STANDALONE) {
        dev_err(sc->dev, "SC853X_STANDALONE\n");
        ...
        for (i = 0; i < num_iio_channels; i++) {
            ...
            chan->datasheet_name = sc853x_iio_psy_channels[i].datasheet_name;
            chan->extend_name = sc853x_iio_psy_channels[i].datasheet_name;
            chan->info_mask_separate = sc853x_iio_psy_channels[i].info_mask;
        }
    }
    ...
    dev_err(sc->dev, "sc853x IIO device\n");
    return rc;
}
...
static const struct dev_pm_ops sc853x_pm_ops = {
    SET_SYSTEM_SLEEP_PM_OPS(sc853x_suspend, sc853x_resume)
};
