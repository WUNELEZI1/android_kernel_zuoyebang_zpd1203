...
#define SC853X_I2C_ADDR1                0x61
#define SC853X_I2C_ADDR2                0x63
...
static const char* sc853x_psy_name[] = {
    [SC853X_STANDALONE] = "primary_dvchg",
...
enum sc853x_fields{
    ...
    CP_SWITCHING_STAT,VAC_INSERT_STAT,/*reg11h*/
...
static const struct reg_field sc853x_reg_fields[] = {
    ...
    /* reg 1a*/
    [ADC_DONE_MSK] = REG_FIELD(0x1a, 7, 7),
    [VAC_OVP_MSK] = REG_FIELD(0x1B, 6, 6),
    /*reg1D*/
...
/*********************************************************/
static void sc853x_detect_addr(struct sc853x_chip *sc)
{
    int i = 0, reg_val = 0;

    for(i = 0; i < 3; i++) {
        sc->client->addr = SC853X_I2C_ADDR2;
        reg_val = i2c_smbus_read_byte_data(sc->client, 0x03);
        if (reg_val < 0) {
            sc->client->addr = SC853X_I2C_ADDR1;
            ...
            dev_err(sc->dev, "get device id succ reg_val = %d\n", reg_val);
            break;
        }
    }
    dev_err(sc->dev, "%s=%d cnt=%d val=%d\n", __func__, sc->client->addr, i, reg_val);
}
...
static int sc853x_plug_in(struct sc853x_chip *sc)
{
    dev_err(sc->dev, "plug in\n");
    sc->plug_in = 1;
...
static int mtk_sc853x_plug_in(struct charger_device *chg_dev)
{
    struct sc853x_chip *sc = charger_get_data(chg_dev);

    dev_err(sc->dev, "sc853x plug in\n");
...
static int sc853x_enable_otg(struct sc853x_chip *sc, bool en)
{
    int ret = 0;
    uint8_t reg = 0;

    ret = sc853x_field_write(sc, OTG_EN, en);
    ret |= sc853x_field_write(sc, ACDRV_MANUAL_EN, en);
    ret |= sc853x_field_write(sc, ACDRV_PRE_DIS, en);
    ret |= sc853x_field_write(sc, ACDRV_EN, en);

    sc853x_read_bulk(sc, 0x0D, &reg, 1);
    dev_info(sc->dev, "%s %d[0x%x], ret =%d\n", __func__, en, reg, ret);

    return ret;
}

static int mtk_sc853x_enable_otg(struct charger_device *chg_dev, bool enable)
{
    struct sc853x_chip *sc = charger_get_data(chg_dev);

    return sc853x_enable_otg(sc, enable);
}

static int mtk_sc853x_dump_registers(struct charger_device *chg_dev)
{
    struct sc853x_chip *sc = charger_get_data(chg_dev);
...
static const struct charger_ops sc853x_chg_ops = {
    ...
    .plug_in = mtk_sc853x_plug_in,
    .enable_otg = mtk_sc853x_enable_otg,
    .dump_registers = mtk_sc853x_dump_registers,
};
...
static void sc853x_dump_check_fault_status(struct sc853x_chip *sc)
{
    ...
    for (i = 0; i <= SC853X_REGMAX; i++) {
        ...
        if (ret < 0)
            dev_err(sc->dev, "%s sc8530 read bulk error", __func__);
        //dev_err(sc->dev, "%s reg[0x%02x] = 0x%02x\n", __func__, i, flag);
...
static irqreturn_t sc853x_irq_handler(int irq, void *dev_id)
{
    ...
    sc853x_field_read(sc, VAC_INSERT_STAT, &vac_status);
    if (vac_status) {
        dev_err(sc->dev, "%s vac insert\n", __func__);
        sc853x_field_write(sc, VAC_OVP_MSK, false);
        sc853x_field_write(sc, ADC_RATE, false); //enable adc
        sc853x_enable_adc(sc, true);
    } else {
        dev_err(sc->dev, "%s vac not insert\n", __func__);
        sc853x_field_write(sc, VAC_OVP_MSK, true);
        sc853x_field_write(sc, ADC_RATE, true); //disable adc
        sc853x_enable_adc(sc, false);
    }
...
static int sc853x_init_device(struct sc853x_chip *sc)
{
    ...
    sc853x_field_write(sc, ADC_DONE_MSK, true);

    //sc853x_field_write(sc, ADC_RATE, true);
    sc853x_enable_adc(sc, true);
    return sc853x_dump_reg(sc);
}
...
static int sc853x_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ret = sc853x_enable_adc(sc, false);
    if (ret < 0) {
        dev_err(sc->dev, "%s disable adc fail(%d)\n", __func__, ret);
        goto err_parse_dt;
    }
...
#endif /* CONFIG_SOUTHCHIP_DVCHG_CLASS */
    sc853x_irq_handler(sc->irq, (void *)sc);
