...
static int set_usbsw(struct device *dev, enum usbsw usbsw)
{
	struct phy *phy;
	int ret, mode = (usbsw == USBSW_CHG) ? PHY_MODE_BC11_SET : PHY_MODE_BC11_CLR;
	dev_info(dev, "usbsw=%d\n", usbsw);
...
static __maybe_unused bool is_usb_rdy(struct device *dev)
{
	bool ready = true;
...
{
	struct delayed_work *bc12_work = to_delayed_work(work);
	struct mtk_ctd_info *mci = container_of(bc12_work, struct mtk_ctd_info, bc12_work);
	struct charger_device *chg = mci->chg_dev[0];
	int ret = 0;

	ret = set_usbsw(mci->dev, USBSW_CHG);
	if (ret) {
		dev_info(mci->dev, "Failed to set usb switch \n");
	}

	ret = charger_dev_force_bc12(chg);
	if (ret) {
		dev_info(mci->dev, "Failed to set chg det, ret=%d\n", ret);
	}
}
				   ...
				   int attach)
{
	...
	if (mci->typec_attach[idx] == ATTACH_TYPE_NONE) { //plug in
		queue_delayed_work(mci->workqueue, &mci->bc12_work, msecs_to_jiffies(350));
		mci->float_retry_cnt = 0;
	} else if (attach == ATTACH_TYPE_NONE){ //plug out
		set_usbsw(mci->dev, USBSW_CHG);
		mci->float_retry_cnt = 0;
	}
...
static int bc12_done_handler(struct mtk_ctd_info *mci)
{
	int ret = 0;
	int i = 0;
	enum lc_chg_type type;

	ret = charger_dev_get_chg_type(mci->chg_dev[0], &type);

	if (LC_CHG_TYPE_UNKNOWN != type && mci->typec_attach[0] >= ATTACH_TYPE_PD) {
		set_usbsw(mci->dev, USBSW_USB);
		dev_info(mci->dev, "%s attach=%d, skip bc12\n", __func__, mci->typec_attach[0]);
	} else {
		switch (type) {
		case LC_CHG_TYPE_FLOAT:
			if (mci->float_retry_cnt < 4 &&
			    mci->typec_attach[0] != ATTACH_TYPE_NONE) {
				mci->float_retry_cnt++;
				queue_delayed_work(mci->workqueue, &mci->bc12_work, msecs_to_jiffies(3000));
			} else {
				xm_charger_dfs_notify(CHG_DFX_NONE_STANDARD_CHG, NULL);
			}
			break;
		case LC_CHG_TYPE_SDP:
		case LC_CHG_TYPE_CDP:
			set_usbsw(mci->dev, USBSW_USB);
			break;
		case LC_CHG_TYPE_DCP:
			charger_dev_set_hvdcp_9V(mci->chg_dev[0]);
			for (i = 0; i < 10; i++) {
				if (type == LC_CHG_TYPE_HVDCP ||
				    mci->typec_attach[0] == ATTACH_TYPE_NONE) {
					dev_info( mci->dev, "%s get hvdcp eixt, type=%d, attach=%d\n",
						__func__, type, mci->typec_attach[0]);
					break;
				}
				msleep(100);
				ret = charger_dev_get_chg_type(mci->chg_dev[0], &type);
	...
	dev_info(mci->dev, "%s chg_type=%s, attach=%d\n", __func__, lc_chg_type_names[type], mci->typec_attach[0]);

	return ret;
}

static int lc_chg_type_chgdev_nb(struct notifier_block *nb, unsigned long event,
				 void *data)
{
	struct mtk_ctd_info *mci =
		container_of(nb, struct mtk_ctd_info, chg_nb);

	dev_info(mci->dev, "%s evt = %lu\n", __func__, event);
	switch (event) {
	case CHARGER_DEV_NOTIFY_FORCE_BC12_DONE:
		bc12_done_handler(mci);
		break;
	default:
		break;
	}
...
skip_get_psy:
	...
	mci->workqueue = create_singlethread_workqueue("mci_bc12_work");
	INIT_DELAYED_WORK(&mci->bc12_work, bc12_work_func);

	mci->chg_nb.notifier_call = lc_chg_type_chgdev_nb;
	ret = register_charger_device_notifier(mci->chg_dev[0], &mci->chg_nb);
