...
static int upm6722_enable_adc_avg(struct upm6722 *upm, bool enable)
{
    u8 val;

    if (enable)
        val = UPM6722_ADC_AVG_RUNNING_VALUE;
    else
        val = UPM6722_ADC_AVG_SINGLE_VALUE;

    val <<= UPM6722_ADC_AVG_SHIFT;

    return upm6722_update_bits(upm, UPM6722_REG_23,
                UPM6722_ADC_AVG_MASK, val);
}
...
{
    upm6722_enable_adc_avg(upm, upm->cfg->adc_avg);
...
static int mtk_upm6722_set_vbusovp(struct charger_device *chg_dev, u32 uV)
{
    ...
    upm_info("mtk_upm6722_set_vbusovp (%d,%d)\n", mv, uV);
...
static int mtk_upm6722_set_ibusocp(struct charger_device *chg_dev, u32 uA)
{
    ...
    int ma;
    ma = uA / 1000;
    upm_info("mtk_upm6722_set_ibusocp = %d ma\n", ma);
...
static int mtk_upm6722_set_vbatovp(struct charger_device *chg_dev, u32 uV)
{
    ...
    upm_info("mtk_upm6722_set_vbatovp = %d mv\n", uV/1000);
    if (ret < 0)
        return ret;
...
static int mtk_upm6722_set_ibatocp(struct charger_device *chg_dev, u32 uA)
{
    ...
    upm_info("mtk_upm6722_set_ibatocp = %d ma\n", uA/1000);
...
static int mtk_upm6722_set_vbusovp_alarm(struct charger_device *chg_dev, u32 uV)
{
    struct upm6722 *upm = charger_get_data(chg_dev);
    int ret;
    upm_info("mtk_upm6722_set_vbusovp_alarm(%d)\n", uV);
...
static int mtk_upm6722_enable_cp_adc(struct charger_device *chg_dev, bool enable)
{
    struct upm6722 *upm = charger_get_data(chg_dev);

    return upm6722_enable_adc(upm, enable);
}

static int mtk_upm6722_enable_otg(struct charger_device *chg_dev, bool enable)
{
    struct upm6722 *upm = charger_get_data(chg_dev);
    int ret = 0;

    if (enable) {
        upm6722_otg_enable(upm->otg_rdev);
    } else {
        upm6722_otg_disable(upm->otg_rdev);
    }
    pr_info("%s: %d, ret =%d\n", __func__, enable, ret);
...
static const struct charger_ops upm_chg_ops = {
	...
	.enable_cp_adc = mtk_upm6722_enable_cp_adc,
	.enable_otg = mtk_upm6722_enable_otg,
...
static int upm6722_psy_register(struct upm6722 *upm)
{
    ...
    if (upm->mode == UPM6722_ROLE_MASTER)
        ...
        upm->psy_desc.name = "primary_dvchg";

    upm->psy_desc.type = POWER_SUPPLY_TYPE_UNKNOWN; //POWER_SUPPLY_TYPE_UNKNOWN;
...
__maybe_unused static void upm6722_cfg_check(struct upm6722 *upm)
{
    u8 val = 0;
    upm_err("CFG  CHECK START\n");
    __upm6722_write_byte(upm, DEBUGMODE_REG, DEBUGMODE_ENTER); //ENTER DEBUG
    
    upm6722_read_byte(upm, CFG_REG01, &val);
    upm_err("RegD5 = 0x%02X\n", val);

    upm6722_read_byte(upm, CFG_REG02, &val);
    upm_err("RegD8 = 0x%02X\n", val);
    __upm6722_write_byte(upm, DEBUGMODE_REG, DEBUGMODE_EXIT); //ENTER DEBUG
}

__maybe_unused static void upm6722_cfg_update(struct upm6722 *upm)
{
    u8 val = 0;
    upm_err("CFG  upadte START\n");
    __upm6722_write_byte(upm, DEBUGMODE_REG, DEBUGMODE_WRITE_EN); //ENTER DEBUG write
    
    upm6722_update_bits(upm, CFG_REG01,
                CFG_REG01_MASK, 1);
    upm6722_update_bits(upm, CFG_REG02,
                CFG_REG02_MASK, 3);  
    ...
    upm6722_read_byte(upm, CFG_REG02, &val);
    upm_err("RegD8 = 0x%02X\n", val);
    __upm6722_write_byte(upm, DEBUGMODE_REG, DEBUGMODE_EXIT); //exit DEBUG
    __upm6722_write_byte(upm, DEBUGMODE_REG, DEBUGMODE_EXIT); //exit DEBUG again
}
...
static int upm6722_otg_enable(struct regulator_dev *rdev)
{
	...
	ret = upm6722_set_vac1ovp_th(upm, 6500);
	if (ret < 0) {
		dev_err(upm->dev, "upm6722 write vac1 ovp threshold fail: %d\n", ret);
		return ret;
	}
...
*/
	ret = upm6722_set_vac1ovp_th(upm, 18000);
	if (ret < 0) {
		dev_err(upm->dev, "upm6722 write vac1 ovp threshold fail: %d\n", ret);
		return ret;
	}
...
static int upm6722_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    struct upm6722 *upm;
    ...
    upm6722_cfg_check(upm);

    //upm6722_cfg_update(upm);  //RELEASE 20241204
