				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SNK_PD30:
			...
			info->pd_type[idx] = pd_connect_tbl[noti->pd_state.connected];
			break;
		...
		int mV, int mA)
{
	...
	if (type == MTK_PD_APDO_START)
		tcpm_ret = tcpm_set_apdo_charging_policy(info->tcpc[active_idx],
		      ...
		      int mV, int mA)
{
	...
	dev_info(info->dev, "%s type:%d %dmV %dmA, ret=%d\n", __func__, type, mV, mA, ret);
				 ...
				 int active_idx, struct adapter_power_cap *cap)
{
	...
	dev_info(info->dev, "%s nr:%d idx:%d\n",
		...
		for (i = 0; i < pd_cap.nr; i++) {
			...
			dev_err(info->dev, "[%s]VBUS = [%d,%d], IBUS = %d, WATT = %d, TYPE = %d\n", __func__,
...
stop_repeat:
	...
	data->support_meas_cap = false;  //for fix huawei P0013-40W reset
	...
	for (i = 0; i < ARRAY_SIZE(apdo_pps_tbl); i++) {
		...
		data->pdp = prog_mv * data->ita_max / 1000000;
                dev->apdo_max = data->pdp;
	}

	ret = pd_set_cap(dev, MTK_PD_APDO_START, max(data->vta_min, 5000), data->ita_max);
	if (ret != MTK_ADAPTER_OK)
		goto out;
...
static int mtk_pd_adapter_probe(struct platform_device *pdev)
{
	...
	dev_info(info->dev, "%s enable force cv = %d\n", __func__, info->force_cv);
