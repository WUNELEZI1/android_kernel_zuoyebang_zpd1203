		...
		struct zram_wb_store_walk *walk)
{
	...
	for (i = 0; i < index; ++i)
		writeback_store_apply(walk->zram, swp_offset(entries[i]), 1, PAGE_WRITEBACK, 0, 0, 0);// 0:wb_max & wb_idle_min & mf_cgroup
}
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (mode == PID_WRITEBACK)
		...
		ret = writeback_store_apply(zram, index, nr_pages, mode, wb_max, wb_idle_min, mf_cgroup);
				       ...
				       struct cftype *cft, u64 val)
{
	...
	if (css->parent && !!val) {
		memcg->android_oem_data1[0] = ANON_WRITEBACK_UNLIMITED;
	}
				       ...
				       struct cftype *cft, u64 val)
{
	...
	if (css->parent && !!val) {
		memcg->android_oem_data1[0] = ANON_WRITEBACK_PROTECTED;
	}

	return 0;
}

static struct cftype memsw_protected_files[] = {
	{
...
#if defined(CONFIG_ZRAM_WRITEBACK)
	WARN_ON(cgroup_add_legacy_cftypes(&memory_cgrp_subsys, memsw_protected_files));
#endif
