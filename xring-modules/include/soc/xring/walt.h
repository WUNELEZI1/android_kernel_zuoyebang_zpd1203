...
struct walt_related_thread_group {
	...
	u64			downmigrate_ts;
	u64			start_ktime_ts;
	cpumask_t		xr_prefer_cpus;
	u32			xr_ed_run_thres;
	u32			xr_ed_wait_thres;
	u32			xr_ed_first_run_thres;
	u64			xr_window_start;
	u64			xr_prev_window_time;
	u64			xr_util;
	u64			xr_prev_running_load;
	u64			xr_prev_running_exec;
	u64			xr_curr_running_load;
	u64			xr_curr_running_exec;
	u64			xr_curr_window_cpu[WALT_NR_CPUS];
	u64			xr_prev_window_cpu[WALT_NR_CPUS];
};
...
struct walt_task_struct {
	...
	u64				mark_start;
	u64				window_start;
	u64				xr_yield_time_sum;
	u64				xr_last_yield_ts;
	u64				xr_running_load_sum;
	u64				xr_running_exec_sum;
	u64				xr_last_exec_sum;
	u64				xr_last_load_sum;
	u64				xr_last_window_exec;
	u64				xr_last_window_load;
	u32				sum, demand;
	u32				coloc_demand;
	...
	u32				curr_window, prev_window, history_window;
	...
	cpumask_t			prior_mask;
	bool				preempt;
	u64				preempt_ts;
	u64				xr_runnable_sum;
	// MIUI ADD: Performance_BoostFramework
	int8_t				prefer_cluster_index;
...
extern int core_ctl_fast_eval(int cpu);
extern int core_ctl_request_resume(int idx, int number);
#endif
...
extern void walt_update_all_cpu(u64 timestamp);
extern void xr_get_task_running_sum(struct task_struct *p, u64 *load_sum, u64 *exec_sum);
extern int sched_set_group_id(struct task_struct *p, unsigned int group_id);
extern unsigned int sched_get_group_id(struct task_struct *p);
extern bool xr_is_rtg_freq_locked(const struct cpumask *query_cpus);

#if IS_ENABLED(CONFIG_XRING_SCHED_VIP)
#define VIP_PRIO_RESERVED	(5)
extern void inherit_vip_prio(struct task_struct *src_p, struct task_struct *dst_p);
extern void restore_vip_prio(struct task_struct *p);
extern int set_vip_prio(struct task_struct *p, int prio, bool set_orig);
#else /* CONFIG_XRING_SCHED_VIP */
static inline void inherit_vip_prio(struct task_struct *src_p, struct task_struct *dst_p)
{

}

static inline void restore_vip_prio(struct task_struct *p) { }
static inline int set_vip_prio(struct task_struct *p, int prio, bool set_orig) { }
#endif

#if IS_ENABLED(CONFIG_XRING_DYN_EM)
extern void xr_update_em_table(int cpu, int em);
#else /* CONFIG_XRING_DYN_EM */
static inline void xr_update_em_table(int cpu, int em) { }
#endif

#else /* CONFIG_XRING_SCHED_WALT */
...
static inline int walt_partial_pause_cpus(struct cpumask *cpus, enum pause_client client)
{
	return 0;
}
static inline int walt_partial_resume_cpus(struct cpumask *cpus, enum pause_client client)
{
	return 0;
}
...
static inline u64 walt_sched_clock(void)
{
	return 0;
}

static inline void walt_update_all_cpu(u64 timestamp)
{
}

static inline void xr_get_task_running_sum(struct task_struct *p, u64 *load_sum, u64 *exec_sum)
{
	if (load_sum != NULL)
		*load_sum = 0;
	if (exec_sum != NULL)
...
static inline int sched_set_group_id(struct task_struct *p, unsigned int group_id)
{
	return 0;
}
...
static inline bool xr_is_rtg_freq_locked(const struct cpumask *query_cpus)
{
	return false;
}
#endif
