		     ...
		     struct WIFI_EVENT *prEvent)
{
	...
	DBGLOG(HAL, INFO, "prEventHifCtrl->u2HifPcieUserCount = %d\n",
	...
	if (prEventHifCtrl->ucHifType == ENUM_HIF_TYPE_PCIE) {
		...
		if (!completion_done(&prAdapter->prGlueInfo->rSRComp)) {
			DBGLOG(HAL, INFO, "PCIE HIFCTRL event done");
			complete(&prAdapter->prGlueInfo->rSRComp);
		} else {
			DBGLOG(HAL, INFO, "PCIE HIFCTRL event no waiter");
		}
#endif
	}
#endif
