...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
...
#ifdef CONFIG_OF
struct WLAN_PINCTRL_OPS __weak fpga_pinctrl_ops = {};
struct WLAN_PINCTRL_OPS __weak f1_pinctrl_ops = {};

const struct of_device_id mtk_wifi_of_ids[] = {
	{	
		.compatible = "xring,fpga-wifi",
		.data = (void *)&fpga_pinctrl_ops,
	},
	{
		.compatible = "xring,f1-wifi",
		.data = (void *)&f1_pinctrl_ops,
	},
	{}
};
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
static int mtk_suspend_recovery(void);
...
struct GLUE_INFO *get_glue_info_isr(void *dev_instance, int irq, int msi_idx)
{
	...
	if (!prGlueInfo) {
		DBGLOG_LIMITED(HAL, INFO, "No glue info in %s(%d, %d),donot enable irq\n",
...
static bool wifiCsrIoremap(struct platform_device *pdev)
{
	...
	struct device_node *node = NULL;
	struct resource res;
#if IS_ENABLED(CFG_XRING_WIFI_PCIE_SUPPORT)
	DBGLOG(INIT, INFO, "%s: start, ignore hostcsr in XRING platform\n", __func__);
	return true;
#endif
	node = of_find_compatible_node(NULL, NULL, "mediatek,wifi");
	if (!node) {
...
static int mtk_wifi_probe(struct platform_device *pdev)
{
	...
	prChipInfo->pinctrl_ops = (struct WLAN_PINCTRL_OPS *)of_device_get_match_data(&pdev->dev);
	DBGLOG(INIT, INFO, "mtk_wifi_probe() SET pinctrl_ops 0x%llx\n", prChipInfo->pinctrl_ops);
#endif
...
static int mtk_pci_suspend(struct pci_dev *pdev, pm_message_t state)
{
...
#endif
	DBGLOG(HAL, STATE, "Only enable MTK PCIE()\n");
	return 0;
#else
	struct device *dev = &pdev->dev;
	struct GLUE_INFO *prGlueInfo = NULL;
	...
	/* Bypass EP Configuration space backup restore by kernel. */
	dev->power.driver_flags = DPM_FLAG_SMART_SUSPEND;
	dev->power.runtime_status = RPM_SUSPENDED;
	pdev->skip_bus_pm = true;
	...
	prAdapter->fgIsInSuspendMode = TRUE;
#endif
	prGlueInfo->rHifInfo.eSuspendtate = PCIE_STATE_PRE_SUSPEND_WAITING;

	rStatus = halPciePreSuspendCmd(prAdapter);
	...
	if (prGlueInfo->rHifInfo.eSuspendtate == PCIE_STATE_PRE_SUSPEND_FAIL) {
		DBGLOG(HAL, INFO, "pcie pre_suspend: FW BUSY, abort suspend\n");
		RECLAIM_POWER_CONTROL_TO_PM(prAdapter, FALSE);
		ret = -EAGAIN;
		goto SUSPEND_PRESUSPEND_FAIL;
	}
...
#if CFG_ENABLE_WAKE_LOCK
	prAdapter->fgIsInSuspendMode = FALSE;
#endif
	DBGLOG(HAL, INFO, "pcie pre_suspend done\n");
	...
	if (prBusInfo->pdmaPollingIdle) {
		if (prBusInfo->pdmaPollingIdle(prGlueInfo) != TRUE) {
			DBGLOG(HAL, ERROR, "POLL DMA IDLE failed, PM TO FW\n");
			RECLAIM_POWER_CONTROL_TO_PM(prAdapter, FALSE);
			ret = -EAGAIN;
	...
	while (wait < 500) {
		 ...
		 * it means that only one FW own is unfinished in suspend flow
		 * We set the SUSPEND_FLAG_FOR_RC_POWER_OFF bit and don't let
		 * FW own up the semaphore.
		 */
		if (KAL_HIF_OWN_TRYLOCK(prAdapter)) {
			if ((prAdapter->u4PwrCtrlBlockCnt == 1)) {
				...
				if (prAdapter->fgIsFwOwn == FALSE) {
					DBGLOG(HAL, INFO, "fgIsFwOwn == FALSE");
					ret = -EAGAIN;
					goto SUSPEND_FW_OWN_FAIL;
				}
				DBGLOG(HAL, STATE, "*********************\n");
				...
				DBGLOG(HAL, INFO, "u4PwrCtrlBlockCnt = %d",
	...
	if (mddpIsMdDrvOwnAcquired())
		...
		if (wait >= 500) {
			DBGLOG(HAL, ERROR, "Set FW Own Timeout, driver is busy !!\n");
			ret = -EAGAIN;
			goto SUSPEND_FW_OWN_FAIL;
		}
...
int mtk_pci_resume(struct pci_dev *pdev)
{
	...
	DBGLOG(HAL, STATE, "mtk_pci_resume(), Only enable MTK PCIE();\n");
	...
	prGlueInfo->prAdapter->fgIsWiFiOnDrvOwn = TRUE;
	up(&prGlueInfo->rDrvOwnSem);
	ACQUIRE_POWER_CONTROL_FROM_PM(prGlueInfo->prAdapter);
	prGlueInfo->prAdapter->fgIsWiFiOnDrvOwn = FALSE;

	DBGLOG(HAL, INFO, "dump ep pcie CR L2 status\n");
	HAL_MCR_WR(prGlueInfo->prAdapter, 0x74030150, 0x3ffff);
	HAL_MCR_RD(prGlueInfo->prAdapter, 0x74030150, &value);
	DBGLOG(HAL, INFO, "0x74030150 = 0x%x\n", value);
	HAL_MCR_RD(prGlueInfo->prAdapter, 0x74030154, &value);
	DBGLOG(HAL, INFO, "0x74030154 = 0x%x\n", value);
	HAL_MCR_RD(prGlueInfo->prAdapter,0x74030184, &value);
	DBGLOG(HAL, INFO, "0x74030184 = 0x%x\n", value);
	HAL_MCR_RD(prGlueInfo->prAdapter,0x7403018C, &value);
	DBGLOG(HAL, INFO, "0x7403018c = 0x%x\n", value);
	HAL_MCR_RD(prGlueInfo->prAdapter, 0x74030348, &value);
	DBGLOG(HAL, INFO, "0x74030348 = 0x%x\n", value);
	HAL_MCR_WR(prGlueInfo->prAdapter, 0x74030168, 0x88880100);
	HAL_MCR_WR(prGlueInfo->prAdapter, 0x74030164, 0x44434241);
	HAL_MCR_RD(prGlueInfo->prAdapter, 0x7403002C, &value);
	DBGLOG(HAL, INFO, "0x7403002C = 0x%x\n", value);
	HAL_MCR_RD(prGlueInfo->prAdapter, 0x740301D4, &value);
	DBGLOG(HAL, INFO, "0x740301D4 = 0x%x\n", value);

	DBGLOG(HAL, STATE, "driver own polling\n");
...
static int mtk_suspend_recovery(void)
{
	...
	DBGLOG(HAL, STATE, "mtk_suspend_recovery()\n");
	...
	DBGLOG(HAL, STATE, "mtk_suspend_recovery() done!\n");
	return 0;
}
#endif
/*----------------------------------------------------------------------------*/
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
static irqreturn_t mtk_oob_interrupt(int irq, void *dev)
{
	struct GLUE_INFO *prGlueInfo = dev;

	DBGLOG(HAL, INFO, "Threaded wakelock irq = %d\n", irq);

	KAL_WAKE_LOCK_TIMEOUT(prGlueInfo->prAdapter,
		prGlueInfo->rTimeoutWakeLock,
		MSEC_TO_JIFFIES(WAKE_LOCK_OOB_TIMEOUT));
...
int32_t glBusSetIrq(void *pvData, void *pfnIsr, void *pvCookie)
{
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
	wlan_host_wake_up = of_get_named_gpio(ppdev->dev.of_node, "wf-wk-gpio", 0);
	if (!gpio_is_valid(wlan_host_wake_up)) {
		DBGLOG(INIT, ERROR, "Invalied gpio pin : %d\n",
			wlan_host_wake_up);
		return -ENODEV;
	}
	DBGLOG(INIT, INFO, "valied gpio pin : %d\n",
		wlan_host_wake_up);

	if (gpio_request(wlan_host_wake_up, "WLAN_HOST_WAKE")) {
		DBGLOG(INIT, ERROR,
			"fail to request gpio(WLAN_HOST_WAKE)\n");
		return -ENODEV;
	}
	DBGLOG(INIT, INFO,
		"success to request gpio(WLAN_HOST_WAKE)\n");
	...
	ret = devm_request_threaded_irq(&ppdev->dev, wlan_host_wake_irq, NULL,
				mtk_oob_interrupt,  IRQF_TRIGGER_FALLING |
				IRQF_NO_SUSPEND | IRQF_ONESHOT,
				"PCIE_WF_WAKE_N", prGlueInfo);

	if (!ret)
		ret = enable_irq_wake(wlan_host_wake_irq);
	else
		DBGLOG(INIT, INFO, "OOB: request threaded irq failed %d\n", ret);

	DBGLOG(INIT, INFO, "OOB: enable irq wake %d\n", ret);
#endif
...
void glBusFreeIrq(void *pvData, void *pvCookie)
{
	...
	if (wlan_host_wake_irq > 0) {
		...
		DBGLOG(INIT, INFO, "free irq and gpio\n");
	}
#endif
}
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
static uint32_t halPciePreSuspendMobileCmd(struct ADAPTER *prAdapter,
		uint8_t ucCID,
		u_int8_t fgSetQuery,
		u_int8_t fgNeedResp,
		u_int8_t fgIsOid,
...
{
	uint32_t rStatus = WLAN_STATUS_PENDING;

#ifdef CFG_SUPPORT_UNIFIED_COMMAND
	rStatus = wlanSendSetQueryCmdHelper(
#else
	rStatus = wlanSendSetQueryCmdAdv(
#endif
	...
	prAdapter->prGlueInfo->fgIsPresuspendEvtWaiting = TRUE;
	DBGLOG(INIT, TRACE, "Donot polling event, using complete...");
	return rStatus;
}
#endif
...
static uint32_t halPciePreSuspendCmd(struct ADAPTER *prAdapter)
{
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
	rStatus = halPciePreSuspendMobileCmd(prAdapter,		/* prAdapter */
	...
	if (rStatus != WLAN_STATUS_PENDING) {
		DBGLOG(HAL, ERROR, "pre_suspend cmd isn't Pending\n");
		return WLAN_STATUS_FAILURE;
	}

	waitRet = wait_for_completion_timeout(&prAdapter->prGlueInfo->rSRComp,
				MSEC_TO_JIFFIES(PM_SR_TIMEOUT));
	if (waitRet > 0) {
		DBGLOG(HAL, INFO, "presuspend cmd OK");
	} else {
		DBGLOG(HAL, WARN, "presuspend cmd TIMEOUT, DUMP dbginfo");
		prAdapter->u4HifDbgFlag |= DEG_HIF_DEFAULT_DUMP;
		halPrintHifDbgInfo(prAdapter);
	}

	return waitRet > 0 ? WLAN_STATUS_SUCCESS : WLAN_STATUS_FAILURE;
#else
...
static uint32_t halPcieResumeCmd(struct ADAPTER *prAdapter)
{
	...
	if (rStatus != WLAN_STATUS_PENDING) {
		DBGLOG(HAL, ERROR, "ResumeCmd isn't Pending\n");
		return WLAN_STATUS_FAILURE;
	}
	...
	waitRet = wait_for_completion_timeout(&prAdapter->prGlueInfo->rSRComp,
				MSEC_TO_JIFFIES(PM_SR_TIMEOUT));
	if (waitRet > 0) {
		DBGLOG(HAL, INFO, "ResumeCmd OK");
	} else {
		DBGLOG(HAL, WARN, "ResumeCmd TIMEOUT");
		prAdapter->u4HifDbgFlag |= DEG_HIF_DEFAULT_DUMP;
		halPrintHifDbgInfo(prAdapter);
	}
...
int32_t glBusFuncOn(void)
{
	...
	if (ret) {
		 ...
		 */
		xring_pcie_remove_port_by_port(1);
		ret = xring_pcie_probe_port_by_port(1);
		if (ret) {
			DBGLOG(HAL, ERROR, "xring_pcie_probe_port_by_port(1) failed, ret=%d\n",
				ret);
			return ret;
		}
#endif
...
{
#if CFG_MTK_WIFI_PCIE_SUPPORT && CFG_MTK_ANDROID_WMT
...
void pcieSetUserCount(uint16_t count)
{
	...
	DBGLOG(HAL, INFO, "pcieSetUserCount: %d\n", count);
}

static uint16_t pcieGetUserCount(void)
{
	DBGLOG(HAL, INFO, "pcieGetUserCount: %d\n", pcie_user_count);
