...
int hif_thread(void *data)
{
	...
	if (prGlueInfo->prAdapter->fgIsInSuspendMode == FALSE) {
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
			if (prGlueInfo->ulFlag & GLUE_FLAG_HIF_TX_PROCESS)
				KAL_WAKE_LOCK_TIMEOUT(prGlueInfo->prAdapter,
						prGlueInfo->rTimeoutWakeLock,
						MSEC_TO_JIFFIES(WAKE_LOCK_TX_TIMEOUT));
#endif
		}
...
{
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
	if (pr->fgIsPresuspendEvtWaiting == FALSE) {
		KAL_WAKE_LOCK(pr->prAdapter, pr->rIntrWakeLock);
		DBGLOG(INIT, TRACE, "%s: Int Presuspend Event Waiting!\n",
			__func__);
	}
#else
	KAL_WAKE_LOCK(pr->prAdapter, pr->rIntrWakeLock);
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
	if (pr->prAdapter->fgIsInSuspendMode == FALSE) {
		KAL_WAKE_LOCK_TIMEOUT(pr->prAdapter, pr->rTimeoutWakeLock,
			MSEC_TO_JIFFIES(
			pr->prAdapter->rWifiVar.u4WakeLockThreadWakeup));
	}
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
	if (KAL_TEST_BIT(SUSPEND_FLAG_FOR_L2_START,
...
void kalPrintUTC(char *msg_buf, int msg_buf_size)
{
	...
	if (ret < 0) {
		...
		printk(KERN_INFO "wifiUTC %s %s", s, msg_buf);
		wifi_salog_write(s,
			strlen(s));
