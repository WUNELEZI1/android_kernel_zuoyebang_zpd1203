		...
		void *pvDriverData)
{
...
#if (CFG_EAP_PCIE_L2_FLOW == 1)
	prGlueInfo->fgIsPresuspendEvtWaiting = FALSE;
#endif
	...
	init_completion(&prGlueInfo->rSRComp);
#endif
...
#if IS_ENABLED(CFG_EAP_PCIE_L2_FLOW)
	sema_init(&prGlueInfo->rDrvOwnSem, 1);
#endif
...
int wlanFuncOn(void)
{
	...
	glGetChipInfo((void **)&chip);
	if (!chip) {
		DBGLOG(HAL, ERROR, "Cannot get CHIP INFO\n");
		return -1;
	}
#endif
...
#ifdef CFG_XRING_PLAT
	DBGLOG(HAL, INFO, "PINCTRL: enable wakeup host irq\n");
	wlan_pinctrl_action(chip, WLAN_PINCTRL_MSG_FUNC_WK_HOST_ON);
#endif
	goto exit;
...
int wlanFuncOff(void)
{
	...
	DBGLOG(HAL, INFO, "PINCTRL: disable wakeup host irq\n");
	wlan_pinctrl_action(chip, WLAN_PINCTRL_MSG_FUNC_WK_HOST_OFF);
#endif
	wlanFuncOffImpl();
	connsys_power_off();
