...
#define WIFI_DBG_FUNC(fmt, arg...)	\
	do { \
		if (gDbgLevel >= WIFI_LOG_DBG) \
			printk_deferred(KERN_INFO PFX "%s[D]: " fmt, __func__, ##arg); \
	} while (0)
#define WIFI_INFO_FUNC(fmt, arg...)	\
	do { \
		if (gDbgLevel >= WIFI_LOG_INFO) \
			printk_deferred(KERN_INFO PFX "%s[I]: " fmt, __func__, ##arg); \
	} while (0)
...
#define WIFI_WARN_FUNC(fmt, arg...)	\
	do { \
		if (gDbgLevel >= WIFI_LOG_WARN) \
			printk_deferred(KERN_WARNING PFX "%s[W]: " fmt, __func__, ##arg); \
	} while (0)
#define WIFI_ERR_FUNC(fmt, arg...)	\
	do { \
		printk_deferred(KERN_ERR PFX "%s[E]: " fmt, __func__, ##arg); \
	} while (0)
