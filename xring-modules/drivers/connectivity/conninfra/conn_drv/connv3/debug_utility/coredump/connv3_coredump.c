...
char * connv3_coredump_get_reason(void *ctx)
{
       struct connv3_dump_ctx *handler = ctx;

       return handler->issue_info.reason;
}

EXPORT_SYMBOL(connv3_coredump_get_reason);
...
static int connv3_dump_end_dump(struct connv3_dump_ctx *ctx)
{
	...
	pr_info("[%s] tag=[%s] dump command=[%s] cmd length=[%lu]\n", __func__, cmd_tag, cmd_str, strlen(cmd_str));
