...
int connv3_plt_pmic_initial_setting_fpga(struct platform_device *pdev, struct connv3_dev_cb* dev_cb);
int connv3_plt_pmic_common_power_ctrl_fpga(u32 enable);
int connv3_plt_pmic_parse_state_fpga(char *buffer, int buf_sz);
static int connv3_plt_pmic_antenna_power_ctrl_fpga(u32 radio, u32 enable);

const struct connv3_platform_pmic_ops g_connv3_platform_pmic_ops_fpga = {
	.pmic_initial_setting = connv3_plt_pmic_initial_setting_fpga,
	.pmic_common_power_ctrl = connv3_plt_pmic_common_power_ctrl_fpga,
	.pmic_parse_state = connv3_plt_pmic_parse_state_fpga,
	.pmic_antenna_power_ctrl = connv3_plt_pmic_antenna_power_ctrl_fpga,
};
...
int connv3_plt_pmic_initial_setting_fpga(struct platform_device *pdev, struct connv3_dev_cb* dev_cb)
{
	...
	if (!g_pmic_en_gpio || !g_faultb_gpio) {
	...
	if (IS_ERR(g_pinctrl_ptr)) {
		pr_err("[%s] get pinctrl fail, %ld fpga has no pinctrl, ignore this", __func__, PTR_ERR(g_pinctrl_ptr));
		//return 0;
	}

	_dump_pmic_gpio_state();
	gpio_request(g_pmic_en_gpio, "fpga-gpio");
	gpio_request(g_faultb_gpio, "fpga-gpio");

	gpio_set_value(g_pmic_en_gpio, 0);
	gpio_set_value(g_faultb_gpio, 0);

	gpio_free(g_pmic_en_gpio);
	gpio_free(g_faultb_gpio);
...
int connv3_plt_pmic_common_power_ctrl_fpga(u32 enable)
{
	...
	if (enable) {
		pr_info("[%s] wcn on using gpio", __func__);

		gpio_request(g_pmic_en_gpio, "fpga-gpio");
		gpio_request(g_faultb_gpio, "fpga-gpio");

		gpio_set_value(g_pmic_en_gpio, 1);

		mdelay(20);

		gpio_set_value(g_faultb_gpio, 1);
...
#define TMP_LOG_SIZE 128
	...
	for (i = 0; i < PMIC_DUMP_REGISTER_SIZE - 4; i++) {
		...
		remain_size -= 3;
		if (i > 0 && (i % 24 == 23)) {
			pr_info("[MT6376-State][MT6639A_ECID][MT6376P_ECID] %s", log_buf);
			log_buf[0] = '\0';
...
int connv3_plt_pmic_antenna_power_ctrl_fpga(u32 radio, u32 enable)
{
	int ret = 0;
