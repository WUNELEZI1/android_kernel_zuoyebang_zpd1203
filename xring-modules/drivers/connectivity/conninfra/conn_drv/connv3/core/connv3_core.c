 ...
 */
#ifndef CONNV3_PWR_OFF_MODE_PMIC_OFF
...
void connectivity_scp_state_notify(enum conn_event_type type)
{
	struct conn_to_shub_msg to_shub_msg;

	pr_info("[%s] type=[%d] init=[%d]", __func__, type, g_conap_inited);
	if (g_conap_inited == false) {
		to_shub_msg.module = MODULE_CONNSYS;
		to_shub_msg.state = DRIVER_STATE_ON;
		pr_info("[%s] connsys power on", __func__);
		shub_conn_notify_state(&to_shub_msg);
		g_conap_inited = true;
	}
	if (type == conn_wifi_on) {
		to_shub_msg.module = MODULE_WIFI;
		to_shub_msg.state = DRIVER_STATE_ON;
		pr_info("[%s] wifi power on", __func__);
		shub_conn_notify_state(&to_shub_msg);
	} else if (type == conn_wifi_off) {
		to_shub_msg.module = MODULE_WIFI;
		to_shub_msg.state = DRIVER_STATE_OFF;
		pr_info("[%s] wifi power off", __func__);
		shub_conn_notify_state(&to_shub_msg);
	} else if (type == conn_bt_on) {
		to_shub_msg.module = MODULE_BT;
		to_shub_msg.state = DRIVER_STATE_ON;
		shub_conn_notify_state(&to_shub_msg);
		pr_info("[%s] bt power on", __func__);
	} else if (type == conn_bt_off) {
		to_shub_msg.module = MODULE_BT;
		to_shub_msg.state = DRIVER_STATE_OFF;
		pr_info("[%s] bt power off", __func__);
		shub_conn_notify_state(&to_shub_msg);
	}
}
...
static int opfunc_power_on_done(struct msg_op_data *op)
{
	...
	if (ret) {
		...
		/* notification */
		if (drv_type == CONNV3_DRV_TYPE_BT)
			connectivity_scp_state_notify(conn_bt_on);
		else if (drv_type == CONNV3_DRV_TYPE_WIFI)
			connectivity_scp_state_notify(conn_wifi_on);
	}
...
static int opfunc_power_off_internal(unsigned int drv_type)
{
	...
	if (drv_type == CONNV3_DRV_TYPE_MAX) {
		...
		/* notification */
		if (drv_type == CONNV3_DRV_TYPE_BT)
			connectivity_scp_state_notify(conn_bt_off);
		else if (drv_type == CONNV3_DRV_TYPE_WIFI)
			connectivity_scp_state_notify(conn_wifi_off);
...
static int opfunc_chip_rst(struct msg_op_data *op)
{
	...
	bt_status = g_connv3_ctx.drv_inst[CONNV3_DRV_TYPE_BT].drv_status;
	wifi_status = g_connv3_ctx.drv_inst[CONNV3_DRV_TYPE_WIFI].drv_status;
					...
					struct connv3_sub_drv_ops_cb *cb)
{
	...
	if (g_connv3_wake_lock.init_flag == 0) {
		pr_info("Skip subsys ops register, perhaps has no connv3 device node in dts");
		return 0;
	}
...
int connv3_core_subsys_ops_unreg(enum connv3_drv_type type)
{
	...
	if (g_connv3_wake_lock.init_flag == 0) {
		pr_info("Skip subsys ops un-register, perhaps has no connv3 device node in dts");
		return 0;
	}
	spin_lock_irqsave(&g_connv3_ctx.infra_lock, flag);
...
int connv3_get_pre_cal_mode(void)
{
	return g_pre_cal_mode;
}
EXPORT_SYMBOL(connv3_get_pre_cal_mode);
