		...
		const struct connlog_emi_config *emi_config)
{
	...
	pr_info("[%s][%s] EMI mapping OK virtual(0x%p) (0x%lx) physical(0x%x) size=%d\n",
		__func__, type_to_title[conn_type],
		(void *)handler->virAddrEmiLogBase, (uintptr_t)handler->virAddrEmiLogBase,
...
int connsys_log_init(int conn_type)
{
	...
	pr_info("[%s][%s] emi=[%llx][%x] block size=[%x][%x]",
				__func__, type_to_title[conn_type],
...
int connsys_dedicated_log_path_apsoc_init(phys_addr_t emiaddr, const struct connlog_emi_config* config)
{
	if (gPhyEmiBase != 0 || emiaddr == 0) {
		pr_err("Connsys log double init or invalid parameter(emiaddr=%llu)\n", emiaddr);
		return -1;
	}
