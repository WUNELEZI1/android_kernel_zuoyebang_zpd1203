...
#define INIT_TX_PINCTRL_NAME	("uart3_4_txd_init")
#define INIT_RX_PINCTRL_NAME	("uart3_4_rxd_init")
#define INIT_CTS_PINCTRL_NAME	("uart3_4_cts_init")
#define INIT_RTS_PINCTRL_NAME	("uart3_4_rts_init")

#define PRE_ON_PINCTRL_NAME		("bt_combo_gpio_pre_on")
#define POWER_ON_TX_PINCTRL_NAME	("uart3_4_txd_default")
#define POWER_ON_RX_PINCTRL_NAME	("uart3_4_rxd_default")
#define POWER_ON_CTS_PINCTRL_NAME	("uart3_4_cts_default")
#define POWER_ON_RTS_PINCTRL_NAME	("uart3_4_rts_default")
...
int btmtk_pre_power_on_handler(void)
{
	...
	btmtk_pinctrl_exec(POWER_ON_CTS_PINCTRL_NAME);
	btmtk_pinctrl_exec(POWER_ON_RTS_PINCTRL_NAME);
...
int btmtk_set_gpio_default(void)
{
	...
	if(!bmain_info->find_my_phone_mode) {
		...
		btmtk_pinctrl_exec(INIT_TX_PINCTRL_NAME);
		btmtk_pinctrl_exec(INIT_RX_PINCTRL_NAME);
		btmtk_pinctrl_exec(INIT_CTS_PINCTRL_NAME);
		btmtk_pinctrl_exec(INIT_RTS_PINCTRL_NAME);
	} else {
...
int btmtk_register_wakeup_irq(struct btmtk_dev *bdev, struct tty_struct *tty) {
	...
	bdev->wakeup_irq = of_get_named_gpio(tty->dev->of_node, "bt-gpio", 0);
...
int btmtk_connv3_sub_drv_init(struct btmtk_dev *bdev)
{
	...
	ret = of_property_read_u32(tty->dev->of_node, "fpga-en", &cif_dev->fpga_en);
	BTMTK_INFO("%s: mediatek,bt fpga-en ret[%d]", __func__, cif_dev->fpga_en);
	if (ret < 0)
		BTMTK_ERR("[ERR] %s: mediatek,bt fpga-en ret[%d]", __func__, ret);

	ret = of_property_read_u32(tty->dev->of_node, "baudrate", &cif_dev->baudrate);
	BTMTK_INFO("%s: mediatek,bt baudrate ret[%d]", __func__, cif_dev->baudrate);
	if (ret < 0)
...
********************************************************************************
*/
int btmtk_send_finder(u8 *buf, u32 len) {
	int ret = 0;
	uint8_t evt[] = {0x04, 0x0E, 0x04, 0x01, 0x64, 0xFD};
...
int btmtk_disable_adv(u8 *buf, u32 len) {
	int ret = 0;
	uint8_t evt[] = {0x04, 0x0E, 0x04, 0x01, 0x39, 0x20, 0x00};
...
int btmtk_uart_launcher_deinit(void)
{
	...
	if (g_sbdev != NULL) {
		...
		while(atomic_read(&g_sbdev->poweroff_finder_mode) && (count <= retry)) {
			BTMTK_WARN("%s:          = %d", __func__, atomic_read(&g_sbdev->poweroff_finder_mode));
			count++;
			mdelay(200);
		}
	}
