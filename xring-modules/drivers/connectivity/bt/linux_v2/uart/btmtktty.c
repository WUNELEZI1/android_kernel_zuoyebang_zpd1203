...
static int btmtk_uart_close(struct hci_dev *hdev)
{
	...
	sys_state_doz2nor_unvote(VOTER_DOZAP_BT);
	BTMTK_INFO("%s bt vote doze!", __func__);
	return 0;
}
...
#endif
{
	...
	case HCIUARTSETBAUD:
		...
		if (g_sbdev != NULL) {
                        if(atomic_read(&g_sbdev->poweroff_finder_mode)) {
				BTMTK_ERR("%s: Set BAUDRATE failed!! ,poweroff_finder_mode:%d", __func__, atomic_read(&g_sbdev->poweroff_finder_mode));
                                return EBUSY;
			}
		}
#if (USE_DEVICE_NODE == 0)
...
void btmtk_wakeup_host(struct btmtk_dev *bdev)
{
	...
	atomic_set(&cif_dev->need_drv_own, 1);
	atomic_set(&cif_dev->fw_wake_isr, 1);
	wake_up_interruptible(&tx_wait_q);
}
...
static int btmtk_uart_driver_own(struct btmtk_dev *bdev)
{
	...
	// bt doze add
	sys_state_doz2nor_vote(VOTER_DOZAP_BT);
	BTMTK_DBG("%s: bt vote normal", __func__);
	// bt doze end
	...
	if (cif_dev->sleep_en && btmtk_get_chip_state(bdev) != BTMTK_STATE_FW_DUMP) {
		u8 wakeup_cmd[] = { 0xFF };
		if(atomic_read(&cif_dev->fw_wake_isr)) {
			/* FW wake up, wait a while for avoid rx pkt error */
			usleep_range(4000, 4100);
		}
		do {
			/* eAP project AP wake up FW flow */
			...
			if (ret < 0)
				BTMTK_ERR("%s btmtk_main_send_cmd 0xFF fai retry[%d]", __func__, retry);
		} while (ret < 0 && --retry);
#if 0
		...
		if (atomic_read(&cif_dev->fw_wake)) {
			...
			usleep_range(4000, 4100);
		} else if(atomic_read(&cif_dev->fw_wake_isr)) {
		    /* eAP project AP wake up by FW isr, still needs to send fw own clr cmd */
			/* wait a while for avoid rx pkt error */
			usleep_range(4000, 4100);
  			do {
  				/* let retry = 0 for only wait for event 500ms */
...
unlock:
	atomic_set(&cif_dev->fw_wake_isr, 0);
	UART_OWN_MUTEX_UNLOCK();
	return ret;
}
#endif
