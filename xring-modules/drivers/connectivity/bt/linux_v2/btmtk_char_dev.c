				...
				if (ioc_buf[4] == 0x05) {
					...
					if (ioc_buf[6] > 240) {
						...
						btmtk_send_finder_eids(ioc_buf, ioc_buf[6] + 7);
                                  	...
                                  	BTMTK_INFO_RAW(ioc_buf, ioc_buf[5] + 6, "%s set header: len[%d] TX: ", __func__, ioc_buf[5] + 6);
					btmtk_send_finder(ioc_buf, ioc_buf[5]+6);
				} else if (ioc_buf[4] == 0x04) {
                                  	BTMTK_INFO_RAW(ioc_buf, ioc_buf[3] + 4, "%s set intervel: len[%d] TX: ", __func__, ioc_buf[3] + 4);
					btmtk_send_finder(ioc_buf, ioc_buf[3]+4);
				}

			}  else if(ioc_buf[0] == 0x01 && ioc_buf[1] == 0x39 && ioc_buf[2] == 0x20) {
					btmtk_disable_adv(ioc_buf, ioc_buf[3]+4);
                          		BTMTK_INFO_RAW(ioc_buf, ioc_buf[3] + 4, "%s disable adv: len[%d] TX: ", __func__, ioc_buf[3] + 4);
			} else
				retval = -EFAULT;
		}
		break;
