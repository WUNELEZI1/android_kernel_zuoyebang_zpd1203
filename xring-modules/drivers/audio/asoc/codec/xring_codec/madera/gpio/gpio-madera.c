...
static int madera_gpio_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(&pdev->dev, "Failed to add pin range (%d)\n", ret);
		return ret;
	}

	dev_info(&pdev->dev, "madera gpio probe finished.\n");
...
{
	return platform_driver_register(&madera_gpio_driver);
}

void madera_gpio_driver_exit(void)
{
	platform_driver_unregister(&madera_gpio_driver);
}
