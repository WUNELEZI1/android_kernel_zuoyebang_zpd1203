...
#define MADERA_IN_UNMUTE_DELAY_MS      100
...
struct madera_interface_params {
	int sample;
	int channel;
	int phy_format;
	int valid_format;
...
struct madera_interface_params madera_interface_p;
#endif

static int madera_usb_port=0;

static int in_unmute_delay = MADERA_IN_UNMUTE_DELAY_MS;

static void madera_in_mute_update_callback(struct work_struct *work)
{
	struct delayed_work *in_unmute_delayed_work = to_delayed_work(work);
	struct in_unmute_work *in_unmute_dwork = container_of(in_unmute_delayed_work, struct in_unmute_work, dwork);
	struct madera_priv *priv = in_unmute_dwork->priv;
	struct snd_soc_component *component = priv->component;
	int decimator = in_unmute_dwork->decimator;
	int reg;

	if (decimator % 2)
		reg = MADERA_ADC_DIGITAL_VOLUME_1L + ((decimator / 2) * 8);
	else
		reg = MADERA_ADC_DIGITAL_VOLUME_1R + ((decimator / 2) * 8);
		  ...
		  struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_info(madera->dev, "madera_clk_ev val:%d, event:%d\n", val, event);
	...
	case MADERA_CLK_SRC_MCLK3:
		...
		break;
	default:
		dev_info(madera->dev, "unused clk_idx\n");
		return 0;
	}

	dev_info(madera->dev, "clk_idx:%d\n", clk_idx);
	switch (event) {
		     ...
		     struct snd_kcontrol *kcontrol, int event)
{
	...
	codec_dbg_trace(trace_codec_event, "%s exit.",
			__func__);
...
int madera_core_init(struct madera_priv *priv)
{
	...
	for (i = 0; i < MADERA_NUM_DECIMATORS; i++) {
		priv->in_unmute_work[i].priv = priv;
		priv->in_unmute_work[i].decimator = i;
		INIT_DELAYED_WORK(&priv->in_unmute_work[i].dwork, madera_in_mute_update_callback);
	}
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	codec_dbg_trace(trace_codec_event, "%s wait wseq.",
				__func__);
	...
	codec_dbg_trace(trace_codec_event, "%s end.",
				__func__);
			    ...
			    struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_info(madera->dev, "mux=%u reg=0x%x inmode=0x%x mask=0x%x val=0x%x\n",
...
static void madera_configure_input_mode(struct madera *madera)
{
	...
	for (i = 0; i < max_dmic_sup; i++) {
		dev_info(madera->dev, "IN%d mode %u:%u:%u:%u\n", i + 1,
...
SOC_ENUM_SINGLE_DECL(madera_anc_ng_enum,
				SND_SOC_NOPM,
				0,
				madera_anc_ng_texts);
		 ...
		 int event)
{
	...
	codec_dbg_trace(trace_codec_event, "%s enter. evt(%d)",
		__func__, event);
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		queue_delayed_work(system_freezable_wq,
				   &priv->in_unmute_work[w->shift].dwork,
				   msecs_to_jiffies(in_unmute_delay));
		  ...
		  struct snd_kcontrol *kcontrol, int event)
{
	...
	codec_dbg_trace(trace_codec_event, "%s evt(%d) shf(%d)",
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		case MADERA_OUT3R_ENA_SHIFT:
			...
			if (!priv->out_up_pending) {
				/* eliminate pop sounds? */
				//msleep(priv->out_up_delay);
				usleep_range(5000, 6000);
				codec_dbg_trace(trace_codec_event, "%s shf:%d.", w->shift,
					__func__);
				priv->out_up_delay = 0;
			}
			break;
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		case MADERA_OUT3R_ENA_SHIFT:
			...
			if (!priv->out_down_pending) {
				...
				usleep_range(5000, 6000);
				codec_dbg_trace(trace_codec_event, "%s post pmd.",
					__func__);
		      ...
		      int source, unsigned int freq, int dir)
{
	...
	*clk = freq;
	...
	dev_info(madera->dev, "%s set to %uHz val:%d\n", name, freq, val);
...
static int madera_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	madera_aif_dbg(dai, "madera_set_fmt enter\n");
			  ...
			  struct snd_soc_dai *dai)
{
	...
	madera_aif_dbg(dai, "base_rate = %d\n", base_rate);
				 ...
				 struct snd_soc_dai *dai)
{
	...
	madera_aif_dbg(dai, "dai_priv->clk = %d\n", dai_priv->clk);
	...
	codec_dbg_trace(trace_codec_event, "%s grp rate.",
				__func__);
...
int madera_usb_port_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	madera_usb_port = ucontrol->value.integer.value[0];
	dev_info(component->dev, "current madera_usb_port value = %d\n", madera_usb_port);
	return 1;
}

void madera_usb_port_set(int value){
	pr_info("%s:enter madera_usb_port:%d",__func__,value);
	madera_usb_port = value;
}
EXPORT_SYMBOL_GPL(madera_usb_port_set);

int madera_usb_port_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	ucontrol->value.integer.value[0] = madera_usb_port;
	dev_info(component->dev, "current madera_usb_port value = %d\n", madera_usb_port);
	return 0;
}

#ifdef CODEC_I2S_DEBUG
void madera_interface_params_init(void)
{
	madera_interface_p.sample = SAMPLE_48000;
	madera_interface_p.channel = CHANNEL_TWO;
	madera_interface_p.valid_format = FMTBIT_32;
	madera_interface_p.phy_format = FMTBIT_32;
	madera_interface_p.tdm_width = FMTBIT_32;
	madera_interface_p.tdm_slots = 0;
};

int madera_interface_sample_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = madera_interface_p.sample;

	return 0;
}

int madera_interface_sample_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	...
	if ((ucontrol->value.integer.value[0] == SAMPLE_8000) ||
		...
		madera_interface_p.sample = ucontrol->value.integer.value[0];
	else
		dev_info(component->dev, "sampel value invalid\n");

	dev_info(component->dev, "current sampel value = %d\n", madera_interface_p.sample);

	return 1;
}

int madera_interface_channel_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = madera_interface_p.channel;

	return 0;
}

int madera_interface_channel_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	...
	if ((ucontrol->value.integer.value[0] == CHANNEL_TWO) ||
		...
		madera_interface_p.channel = ucontrol->value.integer.value[0];
	else
		dev_info(component->dev, "channel value invalid\n");

	dev_info(component->dev, "current channel value = %d\n", madera_interface_p.channel);

	return 1;
}

int madera_interface_phyformat_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = madera_interface_p.phy_format;

	return 0;
}

int madera_interface_phyformat_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	...
	if ((ucontrol->value.integer.value[0] == FMTBIT_16) ||
		...
		madera_interface_p.phy_format = ucontrol->value.integer.value[0];
	else
		dev_info(component->dev, "phy_format value invalid\n");

	dev_info(component->dev, "current phy_format value = %d\n", madera_interface_p.phy_format);

	return 1;
}

int madera_interface_valid_format_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = madera_interface_p.valid_format;

	return 0;
}

int madera_interface_valid_format_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	...
	if ((ucontrol->value.integer.value[0] == FMTBIT_16) ||
		(ucontrol->value.integer.value[0] == FMTBIT_24) ||
		(ucontrol->value.integer.value[0] == FMTBIT_32))
		madera_interface_p.valid_format = ucontrol->value.integer.value[0];
	else
		dev_info(component->dev, "valid_format value invalid\n");

	dev_info(component->dev, "current valid_format value = %d\n", madera_interface_p.valid_format);

	return 1;
}

int madera_interface_tdm_width_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = madera_interface_p.tdm_width;

	return 0;
}

int madera_interface_tdm_width_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	...
	if ((ucontrol->value.integer.value[0] == FMTBIT_16) ||
		(ucontrol->value.integer.value[0] == FMTBIT_24) ||
		(ucontrol->value.integer.value[0] == FMTBIT_32))
		madera_interface_p.tdm_width = ucontrol->value.integer.value[0];
	else
		dev_info(component->dev, "tdm_width value invalid\n");

	dev_info(component->dev, "current tdm_width value = %d\n", madera_interface_p.tdm_width);

	return 1;
}

int madera_interface_tdm_slots_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = madera_interface_p.tdm_slots;

	return 0;
}

int madera_interface_tdm_slots_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	...
	if ((ucontrol->value.integer.value[0] == CHANNEL_TWO) ||
		...
		(ucontrol->value.integer.value[0] == CHANNEL_SIX) ||
		(ucontrol->value.integer.value[0] == CHANNEL_EIGHT))
		madera_interface_p.tdm_slots = ucontrol->value.integer.value[0];
	else
		dev_info(component->dev, "tdm_slots value invalid\n");

	dev_info(component->dev, "current tdm_slots value = %d\n", madera_interface_p.tdm_slots);
			    ...
			    struct snd_soc_dai *dai)
{
	...
	int bclk, lrclk, wl = 0, frame, bclk_target, num_rates;
	int reconfig;
	unsigned int aif_tx_state = 0, aif_rx_state = 0;
	unsigned int format = snd_pcm_format_width(params_format(params));
	unsigned int phy_width = FMTBIT_32;

#ifdef CODEC_I2S_DEBUG
	channels = madera_interface_p.channel;
	rate = madera_interface_p.sample;
#endif

	codec_dbg_trace(trace_codec_event, "%s enter.",
				__func__);

	madera_aif_dbg(dai, "channels=%d,rate=%d,chan_limit=%d,tdm_width=%d,tdm_slots=%d,format=%d\n",
		channels, rate, chan_limit, tdm_width, tdm_slots, format);
	if (rate % 4000) {
...
#ifdef CODEC_I2S_DEBUG
	wl = madera_interface_p.phy_format;
	tdm_slots = madera_interface_p.tdm_slots;
	tdm_width = madera_interface_p.tdm_width;
#endif
	...
	if (tdm_slots) {
...
#ifdef CODEC_I2S_DEBUG
		ret = madera_set_fmt(dai, SND_SOC_DAIFMT_CBC_CFC | SND_SOC_DAIFMT_DSP_A);
		if (ret < 0)
			return ret;
#endif
		...
		if (phy_width >= format)
			bclk_target = channels * rate * phy_width;
		else
	...
	madera_aif_dbg(dai, "bclk_target %dHz format:%d phy_width:%d\n",
		bclk_target, format, phy_width);
				 ...
				 int clk_id, unsigned int freq, int dir)
{
	...
	madera_aif_dbg(dai, "Setting AIF%d to %s\n", dai->id,
			       ...
			       unsigned int rx_mask, int slots, int slot_width)
{
	...
	madera_aif_dbg(dai, "madera_set_tdm_slot enter\n");
...
static int madera_enable_fll(struct madera_fll *fll)
{
	...
	if (already_enabled) {
		...
		codec_dbg_trace(trace_codec_event, "%s pro.",
					__func__);
...
err:
	...
	codec_dbg_trace(trace_codec_event, "%s err.",
			__func__);
