...
const struct snd_soc_dai_ops cs47l92_compr_dai_ops = {
	.compress_new	= snd_soc_new_compress,
};
#endif
enum cs47l92_system_clk {
	CLOCK_NULL,
	CLOCK_FLL1,
	CLOCK_FLL2,
};

static const char * const system_clock_text[] = {"NULL", "FLL1", "FLL2"};
static const struct soc_enum system_clock_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(system_clock_text), system_clock_text);

static int cs47l92_fll1_clock;
static int cs47l92_fll2_clock;

static int cs47l92_set_fll(struct snd_soc_component *component, int fll_id,
			   int source, unsigned int fref, unsigned int fout);

static int cs47l92_fll1_clk_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	...
	cs47l92_fll1_clock = ucontrol->value.integer.value[0];

	codec_dbg_trace(trace_codec_event, "%s enter", __func__);

	if (cs47l92_fll1_clock == CLOCK_FLL1) {
		dev_info(component->dev, "current fll1 clock refclk is 12.288M\n");
		rc = clk_prepare_enable(madera->mclk[MADERA_MCLK2].clk);
		if (rc) {
			dev_err(madera->dev, "mclk1 prepare failed: %d\n", rc);
			return rc;
		}

		rc = clk_prepare_enable(madera->mclk[MADERA_MCLK1].clk);
		if (rc) {
			dev_err(madera->dev, "mclk1 prepare failed: %d\n", rc);
			return rc;
		}

		rc = cs47l92_set_fll(component, MADERA_FLL1_REFCLK,
			MADERA_CLK_SRC_MCLK1, 12288000, 48000 * 2048);
		if (rc < 0) {
			dev_info(component->dev, "Failed to start codec FLL1: %d\n", rc);
			return rc;
		}

		rc = madera_set_sysclk(component, MADERA_CLK_SYSCLK_1,
					       MADERA_CLK_SRC_FLL1,
					       48000 * 2048,/* sysclk_rate */
					       SND_SOC_CLOCK_IN);
		if (rc < 0) {
			dev_info(component->dev, "Failed to set SYSCLK: %d\n", rc);
			return rc;
		}
	} else if (cs47l92_fll1_clock == CLOCK_NULL) {
		dev_info(component->dev, "close codec fll clock\n");
		clk_disable_unprepare(madera->mclk[MADERA_MCLK1].clk);
		clk_disable_unprepare(madera->mclk[MADERA_MCLK2].clk);
	}

	codec_dbg_trace(trace_codec_event, "%s exit", __func__);

	return rc;
}

static int cs47l92_fll1_clk_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = cs47l92_fll1_clock;
	return 0;
}

static int cs47l92_fll2_clk_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct madera_priv *priv = snd_soc_component_get_drvdata(component);
	...
	cs47l92_fll2_clock = ucontrol->value.integer.value[0];

	codec_dbg_trace(trace_codec_event, "%s enter", __func__);

	if (cs47l92_fll2_clock == CLOCK_FLL2) {
		dev_info(component->dev, "current fll2 refclk is 32k\n");
		rc = clk_prepare_enable(madera->mclk[MADERA_MCLK2].clk);
		if (rc) {
			dev_err(madera->dev, "mclk1 prepare failed: %d\n", rc);
			return rc;
		...
		/* mclk2 = 38.4M/2/586 = 32.7645k */
		rc = cs47l92_set_fll(component, MADERA_FLL2_REFCLK,
			MADERA_CLK_SRC_MCLK2, 32764, 48000 * 2048);
		if (rc < 0) {
			dev_info(component->dev, "Failed to start codec FLL2: %d\n", rc);
			return rc;
		}
	...
	} else if (cs47l92_fll2_clock == CLOCK_NULL) {
		dev_info(component->dev, "close codec fll2 clock\n");
		clk_disable_unprepare(madera->mclk[MADERA_MCLK2].clk);
	}
...
static int cs47l92_fll2_clk_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = cs47l92_fll2_clock;
	return 0;
}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	codec_dbg_trace(trace_codec_event, "%s test bits", __func__);
	...
	codec_dbg_trace(trace_codec_event, "%s wsep complete", __func__);
...
#ifdef CODEC_I2S_DEBUG
SOC_SINGLE_EXT("Codec Interface Sample", SND_SOC_NOPM, 0, SAMPLE_48000, 0,
	madera_interface_sample_get, madera_interface_sample_put),
SOC_SINGLE_EXT("Codec Interface Channel", SND_SOC_NOPM, 0, CHANNEL_EIGHT, 0,
	madera_interface_channel_get, madera_interface_channel_put),
SOC_SINGLE_EXT("Codec Interface Physical Formatbit", SND_SOC_NOPM, 0, FMTBIT_32, 0,
	madera_interface_phyformat_get, madera_interface_phyformat_put),
SOC_SINGLE_EXT("Codec Interface Valid Formatbit", SND_SOC_NOPM, 0, FMTBIT_32, 0,
	madera_interface_valid_format_get, madera_interface_valid_format_put),
SOC_SINGLE_EXT("Codec Interface Tdm Width", SND_SOC_NOPM, 0, FMTBIT_32, 0,
	madera_interface_tdm_width_get, madera_interface_tdm_width_put),
SOC_SINGLE_EXT("Codec Interface Tdm Slots", SND_SOC_NOPM, 0, CHANNEL_EIGHT, 0,
	madera_interface_tdm_slots_get, madera_interface_tdm_slots_put),
#endif
SOC_SINGLE_EXT("Codec USB Port", SND_SOC_NOPM, 0, 2, 0,
	madera_usb_port_get, madera_usb_port_put),
SOC_ENUM_EXT("Codec Fll1 Clock", system_clock_enum,
	cs47l92_fll1_clk_get, cs47l92_fll1_clk_put),
SOC_ENUM_EXT("Codec Fll2 Clock", system_clock_enum,
	cs47l92_fll2_clk_get, cs47l92_fll2_clk_put),
};
...
//add dmic
static const char *dmic_supply_text[] = { "OFF", "ON"};
static SOC_ENUM_SINGLE_DECL(dmic_supply_enum, SND_SOC_NOPM, ARRAY_SIZE(dmic_supply_text), dmic_supply_text);

static const struct snd_kcontrol_new dmic_supply =
	SOC_DAPM_ENUM("Dmic Supply", dmic_supply_enum);
...
//add dmic
SND_SOC_DAPM_MUX("DMIC Supply Mux", SND_SOC_NOPM, 0, 0, &dmic_supply),
SND_SOC_DAPM_OUTPUT("DUMMY DMIC OUTPUT"),
SND_SOC_DAPM_INPUT("DUMMY DMIC INPUT"),
...
static struct snd_soc_dapm_route cs47l92_dapm_routes[] = {
	...
	/* top mic */
	{"IN2ALP", NULL, "MICBIAS1A"},
	{"IN2ALN", NULL, "MICBIAS1A"},
	{"IN2BR", NULL, "MICBIAS1A"},
	/* bottom mic */
	{"IN1ALP", NULL, "MICBIAS1B"},
	{"IN1ALN", NULL, "MICBIAS1B"},
	{"IN1BR", NULL, "MICBIAS1B"},
	/* sub mic */
	{"IN1ARP", NULL, "MICBIAS1B"},
	{"IN1ARN", NULL, "MICBIAS1B"},
	/* deco mic */
	{"IN2ARP", NULL, "MICBIAS1D"},
	{"IN2ARN", NULL, "MICBIAS1D"},
	/* headset mic for F1 and o2s p0.1*/
	{"IN1BLP", NULL, "MICBIAS2A"},
	{"IN1BLN", NULL, "MICBIAS2A"},

	/*dmic suppli route*/
	{"DUMMY DMIC OUTPUT", NULL, "DMIC Supply Mux"},
	{"DMIC Supply Mux", "ON", "DUMMY DMIC INPUT"},
	{"DUMMY DMIC INPUT", NULL, "MICBIAS2A"},
};
...
{
	struct cs47l92 *cs47l92 = snd_soc_component_get_drvdata(component);
	codec_dbg_trace(trace_codec_event, "%s enter.",
				__func__);
...
void cs47l92_dapm_ignore_suspend(struct snd_soc_component *component)
{
	struct cs47l92 *cs47l92 = snd_soc_component_get_drvdata(component);
	...
	ret = snd_soc_dapm_ignore_suspend(dapm, "AIF1TX1 Input 1");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1 Input 1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "AIF1TX2 Input 1");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2 Input 1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "LHPF1 Input 1");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "LHPF1 Input 1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "LHPF2 Input 1");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "LHPF2 Input 1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN1L");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "IN1L");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN2L");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "IN2L");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "LHPF1");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "LHPF1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "LHPF2");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "LHPF2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "AIF1TX2 Mixer");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "AIF1TX1 Mixer");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "LHPF1 Mixer");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "LHPF2 Mixer");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN1ALN");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN1ALP");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN2ALN");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN2ALP");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN1BLN");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN1BLP");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN1L Analog Mux");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN2L Analog Mux");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN1L Mode");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "IN2L Mode");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "AIF1 Capture");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "AIF1TX2");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "AIF1TX1");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "AIF1TXCLK");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "FXCLK");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "AIF1TX2");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "MICBIAS1");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "MICBIAS1");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "MICBIAS1B");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "MICBIAS1A");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "MICBIAS1A");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "MICBIAS1A");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "MICVDD");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "MICVDD");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "MICSUPP");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "MICVDD");
		return;
	}

	ret = snd_soc_dapm_ignore_suspend(dapm, "SYSCLK");
	if (ret < 0) {
		dev_err(madera->dev, "Failed to set ignore suspend for pin %s.\n", "SYSCLK");
		return;
	}

	snd_soc_dapm_ignore_suspend(dapm, "AIF1TX3 Input 1");
	snd_soc_dapm_ignore_suspend(dapm, "AIF1TX4 Input 1");
	snd_soc_dapm_ignore_suspend(dapm, "HPOUT1R Input 1");
	snd_soc_dapm_ignore_suspend(dapm, "HPOUT1L Input 1");
	snd_soc_dapm_ignore_suspend(dapm, "AIF1 Playback");
	snd_soc_dapm_ignore_suspend(dapm, "HPOUT1R");
	snd_soc_dapm_ignore_suspend(dapm, "HPOUT1L");
	snd_soc_dapm_ignore_suspend(dapm, "AIF1TX3 Mixer");
	snd_soc_dapm_ignore_suspend(dapm, "AIF1TX4 Mixer");
	snd_soc_dapm_ignore_suspend(dapm, "HPOUT1R Mixer");
	snd_soc_dapm_ignore_suspend(dapm, "HPOUT1L Mixer");
	snd_soc_dapm_ignore_suspend(dapm, "AIF1RX2");
	snd_soc_dapm_ignore_suspend(dapm, "AIF1RX1");
	snd_soc_dapm_ignore_suspend(dapm, "OUT1R");
	snd_soc_dapm_ignore_suspend(dapm, "OUT1L");
	snd_soc_dapm_ignore_suspend(dapm, "AIF1TX4");
	snd_soc_dapm_ignore_suspend(dapm, "AIF1TX3");
	snd_soc_dapm_ignore_suspend(dapm, "NOISE");
	snd_soc_dapm_ignore_suspend(dapm, "TONE");
	snd_soc_dapm_ignore_suspend(dapm, "OUTCLK");
	snd_soc_dapm_ignore_suspend(dapm, "MICBIAS1C");
	snd_soc_dapm_ignore_suspend(dapm, "CPVDD2");
	snd_soc_dapm_ignore_suspend(dapm, "CPVDD1");
}
...
static int cs47l92_component_probe(struct snd_soc_component *component)
{
	...
	cs47l92->core.component = component;
...
#ifdef CODEC_I2S_DEBUG
	madera_interface_params_init();
#endif

	cs47l92_dapm_ignore_suspend(component);
...
static int cs47l92_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "cs47l92 probe\n");
	...
	if (madera->bind_product && !strcmp(madera->bind_product, "dijun")){
		dev_info(madera->dev, "config headset_mic supply to MICBIAS1C for o2s p1 \n");
		bool found_IN1BLP = false;
		bool found_IN1BLN = false;
		for(int i = ARRAY_SIZE(cs47l92_dapm_routes) -1; (i >= 0 && !(found_IN1BLP && found_IN1BLN)); i--){
			if(!strcmp(cs47l92_dapm_routes[i].sink,"IN1BLP") && !strcmp(cs47l92_dapm_routes[i].source,"MICBIAS2A")){
				cs47l92_dapm_routes[i].source = "MICBIAS1C";
				found_IN1BLP = true;
			}
			if(!strcmp(cs47l92_dapm_routes[i].sink,"IN1BLN")  && !strcmp(cs47l92_dapm_routes[i].source,"MICBIAS2A")){
				cs47l92_dapm_routes[i].source = "MICBIAS1C";
				found_IN1BLN = true;
			}
		}
	}
	if (madera->bind_product && !strcmp(madera->bind_product, "jinghu")) {
		dev_info(madera->dev, "config headset_mic supply to MICBIAS1C for o80 p0\n");

		//{"IN2ALP", NULL, "MICBIAS1A"} --> {"IN2ALP", NULL, "MICBIAS1C"}
		bool found_IN2ALP = false;
		bool found_IN2ALN = false;
		for (int i = ARRAY_SIZE(cs47l92_dapm_routes)-1; (i >= 0 && !(found_IN2ALP && found_IN2ALN)); i--) {
			if (!strcmp(cs47l92_dapm_routes[i].sink, "IN2ALP") && !strcmp(cs47l92_dapm_routes[i].source, "MICBIAS1A")) {
				cs47l92_dapm_routes[i].source = "MICBIAS1C";
				found_IN2ALP = true;
			}
			if (!strcmp(cs47l92_dapm_routes[i].sink, "IN2ALN")  && !strcmp(cs47l92_dapm_routes[i].source, "MICBIAS1A")) {
				cs47l92_dapm_routes[i].source = "MICBIAS1C";
				found_IN2ALN = true;
			}
		}

		//{"IN2ARP", NULL, "MICBIAS1D"} --> {"IN2ALP", NULL, "MICBIAS1B"}
		bool found_IN2ARP = false;
		bool found_IN2ARN = false;
		for (int i = ARRAY_SIZE(cs47l92_dapm_routes)-1; (i >= 0 && !(found_IN2ARP && found_IN2ARN)); i--) {
			if (!strcmp(cs47l92_dapm_routes[i].sink, "IN2ARP") && !strcmp(cs47l92_dapm_routes[i].source, "MICBIAS1D")) {
				cs47l92_dapm_routes[i].source = "MICBIAS1B";
				found_IN2ARP = true;
			}
			if (!strcmp(cs47l92_dapm_routes[i].sink, "IN2ARN")  && !strcmp(cs47l92_dapm_routes[i].source, "MICBIAS1D")) {
				cs47l92_dapm_routes[i].source = "MICBIAS1B";
				found_IN2ARN = true;
			}
		}

		//{"IN1ALP", NULL, "MICBIAS1B"} --> {"IN1ALP", NULL, "MICBIAS1A"}
		bool found_IN1ALP = false;
		bool found_IN1ALN = false;
		for (int i = ARRAY_SIZE(cs47l92_dapm_routes)-1; (i >= 0 && !(found_IN1ALP && found_IN1ALN)); i--) {
			if (!strcmp(cs47l92_dapm_routes[i].sink, "IN1ALP") && !strcmp(cs47l92_dapm_routes[i].source, "MICBIAS1B")) {
				cs47l92_dapm_routes[i].source = "MICBIAS1A";
				found_IN1ALP = true;
			}
			if (!strcmp(cs47l92_dapm_routes[i].sink, "IN1ALN")  && !strcmp(cs47l92_dapm_routes[i].source, "MICBIAS1B")) {
				cs47l92_dapm_routes[i].source = "MICBIAS1A";
				found_IN1ALN = true;
			}
		}

		//{"IN1ARP", NULL, "MICBIAS1B"} --> {"IN1ARP", NULL, "MICBIAS1D"}
		bool found_IN1ARP = false;
		bool found_IN1ARN = false;
		for (int i = ARRAY_SIZE(cs47l92_dapm_routes)-1; (i >= 0 && !(found_IN1ARP && found_IN1ARN)); i--) {
			if (!strcmp(cs47l92_dapm_routes[i].sink, "IN1ARP") && !strcmp(cs47l92_dapm_routes[i].source, "MICBIAS1B")) {
				cs47l92_dapm_routes[i].source = "MICBIAS1D";
				found_IN1ARP = true;
			}
			if (!strcmp(cs47l92_dapm_routes[i].sink, "IN1ARN")  && !strcmp(cs47l92_dapm_routes[i].source, "MICBIAS1B")) {
				cs47l92_dapm_routes[i].source = "MICBIAS1D";
				found_IN1ARN = true;
			}
		}
	...
	dev_info(&pdev->dev, "cs47l92 driver probe finished.\n");
...
{
	return platform_driver_register(&cs47l92_codec_driver);
}

void cs47l92_codec_driver_exit(void)
{
	platform_driver_unregister(&cs47l92_codec_driver);
}
