			      ...
			      unsigned int group)
{
	...
	dev_info(priv->dev, "%s selecting %u (%s) for group %u (%s)\n",
	...
	if (madera_mux_funcs[selector].func == 0) {
		...
		for (i = 0; i < pin_group->n_pins; ++i) {
			...
			dev_info(priv->dev, "%s setting 0x%x func bits to 0\n",
				__func__, reg);
		...
		dev_info(priv->dev, "%s setting 0x%x func bits to 0x%x\n",
			       ...
			       unsigned long *configs, unsigned int num_configs)
{
	...
	while (num_configs) {
		dev_info(priv->dev, "%s config 0x%lx\n", __func__, *configs);
				     ...
				     unsigned int num_configs)
{
	...
	dev_info(priv->dev, "%s setting group %s\n", __func__,
...
static int madera_pin_probe(struct platform_device *pdev)
{
	...
	dev_info(priv->dev, "pinctrl probed ok\n");
...
{
	return platform_driver_register(&madera_pin_driver);
}

void madera_pin_driver_exit(void)
{
	platform_driver_unregister(&madera_pin_driver);
}
