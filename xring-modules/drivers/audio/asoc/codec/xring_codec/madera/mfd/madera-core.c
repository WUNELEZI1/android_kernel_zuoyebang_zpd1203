...
#define MADERA_32KZ_MCLK1	0
#define MADERA_32KZ_MCLK2	1
#define MADERA_32KZ_SYSCLK_AUTO_DIV 2
...
#define CODEC_CLK 12288000
#define DCVDD_POWER_ALWAYSON
...
EXPORT_SYMBOL_GPL(madera_runtime_resume);
...
EXPORT_SYMBOL_GPL(madera_runtime_suspend);
...
static int madera_get_reset_gpio(struct madera *madera)
{
	...
	reset = devm_gpiod_get_optional(madera->dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(reset))
...
static void madera_configure_micbias(struct madera *madera)
{
	...
	for (i = 0; i < madera->num_micbias; ++i) {
		...
		dev_info(madera->dev, "Set MICBIAS_CTRL%d mask=0x%x val=0x%x\n",
			i + 1, mask, val);
	}
}

#ifdef MADERA_FPGA_DEBUG
void madera_configure_clk_fpga(struct madera *madera)
{
	int ret;

	/* FPGA SYSCLK SRC from MCLK1; FREQ set 12.288M */
	ret = regmap_update_bits(madera->regmap, MADERA_SYSTEM_CLOCK_1,
				MADERA_SYSCLK_FREQ_MASK | MADERA_SYSCLK_SRC_MASK |
				MADERA_SYSCLK_ENA_MASK,
				0x100 | 0x0 | MADERA_SYSCLK_ENA);
}
...
void madera_configure_power_fpga(struct madera *madera)
{
	...
	/* FPGA CP2 ENA, CP2 BYPASS, discharged mode */
	ret = regmap_write(madera->regmap, MADERA_MIC_CHARGE_PUMP_1, 0x7);
	regmap_read(madera->regmap, MADERA_MIC_CHARGE_PUMP_1, &val);

	/* MICB1 ENA, REGULATOR MODE, LVL:1.8V */
	ret = regmap_write(madera->regmap, MADERA_MIC_BIAS_CTRL_1, 0x1a5);
	regmap_read(madera->regmap, MADERA_MIC_BIAS_CTRL_1, &val);

	/* MICB1A-MICB1D ENA */
	ret = regmap_write(madera->regmap, MADERA_MIC_BIAS_CTRL_5, 0x3333);
	regmap_read(madera->regmap, MADERA_MIC_BIAS_CTRL_5, &val);

	/* MICB2 ENA, REGULATOR MODE, LVL:1.8V */
	ret = regmap_write(madera->regmap, MADERA_MIC_BIAS_CTRL_2, 0x1a5);
	regmap_read(madera->regmap, MADERA_MIC_BIAS_CTRL_1, &val);

	/* MICB2A-MICB2B ENA */
	ret = regmap_write(madera->regmap, MADERA_MIC_BIAS_CTRL_6, 0x33);
	regmap_read(madera->regmap, MADERA_MIC_BIAS_CTRL_5, &val);
...
EXPORT_SYMBOL_GPL(madera_configure_power_fpga);
...
void madera_disable_power_fpga(struct madera *madera)
{
	...
	/* FPGA CP2 ENA, CP2 BYPASS, discharged mode */
	ret = regmap_write(madera->regmap, MADERA_MIC_CHARGE_PUMP_1, 0x6);
	regmap_read(madera->regmap, MADERA_MIC_CHARGE_PUMP_1, &val);

	/* MICB1 ENA, REGULATOR MODE, LVL:2.5V */
	ret = regmap_write(madera->regmap, MADERA_MIC_BIAS_CTRL_1, 0x1a4);
	regmap_read(madera->regmap, MADERA_MIC_BIAS_CTRL_1, &val);

	/* MICB1A-MICB1D ENA */
	ret = regmap_write(madera->regmap, MADERA_MIC_BIAS_CTRL_5, 0x2222);
	regmap_read(madera->regmap, MADERA_MIC_BIAS_CTRL_5, &val);
}
EXPORT_SYMBOL_GPL(madera_disable_power_fpga);
#endif

/*static int codec_reset(struct device *dev)
{
	...
	if (!reset_gpio) {
		dev_err(dev, "%s: Looking up %s property in node %s failed\n",
				__func__, "reset_gpios",
				dev->of_node->full_name);
		return 0;
	}

	usleep_range(2000, 2100);
	gpio_request(reset_gpio, "codec-reset-gpios");
	gpio_set_value(reset_gpio, 1);

	return 0;
}*/

struct debugfs_codec {
	unsigned int reg;
	unsigned int val;
	struct regmap *regmap;
};

struct debugfs_codec debugfs_data;
...
EXPORT_SYMBOL_GPL(codec_reg_debugs_write);
...
EXPORT_SYMBOL_GPL(codec_reg_debugs_read);
...
int madera_dev_init(struct madera *madera)
{
	...
	ret = clk_prepare_enable(madera->mclk[MADERA_MCLK1].clk);
	if (ret) {
		dev_err(madera->dev, "mclk1 prepare failed: %d\n", ret);
		return ret;
	}

	ret = clk_set_rate(madera->mclk[MADERA_MCLK1].clk, CODEC_CLK);
	if (ret) {
		dev_err(madera->dev, "mclk1 set failed: %d\n", ret);
		return ret;
	}
	...
	//codec_reset(madera->dev);

	regcache_cache_only(madera->regmap, true);
	regcache_cache_only(madera->regmap_32bit, true);

	debugfs_data.regmap = madera->regmap;


	if(madera->dev->of_node){
		if(!of_property_read_string(madera->dev->of_node, "bind-product", &madera->bind_product)){
			dev_info(madera->dev, "note, it's %s product \n", madera->bind_product);
		}
	}

	if (madera->bind_product && !strcmp(madera->bind_product, "dijun")){
		dev_info(madera->dev, "provide secure_ic_vdd for o2s \n");
		struct regulator *secure_ic_vdd;
		secure_ic_vdd = devm_regulator_get(madera->dev, "SECUREIC");
		if (!IS_ERR(secure_ic_vdd)) {
			ret = regulator_enable(secure_ic_vdd);
			if (ret){
				goto err_devs;
				dev_err(madera->dev, "Failed to request core supplies: %d\n", ret);
			}
		}else{
			dev_err(madera->dev, "Failed to request core supplies: %d\n", ret);
			goto err_devs;
	...
	dev_info(madera->dev, "madera type is %d\n", madera->type);
...
//#endif
	...
	case CS47L15_SILICON_ID:
		if (IS_ENABLED(CONFIG_XRING_MFD_CS47L15)) {
			switch (madera->type) {
			case CS47L15:
	...
	case CS47L35_SILICON_ID:
		if (IS_ENABLED(CONFIG_XRING_MFD_CS47L35)) {
			switch (madera->type) {
			case CS47L35:
	...
	case CS47L85_SILICON_ID:
		if (IS_ENABLED(CONFIG_XRING_MFD_CS47L85)) {
			switch (madera->type) {
			case CS47L85:
			case WM1840:
	...
	case CS47L90_SILICON_ID:
		if (IS_ENABLED(CONFIG_XRING_MFD_CS47L90)) {
			switch (madera->type) {
			case CS47L90:
			case CS47L91:
...
#else
	/* FPGA 32k clock sourced from sysclk auto div*/
	ret = regmap_update_bits(madera->regmap,
			MADERA_CLOCK_32K_1,
			MADERA_CLK_32K_ENA_MASK | MADERA_CLK_32K_SRC_MASK,
			MADERA_CLK_32K_ENA | MADERA_32KZ_SYSCLK_AUTO_DIV);
#endif
	if (ret) {
...
#ifdef MADERA_FPGA_DEBUG
	madera_configure_clk_fpga(madera);
	...
	dev_info(dev, "%s mfd_add_devices %d\n", madera->type_name,
		 madera->rev);
...
#if IS_ENABLED(CONFIG_MIEV)
	mievent  = cdev_tevent_alloc(906001434);
	cdev_tevent_add_str(mievent, "Keyword", "wcd probe failed");
	cdev_tevent_write(mievent);
	cdev_tevent_destroy(mievent);
#endif
	dev_err(madera->dev, "madera_dev_init error: %d\n", ret);
	return ret;
}
