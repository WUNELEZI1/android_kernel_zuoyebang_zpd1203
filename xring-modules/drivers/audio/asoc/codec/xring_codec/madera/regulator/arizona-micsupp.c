...
static void arizona_micsupp_check_cp(struct work_struct *work)
{
	...
	dev_info(micsupp->dev, "val = %d\n", val);

	if (dapm) {
		component = snd_soc_dapm_to_component(dapm);

		dev_info(micsupp->dev, "enable micsupp\n");
					...
					const struct regulator_desc *desc)
{
	...
	if (np) {
		...
		dev_err(micsupp->dev, "np get failed\n");

	return 0;
}

#ifdef MADERA_FPGA_DEBUG
static void arizona_micsupp_configure_fpga(struct arizona_micsupp *micsupp)
{
	int ret, val;

	/* FPGA CP2 ENA, CP2 BYPASS, discharged mode */
	ret = regmap_write(micsupp->regmap, MADERA_MIC_CHARGE_PUMP_1, 0x7);
	regmap_read(micsupp->regmap, MADERA_MIC_CHARGE_PUMP_1, &val);
	dev_info(micsupp->dev, "CP2 val = %d\n", val);
	/* MICB1 ENA, REGULATOR MODE, LVL:2.5V */
	ret = regmap_write(micsupp->regmap, MADERA_MIC_BIAS_CTRL_1, 0x1a5);
	regmap_read(micsupp->regmap, MADERA_MIC_BIAS_CTRL_1, &val);
	dev_info(micsupp->dev, "MICB1 val = %d\n", val);
	/* MICB1A-MICB1D ENA */
	ret = regmap_write(micsupp->regmap, MADERA_MIC_BIAS_CTRL_5, 0x3333);
	regmap_read(micsupp->regmap, MADERA_MIC_BIAS_CTRL_5, &val);
	dev_info(micsupp->dev, "MICB1A-MICB1D val = %d\n", val);
	ret = regmap_write(micsupp->regmap, MADERA_JACK_DETECT_ANALOGUE, 0x0);
	regmap_read(micsupp->regmap, MADERA_JACK_DETECT_ANALOGUE, &val);
	dev_info(micsupp->dev, "MICB1A-MICB1D val = %d\n", val);
...
#else
	arizona_micsupp_configure_fpga(micsupp);
#endif
	return 0;
}
...
void arizona_micsupp_exit(void)
{
