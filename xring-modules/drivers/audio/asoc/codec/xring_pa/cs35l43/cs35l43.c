...
//#define SMARTPA_I2S_DEBUG
#ifdef SMARTPA_I2S_DEBUG
#define SAMPLE_8000	    (8000)
#define SAMPLE_16000	(16000)
#define SAMPLE_48000	(48000)
...
#ifdef SMARTPA_I2S_DEBUG
struct cs35l43_interface_params {
	int sample;
	int channel;
	int phy_format;
	int valid_format;
...
struct cs35l43_interface_params cs35l43_interface_p;
#endif
...
#ifdef SMARTPA_I2S_DEBUG
static void cs35l43_interface_params_init(void)
{
	cs35l43_interface_p.sample = SAMPLE_48000;
	cs35l43_interface_p.channel = CHANNEL_TWO;
	cs35l43_interface_p.valid_format = FMTBIT_32;
	cs35l43_interface_p.phy_format = FMTBIT_32;
};

static int cs35l43_interface_sample_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = cs35l43_interface_p.sample;

	return 0;
}

static int cs35l43_interface_sample_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private *cs35l43 =snd_soc_component_get_drvdata(component);
	...
	if ((ucontrol->value.integer.value[0] == SAMPLE_8000) ||
		...
		cs35l43_interface_p.sample = ucontrol->value.integer.value[0];
	else
		dev_info(cs35l43->dev, "sampel value invalid\n");

	dev_info(cs35l43->dev, "current sampel value = %d\n", cs35l43_interface_p.sample);

	return 1;
}

static int cs35l43_interface_channel_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = cs35l43_interface_p.channel;

	return 0;
}

static int cs35l43_interface_channel_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private *cs35l43 =snd_soc_component_get_drvdata(component);
	...
	if ((ucontrol->value.integer.value[0] == CHANNEL_TWO) ||
		...
		cs35l43_interface_p.channel = ucontrol->value.integer.value[0];
	else
		dev_info(cs35l43->dev, "channel value invalid\n");

	dev_info(cs35l43->dev, "current channel value = %d\n", cs35l43_interface_p.channel);

	return 1;
}

static int cs35l43_interface_phyformat_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = cs35l43_interface_p.phy_format;

	return 0;
}

static int cs35l43_interface_phyformat_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private *cs35l43 =snd_soc_component_get_drvdata(component);
	...
	if ((ucontrol->value.integer.value[0] == FMTBIT_16) ||
		...
		cs35l43_interface_p.phy_format = ucontrol->value.integer.value[0];
	else
		dev_info(cs35l43->dev, "phy_format value invalid\n");

	dev_info(cs35l43->dev, "current phy_format value = %d\n", cs35l43_interface_p.phy_format);

	return 1;
}

static int cs35l43_interface_valid_format_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = cs35l43_interface_p.valid_format;

	return 0;
}

static int cs35l43_interface_valid_format_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private *cs35l43 =snd_soc_component_get_drvdata(component);
	...
	if ((ucontrol->value.integer.value[0] == FMTBIT_16) ||
		(ucontrol->value.integer.value[0] == FMTBIT_24) ||
		(ucontrol->value.integer.value[0] == FMTBIT_32))
		cs35l43_interface_p.valid_format = ucontrol->value.integer.value[0];
	else
		dev_info(cs35l43->dev, "valid_format value invalid\n");

	dev_info(cs35l43->dev, "current valid_format value = %d\n", cs35l43_interface_p.valid_format);
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct cs35l43_private	*cs35l43 = snd_soc_component_get_drvdata(component);
	if(cs35l43->bind_product && !strcmp(cs35l43->bind_product, "dijun")){
		ucontrol->value.integer.value[0] = 0;
				    ...
				    struct snd_ctl_elem_value *ucontrol)
{
	...
	pa_dbg_trace(trace_pa_event, "%s enter", __func__);
	if(cs35l43->bind_product && !strcmp(cs35l43->bind_product, "dijun")){
		...
		if (cs35l43->cs35l43_rcv_pinctrl && cs35l43->enable_rcv_pin_control) {
			...
			if (cs35l43->enable_rcv_switch) {
				...
				ret = pinctrl_select_state(cs35l43->cs35l43_rcv_pinctrl,
			...
			if(ret){
				dev_err(cs35l43->dev, "%s: rcv switch gpio select %d fail \n", __func__,
						cs35l43->enable_rcv_switch);
			}
		}
	}
	pa_dbg_trace(trace_pa_event, "%s exit", __func__);
    return 0;
}
...
static const struct snd_kcontrol_new cs35l43_aud_controls[] = {
	...
	SOC_SINGLE_RANGE_TLV("Digital PCM Volume", CS35L43_AMP_CTRL,
			CS35L43_AMP_VOL_PCM_SHIFT, 0, 0x391, 0,
			dig_vol_tlv),
...
#ifdef SMARTPA_I2S_DEBUG
	SOC_SINGLE_EXT("Interface Sample", SND_SOC_NOPM, 0, SAMPLE_48000, 0,
		cs35l43_interface_sample_get, cs35l43_interface_sample_put),
	SOC_SINGLE_EXT("Interface Channel", SND_SOC_NOPM, 0, CHANNEL_EIGHT, 0,
		cs35l43_interface_channel_get, cs35l43_interface_channel_put),
	SOC_SINGLE_EXT("Interface Physical Formatbit", SND_SOC_NOPM, 0, FMTBIT_32, 0,
		cs35l43_interface_phyformat_get, cs35l43_interface_phyformat_put),
	SOC_SINGLE_EXT("Interface Valid Formatbit", SND_SOC_NOPM, 0, FMTBIT_32, 0,
		cs35l43_interface_valid_format_get, cs35l43_interface_valid_format_put),
#endif
...
static int cs35l43_dsp_reset(struct cs35l43_private *cs35l43)
{
	...
	ret = cs35l43_wm_adsp_write_ctl(&cs35l43->dsp, "CALL_RAM_INIT", WMFW_ADSP2_XM,
				cs35l43->dsp.cs_dsp.fw_id,
				&val, sizeof(u32));
	if (ret < 0)
		dev_err(cs35l43->dev, "Failed to clear CALL_RAM_INIT: %d\n", ret);

	ret = cs35l43_wm_adsp_write_ctl(&cs35l43->dsp, "HALO_STATE", WMFW_ADSP2_XM,
				cs35l43->dsp.cs_dsp.fw_id,
				&val, sizeof(u32));
	if (ret < 0)
...
static int cs35l43_check_dsp_regs(struct cs35l43_private *cs35l43)
{
	...
	ret = cs35l43_wm_adsp_read_ctl(&cs35l43->dsp, "HALO_STATE", WMFW_ADSP2_XM,
				cs35l43->dsp.cs_dsp.fw_id,
				&val, sizeof(u32));
	if (ret < 0) {
				 ...
				 struct snd_soc_dai *dai)
{
	...
	if (asp_width <= FMTBIT_32)
		asp_width = FMTBIT_32;

#ifdef SMARTPA_I2S_DEBUG
	rate = cs35l43_interface_p.sample;
	channel = cs35l43_interface_p.channel;
	asp_wl = cs35l43_interface_p.valid_format;
	asp_width = cs35l43_interface_p.phy_format;
#endif
	dev_info(cs35l43->dev, "%s: wl=%d, width=%d, rate=%d, channels=%d\n", __func__,
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		cs35l43_component_set_sysclk(dai->component, 0, 0, channel * rate * asp_width, 0);
		regmap_update_bits(cs35l43->regmap, CS35L43_ASP_CONTROL2,
...
static int cs35l43_component_probe(struct snd_soc_component *component)
{
	...
	struct cs35l43_private *cs35l43 =
		snd_soc_component_get_drvdata(component);
	dev_info(cs35l43->dev, "DEBUG cs35l43_component_probe enter\n");
#ifdef SMARTPA_I2S_DEBUG
	cs35l43_interface_params_init();
#endif
				    ...
				    int slots, int slot_width)
{
	...
	dev_info(cs35l43->dev, "%s: slots=%d, slot_width=%d\n", __func__, slots, slot_width);
				...
				struct cs35l43_platform_data *pdata)
{
	...
	dev_info(cs35l43->dev, "DEBUG cs35l43 probe E\n");
	...
	if(cs35l43->dev->of_node){
		if(!of_property_read_string(cs35l43->dev->of_node, "bind-product", &cs35l43->bind_product)){
			dev_info(cs35l43->dev, "note, it's %s product \n", cs35l43->bind_product);
		}
	}
	if(cs35l43->bind_product && !strcmp(cs35l43->bind_product, "dijun")){
		struct pinctrl *cs35l43_len_pinctrl = devm_pinctrl_get(cs35l43->dev);
		if (IS_ERR(cs35l43_len_pinctrl)) {
			if (PTR_ERR(cs35l43_len_pinctrl) == -EPROBE_DEFER) {
				dev_err(cs35l43->dev, "pinctrl not ready\n");
				goto err;
			}
			dev_info(cs35l43->dev, "Target does not use pinctrl\n");
			cs35l43_len_pinctrl = NULL;
			goto err;
		}
		struct pinctrl_state *state = pinctrl_lookup_state(cs35l43_len_pinctrl, "gpio_len_enable");
		if (IS_ERR(state)) {
			dev_err(cs35l43->dev, "cannot find'\n");
			goto err;
		}
		ret = pinctrl_select_state(cs35l43_len_pinctrl, state);
		if (ret) {
			dev_err(cs35l43->dev, "cannot select pinctrl state'\n");
			goto err;
		}
		struct pinctrl *cs35l43_default_pinctrl = devm_pinctrl_get(cs35l43->dev);
		if (IS_ERR(cs35l43_default_pinctrl)) {
			if (PTR_ERR(cs35l43_default_pinctrl) == -EPROBE_DEFER) {
				dev_err(cs35l43->dev, "pinctrl not ready\n");
				goto err;
			}
			dev_info(cs35l43->dev, "Target does not use pinctrl\n");
			cs35l43_default_pinctrl = NULL;
			goto err;
		}
		struct pinctrl_state *state1 = pinctrl_lookup_state(cs35l43_default_pinctrl, "default");
		if (IS_ERR(state1)) {
			dev_err(cs35l43->dev, "cannot find'\n");
			goto err;
		}
		ret = pinctrl_select_state(cs35l43_default_pinctrl, state1);
		if (ret) {
			dev_err(cs35l43->dev, "cannot select pinctrl state'\n");
			goto err;
		}
	...
	if(cs35l43->bind_product && !strcmp(cs35l43->bind_product, "jinghu")) {
		struct pinctrl *cs35l43_len_pinctrl = devm_pinctrl_get(cs35l43->dev);
		if (IS_ERR(cs35l43_len_pinctrl)) {
			if (PTR_ERR(cs35l43_len_pinctrl) == -EPROBE_DEFER) {
				dev_err(cs35l43->dev, "pinctrl not ready\n");
