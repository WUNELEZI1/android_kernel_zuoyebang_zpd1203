		      ...
		      unsigned int alg, void *buf, size_t len)
{
	...
	if (cs_ctl == NULL) {
		adsp_err(dsp, "we can't find control '%s'in this firmware\n", name);
	...
	// adsp_info(dsp, "cs35l43_wm_adsp_write_ctl control '%s' with data: 0x%x(ctl sts:%d, cache=0x%x), ret= %d\n",
		     ...
		     unsigned int alg, void *buf, size_t len)
{
	...
	ret = cs_dsp_coeff_read_ctrl(cs_dsp_get_ctl(&dsp->cs_dsp, name, type, alg),
				     0, buf, len);
	mutex_unlock(&dsp->cs_dsp.pwr_lock);
	adsp_info(dsp, "cs35l43_wm_adsp_read_ctl control '%s' with data: 0x%x, ret= %d\n", name, *((int*)buf), ret);
	return ret;
}
...
int cs35l43_wm_adsp_power_up(struct wm_adsp *dsp, bool load_firmware)
{
	int ret = 0;
	char *wmfw_filename = NULL;
	const struct firmware *wmfw_firmware = NULL;
...
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_power_up);

void cs35l43_wm_adsp_power_down(struct wm_adsp *dsp)
{
	cs_dsp_power_down(&dsp->cs_dsp);
}
EXPORT_SYMBOL_GPL(cs35l43_wm_adsp_power_down);
...
static void wm_adsp_boot_work(struct work_struct *work)
{
	...
	cs35l43_wm_adsp_power_up(dsp, true);
}
			...
			struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		break;
	case SND_SOC_DAPM_PRE_PMD:
		cs35l43_wm_adsp_power_down(dsp);
		break;
	default:
		break;
	}
...
static int cs35l43_wm_adsp_event_post_run(struct cs_dsp *cs_dsp)
{
	struct wm_adsp *dsp = container_of(cs_dsp, struct wm_adsp, cs_dsp);
...
static void cs35l43_wm_adsp_event_post_stop(struct cs_dsp *cs_dsp)
{
	struct wm_adsp *dsp = container_of(cs_dsp, struct wm_adsp, cs_dsp);
...
{
	struct wm_adsp_compr_buf *buf = NULL, *tmp = NULL;

	if (compr->dsp->fatal_error)
		return -EINVAL;

	list_for_each_entry(tmp, &compr->dsp->buffer_list, list) {
		if (tmp != NULL && (!tmp->name || !strcmp(compr->name, tmp->name))) {
			buf = tmp;
			break;
		}
	}
...
static int wm_adsp_buffer_init(struct wm_adsp *dsp)
{
	struct cs_dsp_coeff_ctl *cs_ctl = NULL;
	int ret;

	list_for_each_entry(cs_ctl, &dsp->cs_dsp.ctl_list, list) {
		if (cs_ctl != NULL && cs_ctl->type != WMFW_CTL_TYPE_HOST_BUFFER)
			continue;
...
{
	struct wm_adsp *dsp = container_of(cs_dsp, struct wm_adsp, cs_dsp);
	struct wm_adsp_compr *compr = NULL;

	dsp->fatal_error = true;

	list_for_each_entry(compr, &dsp->compr_list, list) {
		if (compr != NULL && compr->stream)
			snd_compr_fragment_elapsed(compr->stream);
	}
}
...
static const struct cs_dsp_client_ops wm_adsp2_client_ops = {
	...
	.post_run = cs35l43_wm_adsp_event_post_run,
	.post_stop = cs35l43_wm_adsp_event_post_stop,
