...
static void cs35l45_wm_adsp2_init_debugfs(struct wm_adsp *dsp,
...
#else
static inline void cs35l45_wm_adsp2_init_debugfs(struct wm_adsp *dsp,
					 struct snd_soc_component *component)
{
}
...
#endif
int cs35l45_wm_adsp_fw_get(struct snd_kcontrol *kcontrol,
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_fw_get);
int cs35l45_wm_adsp_fw_put(struct snd_kcontrol *kcontrol,
		   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct soc_enum *e = (struct soc_enum *)kcontrol->private_value;
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_fw_put);
const struct soc_enum cs35l45_wm_adsp_fw_enum[] = {
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_fw_enum);
...
int cs35l45_wm_adsp_handle_fw_event(struct wm_adsp *dsp)
{
	struct wm_coeff_ctl *ctl;
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_handle_fw_event);
...
int cs35l45_wm_adsp_write_ctl(struct wm_adsp *dsp, const char *name, int type,
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_write_ctl);
int cs35l45_wm_adsp_read_ctl(struct wm_adsp *dsp, const char *name, int type,
		     unsigned int alg, void *buf, size_t len)
{
	struct wm_coeff_ctl *ctl;
	ctl = wm_adsp_get_ctl(dsp, name, type, alg);
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_read_ctl);
...
int cs35l45_wm_adsp_load_coeff(struct wm_adsp *dsp)
{
	LIST_HEAD(buf_list);
	struct regmap *regmap = dsp->regmap;
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_load_coeff);
...
int cs35l45_wm_adsp1_init(struct wm_adsp *dsp)
{
	dsp->ops = &wm_adsp1_ops;
	return wm_adsp_common_init(dsp);
}
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp1_init);
int cs35l45_wm_adsp1_event(struct snd_soc_dapm_widget *w,
		   struct snd_kcontrol *kcontrol,
		   int event)
{
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp1_event);
...
int cs35l45_wm_adsp2_set_dspclk(struct snd_soc_dapm_widget *w, unsigned int freq)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
	struct wm_adsp *dsps = snd_soc_component_get_drvdata(component);
	struct wm_adsp *dsp = &dsps[w->shift];
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp2_set_dspclk);
int cs35l45_wm_adsp2_preloader_get(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct wm_adsp *dsps = snd_soc_component_get_drvdata(component);
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp2_preloader_get);
int cs35l45_wm_adsp2_preloader_put(struct snd_kcontrol *kcontrol,
			   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct wm_adsp *dsps = snd_soc_component_get_drvdata(component);
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp2_preloader_put);
...
int cs35l45_wm_adsp_early_event(struct snd_soc_dapm_widget *w,
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_early_event);
...
static int wm_halo_apply_calibration(struct snd_soc_dapm_widget *w)
{
	...
	struct wm_adsp *dsp = &dsps[w->shift];
	int ret, val;
	__be32 ambient = cpu_to_be32(0);
	adsp_dbg(dsp, "%s:\n", __func__);
	switch (dsp->fw) {
	case WM_ADSP_FW_SPK_CALI:
	case WM_ADSP_FW_SPK_DIAG:
		val = cpu_to_be32(dsp->cal_ambient);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "CAL_AMBIENT", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_dbg(dsp, "%s: Write CAL_AMBIENT, ret=%d\n", __func__, ret);
		cs35l45_wm_adsp_read_ctl(dsp, "CAL_AMBIENT", WMFW_ADSP2_XM, CS35L45_ALGID, &ambient, sizeof(__be32));
		adsp_info(dsp, "Read CAL_AMBIENT => %d \n", be32_to_cpu(ambient));
		break;
	case WM_ADSP_FW_SPK_PROT:
		val = cpu_to_be32(CAL_STATUS_DEFAULT);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "CAL_STATUS", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_dbg(dsp, "%s: Write CAL_STATUS ret=%d\n", __func__, ret);

		val = cpu_to_be32(dsp->cal_checksum);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "CAL_CHECKSUM", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_dbg(dsp, "%s: Write CAL_CHECKSUM ret=%d\n", __func__, ret);

		val = cpu_to_be32(dsp->cal_r);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "CAL_R", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_dbg(dsp, "%s: Write CAL_R, ret=%d\n", __func__, ret);
		val = cpu_to_be32(1);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "EXT_BST_CTRL_EN", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_warn(dsp, "%s: Write EXT_BST_CTRL_EN ret=%d\n", __func__, ret);
		val = cpu_to_be32(0x898);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "EXT_VBST_VALUE_THLD", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_warn(dsp, "%s: Write EXT_VBST_VALUE_THLD ret=%d\n", __func__, ret);
		val = cpu_to_be32(0x269a69);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "EXT_AUDIO_PEAK_ON_THD", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_warn(dsp, "%s: Write EXT_AUDIO_PEAK_ON_THD ret=%d\n", __func__, ret);
		val = cpu_to_be32(0x0);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "EXT_UVP_THLD", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_warn(dsp, "%s: Write EXT_UVP_THLD ret=%d\n", __func__, ret);
		val = cpu_to_be32(0xce4);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "EXT_OVP_THLD", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
			adsp_warn(dsp, "%s: Write EXT_OVP_THLD ret=%d\n", __func__, ret);
		#if 0
		val = cpu_to_be32(150);
		ret = cs35l45_wm_adsp_write_ctl(dsp, "DC_OFFSET_HOLD_TIME", WMFW_ADSP2_XM, CS35L45_ALGID, (void *)&val, sizeof(__be32));
		if (ret)
...
int cs35l45_wm_adsp_event(struct snd_soc_dapm_widget *w,
		  struct snd_kcontrol *kcontrol, int event)
{
	struct snd_soc_component *component = snd_soc_dapm_to_component(w->dapm);
	struct wm_adsp *dsps = snd_soc_component_get_drvdata(component);
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_event);
...
int cs35l45_wm_adsp2_component_probe(struct wm_adsp *dsp, struct snd_soc_component *component)
{
	char preload[32];
	if (!dsp->no_preloader) {
		snprintf(preload, ARRAY_SIZE(preload), "%s Preload", dsp->name);
	...
	cs35l45_wm_adsp2_init_debugfs(dsp, component);
	dsp->component = component;
	snd_soc_add_component_controls(component, wm_adsp_cal_controls, ARRAY_SIZE(wm_adsp_cal_controls));
	return 0;
}
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp2_component_probe);
int cs35l45_wm_adsp2_component_remove(struct wm_adsp *dsp, struct snd_soc_component *component)
{
	wm_adsp2_cleanup_debugfs(dsp);
	return 0;
}
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp2_component_remove);
int cs35l45_wm_adsp2_init(struct wm_adsp *dsp)
{
	int ret;
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp2_init);
int cs35l45_wm_halo_init(struct wm_adsp *dsp)
{
	int ret;
	ret = wm_adsp_common_init(dsp);
	if (ret)
...
EXPORT_SYMBOL_GPL(cs35l45_wm_halo_init);
void cs35l45_wm_adsp2_remove(struct wm_adsp *dsp)
{
	struct wm_coeff_ctl *ctl;
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp2_remove);
...
int cs35l45_wm_adsp_compr_open(struct wm_adsp *dsp, struct snd_compr_stream *stream)
{
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_compr_open);
int cs35l45_wm_adsp_compr_free(struct snd_soc_component *component,
		       struct snd_compr_stream *stream)
{
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_compr_free);
...
int cs35l45_wm_adsp_compr_set_params(struct snd_soc_component *component,
			     struct snd_compr_stream *stream,
			     struct snd_compr_params *params)
{
	struct wm_adsp_compr *compr = stream->runtime->private_data;
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_compr_set_params);
int cs35l45_wm_adsp_compr_get_caps(struct snd_soc_component *component,
			   struct snd_compr_stream *stream,
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_compr_get_caps);
...
int cs35l45_wm_adsp_compr_trigger(struct snd_soc_component *component,
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_compr_trigger);
...
int cs35l45_wm_adsp_compr_handle_irq(struct wm_adsp *dsp)
{
	struct wm_adsp_compr_buf *buf;
	struct wm_adsp_compr *compr;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_compr_handle_irq);
...
int cs35l45_wm_adsp_compr_pointer(struct snd_soc_component *component,
			  struct snd_compr_stream *stream,
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_compr_pointer);
...
int cs35l45_wm_adsp_compr_copy(struct snd_soc_component *component,
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp_compr_copy);
...
irqreturn_t cs35l45_wm_adsp2_bus_error(int irq, void *data)
{
...
EXPORT_SYMBOL_GPL(cs35l45_wm_adsp2_bus_error);
irqreturn_t cs35l45_wm_halo_bus_error(int irq, void *data)
{
	struct wm_adsp *dsp = (struct wm_adsp *)data;
	struct regmap *regmap = dsp->regmap;
...
EXPORT_SYMBOL_GPL(cs35l45_wm_halo_bus_error);
irqreturn_t cs35l45_wm_halo_wdt_expire(int irq, void *data)
{
...
EXPORT_SYMBOL_GPL(cs35l45_wm_halo_wdt_expire);
static const struct wm_adsp_ops wm_adsp1_ops = {
