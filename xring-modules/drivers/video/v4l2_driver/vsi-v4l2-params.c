...
#define INIT_PARAM(member, id)	\
	...
	INIT_PARAM(param->m_jpgquality, V4L2_ENCCFG_JPGQUALITY);
	INIT_PARAM(param->m_restartinterval, V4L2_ENCCFG_RESTARTINTERVAL);
	INIT_PARAM(param->m_skipmode, V4L2_ENCCFG_SKIPMODE);
	...
	INIT_PARAM(param->m_sliceencoding, V4L2_ENCCFG_SLICEENCODING);
}
...
#define COPY_PARAM(member, buf)	\
	...
	for (id = V4L2_ENCCFG_UNKNOWN + 1; id < V4L2_ENCCFG_NO; id++) {
		...
		case V4L2_ENCCFG_JPGQUALITY:
			COPY_PARAM(param->m_jpgquality, buf)
		...
		case V4L2_ENCCFG_SLICEENCODING:
			if (param->m_codecfmt.codecFormat == V4L2_DAEMON_CODEC_ENC_HEVC ||
				param->m_codecfmt.codecFormat == V4L2_DAEMON_CODEC_ENC_H264)
				COPY_PARAM(param->m_sliceencoding, buf)
			break;
		default:
			break;
		}
