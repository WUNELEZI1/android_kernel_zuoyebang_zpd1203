...
#else
#include <dt-bindings/xring/platform-specific/vpu/v4l2/hantro_vpu.h>
#endif
...
#define VSI_VDEV_MAGIC		v4l2_fourcc('V', 'S', 'I', 'X')

/*in enter kernel v4l2's ioctl, it uses device's lock to protect critical area, which is a big
 * bottleneck. XM's ticket 756 is related with it. We build a fake m2m_ctx ptr in non-m2m config
 * to make kernel use instance's lock to workaround this issue. Need regression test.
 **/
#define USE_FAKE_M2M	1
...
#define XRING_STREAM_OFF_TIMEOUT 3000
...
#define v4l2_klog(lvl, fmt, ...) {	\
	if (lvl <= LOGLVL_WARNING) {	\
		vpu_klog_base("[VPU  %s][V4L2 ]", lvl, fmt, ##__VA_ARGS__);	\
	} else if (lvl <= vsi_kloglvl) {	\
		vpu_klog_base("[VPU  %s][V4L2 ]", lvl, fmt, ##__VA_ARGS__);	\
	}	\
}

#define v4l2_init_klog(lvl, fmt, ...) {	\
	if (lvl <= LOGLVL_INFO) {	\
		vpu_klog_base("[VPU  %s][V4L2 ]", lvl, fmt, ##__VA_ARGS__);	\
	} else if (lvl <= vsi_kloglvl) {	\
		vpu_klog_base("[VPU  %s][V4L2 ]", lvl, fmt, ##__VA_ARGS__);	\
...
struct vsi_video_fmt {
	char *name;		//don't exceed 31 characters based on v4l2_fmtdesc definition
...
struct vsi_v4l2_encparams {
	...
	struct v4l2_enccfg_jpgquality m_jpgquality;
	...
	struct v4l2_enccfg_sliceencoding m_sliceencoding;
};
...
struct vsi_v4l2_mediacfg {
	...
	s32 sizeimagedst_bkup[MAX_PLANENO];
};

struct vsi_device_node {
	struct class *mclass;
	struct vsi_v4l2_device *node[MAX_VIDEODEV_NO];
};

struct vsi_v4l2_device {
	u32 deviceid;	//0..MAX_VIDEODEV_NO
	u32 magic;		//for verification, see VSI_VDEV_MAGIC
...
//from daemon
	...
	wait_queue_head_t instance_queue;
	s32 v4l2_fn;
...
//from vsi-v4l2
	...
	struct device vsidaemondev;
	struct mutex vsi_ctx_array_lock;		//it only protect ctx between release from app and msg from daemon
	u64 ctx_seqid;
};
...
enum {
	...
	CTX_FLAG_MAPMETA,				//meta buffer vaddr is got through ioremap, so need iounmap in dqbuf
};
...
struct vsi_v4l2_ctx {
	...
	struct vb2_queue meta_que;
	atomic_t vb2_queue_complete;
	...
	u32 metabufoffset[1];
	u32 inputbufoffset[VIDEO_MAX_FRAME][4];
	u32 outputbufoffset[VIDEO_MAX_FRAME][4];

	s32 queued_srcnum;
	s32 buffed_capnum;
	s32 buffed_cropcapnum;
	u32 lastcapbuffer_idx;	//latest received capture buffer index
	atomic_t buffed_msgnum;
	...
	u64	frameidx;
	s64	addr_offset;
	void *pmeta;		//meta buf's virtual addr
...
#ifdef USE_FAKE_M2M
	struct v4l2_m2m_ctx m2mfake;
	struct mutex ioctl_lock;
#endif

	struct list_head x_pp_list;		//for second pp buf
};

struct vsi_msghdr_node {
	struct list_head list;
...
int vsi_v4l2_handleerror(struct vsi_v4l2_device *vpu, unsigned long ctxtid, int error);
...
static inline int isvalidtype(int buftype, int ctxtype)
{
	if (((ctxtype & CTX_FLAG_DEC) == 0) &&
		...
		buftype == V4L2_BUF_TYPE_META_OUTPUT))
		return 1;
	if (((ctxtype & CTX_FLAG_DEC) != 0) &&
		(buftype == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE ||
		buftype == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE ||
		buftype == V4L2_BUF_TYPE_META_CAPTURE))
		return 1;
	return 0;
}

static inline int ismetabuf(int buftype)
{
	if (buftype == V4L2_BUF_TYPE_META_OUTPUT ||
		buftype == V4L2_BUF_TYPE_META_CAPTURE)
		return 1;
	return 0;
}
...
static inline int brawfmt(int ctxtype, int qtype)
{
	if ((((ctxtype & CTX_FLAG_DEC) == 0) && qtype == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) ||
		(((ctxtype & CTX_FLAG_DEC) != 0) && qtype == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE))
		return 1;
	return 0;
}
...
{
	struct vsi_vpu_buf *buf = NULL, *node = NULL;
...
static inline void return_all_buffers(struct vb2_queue *vq, int status, int bRelbuf)
{
	...
	for (i = 0; i < vq->num_buffers; ++i) {
		if (vq->bufs[i]->state == VB2_BUF_STATE_ACTIVE) {
			v4l2_klog(LOGLVL_DEBUG, "return buffer %d", i);
	...
	if (bRelbuf) {
		list_for_each_entry_safe(buf, node, plist, list) {
			...
			list_del(&buf->list);
			v4l2_klog(LOGLVL_DEBUG, "clear buffer %d", buf->vb.vb2_buf.index);
		}
	}
}
