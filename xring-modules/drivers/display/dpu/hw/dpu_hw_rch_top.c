		...
		struct dpu_iomem *base_mem)
{
	...
	if (blk_end > base_end) {
		DPU_ERROR("wrong rch_top hw info 0x%x, %d, 0x%x, %d\n",
		...
		struct dpu_hw_rch_top_ops *ops)
{
	...
	ops->status_dump = dpu_hw_rdma_top_status_dump;
}
		...
		struct dpu_iomem *base_mem)
{
	...
	hw_rch_top = kzalloc(sizeof(*hw_rch_top), GFP_KERNEL);
	if (ZERO_OR_NULL_PTR(hw_rch_top))
		return ERR_PTR(-ENOMEM);

	ret = dpu_hw_rch_top_res_init(hw_rch_top, rch_top_cap, base_mem);
	if (ret) {
		DPU_ERROR("failed to init rch_top hw blk %d\n",
				rch_top_cap->base.id);
		kfree(hw_rch_top);
		return ERR_PTR(ret);
	}
