		...
		struct dpu_iomem *base_mem)
{
	...
	if (blk_end > base_end) {
		DPU_ERROR("wrong postpq top hw info 0x%x, %d, 0x%x, %d\n",
		...
		struct dpu_iomem *base_mem)
{
	...
	hw_pospq_top = kzalloc(sizeof(*hw_pospq_top), GFP_KERNEL);
	if (ZERO_OR_NULL_PTR(hw_pospq_top))
		return ERR_PTR(-ENOMEM);

	ret = dpu_post_pq_top_cap_init(hw_pospq_top, cap, base_mem);
	if (ret) {
		DPU_ERROR("failed to init post pq top hw blk %d\n",
				cap->base.id);
		kfree(hw_pospq_top);
		return ERR_PTR(ret);
	}
