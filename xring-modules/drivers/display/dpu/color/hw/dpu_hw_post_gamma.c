		...
		struct dpu_iomem *base_mem)
{
	...
	if (blk_end > base_end) {
		DPU_ERROR("wrong post gma hw info 0x%x, %d, 0x%x, %d\n",
		...
		struct dpu_iomem *base_mem)
{
	...
	hw_gamma = kzalloc(sizeof(*hw_gamma), GFP_KERNEL);
	if (ZERO_OR_NULL_PTR(hw_gamma))
		return ERR_PTR(-ENOMEM);

	ret = dpu_hw_post_gamma_cap_init(hw_gamma, cap, base_mem);
	if (ret) {
		DPU_ERROR("failed to init post gamma hw blk %d\n",
				cap->base.id);
		kfree(hw_gamma);
		return ERR_PTR(ret);
	}
