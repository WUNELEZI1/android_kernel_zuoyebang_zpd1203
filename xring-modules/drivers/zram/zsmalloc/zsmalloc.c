...
#define pr_fmt(fmt) "[ZSMALLOC]%s: " fmt, __func__
...
#define ZSMALLOC_MULTI_PAGES_ORDER	(_AC(CONFIG_XRING_ZSMALLOC_MULTI_PAGES_ORDER, UL))
...
#define ZS_MAX_PAGES_PER_ZSPAGE	(_AC(CONFIG_XRING_ZSMALLOC_CHAIN_SIZE, UL))
...
#ifdef CONFIG_XRING_ZSMALLOC_STAT
...
{
	int idx = 0;
	int nr_size_classes = 0;

#ifdef CONFIG_XRING_ZSMALLOC_MULTI_PAGES
	if (size > PAGE_SIZE) {
		idx = ZS_PAGE_SIZE_CLASSES - 1;
		idx += DIV_ROUND_UP(size - PAGE_SIZE,
				ZS_MULTI_PAGES_SIZE_CLASS_DELTA);
		nr_size_classes = ZS_SIZE_CLASSES - 1;
		goto out;
	}
#endif
	...
	if (likely(size > ZS_MIN_ALLOC_SIZE)) {
		...
		nr_size_classes = ZS_PAGE_SIZE_CLASSES - 1;
	}

#ifdef CONFIG_XRING_ZSMALLOC_MULTI_PAGES
out:
#endif
	pr_debug("size=%d, idx=%d, nr_size_classes=%d", size, idx, nr_size_classes);
	return min_t(int, nr_size_classes, idx);
}
...
{
	if (!zs_stat_root) {
		pr_err("no root stat dir, not creating <%s> stat dir\n", name);
		return;
	}
...
#else /* CONFIG_XRING_ZSMALLOC_STAT */
static void __init zs_stat_init(void)
{
}
	...
	for (i = 0; i < class->pages_per_zspage; i++) {
		...
		if (!page) {
			while (--i >= 0) {
				...
				clear_bit(PG_oem_reserved_1, &(pages[i]->flags));
...
unsigned long zs_malloc(struct zs_pool *pool, size_t size, gfp_t gfp)
{
	...
	size += ZS_HANDLE_SIZE;
	class = pool->size_class[get_size_class_index(size)];
	pr_debug("size=%lu, class->size=%d", size, class->size);
	...
	if (likely(zspage)) {
		...
		pr_debug("found obj, zspage=%p, obj=%lu", zspage, obj);
		goto out;
	}
	...
	spin_lock(&pool->lock);
	obj = obj_malloc(pool, zspage, handle);
	pr_debug("alloc new zspage, zspage=%p, obj=%lu", zspage, obj);
	newfg = get_fullness_group(class, zspage);
	insert_zspage(class, zspage, newfg);
		...
		enum migrate_mode mode)
{
	...
	set_bit(PG_oem_reserved_1, &newpage->flags);
...
static int calculate_zspage_chain_size(int class_size)
{
	...
	if (is_power_of_2(class_size)) {
		pr_debug("is power of 2, class_size=%d", class_size);
		return chain_size;
	}
	...
	pr_debug("class_size=%d, page_size=%lu, min_waste=%d, chain_size=%d",
		 class_size, page_size, min_waste, chain_size);
...
struct zs_pool *zs_create_pool(const char *name)
{
	...
	for (i = ZS_SIZE_CLASSES - 1; i >= 0; i--) {
...
#ifdef CONFIG_XRING_ZSMALLOC_MULTI_PAGES
		if (i >= ZS_PAGE_SIZE_CLASSES)
			size = PAGE_SIZE + (i - ZS_PAGE_SIZE_CLASSES + 1) *
					   ZS_MULTI_PAGES_SIZE_CLASS_DELTA;
		...
		if (!huge_class_size[idx]) {
			...
			pr_debug("set huge_class_size[%d]=%lu, pages_per_zspage=%d, objs_per_zspage=%d",
				idx, huge_class_size[idx], pages_per_zspage, objs_per_zspage);
		}
