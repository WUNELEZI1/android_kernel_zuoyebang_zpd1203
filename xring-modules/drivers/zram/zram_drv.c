...
#ifdef CONFIG_XRING_ZRAM_MULTI_PAGES
static ssize_t multi_pages_stat_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct zram *zram = dev_to_zram(dev);
	ssize_t ret = 0;
...
#ifdef CONFIG_XRING_ZRAM_MULTI_PAGES
static DEVICE_ATTR_RO(multi_pages_stat);
#endif
...
#ifdef CONFIG_XRING_ZRAM_MULTI_PAGES
	&dev_attr_multi_pages_stat.attr,
#endif
#ifdef CONFIG_ZRAM_WRITEBACK
