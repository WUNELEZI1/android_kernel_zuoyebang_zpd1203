...
static int ffa_version_check(u32 *version)
{
	...
	if (ver.a0 == FFA_RET_NOT_SUPPORTED) {
		pr_err("[%s %d] FFA_VERSION returned not supported\n", __func__, __LINE__);
		return -EOPNOTSUPP;
	}

	if (ver.a0 < FFA_MIN_VERSION) {
		pr_err("[%s %d] Incompatible v%d.%d! Earliest supported v%d.%d\n", __func__, __LINE__,
...
ffa_partition_probe(const uuid_t *uuid, struct ffa_partition_info **buffer)
{
	...
	count = __ffa_partition_info_get(uuid0_4[0], uuid0_4[1], uuid0_4[2],
					 uuid0_4[3], NULL, 0);
	if (count <= 0) {
		pr_err("[%s %d] count is %d\n", __func__, __LINE__, count);
		return count;
	}

	pbuf = kcalloc(count, sizeof(*pbuf), GFP_KERNEL);
	if (!pbuf) {
		pr_err("[%s %d] pbuf alloc is failed\n", __func__, __LINE__);
		return -ENOMEM;
	}
				   ...
				   struct ffa_send_direct_data *data)
{
	...
	pr_err("[%s %d] a0 is error 0x%lx\n", __func__, __LINE__, ret.a0);
	return -EINVAL;
}
		       ...
		       struct ffa_mem_ops_args *args)
{
	...
	if (frag_len > max_fragsize) {
		pr_err("[%s %d] frag_len is 0x%x\n", __func__, __LINE__, frag_len);
		return -ENXIO;
	}
	...
	do {
		if (frag_len == max_fragsize) {
			rc = ffa_transmit_fragment(func_id, addr, buf_sz,
						   ...
						   &args->g_handle, first);
			if (rc < 0) {
				pr_err("[%s %d] ffa_transmit_fragment is failed\n", __func__, __LINE__);
				return -ENXIO;
			}
		...
		if ((void *)constituents - buffer > max_fragsize) {
			pr_err("[%s %d] Memory Region Fragment > Tx Buffer size\n", __func__, __LINE__);
			return -EFAULT;
		}
...
static int ffa_memory_ops(u32 func_id, struct ffa_mem_ops_args *args)
{
	...
	if (!args->use_txbuf) {
		...
		if (!buffer) {
			pr_err("[%s %d] alloc_pages_exact is failed\n", __func__, __LINE__);
			return -ENOMEM;
		}
	} else {
	...
	if (ret < 0)
		pr_err("[%s %d] ffa_setup_and_transmit is failed\n", __func__, __LINE__);
				  ...
				  struct ffa_partition_info *buffer)
{
	...
	if (count <= 0) {
		pr_err("[%s %d] ffa_partition_probe is failed\n", __func__, __LINE__);
		return -ENOENT;
	}
...
void ffa_device_match_uuid(struct ffa_device *ffa_dev, const uuid_t *uuid)
{
	 ...
	 */
	if (drv_info->version > FFA_VERSION_1_0) {
		pr_err("[%s %d] version is 0x%x\n", __func__, __LINE__, drv_info->version);
		return;
	}
...
static int __init ffa_init(void)
{
	...
	if (ret) {
		pr_err("[%s %d] ffa_transport_init is failed\n", __func__, __LINE__);
		return ret;
	}

	ret = arm_ffa_bus_init();
	if (ret) {
		pr_err("[%s %d] arm_ffa_bus_init is failed\n", __func__, __LINE__);
		return ret;
	}

	drv_info = kzalloc(sizeof(*drv_info), GFP_KERNEL);
	if (!drv_info) {
		pr_err("[%s %d] drv_info alloc is failed\n", __func__, __LINE__);
		ret = -ENOMEM;
		goto ffa_bus_exit;
	}
	...
	if (ret) {
		pr_err("[%s %d] version check is failed\n", __func__, __LINE__);
		goto free_drv_info;
	}

	if (ffa_id_get(&drv_info->vm_id)) {
		pr_err("[%s %d] failed to obtain VM id for self\n", __func__, __LINE__);
		ret = -ENODEV;
		goto free_drv_info;
	}
	...
	if (!drv_info->rx_buffer) {
		pr_err("[%s %d] rx_buffer alloc is failed\n", __func__, __LINE__);
		ret = -ENOMEM;
		goto free_pages;
	}
	...
	if (!drv_info->tx_buffer) {
		pr_err("[%s %d] tx_buffer alloc is failed\n", __func__, __LINE__);
		ret = -ENOMEM;
		goto free_pages;
	}
	...
	if (ret) {
		pr_err("[%s %d] failed to register FFA RxTx buffers\n", __func__, __LINE__);
		goto free_pages;
	}
