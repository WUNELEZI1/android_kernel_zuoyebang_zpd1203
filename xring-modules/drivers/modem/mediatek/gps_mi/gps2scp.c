...
static const char *g_name_for_log[MODULE_NUM_MAX] = {"GNSS", "BT", "WIFI", "MODEM"};
static struct conn_to_shub_msg g_to_shub_msg[MODULE_NUM_MAX] = {
	[MODULE_GNSS] = {MODULE_GNSS, DRIVER_STATE_ON},
    [MODULE_BT] = {MODULE_GNSS, DRIVER_STATE_OFF},
};
...
static atomic_t my_counter = ATOMIC_INIT(2);
struct scp2gps_data {
	int size;
	unsigned int msg_id;
...
static void my_work_function(struct work_struct *work)
{
	...
	pr_info("val is %d\n",val);
	if (2 == val) {
		pr_info("gps2scp_poll have none data\n");
	} else {
		debug_cmd.drv = 2;
		debug_cmd.cmd = val;
		debug_cmd.param =  0;
		size = sizeof(debug_cmd);
		save_scp2gps_data(msg_id, (unsigned int *)&debug_cmd, size);
		pr_info("lpp  message save");
		atomic_set(&my_counter, 2);
	}
    wake_up_interruptible_all(&GPS2SCP_wq);  // wake up epoll_wait()
}

static void _gnss_conn_cb_for_shub(struct shub_conn_to_ap_msg *shub_request_msg)
{
	/*unsigned int msg_id = 1;
	//unsigned int *buf;
	unsigned int size;
	struct conn_debug_cmd debug_cmd;*/
	if (shub_request_msg->module != MODULE_GNSS) {
		pr_info("Shub msg target:%s, not GNSS.\n", g_name_for_log[shub_request_msg->module]);
		return;
	}
	/* SHUB_REQUEST_GNSS_ON = 0,
	 * SHUB_REQUEST_BT_ON = 1,
	 * SHUB_REQUEST_WIFI_ON = 2,
	 * SHUB_REQUEST_MODEM_ON = 3,
	 * SHUB_REQUEST_MODULE_STATUS = 4,
	 */
	pr_info("Sensorhub request cmd is %d.\n", shub_request_msg->request_cmd);
	//pr_info("GNSS status is %d.\n", g_to_shub_msg[MODULE_GNSS].state);
	if (1 == shub_request_msg->request_cmd ) {
		/*debug_cmd.drv = 2;
		debug_cmd.cmd = 1;
		debug_cmd.param =  0;
		size = sizeof(debug_cmd);
		save_scp2gps_data(msg_id, (unsigned int *)&debug_cmd, size);*/
		atomic_set(&my_counter, 1);
		pr_info("lpp init message send");
		schedule_work(&my_work);

	}else if (0 == shub_request_msg->request_cmd){
		/*debug_cmd.drv = 2;
		debug_cmd.cmd = 0;
		debug_cmd.param =  0;
		size = sizeof(debug_cmd);
		...
		pr_info("lpp deinit message send");
		schedule_work(&my_work);
		pr_info("deinit lpp message send");
	}
	//pr_info("GNSS status is %d.\n", g_to_shub_msg[MODULE_GNSS].state);
	//notify_shub_conn_state(&g_to_shub_msg[MODULE_GNSS]);
}
...
void gps2scp_msg_cb(unsigned int msg_id, unsigned int *buf, unsigned int size)
{
	...
	if (msg_id == 0) {
		...
		wake_up_interruptible_all(&GPS2SCP_wq);
#ifdef GPS2SCP_TEST_ENABLE
	} else if (msg_id == 1) {
...
static ssize_t gps2scp_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	result_val = o_buf[4];
	pr_info("gps2scp buf[4] = %d.\n", result_val);
	if (result_val == 4) {
		 shub_conn_notify_state(&g_to_shub_msg[0]);
	} else if (result_val == 5){
		shub_conn_notify_state(&g_to_shub_msg[1]);
	} else {
		pr_info("gps2scp cmd is fail");
	}
...
static unsigned int gps2scp_poll(struct file *file, poll_table *wait)
{
	...
	if (scp2gps_data_received.msg_read != scp2gps_data_received.msg_write)
		mask = (POLLIN | POLLRDNORM);
	pr_info("gps2scp_poll has been return , mask = %d\n", mask);
...
static int gps2scp_init(void)
{
    ...
    shub_conn_regist_callback(MODULE_GNSS, _gnss_conn_cb_for_shub);
