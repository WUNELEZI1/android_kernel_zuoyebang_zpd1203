...
#define MAX_DELTA_TICKS		0x7fffffff
#define MIN_DELTA_TICKS		5000
#define MAX_LOAD_COUNT		~0

#ifdef pr_fmt
#undef pr_fmt
#endif
#define pr_fmt(fmt)  "XR_APBTIMER " fmt
...
static void apbt_disable_int(struct dw_apb_timer *timer)
{
	...
	spin_lock_irqsave(&timer->slock, flags);
	u32 ctrl = apbt_readl(timer, APBTMR_N_CONTROL);

	ctrl |= APBTMR_CONTROL_INT;
	apbt_writel(timer, ctrl, APBTMR_N_CONTROL);
	spin_unlock_irqrestore(&timer->slock, flags);
}
...
static int apbt_shutdown(struct clock_event_device *evt)
{
	...
	spin_lock_irqsave(&dw_ced->timer.slock, flags);
	ctrl = apbt_readl(&dw_ced->timer, APBTMR_N_CONTROL);
	ctrl &= ~APBTMR_CONTROL_ENABLE;
	apbt_writel(&dw_ced->timer, ctrl, APBTMR_N_CONTROL);
	spin_unlock_irqrestore(&dw_ced->timer.slock, flags);
	return 0;
}
...
static int apbt_set_oneshot(struct clock_event_device *evt)
{
	 ...
	 * running mode.
	 */
	apbt_writel(&dw_ced->timer, MAX_LOAD_COUNT, APBTMR_N_LOAD_COUNT);
	ctrl &= ~APBTMR_CONTROL_INT;
	ctrl |= APBTMR_CONTROL_ENABLE;
	apbt_writel(&dw_ced->timer, ctrl, APBTMR_N_CONTROL);
...
dw_apb_clockevent_init(int cpu, const char *name, unsigned int rating,
		       void __iomem *base, int irq, unsigned long freq)
{
	...
	dw_ced->ced.max_delta_ns = clockevent_delta2ns(MAX_DELTA_TICKS,
						       &dw_ced->ced);
	dw_ced->ced.max_delta_ticks = MAX_DELTA_TICKS;
	dw_ced->ced.min_delta_ns = clockevent_delta2ns(MIN_DELTA_TICKS, &dw_ced->ced);
	dw_ced->ced.min_delta_ticks = MIN_DELTA_TICKS;
	...
	dw_ced->eoi = apbt_eoi;
	spin_lock_init(&dw_ced->timer.slock);
...
void dw_apb_clocksource_start(struct dw_apb_clocksource *dw_cs)
{
	 ...
	 */
	spin_lock_irqsave(&dw_cs->timer.slock, flags);
	u32 ctrl = apbt_readl(&dw_cs->timer, APBTMR_N_CONTROL);

	ctrl &= ~APBTMR_CONTROL_ENABLE;
	apbt_writel(&dw_cs->timer, ctrl, APBTMR_N_CONTROL);
	apbt_writel(&dw_cs->timer, MAX_LOAD_COUNT, APBTMR_N_LOAD_COUNT);
	/* enable, mask interrupt */
	ctrl &= ~APBTMR_CONTROL_MODE_PERIODIC;
	ctrl |= (APBTMR_CONTROL_ENABLE | APBTMR_CONTROL_INT);
	apbt_writel(&dw_cs->timer, ctrl, APBTMR_N_CONTROL);
	spin_unlock_irqrestore(&dw_cs->timer.slock, flags);
...
dw_apb_clocksource_init(unsigned int rating, const char *name, void __iomem *base,
			unsigned long freq)
{
	...
	spin_lock_init(&dw_cs->timer.slock);
	return dw_cs;
}
