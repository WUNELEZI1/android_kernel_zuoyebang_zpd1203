 ...
 *
 * @read_byte_reg: Reads a bytes with addr.
 * @return the byte read
 *
 * @write_byte: Writes a byte. Same as 8 touch_bit(x) calls.
 *
 * @write_byte_reg: Writes a byte with addr.
 *
 ...
 * @return -1=Error, 0=completed* @return -1=Error, 0=completed
 *
 ...
 *
 * @skip: Search slave when there is only one slave on bus
 * u8 is search_type, W1_SKIP_ROM
 *
 * @match: Search slave with id use addr
 * u8 is search_type, W1_MATCH_ROM
 *
 * @cmd_start: Start cmd translate
 * @return -x=Error, 0=completed
 *
 * @read_block_reg:Read a series of bytes
 *
 * @write_block_reg: Write a series of bytes
 * @return -1=Error, 0 or positve int=completed
 *
...
struct w1_bus_master {
	...
	/*add for xr 1-wire*/
	void (*skip)(void *, u8);
	void (*match)(void *, const u8 *, int);
	int (*read_rom)(void *data, u8 val);
	int (*cmd_start)(void *);
	void (*set_resp_delay)(void *data, int val);
	void (*read_block_reg)(void *, u8 *, int);
	int (*write_block_reg)(void *, const u8 *, int);
};
...
int w1_write_block_reg(struct w1_master *, const u8 *, int);
void w1_touch_block(struct w1_master *, u8 *, int);
u8 w1_read_block(struct w1_master *, u8 *, int);
u8 w1_read_block_reg(struct w1_master *, u8 *, int);
