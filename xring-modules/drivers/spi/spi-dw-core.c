...
static irqreturn_t dw_spi_irq(int irq, void *dev_id)
{
	...
	irq_status = dw_readl(dws, DW_SPI_ISR) & 0x3f;
	if (!irq_status) {
		spin_unlock_irqrestore(&dws->lock, flags);
		return IRQ_NONE;
	}
...
static inline u32 get_spi_rate(u32 pll_rate, u32 crg_div, u32 spi_div)
{
	return pll_rate / (crg_div * spi_div);
}

static int spi_rate_check(struct dw_spi *dws,
	u32 freq, u32 pll_rate)
{
	int ret = 0;
	u32 max_rate = 0, min_rate = 0;

	max_rate = get_spi_rate(pll_rate, dws->crg_div_min, SPI_DIV_MIN);
	min_rate = get_spi_rate(pll_rate, dws->crg_div_max, SPI_DIV_MAX);

	if (freq > max_rate) {
		dev_err(dws->dev,
			"max speed is %d Hz, request speed is %d Hz\n",
			max_rate, freq);
		ret = -EINVAL;
	}
	...
	if (freq < min_rate) {
		dev_err(dws->dev,
			"min speed is %d Hz, request speed is %d Hz\n",
			min_rate, freq);
		ret = -EINVAL;
	}
...
static int set_closet_spi_freq(struct dw_spi *dws, u32 freq, struct chip_data *chip)
{
	u32 spi_div = 0, crg_div = 0;
	u32 pll_freq = 0, clk_div_freq = 0;
	u32 best_freq = 0, best_spi_div = 0, best_crg_div = 0;
	u32 tmp = 0;
	struct dw_spi_mmio *dwsmmio = container_of(dws, struct dw_spi_mmio, dws);
	int ret;

	pll_freq = clk_get_rate(clk_get_parent(dwsmmio->clk_div));

	if (spi_rate_check(dws, freq, pll_freq))
		return -EINVAL;

	for (spi_div = SPI_DIV_MIN; spi_div <= SPI_DIV_MAX; spi_div += 2) {
		for (crg_div = dws->crg_div_min; crg_div <= dws->crg_div_max; crg_div++) {
			tmp = get_spi_rate(pll_freq, crg_div, spi_div);
			if (tmp > (u32)freq)
				continue;

			if (tmp > best_freq) {
				best_freq = tmp;
				best_crg_div = crg_div;
				best_spi_div = spi_div;
			}
			/*
	...
	if (!best_crg_div || !best_spi_div)
		return -EINVAL;

	clk_div_freq = DIV_ROUND_UP(pll_freq, best_crg_div);
	ret = clk_set_rate(dwsmmio->clk_div, clk_div_freq);
	if (ret < 0) {
		dev_err(&dws->master->dev, "failed to set the clk_div freq\n");
		return ret;
	}

	dws->max_freq = clk_get_rate(dwsmmio->clk);
	dw_writel(dws, DW_SPI_BAUDR, best_spi_div);
	chip->rx_sample_dly = best_spi_div / 2;

	dev_dbg(&dws->master->dev, "req_freq = %d, actual_freq = %d",
		freq, dws->max_freq / best_spi_div);
...
#if IS_ENABLED(CONFIG_XRING_SPI)
int dw_spi_update_config(struct dw_spi *dws, struct spi_device *spi,
			  struct dw_spi_cfg *cfg)
{
	...
	if (dws->current_freq != cfg->freq) {
		ret = set_closet_spi_freq(dws, cfg->freq, chip);
		if (ret < 0) {
			dev_err(&dws->master->dev, "fail to set closet spi clk\n");
			return ret;
		}
		dws->current_freq = cfg->freq;
	}
...
#if IS_ENABLED(IS_ENABLED(CONFIG_XRING_SPI))
int dw_spi_prepare_transfer_hardware(struct spi_master *master)
{
	struct dw_spi *dws = spi_controller_get_devdata(master);
	struct hwspinlock *hwlock = dws->spi_hwspin_lock;
	...
	if (dws->hardware_lock) {
		do {
			...
			if (time_after(time, timeout)) {
				...
				if (ret) {
					dev_err(&dws->master->dev, "Get hwspinlock(%d) mid failed\n",
						lock_id);
					return -EINVAL;
				}
				dev_err(&dws->master->dev,
					"Get hardware_mutex(%d) complete timeout, used by %d\n",
					lock_id, cur_mid_r);
				return -ETIME;
			}
	...
	ret = spi_runtime_resume(dws->dev);
	if (ret < 0) {
		dev_err(&dws->master->dev, "spi_runtime_resume failed %d\n", ret);
		return ret;
	}

	dw_writel(dws, DW_SPI_SER, 0x1);
...
int dw_spi_unprepare_transfer_hardware(struct spi_master *master)
{
	...
	if (dws->transfer_state == SPI_TRANSFER_PREPARED ||
			dws->transfer_state == SPI_TRANSFER_ERR) {
		dw_writel(dws, DW_SPI_SER, 0x0);
		ret = spi_runtime_suspend(dws->dev);
		if (ret)
			dev_err(&dws->master->dev, "spi_runtime_suspend failed %d\n", ret);
	}
...
static int dw_spi_exec_mem_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	ret = dw_spi_update_config(dws, mem->spi, &cfg);
	if (ret < 0)
		return ret;
...
int dw_spi_add_host(struct device *dev, struct dw_spi *dws)
{
	...
	ret = request_irq(dws->irq, dw_spi_irq, IRQF_TRIGGER_HIGH, dev_name(dev),
				master);
...
void dw_spi_remove_host(struct dw_spi *dws)
{
	...
	if (dws->is_spi_dma)
		free_irq(dws->dma_complete_irq, dws);
}
