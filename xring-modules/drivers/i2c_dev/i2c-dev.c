...
#define I2C_RX_DATA_MAX_COUNT	8192
...
int check_test_case(struct i2c_client *client, unsigned int cmd)
{
	if (cmd == I2C_SMBUS) {
		dev_info(&client->adapter->dev, "[%s:%d], not support i2c_smbus test.\n",
			__func__, __LINE__);
		return -EINVAL;
	}
		...
		loff_t *offset)
{
	...
	if (count > I2C_RX_DATA_MAX_COUNT)
		count = I2C_RX_DATA_MAX_COUNT;
		...
		unsigned int nmsgs, struct i2c_msg *msgs)
{
	...
	for (i = 0; i < nmsgs; i++) {
		...
		if (msgs[i].len > I2C_RX_DATA_MAX_COUNT) {
			res = -EINVAL;
			break;
		}
...
static long i2cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	status = check_test_case(client, cmd);
	if (status)
		return 0;
	...
	case I2C_RDWR: {
		status = copy_from_user(&rdwr_arg,
	...
	case I2C_SMBUS: {
		status = copy_from_user(&data_arg,
...
static long compat_i2cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case I2C_RDWR: {
		...
		for (i = 0; i < rdwr_arg.nmsgs; i++) {
			status = copy_from_user(&umsg, p + i, sizeof(umsg));
			if (status) {
				kfree(rdwr_pa);
				return -EFAULT;
			}
