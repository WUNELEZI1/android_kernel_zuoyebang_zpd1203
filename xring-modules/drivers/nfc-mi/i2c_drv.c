...
void print_rw_buffer(const unsigned char *buf, const int len, bool is_send)
{
	unsigned char output[MAX_DL_BUFFER_SIZE*2 + 1];
	int i;
			...
			if (!gpio_get_value(nfc_gpio->irq)) {
				if (timeout) {
					...
					ret = wait_event_interruptible(
						...
						!i2c_dev->irq_enabled);
					if (ret) {
						pr_debug("%s: spurious wakeup of read wq, try wait again.\n",
						       __func__);
						goto err;
					}
				}
...
#ifdef DEBUG
	print_rw_buffer(buf, count, false);
#endif
...
#ifdef DEBUG
	print_rw_buffer(buf, count, true);
#endif
	/*
