...
static inline int nfc_pinctrl_exec(const char *name)
{
	...
	if (IS_ERR_OR_NULL(pinctrl_ptr)) {
		pr_err("[ERR] %s: fail to get nfc pinctrl", __func__);
		return -1;
	}
		 ...
		 uint8_t interface)
{
	...
	if (IS_ERR_OR_NULL(pinctrl_ptr)) {
		pr_err("failed to get nfc pinctrl handle\n");
	} else {
		nfc_pinctrl_exec(DTS_PINCTL_I2C_INIT);
	}
...
void gpio_set_ven(struct nfc_dev *nfc_dev, int value)
{
	...
	if (gpio_get_value(nfc_gpio->ven) != value) {
		...
		int cur_ven = gpio_get_value(nfc_gpio->ven);
		pr_info("%s: cur_ven %d\n", __func__, cur_ven);
		/* hardware dependent delay */
		if(value == 0)
		{
...
static int nfc_gpio_info(struct nfc_dev *nfc_dev, unsigned long arg)
{
	...
	if (ret < 0) {
		pr_err("nfc_gpio_info: Unable to copy data from kernel space to user space");
		return -EFAULT;
	}
	return 0;
}
...
static int nfc_ioctl_power_states(struct nfc_dev *nfc_dev, unsigned long arg)
{
	...
	if (arg == NFC_POWER_OFF) {
		 ...
		 */
		pr_info("NxpDrv: set fw gpio LOW\n");
		set_valid_gpio(nfc_gpio->dwl_req, 0);
		nfc_dev->nfc_state = NFC_STATE_NCI;
...
int nfc_clock_select(struct nfc_dev *nfc_dev)
{
	int ret = 0;
	pr_info("%s: enter.\n", __func__);
	nfc_dev->s_clk = devm_clk_get(&nfc_dev->i2c_dev.client->dev, "clk_38m4_o4");

	if (IS_ERR_OR_NULL(nfc_dev->s_clk)) {
		pr_err("%s: no clk_38m4_o4 configured\n", __func__);
		return ret;
	}

	if (!nfc_dev->clk_run) {
		ret = clk_prepare_enable(nfc_dev->s_clk);
	}
