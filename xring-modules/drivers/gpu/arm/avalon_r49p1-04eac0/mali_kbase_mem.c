...
static enum kbase_large_page_state large_page_conf =
	IS_ENABLED(CONFIG_MALI_LARGE_PAGE_SUPPORT) ? LARGE_PAGE_AUTO : LARGE_PAGE_OFF;

static int set_large_page_conf(const char *val, const struct kernel_param *kp)
{
	char *user_input = strstrip((char *)val);

	if (!IS_ENABLED(CONFIG_MALI_LARGE_PAGE_SUPPORT))
		return 0;
...
#endif
	trace_kbase_mem_free_region(reg);
			      ...
			      unsigned long flags)
{
	...
	if (flags & BASE_MEM_DISARRANGE_BIT)
		reg->flags |= KBASE_REG_DISARRANGE_BIT;
#endif
