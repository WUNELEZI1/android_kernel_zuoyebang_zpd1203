						 ...
						 u32 clk_index, u32 clk_rate_hz)
{
	if ((clk_index == KBASE_CLOCK_DOMAIN_TOP) && (clk_rate_hz != 0)) {
		...
		atomic_set(&listener_data->rate, DEFAULT_REF_TIMEOUT_FREQ_KHZ * 1000);
#else
...
static void kbase_ipa_ctrl_rate_change_worker(struct work_struct *data)
{
	...
	if (!kbdev->pm.backend.gpu_ready) {
		dev_dbg(kbdev->dev, "GPU frequency cannot change while GPU is off");
...
void kbase_xr_ipa_control_rate_change_notify(struct kbase_device *kbdev,
						u64 clk_rate_hz)
{
	struct kbase_ipa_control *ipa_ctrl = &kbdev->csf.ipa_control;
	struct kbase_ipa_control_listener_data *listener_data =
	...
	if (!listener_data)
		return;
	/*
	 * Here you need to pay attention to the measurement
	 * of GPU frequency obtained from the gpufreq module.
	 * After getting the GPU frequency from gpufreq,
	 * kbase_xr_ipa_control_rate_change_notify need to
	 * convert it to Hz and pass it to IPA Control
	 */
	kbase_ipa_control_rate_change_notify(&listener_data->listener,
					KBASE_CLOCK_DOMAIN_TOP, (u32)clk_rate_hz);
}
KBASE_EXPORT_TEST_API(kbase_xr_ipa_control_rate_change_notify);
...
#if IS_ENABLED(CONFIG_MALI_IS_FPGA)
	ipa_ctrl->cur_gpu_rate = DEFAULT_REF_TIMEOUT_FREQ_KHZ * 1000;
#else
	if (kbdev->gpu_dev_data.opp_lowest_freq_khz) {
		ipa_ctrl->cur_gpu_rate = kbdev->gpu_dev_data.opp_lowest_freq_khz * 1000;
	} else {
		ipa_ctrl->cur_gpu_rate = DEFAULT_REF_TIMEOUT_FREQ_KHZ * 1000;
		dev_err(kbdev->dev, "opp_lowest_freq_khz is not initialized, set default value");
	}
#endif
#endif
...
int kbase_ipa_control_query(struct kbase_device *kbdev, const void *client, u64 *values,
			    size_t num_values, u64 *protected_time, bool sample_command)
{
	struct kbase_ipa_control *ipa_ctrl;
	struct kbase_ipa_control_session *session;
	size_t i;
