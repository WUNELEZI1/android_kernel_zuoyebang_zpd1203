...
#define difftime(kbdev, start, value, kcpu_queue, fence)                                \
	ktime_t now, diff_time;                                                       \
	s64     diff_time_ms;                                                         \
	now = ktime_get_raw();                                                        \
	diff_time = ktime_sub(now, (start));                                          \
	diff_time_ms = ktime_to_ms(diff_time);                                        \
	if ((diff_time_ms > value) && (kbdev))                                                     \
		dev_err(kbdev->dev, "difftime %s line(%d) [%lld]ms KCPU[%pK] fence[%pK] f[%u] u[%u] c[%llx]",       \
			 __func__, __LINE__, diff_time_ms, kcpu_queue, fence,                 \
			kbdev->csf.ipa_control.cur_gpu_rate,                                  \
			kbdev->pm.backend.metrics.utilisation,                                \
...
static void enqueue_kcpuq_work(struct kbase_kcpu_command_queue *queue)
{
	...
	if (!atomic_read(&kctx->prioritized)) {
		...
		queue->queuework_time = ktime_get_raw();
	} else {
					 ...
					 struct kbase_kcpu_command_fence_info *fence_info)
{
	...
	if (fence_status < 0)
		dev_warn(kctx->kbdev->dev,
			"Fence wait error %d\n"
			"ctx:%d_%d_%d kcpu queue:%u\n"
			"fence[%pK] seqno:%llu, timeline:%s driver_name:%s\n",
			fence_status, kctx->tgid, kctx->pid, kctx->id, kcpu_queue->id,
			fence, fence->seqno, fence->ops->get_timeline_name(fence), fence->ops->get_driver_name(fence));
...
#ifdef CONFIG_MALI_FENCE_DEBUG
		kcpu_queue->start_time = ktime_get_raw();
#endif
	}
}
...
#ifdef CONFIG_MALI_FENCE_DEBUG
	difftime(kctx->kbdev, fence_info->start_time, 2000, kcpu_queue, fence_info->fence);
#endif
...
static void kcpu_queue_force_fence_signal(struct kbase_kcpu_command_queue *kcpu_queue)
{
	 ...
	 */
	if (atomic_read(&kcpu_queue->kctx->kbdev->fence_signal_timeout_enabled)) {
...
#ifdef CONFIG_MALI_FENCE_DEBUG
	difftime(kctx->kbdev, kcpu_queue->start_time, 0, kcpu_queue, kctx);
	dev_warn(kctx->kbdev->dev,
			"kbase KCPU fence signal timeout callback triggered:"
			"ctx:%d_%d(%s:%d)",
			kctx->tgid, kctx->id, kctx->comm, kctx->pid);
	kcpu_queue->timeout_time = ktime_get_raw();
#endif
}
					    ...
					    struct kbase_kcpu_command_fence_info *fence_info)
{
	...
	if (fence_info->frame_complete &&
		kcpu_queue->pid == kctx->kbdev->gpu_dev_data.query_pid)
		kctx->kbdev->gpu_dev_data.fence_count++;
#endif
...
#if IS_ENABLED(CONFIG_MALI_XR_SMART_GAMING)
	current_command->info.fence.frame_complete = fence->basep.frame_complete;
#endif
#ifdef CONFIG_MALI_FENCE_DEBUG
	current_command->info.fence.start_time = ktime_get_raw();
#endif
				    ...
				    struct kbasep_printer *kbpr)
{
	...
	if (i == queue->num_pending_cmds) {
		int fence_signal_pending_cnt = atomic_read(&queue->fence_signal_pending_cnt);
		dev_err(kctx->kbdev->dev,
			"%s: No fence signal command found in ctx:%d_%d kcpu queue:%u,"
			 "fence_signal_pending_cnt = %d, KCPU[%pK]",
			__func__, kctx->tgid, kctx->id, queue->id, fence_signal_pending_cnt, queue);
...
static void kcpu_queue_timeout_worker(struct work_struct *data)
{
	...
	struct kbase_device *kbdev = queue->kctx->kbdev;
#ifdef CONFIG_MALI_FENCE_DEBUG
	difftime(queue->kctx->kbdev, queue->timeout_time, 500, queue, queue->kctx);
#endif

	if (!queue->kctx->has_dump_fence) {
		queue->kctx->has_dump_fence = true;
	...
	/* using GPU reset to clear the current GPU queue and reset the MCU */
	if (kbase_prepare_to_reset_gpu(kbdev, RESET_FLAGS_NONE)) {
		dev_info(kbdev->dev, "Trying to recover GPU by triggering GPU reset\n");
		queue->kctx->kbdev->force_reset_recovery = true;
...
#if IS_ENABLED(CONFIG_MALI_XR_MDR)
	kbase_xr_mdr_system_error(queue->kctx->kbdev, queue->kctx, MODID_GPU_FENCE_TIMEOUT, 0, 0);
#endif
}
...
#ifdef CONFIG_MALI_FENCE_DEBUG
	difftime(queue->kctx->kbdev, queue->queuework_time, 500, queue, queue->kctx);
#endif
...
int kbase_csf_kcpu_queue_context_init(struct kbase_context *kctx)
{
	kctx->csf.kcpu_queues.kcpu_wq =
		alloc_workqueue("mali_kcpu_wq_%i_%i", WQ_HIGHPRI | WQ_UNBOUND, 0, kctx->tgid, kctx->id);
