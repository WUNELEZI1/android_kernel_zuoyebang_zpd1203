...
#include <platform/xring/mali_kbase_xr_csf_protected_memory.h>
#else
...
extern const u8 kbasep_csf_queue_group_throttle_to_relative[BASE_QUEUE_GROUP_THROTTLE_COUNT];
...
/**
 * kbase_csf_throttle_queue_group_throttle_to_relative - Convert base throttle to relative
 *
 * @priority: base_queue_group_throttle throttle
 *
 * This will convert the fixed base throttle list to monotonically increasing realtive throttle.
 *
 * Return: kbase relative throttle.
 */
static inline u8 kbase_csf_throttle_queue_group_throttle_to_relative(u8 throttle)
{
	/* Apply low throttle in case of invalid throttle */
	if (throttle >= BASE_QUEUE_GROUP_THROTTLE_COUNT)
		throttle = BASE_QUEUE_GROUP_THROTTLE_NO_BOOST;
	return kbasep_csf_queue_group_throttle_to_relative[throttle];
}
