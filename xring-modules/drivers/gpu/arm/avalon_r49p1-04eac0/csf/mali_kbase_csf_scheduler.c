					 ...
					 struct list_head *evicted_groups)
{
	...
	if (kbdev->force_reset_recovery) {
		int i;
		bool as_fault;
		bool kctx_is_resident_group = false;
		...
		if (!kctx_is_resident_group) {
			...
			dev_info(kbdev->dev, "Force Evicting context %d_%d\n",
					kctx->tgid, kctx->id);
		}
	} else {
		for (slot = 0; slot < num_groups; slot++) {
...
void kbase_csf_scheduler_reset(struct kbase_device *kbdev)
{
	...
	/* force reset go to trigger call CSG delete and recreate */
	if (!kbdev->force_reset_recovery) {
	...
	kbdev->force_reset_recovery = false;
...
int kbase_csf_scheduler_init(struct kbase_device *kbdev)
{
	...
	/* For realtime process, the prio will equal "MAX_RT_PRIO - 1 - rt_prio",
	 * set rt_prio to 1 in order to equal the prio value of surfaceflinger.
	 */
	param.sched_priority = 1;
	/* Must use nocheck version of sched_setschedule in kernel mode.*/
