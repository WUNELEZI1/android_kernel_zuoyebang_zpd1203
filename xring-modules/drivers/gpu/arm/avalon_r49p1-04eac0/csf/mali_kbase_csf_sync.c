							    ...
							    const char *cmd_name)
{
	...
	driver_name = fence->ops->get_driver_name(fence);
	...
	*length += scnprintf(buffer + *length, CSF_SYNC_DUMP_SIZE - *length,
				"timeline_name:%s driver_name:%s timeline_context:0x%.16llx fence_seqno:0x%.16llx",
				timeline_name, driver_name, fence->context, (u64)fence->seqno);
