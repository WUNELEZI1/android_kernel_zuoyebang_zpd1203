...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
#include <linux/dma-heap.h>
#include <platform/xring/mali_kbase_xr_protected_memory_allocator.h>
#endif
...
const u8 kbasep_csf_queue_group_throttle_to_relative[BASE_QUEUE_GROUP_THROTTLE_COUNT] = {
	KBASE_QUEUE_GROUP_THROTTLE_NONE, KBASE_QUEUE_GROUP_THROTTLE_HIGH,
	KBASE_QUEUE_GROUP_THROTTLE_MEDIUM, KBASE_QUEUE_GROUP_THROTTLE_LOW,
	KBASE_QUEUE_GROUP_THROTTLE_NO_BOOST
};
...
int kbase_csf_queue_kick(struct kbase_context *kctx, struct kbase_ioctl_cs_queue_kick *kick)
{
	...
	if (!kbase_is_region_invalid_or_free(region)) {
		...
		if (queue && (queue->bind_state == KBASE_CSF_QUEUE_BOUND)) {
			...
			if (queue->group != NULL) {
				if (queue->group->throttle
						== KBASE_QUEUE_GROUP_THROTTLE_HIGH) {
					kbdev->gpu_dev_data.cl_high_accel_flag = true;
				} else if (queue->group->throttle
						== KBASE_QUEUE_GROUP_THROTTLE_MEDIUM) {
					kbdev->gpu_dev_data.cl_med_accel_flag = true;
				} else if (queue->group->throttle
						== KBASE_QUEUE_GROUP_THROTTLE_LOW) {
					kbdev->gpu_dev_data.cl_low_accel_flag = true;
				} else if (queue->group->throttle
						== KBASE_QUEUE_GROUP_THROTTLE_NO_BOOST) {
					kbdev->gpu_dev_data.cl_no_accel_flag = true;
				}
			}
#endif
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
	group->protected_suspend_buf.pdb = NULL;
#endif
			      ...
			      union kbase_ioctl_cs_queue_group_create *const create)
{
	...
	if (group_handle < 0) {
		...
		if (!group) {
			...
			group->throttle = kbase_csf_throttle_queue_group_throttle_to_relative(create->in.throttle);
				 ...
				 union kbase_ioctl_cs_queue_group_create *const create)
{
	...
	if ((create->in.tiler_max > tiler_count) || (create->in.fragment_max > fragment_count) ||
		...
		err = -EINVAL;
	} else if (create->in.throttle >= BASE_QUEUE_GROUP_THROTTLE_COUNT) {
		dev_dbg(kctx->kbdev->dev, "Invalid queue group throttle %u",
			(unsigned int)create->in.throttle);
		err = -EINVAL;
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
		kbase_csf_protected_memory_free(kbdev, sbuf->pma, nr_pages, true, sbuf->pdb);
		sbuf->pdb = NULL;
#else
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
	sbuf->pma = kbase_csf_protected_memory_alloc(kbdev, phys, nr_pages, true, &sbuf->pdb);
	if (unlikely(!sbuf->pdb)) {
		err = -ENOMEM;
		goto unlock;
	}
#else
...
static void handle_fault_event(struct kbase_queue *const queue, const u32 cs_ack)
{
	...
	if (use_old_log_format && !skip_fault_report) {
		dev_warn(kbdev->dev,
			 ...
			 "CS_FAULT_INFO.EXCEPTION_DATA: 0x%llx, pid = %d, comm: %s\n",
			 ...
			 cs_fault_info_exception_data, queue->kctx->pid, queue->kctx->comm);
#if IS_ENABLED(CONFIG_MALI_XR_MDR)
		kbase_xr_mdr_system_error(kbdev, queue->kctx, MODID_GPU_CS_FAULT, 0, 0);
#endif
	}
			       ...
			       struct kbase_csf_cmd_stream_info const *const stream, u32 cs_ack)
{
	...
	if (use_old_log_format && !skip_fault_report) {
		dev_warn(kbdev->dev,
			 ...
			 "CS_FATAL_INFO.EXCEPTION_DATA: 0x%llx, pid = %d, comm: %s\n",
			 queue->kctx->tgid, queue->kctx->id, queue->group->handle,
			 ...
			 cs_fatal_info_exception_data, queue->kctx->pid, queue->kctx->comm);
#if IS_ENABLED(CONFIG_MALI_XR_MDR)
		kbase_xr_mdr_system_error(kbdev, queue->kctx, MODID_GPU_CS_FATAL, 0, 0);
#endif
	}
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
/**
 * defer_alloc_fw_protected_pages() -  Allocate protected memory for firmware.
 * @kbdev: Pointer to the device allocated for.
 *
 * Return: 0 if protect fw memory allocation is successful, otherwise
 * negative error value.
 *
 * If there is no CSF_FIRMWARE_ENTRY_PROTECTED, no allocation is required,
 * that is, need_defer_alloc_protected_fw=false
 */
static int defer_alloc_fw_protected_pages(struct kbase_device *kbdev)
{
	int ret = 0;
	...
	list_for_each_entry(interface, &kbdev->csf.firmware_interfaces, node) {
		if ((interface->flags & CSF_FIRMWARE_ENTRY_PROTECTED)) {
			mem_flags = convert_mem_flags(kbdev, interface->flags, &cache_mode);

			kbase_csf_scheduler_lock(kbdev);
			interface->pma = kbase_csf_protected_memory_alloc(kbdev, interface->phys, interface->num_pages_aligned,
							       true, &interface->pdb);
			if (!interface->pma) {
				/* If allocation fails, return. At this time, interface->pma/phys are all NULL. */
				dev_err(kbdev->dev, "Failed to alloc protect firmware pages\n");
				ret = -ENOMEM;
				goto unlock;
			}
			/* Only when allocating protect firmware entry successfully,
			 * alloc_protected_fw_done=true is set, that is, pma!=null.
			 * If allocation fails, that is, pma==null, and alloc_protected_fw_done=false,
			 * protm_event_worker can still allocate protect firmware
			 * if alloc_protected_fw_retries <= PROTM_ALLOC_MAX_RETRIES.
			 * But if mmu fails, no retry allocation protect firmware will be made.
			 * The memory that has been allocated will be freed by firmware unloading.
			 */
			atomic_set(&kbdev->csf.pma_dev->alloc_protected_fw_done, 1);

			ret = kbase_mmu_insert_pages_no_flush(kbdev, &kbdev->csf.mcu_mmu,
							      interface->virtual >> PAGE_SHIFT, interface->phys,
							      interface->num_pages_aligned, mem_flags,
							      KBASE_MEM_GROUP_CSF_FW, NULL, NULL);

			if (ret != 0) {
				dev_err(kbdev->dev, "Failed to insert protect firmware pages\n");
			}

unlock:
			kbase_csf_scheduler_unlock(kbdev);

			/* phys will not be freed until kbase_csf_firmware_unload_term. */
			break;
		}
	}
...
void kbase_csf_process_protm_event_request(struct kbase_queue_group *group)
{
	...
	bool need_alloc_fw = kbdev->csf.pma_dev->need_defer_alloc_protected_fw;
#endif

	KBASE_KTRACE_ADD_CSF_GRP(group->kctx->kbdev, PROTM_EVENT_WORKER_START, group, 0u);

#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
	need_alloc_fw &= !atomic_read(&kbdev->csf.pma_dev->alloc_protected_fw_done);
	if(need_alloc_fw) {
		err = defer_alloc_fw_protected_pages(kbdev);
		if(err) {
			if (err == -ENOMEM && kbdev->csf.pma_dev->alloc_protected_fw_retries <= PROTM_ALLOC_MAX_RETRIES) {
				kbdev->csf.pma_dev->alloc_protected_fw_retries++;
				/* try again to allocate pages */
				dev_info(kbdev->dev,
					"try again to allocate protected firmeware entry %d", kbdev->csf.pma_dev->alloc_protected_fw_retries);
				kbase_csf_scheduler_enqueue_protm_event_work(group);
				KBASE_KTRACE_ADD_CSF_GRP(group->kctx->kbdev, PROTM_EVENT_WORKER_END, group, 0u);
				return;
			} else if (kbdev->csf.pma_dev->alloc_protected_fw_retries >= PROTM_ALLOC_MAX_RETRIES || err != -ENOMEM) {
				if (kbdev->csf.pma_dev->alloc_protected_fw_retries >= PROTM_ALLOC_MAX_RETRIES && err == -ENOMEM) {
					kbdev->csf.pma_dev->alloc_protected_fw_retries = 0;
				}
				dev_err(kbdev->dev,
					"Failed to allocate physical pages for protected firmeware entry");
