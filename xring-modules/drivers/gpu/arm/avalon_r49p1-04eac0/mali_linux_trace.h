...
TRACE_EVENT(
	...
	TP_STRUCT__entry(__field(u64, start_addr) __field(u64, fault_addr)
				 __field(u64, fault_extra_addr) __field(size_t, new_size)
					 __field(u32, status)),
	TP_fast_assign(__entry->start_addr = ((u64)reg->start_pfn) << PAGE_SHIFT;
		       ...
		       __entry->new_size = new_pages * PAGE_SIZE; __entry->status = fault->status;),
	TP_printk(
		"start=0x%llx fault_addr=0x%llx fault_extra_addr=0x%llx new_size=%zu raw_fault_status=0x%x decoded_faultstatus=%s exception_type=0x%x,%s%u access_type=0x%x,%s source_id=0x%x",
		__entry->start_addr, __entry->fault_addr, __entry->fault_extra_addr,
		__entry->new_size, __entry->status,
...
TRACE_EVENT(
	...
	TP_ARGS(reg, fault, new_pages),
	TP_STRUCT__entry(__field(u64, start_addr) __field(u64, fault_addr)
				 __field(size_t, new_size) __field(u32, status)),
	TP_fast_assign(__entry->start_addr = ((u64)reg->start_pfn) << PAGE_SHIFT;
		       __entry->fault_addr = fault->addr; __entry->new_size = new_pages * PAGE_SIZE;
		       __entry->status = fault->status;),
	TP_printk(
		"start=0x%llx fault_addr=0x%llx new_size=%zu raw_fault_status=0x%x decoded_faultstatus=%s exception_type=0x%x,%s%u access_type=0x%x,%s source_id=0x%x",
		__entry->start_addr, __entry->fault_addr, __entry->new_size, __entry->status,
		KBASE_MMU_FAULT_STATUS_DECODED_STRING(__entry->status),
		KBASE_MMU_FAULT_STATUS_CODE(__entry->status),
		KBASE_MMU_FAULT_STATUS_EXCEPTION_NAME_PRINT(__entry->status),
		KBASE_MMU_FAULT_STATUS_LEVEL(__entry->status),
...
/* trace_kbase_mem_alloc
 *
 * Tracepoint about kbase_mem_alloc
 */
TRACE_EVENT(
	kbase_mem_alloc,
	TP_PROTO(struct kbase_va_region *reg, u64 gpu_va),
	TP_ARGS(reg, gpu_va),
	TP_STRUCT__entry(__field(u64, gpu_va) __field(u64, flags)
				 __field(u64, va_size) __field(u64, commit_size)),
	TP_fast_assign(__entry->gpu_va = gpu_va;
		       __entry->flags = reg->flags;
		       __entry->va_size = reg->nr_pages * PAGE_SIZE;
		       __entry->commit_size = reg->initial_commit * PAGE_SIZE;),
	TP_printk(
		"gpu_va=0x%llx flags=0x%llx va_size=%llu commit_size=%llu",
		__entry->gpu_va, __entry->flags, __entry->va_size,
		__entry->commit_size));

/* trace_kbase_mem_free
 *
 * Tracepoint about kbase_mem_free
 */
TRACE_EVENT(
	kbase_mem_free,
	TP_PROTO(struct kbase_va_region *reg),
	TP_ARGS(reg),
	TP_STRUCT__entry(__field(u64, gpu_va) __field(u64, va_size)),
	TP_fast_assign(__entry->gpu_va = reg->start_pfn << PAGE_SHIFT;
				   __entry->va_size = reg->nr_pages * PAGE_SIZE;),
	TP_printk(
		"gpu_va=0x%llx va_size=%llu",
		__entry->gpu_va, __entry->va_size));

/* trace_kbasep_reg_mmap
 *
 * Tracepoint about kbasep_reg_mmap
 */
TRACE_EVENT(
	kbasep_reg_mmap,
	TP_PROTO(struct kbase_va_region *reg),
...
/* trace_kbase_mem_free_region
 *
 * Tracepoint about kbase_mem_free_region
 */
TRACE_EVENT(
	kbase_mem_free_region,
	TP_PROTO(struct kbase_va_region *reg),
	TP_ARGS(reg),
	TP_STRUCT__entry(__field(u64, gpu_va) __field(u64, va_size)),
	TP_fast_assign(__entry->gpu_va = reg->start_pfn << PAGE_SHIFT;
...
/* trace_kbase_mem_import
 *
 * Tracepoint about kbase_mem_import
 */
TRACE_EVENT(
	kbase_mem_import,
	TP_PROTO(struct kbase_va_region *reg, u64 gpu_va, u8 import_type),
	TP_ARGS(reg, gpu_va, import_type),
	TP_STRUCT__entry(__field(u64, gpu_va) __field(u64, flags)
				 __field(u64, va_size) __field(u8, import_type)),
	TP_fast_assign(__entry->gpu_va = gpu_va;
		       __entry->flags = reg->flags;
		       __entry->va_size = reg->nr_pages * PAGE_SIZE;
		       __entry->import_type = import_type;),
	TP_printk(
		"gpu_va=0x%llx flags=0x%llx va_size=%llu import_type=%d",
		__entry->gpu_va, __entry->flags, __entry->va_size,
		__entry->import_type));

/* trace_kbase_mem_commit
 *
 * Tracepoint about kbase_mem_commit
 */
TRACE_EVENT(
	kbase_mem_commit,
	TP_PROTO(struct kbase_va_region *reg, u64 old_pages, u64 new_pages),
	TP_ARGS(reg, old_pages, new_pages),
	TP_STRUCT__entry(__field(u64, gpu_va) __field(u64, old_size)
				 __field(u64, new_size)),
	TP_fast_assign(__entry->gpu_va = reg->start_pfn << PAGE_SHIFT;
		       __entry->old_size = old_pages * PAGE_SIZE;
		       __entry->new_size = new_pages * PAGE_SIZE;),
	TP_printk(
		"gpu_va=0x%llx old_size=%llu new_size=%llu",
		__entry->gpu_va, __entry->old_size, __entry->new_size));
...
/* trace_gpu_common_event
 *
 * Tracepoint about gpu_common_event
 */
TRACE_EVENT(
	gpu_common_event,
	TP_PROTO(const char* func, int line, const char* tag0, int arg0,
		 const char* tag1, int arg1, const char* tag2, int arg2,
		 const char* tag3, int arg3, const char* tag4, int arg4),
	TP_ARGS(func, line, tag0, arg0, tag1, arg1, tag2, arg2,
		  tag3, arg3, tag4, arg4),
	TP_STRUCT__entry(
		  __string(trace_func, func)
		  __field(int, line)
		  __string(trace_tag0, tag0)
		  __field(int, arg0)
		  __string(trace_tag1, tag1)
		  __field(int, arg1)
		  __string(trace_tag2, tag2)
		  __field(int, arg2)
		  __string(trace_tag3, tag3)
		  __field(int, arg3)
		  __string(trace_tag4, tag4)
		  __field(int, arg4)),
	TP_fast_assign(
		  __assign_str(trace_func, func);
		  __entry->line = line;
		  __assign_str(trace_tag0, tag0);
		  __entry->arg0 = arg0;
		  __assign_str(trace_tag1, tag1);
		  __entry->arg1 = arg1;
		  __assign_str(trace_tag2, tag2);
		  __entry->arg2 = arg2;
		  __assign_str(trace_tag3, tag3);
		  __entry->arg3 = arg3;
		  __assign_str(trace_tag4, tag4);
		  __entry->arg4 = arg4;),
	TP_printk("func: %s, line: %d, %s: %d, %s: %d, %s: %d, %s: %d, %s: %d",
		  __get_str(trace_func), __entry->line,
		  __get_str(trace_tag0), __entry->arg0,
		  __get_str(trace_tag1), __entry->arg1,
		  __get_str(trace_tag2), __entry->arg2,
		  __get_str(trace_tag3), __entry->arg3,
		  __get_str(trace_tag4), __entry->arg4));
