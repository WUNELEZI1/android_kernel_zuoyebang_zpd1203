...
#define ENTRY_DISARRANGE_BIT (1ULL << 38)
...
static void entry_set_ate_disarrange_bit(u64 *entry,
		...
		int const level)
{
	if (level == MIDGARD_MMU_BOTTOMLEVEL)
		page_table_entry_set(entry, as_phys_addr_t(phy) |
				get_mmu_flags(flags) |
				ENTRY_ACCESS_BIT | ENTRY_IS_ATE_L3 | ENTRY_DISARRANGE_BIT);
	else
		page_table_entry_set(entry, as_phys_addr_t(phy) |
				get_mmu_flags(flags) |
				ENTRY_ACCESS_BIT | ENTRY_IS_ATE_L02 | ENTRY_DISARRANGE_BIT);
}
#endif
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_DISARRANGE)
						    .entry_set_ate_disarrange_bit = entry_set_ate_disarrange_bit,
#endif
