...
#include <platform/xring/mali_kbase_xr_core_hotplug.h>
#endif
...
#if IS_ENABLED(CONFIG_MALI_XR_MDR)
			kbase_xr_mdr_system_error(kbdev, NULL, MODID_GPU_IRQ_FAULT, 0, 0);
#endif
			return irq;
		}
...
#if IS_ENABLED(CONFIG_MALI_XR_CORE_HOTPLUG)
	err = kbase_xr_hotplug_core_set_mask(kbdev, HOTPLUG_POLICY_MANUAL_CONTROL, core_mask.new_core_mask);
	CSTD_UNUSED(flags);
#else
				  ...
				  size_t count)
{
	...
	if (flag == 1) {
		atomic_set(&kbdev->gpu_dev_data.vfreq_status, 1);
	} else if (flag == 0) {
		atomic_set(&kbdev->gpu_dev_data.vfreq_status, 0);
		kbase_xr_vfreq_core_set(kbdev->dev, hweight64(kbdev->gpu_props.shader_present));
	}
	return count;
}
