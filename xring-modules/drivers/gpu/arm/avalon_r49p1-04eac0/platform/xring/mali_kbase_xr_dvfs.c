...
u32 kbase_xr_get_utilisation(void *data)
{
	...
	if (kbdev->gpu_dev_data.cl_no_accel_flag == true)
	{
		/* We give a weight 1.5 here, this may change in the future depending on the actual scene */
		utilisation = utilisation * 3 / 2 > 100 ? 100 : utilisation * 3 / 2;
...
#if IS_ENABLED(CONFIG_MALI_XRING_CL_DVFS)
u32 kbase_xr_get_cl_accel(void *data)
{
	struct device *dev = NULL;
	struct kbase_device *kbdev = NULL;
	u32 accel_level = 0;
	...
	if (kbdev->gpu_dev_data.cl_high_accel_flag) {
		accel_level = 1;
	...
	} else if (kbdev->gpu_dev_data.cl_med_accel_flag) {
		accel_level = 2;
		kbdev->gpu_dev_data.cl_med_accel_flag = false;
		kbdev->gpu_dev_data.cl_low_accel_flag = false;
	} else if (kbdev->gpu_dev_data.cl_low_accel_flag) {
		accel_level = 3;
		kbdev->gpu_dev_data.cl_low_accel_flag = false;
	}
...
#if IS_ENABLED(CONFIG_MALI_XR_VIRTUAL_DEVFREQ)
	gpufreq_ops->update_coremask = kbase_xr_vfreq_core_set;
