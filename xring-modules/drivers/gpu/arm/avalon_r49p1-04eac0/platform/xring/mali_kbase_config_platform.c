...
#include <platform/xring/mali_kbase_xr_thermal_control.h>
#endif
...
static int kbase_xr_platform_early_init(struct kbase_device *kbdev)
{
	...
	kbdev->gpu_dev_data.cl_high_accel_flag = false;
	kbdev->gpu_dev_data.cl_med_accel_flag = false;
	kbdev->gpu_dev_data.cl_low_accel_flag = false;
	kbdev->gpu_dev_data.cl_no_accel_flag = false;
#endif
#endif

#if IS_ENABLED(CONFIG_MALI_XR_CORE_HOTPLUG)
	mutex_init(&kbdev->gpu_dev_data.core_info.hotplug_policy_lock);
#endif
...
#if IS_ENABLED(CONFIG_MALI_XR_CORE_HOTPLUG)
	kbase_xr_hotplug_core_init(kbdev);
#endif

#if IS_ENABLED(CONFIG_MALI_XR_SMART_GAMING)
	kbase_gpu_query_register();
#endif
#if IS_ENABLED(CONFIG_MALI_XR_MDR)
...
#endif
#if IS_ENABLED(CONFIG_MALI_XR_THERMAL_CONTROL)
	kbase_xr_thermal_control_init(kbdev);
#endif
	return err;
}
...
#endif
#if IS_ENABLED(CONFIG_MALI_XR_THERMAL_CONTROL)
	kbase_xr_thermal_control_term();
#endif
#if IS_ENABLED(CONFIG_MALI_XR_CORE_HOTPLUG)
	kbase_xr_hotplug_core_term(kbdev);
#endif
}
