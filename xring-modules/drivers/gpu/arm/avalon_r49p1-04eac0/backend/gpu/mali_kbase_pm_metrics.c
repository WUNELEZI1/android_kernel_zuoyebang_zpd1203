...
#include <platform/xring/mali_kbase_xr_smart_gaming.h>
#endif
...
#define MALI_XRING_DVFS_TIMER 0
...
int kbasep_pm_metrics_init(struct kbase_device *kbdev)
{
	...
	struct kbase_ipa_control_perf_counter perf_counter[NUM_PERF_COUNTERS] = {
		{GPU_ACTIVE_SCALING_FACTOR, true, KBASE_IPA_CORE_TYPE_CSHW, GPU_ACTIVE_CNT_IDX},
		{GPU_ACTIVE_SCALING_FACTOR, true, KBASE_IPA_CORE_TYPE_CSHW, ITER_TILER_ACTIVE_IDX},
		{GPU_ACTIVE_SCALING_FACTOR, true, KBASE_IPA_CORE_TYPE_CSHW, ITER_COMP_ACTIVE_IDX},
		{GPU_ACTIVE_SCALING_FACTOR, true, KBASE_IPA_CORE_TYPE_CSHW, ITER_FRAG_ACTIVE_IDX},
	};
...
#if IS_ENABLED(CONFIG_MALI_XR_SMART_GAMING)
	err = kbase_xr_ai_control_register(kbdev);
	if (err) {
		dev_err(kbdev->dev,
			"Failed to register ai control client: err=%d",
			err);
		return -1;
	}
#endif
...
void kbasep_pm_metrics_term(struct kbase_device *kbdev)
{
	...
	kbase_ipa_control_unregister(
		kbdev, kbdev->pm.backend.metrics.ai_control_client);
#endif
#else
	CSTD_UNUSED(kbdev);
#endif
...
static void kbase_pm_get_dvfs_utilisation_calc(struct kbase_device *kbdev)
{
	...
	err = kbase_ipa_control_query(
		...
		gpu_active_counter, NUM_PERF_COUNTERS, &protected_time, false);
#else
	err = kbase_ipa_control_query(
		kbdev, kbdev->pm.backend.metrics.ipa_control_client,
		&gpu_active_counter, 1, &protected_time, false);
#endif
...
void kbase_pm_get_dvfs_action(struct kbase_device *kbdev)
{
	...
	u32 current_compute;
	u32 current_total;
	int current_compute_proportion;
	int tiler_utilisation;
	int compute_utilisation;
	int fragment_utilisation;
	struct kbasep_pm_metrics *diff;
	...
	tiler_utilisation = (100 * diff->time_busy[1]) /
			max(diff->time_busy[1] + diff->time_idle[1], 1u);
	compute_utilisation = (100 * diff->time_busy[2]) /
			max(diff->time_busy[2] + diff->time_idle[2], 1u);
	fragment_utilisation = (100 * diff->time_busy[3]) /
			max(diff->time_busy[3] + diff->time_idle[3], 1u);

	/* Krake GPU CL Boost stage1 */
	current_tiler = diff->time_busy[1];
	current_compute = diff->time_busy[2];
	current_fragment = diff->time_busy[3];
	current_total = current_tiler + current_compute + current_fragment;

	if (current_compute > 0 && current_total > 0) {
		current_compute_proportion = (100 * current_compute) / current_total;
		utilisation = utilisation * ((current_compute_proportion + 100) / 100);
