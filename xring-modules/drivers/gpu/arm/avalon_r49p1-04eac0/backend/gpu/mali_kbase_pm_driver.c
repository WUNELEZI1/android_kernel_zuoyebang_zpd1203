...
void kbase_pm_enable_interrupts(struct kbase_device *kbdev)
{
	...
	kbase_reg_write32(kbdev, GPU_CONTROL_ENUM(PWR_OVERRIDE0), 0x10000);
#else
...
static int kbase_pm_do_reset(struct kbase_device *kbdev)
{
	...
	if (kbase_reg_read32(kbdev, reg_offset) & reg_val) {
		...
		destroy_hrtimer_on_stack(&rtdata.timer);
		kbase_xr_mdr_system_error(kbdev, NULL, MODID_GPU_INTERRUPT_TIMEOUT, 0, 0);
		return -EINVAL;
	}
	...
	if (!kbdev->arb.arb_if) {
		...
		dev_err(kbdev->dev, "Failed to hard-reset the GPU (timed out after %d ms)\n",
			RESET_TIMEOUT);
#if IS_ENABLED(CONFIG_MALI_XR_MDR)
		kbase_xr_mdr_system_error(kbdev, NULL, MODID_GPU_HARD_RESET_TIME_OUT, 0, 0);
#endif
