...
static int kbasep_xr_counter_query_show(struct seq_file *sfile, void *data)
{
	...
	list_for_each(entry, kbdev_list) {
		...
		kbdev = list_entry(entry, struct kbase_device, entry);
		kbase_xr_query_hwcounter(kbdev->dev, counter_value, XR_NUM_PERF_COUNTERS);
		for (i = 0; i < XR_NUM_PERF_COUNTERS; i++) {
			seq_printf(sfile, "%-16s %llu\n",
				kbdev->devname,
				counter_value[i]);
		}
...
#if IS_ENABLED(CONFIG_MALI_XR_MDR)
static void kbasep_xr_report_mdr(struct kbase_device *kbdev, u8 mdr_type)
{
	switch (mdr_type) {
	case 0:
		...
		break;
	case 1:
		mdr_system_error(MODID_GPU_BUS_FAULT, 0, 0);
		break;
	case 2:
		mdr_system_error(MODID_GPU_FAULT, 0, 0);
		break;
	case 3:
		mdr_system_error(MODID_GPU_CS_FAULT, 0, 0);
		break;
	case 4:
		mdr_system_error(MODID_GPU_UNHANDLE_PAGE_FAULT, 0, 0);
		break;
	case 5:
		mdr_system_error(MODID_MCU_UNHANDLE_PAGE_FAULT, 0, 0);
		break;
	case 6:
		mdr_system_error(MODID_GPU_SOFT_RESET_TIME_OUT, 0, 0);
		break;
	case 7:
		mdr_system_error(MODID_GPU_HARD_RESET_TIME_OUT, 0, 0);
		break;
	case 8:
		mdr_system_error(MODID_GPU_BIT_STUCK, 0, 0);
		break;
	default:
...
/**
 * kbasep_xr_mdr_show - Show mdr info
 * @sfile: The debugfs entry
 * @data: Data associated with the entry
 *
 ...
 * * -1 if it encountered an error
 */
static int kbasep_xr_mdr_show(struct seq_file *sfile, void *data)
{
	struct list_head *entry = NULL;
	const struct list_head *kbdev_list = NULL;
	...
	list_for_each(entry, kbdev_list) {
		...
		kbasep_xr_report_mdr(kbdev, kbdev->gpu_dev_data.mdr_type);
	}
	kbase_device_put_list(kbdev_list);
	return 0;
}
 ...
 *  File operations related to debugfs entry for fence query
 */
static int kbasep_xr_mdr_debugfs_open(struct inode *in, struct file *file)
{
	return single_open(file, kbasep_xr_mdr_show, NULL);
}

static ssize_t kbase_xr_mdr_debugfs_write(struct file *file,
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	list_for_each(entry, kbdev_list) {
		...
		if (kstrtoul(raw_str, 0, &type)) {
			dev_err(kbdev->dev, "Kstrtoul failed");
			return -EINVAL;
		}
		if (type < MDR_MAX_TYPE_NUM)
			kbdev->gpu_dev_data.mdr_type = type;
		dev_err(kbdev->dev, "MDR write %lu", type);
	}
	kbase_device_put_list(kbdev_list);
	return count;
}

static const struct file_operations kbasep_mdr_debugfs_fops = {
	.owner = THIS_MODULE,
	.open = kbasep_xr_mdr_debugfs_open,
	.read = seq_read,
	.write = kbase_xr_mdr_debugfs_write,
	.llseek = seq_lseek,
	.release = single_release,
};

static void kbasep_xr_mdr_debugfs_init(struct kbase_device *kbdev, struct dentry *dentry)
{
	debugfs_create_file("mdr", 0644,
			dentry, kbdev,
			&kbasep_mdr_debugfs_fops);
}
#endif
		...
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	list_for_each(entry, kbdev_list) {
		...
		dev_err(kbdev->dev, "DVFS write %lu", type);
	}
	kbase_device_put_list(kbdev_list);
	return count;
}
