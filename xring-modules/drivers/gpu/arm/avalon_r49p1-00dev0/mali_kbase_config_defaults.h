 ...
 * GPU clock frequency.
 *
 * The frequency of FPGA only is 10MHZ.
 */
#if IS_ENABLED(CONFIG_MALI_IS_FPGA) && !IS_ENABLED(CONFIG_MALI_NO_MALI)
#define DEFAULT_REF_TIMEOUT_FREQ_KHZ (10000)
#else
 ...
 *
 * The frequency of FPGA is 10MHZ, we have already scaled it by correct the value of
 * "DEFAULT_REF_TIMEOUT_FREQ_KHZ", however, the antutu still reported the fence
 * timeout, thus, we enlarge this cycle 10 times here.
 */
...
#elif IS_ENABLED(CONFIG_MALI_IS_FPGA) && !IS_ENABLED(CONFIG_MALI_NO_MALI)
#define KCPU_FENCE_SIGNAL_TIMEOUT_CYCLES (10000000000ull)
#else
