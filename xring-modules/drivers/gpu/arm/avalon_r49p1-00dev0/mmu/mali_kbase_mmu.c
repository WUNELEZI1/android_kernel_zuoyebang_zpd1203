...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_DISARRANGE)
						   ...
						   struct kbase_mmu_table *mmut, const u64 vpfn)
{
	...
	if (!IS_ENABLED(CONFIG_MALI_PAGE_MIGRATION_SUPPORT))
		return;
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_DISARRANGE)
static inline u64 kbase_mmu_create_ate_disarrange_bit(struct kbase_device *const kbdev,
	struct tagged_addr const phy, unsigned long const flags,
	int const level, int const group_id)
{
	u64 entry;

	kbdev->mmu_mode->entry_set_ate_disarrange_bit(&entry, phy, flags, level);
	return kbdev->mgm_dev->ops.mgm_update_gpu_pte(kbdev->mgm_dev,
		(unsigned int)group_id, level, entry);
}
#endif
	...
	int const level, int const group_id)
{
	u64 entry;
#if IS_ENABLED(CONFIG_MALI_XR_GPU_DISARRANGE)
	/* only for gpu internal buffer */
	if (flags & KBASE_REG_DISARRANGE_BIT)
		return kbase_mmu_create_ate_disarrange_bit(kbdev, phy, flags, level, group_id);
#endif
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_DISARRANGE)
/* Return Disarrange Skip Pages. */
static size_t caculate_disarrange_skip_pages(struct kbase_va_region *reg, size_t nr)
{
	...
	if (reg->gpu_alloc->type == KBASE_MEM_TYPE_IMPORTED_UMM &&
		...
		/* xr_dmabuf_get_headersize return 0 for dma_buf without disarrange
		 * return -1 for dma_buf is which is invalid
		 * return header size for dma_buf which need to be disarranged.
		 * For dma_buf, only body_pages is disarranged.
		 */
		long long header_size = xr_dmabuf_get_headersize(
			reg->gpu_alloc->imported.umm.dma_buf);

		if (header_size > nr)
			return nr;

		/* The return value of xr_dmabuf_get_headersize is less than or equal to 0,
		 * which means no disarrange is required, so disarrange_skip_pages is nr.
		 * Otherwise here return header size.
		 */
		if (header_size > 0)
			return header_size;

		return nr;
	} else if (reg->gpu_alloc->type ==  KBASE_MEM_TYPE_NATIVE) {
		/* For gpu internal buffer, total buffer is disarranged,
		 * and xr_dmabuf_get_headersize is 0 with disarranged enabled.
		 * If no disarrange is required, disarrange_skip_pages is equal to nr.
		 */
		if (reg->flags & KBASE_REG_DISARRANGE_BIT)
			return 0;
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_DISARRANGE)
	/* header_index and disarrange_skip_pages are GPU_PAGE_SIZE units. */
	size_t header_index = 0;
	size_t disarrange_skip_pages = caculate_disarrange_skip_pages(reg, nr) * GPU_PAGES_PER_CPU_PAGE;
#endif
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_DISARRANGE)
			if ((nr * GPU_PAGES_PER_CPU_PAGE) == disarrange_skip_pages) {
				pgd_page[level_index] = kbase_mmu_create_ate(kbdev,
					*phys, flags, cur_level, group_id);
			} else {
				pgd_page[level_index] = kbase_mmu_create_ate_disarrange_bit(kbdev,
					*phys, flags, cur_level, group_id);
			}
#else
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_DISARRANGE)
					if (header_index < disarrange_skip_pages) {
						*target = kbase_mmu_create_ate(kbdev,
									       as_tagged(page_address),
									       flags, cur_level, group_id);
						header_index++;
					} else {
						*target = kbase_mmu_create_ate_disarrange_bit(kbdev,
