...
void kbase_sync_fence_info_get(struct dma_fence *fence, struct kbase_sync_fence_info *info)
{
	...
	scnprintf(info->name, sizeof(info->name), "%llu#%u#%s#%s",
		fence->context, fence->seqno, fence->ops->get_timeline_name(fence), fence->ops->get_driver_name(fence));
#else
	scnprintf(info->name, sizeof(info->name), "%llu#%llu#%s#%s",
		fence->context, fence->seqno, fence->ops->get_timeline_name(fence), fence->ops->get_driver_name(fence));
#endif
}
