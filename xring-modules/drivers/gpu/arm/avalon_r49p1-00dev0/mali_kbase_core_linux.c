...
#include <mdr_pub.h>
#endif
#include <platform/xring/mali_kbase_xr_debugfs.h>
...
#if IS_ENABLED(CONFIG_MALI_XR_MDR)
			mdr_system_error(MODID_GPU_IRQ_FAULT, 0, 0);
#endif
			return irq;
		}
...
// sysfs node, output the information of the gpu_kmd_data structure to user space.
static ssize_t show_gpu_error_info(struct device *dev, struct device_attribute *attr,
		char * const buf) {
	struct kbase_device *kbdev;
	ssize_t ret = 0;
	...
	if (!kbdev)
		return -ENODEV;
	kbdev->gpu_kmd_data.total_error_num =
		kbdev->gpu_kmd_data.unhandled_page_fault +
		kbdev->gpu_kmd_data.gpu_fault_worker +
		kbdev->gpu_kmd_data.cs_fault +
		kbdev->gpu_kmd_data.soft_reset_gpu_fault +
		kbdev->gpu_kmd_data.hard_reset_gpu_fault +
		kbdev->gpu_kmd_data.as_active_bit_stuck +
		kbdev->gpu_kmd_data.unsuccessful_gpu_reset_detected;
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"total_error_num:%d\n",
		kbdev->gpu_kmd_data.total_error_num);

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"unhandled_page_fault num:%d\n",
		kbdev->gpu_kmd_data.unhandled_page_fault);

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"gpu_fault_worker num:%d\n",
		kbdev->gpu_kmd_data.gpu_fault_worker);
	...
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"soft_reset_gpu_fault num:%d\n",
		kbdev->gpu_kmd_data.soft_reset_gpu_fault);

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"hard_reset_gpu_fault num:%d\n",
		kbdev->gpu_kmd_data.hard_reset_gpu_fault);

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"as_active_bit_stuck num:%d\n",
		kbdev->gpu_kmd_data.as_active_bit_stuck);

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"unsuccessful_gpu_reset_detected num:%d\n",
		kbdev->gpu_kmd_data.unsuccessful_gpu_reset_detected);
	return ret;
}

// Zeroing the gpu_kmd_data structure. Don`t support others functions.
static ssize_t set_gpu_error_info(struct device *dev, struct device_attribute *attr,
	...
	if (!kbdev)
		return -ENODEV;
	kbdev->gpu_kmd_data.total_error_num = 0;
	kbdev->gpu_kmd_data.unhandled_page_fault = 0;
	kbdev->gpu_kmd_data.gpu_fault_worker = 0;
	kbdev->gpu_kmd_data.cs_fault = 0;
	kbdev->gpu_kmd_data.soft_reset_gpu_fault = 0;
	kbdev->gpu_kmd_data.hard_reset_gpu_fault = 0;
	kbdev->gpu_kmd_data.as_active_bit_stuck = 0;
	kbdev->gpu_kmd_data.unsuccessful_gpu_reset_detected = 0;
	return count;
}

// new attribute, which will register in "attribute *kbase_attrs"
static struct device_attribute gpu_kmd_error_info = __ATTR(gpu_error_info, 0644,
...
/*
 * core_num_show - Show callback for the core_num sysfs file.
 *
 * @dev:	The device this sysfs file is for
 * @attr:	The attributes of the sysfs file
 * @buf:	The output buffer for the sysfs file contents
 *
 * This function is called to get the contents of the core_num sysfs file.
 *
 * Return: The number of bytes output to @buf.
 */
static ssize_t core_num_show(struct device *dev, struct device_attribute *attr, char *const buf)
{
	struct kbase_device *kbdev;
	unsigned long flags;
	ssize_t ret = 0;
	...
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
	kbase_xr_core_mask_to_num(kbdev, kbdev->pm.debug_core_mask, &core_num);
	ret += scnprintf(buf + ret, (size_t)(PAGE_SIZE - ret),
			 "Desired core number: %d\n", core_num);
	kbase_xr_core_mask_to_num(kbdev, kbdev->pm.backend.shaders_avail, &core_num);
	ret += scnprintf(buf + ret, (size_t)(PAGE_SIZE - ret),
			 "Current in use core number : %d\n", core_num);
	kbase_xr_core_mask_to_num(kbdev, kbdev->gpu_props.shader_present, &core_num);
	ret += scnprintf(buf + ret, (size_t)(PAGE_SIZE - ret),
			 "Available max core number: %d\n", core_num);
	spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
...
/*
* core_num_store - Store callback for the core_num sysfs file.
*
* @dev:	The device with sysfs file is for
* @attr:	The attributes of the sysfs file
* @buf:	The value written to the sysfs file
* @count:	The number of bytes to write to the sysfs file
*
* This function is called when the core_num sysfs file is written to.
*
* Return: @count if the function succeeded. An error code on failure.
*/
static ssize_t core_num_store(struct device *dev, struct device_attribute *attr, const char *buf,
			      size_t count)
{
	struct kbase_device *kbdev;
	u32 core_num;
	...
	err = kstrtoint(buf, 0, &core_num);
	if (err) {
		dev_err(kbdev->dev, "Couldn't process core num write operation.\n");
		return err;
	}

	if((core_num == 0) || (core_num > gpu_coremask_data->max_core_num)) {
		dev_err(kbdev->dev, "input invalid core number:%d.\n"
			"Use number between 1 to %d.\n",
			core_num,
			gpu_coremask_data->max_core_num);
		return -EINVAL;
	}

	kbase_xr_core_num_to_mask(kbdev, core_num, &core_mask.new_core_mask);
...
/*
 * The sysfs file core_num.
 *
 * This is used to restrict shader core availability for debugging purposes.
 * Reading it will show the current core number and the max number of cores available.
 * Writing to it will set the current core mask.
 */
static DEVICE_ATTR_RW(core_num);
#endif /* CONFIG_MALI_XR_VIRTUAL_DEVFREQ */

#if IS_ENABLED(CONFIG_MALI_XR_VIRTUAL_DEVFREQ)
/*
 * vfreq_status_show - Shown callback for the vfreq_status sysfs file.
 *
 * @dev:	The device this sysfs file is for
 * @attr:	The attributes of the sysfs file
 * @buf:	The output buffer for the sysfs file contents
 *
 * This function is called to get the contents of the vfreq_status sysfs file.
 *
 * Return: The number of bytes output to @buf.
 */
static ssize_t vfreq_status_show(struct device *dev, struct device_attribute *attr, char *const buf)
{
	struct kbase_device *kbdev;
	ssize_t ret = 0;
	bool vfreq_status;
	...
	ret = scnprintf(buf, (size_t)(PAGE_SIZE), "vfreq status: %s\n",
			vfreq_status ? "enable" : "disable");

	return ret;
}

/**
 * vfreq_status_store - Store callback for the vfreq_status sysfs file.
 *
 * @dev:	The device with sysfs file is for
 * @attr:	The attributes of the sysfs file
 * @buf:	The value written to the sysfs file
 * @count:	The number of bytes to write to the sysfs file
 *
 * This function is called when the vfreq_status sysfs file is written to.
 *
 * Return: @count if the function succeeded. An error code on failure.
 */
static ssize_t vfreq_status_store(struct device *dev, struct device_attribute *attr, const char *buf,
				  size_t count)
{
	struct kbase_device *kbdev;
	int flag;
	...
	if (ret || ((flag != 0) && (flag != 1))) {
		dev_err(kbdev->dev, "Couldn't process virtual devfreq state write operation.\n"
				    "Use 0 disable, 1 enable.\n");
		return -EINVAL;
	}

	vfreq_status = flag != 0;

	kbase_xr_vfreq_set_status(kbdev, vfreq_status);

	if (!vfreq_status) {
		core_mask.new_core_mask = kbdev->gpu_props.shader_present;
		mutex_lock(&kbdev->pm.lock);
		spin_lock_irqsave(&kbdev->hwaccess_lock, flags);

		core_mask_set(kbdev, &core_mask);
...
/*
 * The sysfs file vfreq_status.
 *
 * This is used to set disable virtual freq for debugging purposes.
 * The set coremask via core_mask sysfile would be useless when virtual
 * devfreq is enabled.
 *
 */
static DEVICE_ATTR_RW(vfreq_status);
#endif /* CONFIG_MALI_XR_VIRTUAL_DEVFREQ */
...
#if !IS_ENABLED(CONFIG_MALI_XR_PROP_VISIBLE)
	product_name = "Mali-G710";
#endif
...
int kbase_pm_gpu_freq_init(struct kbase_device *kbdev)
{
	...
	if (IS_ERR(opp_ptr)) {
		...
		kbdev->gpu_dev_data.opp_lowest_freq_khz = found_freq;
#endif
		dev_info(kbdev->dev, "dts lowest %lu khz, lowest %llu khz",
			found_freq, lowest_freq_khz);
	}
#else
...
int kbase_device_procfs_init(struct kbase_device *kbdev)
{
	struct proc_dir_entry *entry = kbasep_gpu_memory_procfs_init(kbdev);

	if (IS_ERR_OR_NULL(entry)) {
		dev_err(kbdev->dev, "Unable to create proc gpu_memory entry\n");
		return IS_ERR(entry) ? PTR_ERR(entry) : -ENOMEM;
	}
	return 0;
}

void kbase_device_procfs_term(struct kbase_device *kbdev)
{
	CSTD_UNUSED(kbdev);
	kbasep_gpu_memory_procfs_exit();
}
...
#endif /* !MALI_USE_CSF */
#if IS_ENABLED(CONFIG_MALI_XR_VIRTUAL_DEVFREQ)
	&dev_attr_vfreq_status.attr,
#endif
	&gpu_kmd_error_info.attr,
	NULL
};
