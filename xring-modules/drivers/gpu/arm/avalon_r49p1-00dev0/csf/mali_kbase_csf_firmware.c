 ...
 */
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
unsigned long convert_mem_flags(const struct kbase_device *const kbdev, const u32 flags,
				       u32 *cm)
#else
...
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
	struct protected_dma_buffer *pdb = NULL;
#endif
...
retry_alloc:
	...
	if (protected_mode) {
		if (!reuse_pages) {
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
			kbdev->csf.pma_dev->need_defer_alloc_protected_fw = true;
#else
	...
	if (!reuse_pages) {
#if IS_ENABLED(CONFIG_MALI_XR_GPU_PROTECTED_MEMORY)
		if (!protected_mode || !kbdev->csf.pma_dev->need_defer_alloc_protected_fw) {
#endif
		...
		if (!interface->reuse_pages) {
			if (interface->flags & CSF_FIRMWARE_ENTRY_PROTECTED) {
				...
				kbase_csf_protected_memory_free(kbdev, interface->pma,
								...
								interface->is_small_page, interface->pdb);
#else
				kbase_csf_protected_memory_free(kbdev, interface->pma,
								interface->num_pages_aligned,
