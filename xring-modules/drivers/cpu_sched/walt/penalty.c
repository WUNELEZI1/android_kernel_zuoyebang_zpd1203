...
static unsigned int __read_mostly sysctl_sched_auto_penaty;
static unsigned int __read_mostly sysctl_penalty_debug;
...
static void android_rvh_before_do_sched_yield(void *unused, long *skip)
{
	...
	*skip = 1;
	...
	if (ps->last_yield_time > 0 && (clock - ps->last_yield_time) > MAX_YIELD_SLEEP) {   /*refresh start of yield group*/
		...
		ps->last_sleep_ns = min_t(u32, ps->last_sleep_ns, MAX_YIELD_SLEEP);
	...
	sleep_ns = max_t(u64, sleep_ns, penalty_headroom);
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (target_fps > 0) {
		...
		pr_debug("sched_penalty:fps=%llu, frame_time_ns=%llu\n",
...
MODULE_DESCRIPTION("WALT optimization module");
