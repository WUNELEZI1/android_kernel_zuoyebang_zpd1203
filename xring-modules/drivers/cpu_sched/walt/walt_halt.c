...
#define pr_fmt(fmt) "walt_halt: " fmt
...
static int drain_rq_cpu_stop(void *data)
{
	...
	/* Notice: The symbol has not been exported. */
...
static int __ref __maybe_unused try_drain_rqs(void *data)
{
...
void restrict_cpus_and_freq(struct cpumask *cpus)
{
	...
	int i = 0;
	struct cpumask halt_cpus;
	struct walt_sched_cluster *cluster = NULL;

	cpumask_or(&halt_cpus, cpu_partial_halt_mask, cpu_halt_mask);

	for (i = 0; i < num_sched_clusters; i++) {
		cluster = sched_cluster[i];
		if (!cpumask_intersects(cpus, &cluster->cpus))
			continue;

		if (cpumask_intersects(&cluster->cpus, cpu_partial_halt_mask) &&
		    cpumask_subset(&cluster->cpus, &halt_cpus)) {
			add_freq_qos_request(cluster->cpus,
						sysctl_max_freq_partial_halt[i],
						QOS_PARTIAL_HALT,
						MAX_REQUEST);
		} else {
			add_freq_qos_request(cluster->cpus,
...
static void __maybe_unused android_rvh_get_nohz_timer_target(void *unused, int *cpu, bool *done)
{
	int i, default_cpu = -1;
	struct sched_domain *sd = NULL;
	cpumask_t unhalted;
	const cpumask_t *hk_mask;
	...
	*done = true;
	...
	for_each_domain(*cpu, sd) {
		for_each_cpu_and(i, sched_domain_span(sd), hk_mask) {
			...
			/*
			 * FIXME:
			 * cpuset hotplug is handled asynchronously in kernel 6.6,
			 * so it's possible to see an inactive core from the sched
			 * domain even after the hotplug process for a while.
			 *
			 * The problem above is handled by the following commit for
			 * kernel 6.9:
			 * https://lore.kernel.org/all/20240625085549.981682583@linuxfoundation.org/
			 *
			 * Here, to work around the problem, we check the cpu active
			 * status explicitly.
			 */
			if (!cpu_active(i))
				continue;
	...
	if (default_cpu == -1) {
		cpumask_andnot(&unhalted, cpu_online_mask, cpu_halt_mask);
		for_each_cpu_and(i, &unhalted, hk_mask) {
			if (*cpu == i)
				continue;
 ...
 */
static void __maybe_unused android_rvh_set_cpus_allowed_by_task(void *unused,
						 ...
						 unsigned int *dest_cpu)
{
	...
	const struct cpumask *prior_mask = new_mask;

	if (unlikely(walt_disabled))
		return;

	if (!cpumask_empty(&wts->prior_mask)) {
		prior_mask = &wts->prior_mask;
		*dest_cpu = cpumask_any_and_distribute(cpu_valid_mask, prior_mask);
	}
	...
	if (cpu_halted(*dest_cpu) && !p->migration_disabled) {
		...
		cpumask_and(&allowed_cpus, &allowed_cpus, prior_mask);
 ...
 */
static void __maybe_unused android_rvh_rto_next_cpu(void *unused,
 ...
 */
static void __maybe_unused android_rvh_is_cpu_allowed(void *unused,
						struct task_struct *p, int cpu, bool *allowed)
{
	if (unlikely(walt_disabled))
		return;
