...
static void create_util_to_cost_pd(struct em_perf_domain *pd)
{
	...
	struct em_perf_state *ps, *table;

	table = em_perf_state_from_pd(pd);

	fmax = (u64)table[pd->nr_perf_states - 1].frequency;
...
unsigned int sched_cpu_capacity_margin[WALT_NR_CPUS] = {
			[0 ... WALT_NR_CPUS-1] = 1078 /* ~5% margin */
};
...
bias_to_this_cpu(struct task_struct *p, int cpu, int start_cpu)
{
	...
	bool start_cap_test = !capacity_higher(start_cpu, cpu);
	...
	*end_index = 0;
	...
	for (; *order_index < num_sched_clusters - 1; (*order_index)++) {
		if (task_demand_fits(p, cpumask_first(&cpu_array[*order_index][0])) &&
		    cpumask_intersects(&cpu_array[*order_index][0], &prefer_cpus))
			break;
	}
						...
						struct find_best_target_env *fbt_env)
{
	...
	if (arch_scale_cpu_capacity(cpu) < xr_get_group_prefer_util(p))
		return true;
		...
		struct task_struct *p)
{
	...
	if (!xr_is_task_prefer_cpu(p, prev_cpu))
		return false;
					...
					struct find_best_target_env *fbt_env)
{
	...
	cpumask_t visit_cpus, prefer_cpus;
	struct walt_task_struct *wts = (struct walt_task_struct *) p->android_vendor_data1;
	int packing_cpu;
	...
	for (cluster = 0; cluster < num_sched_clusters; cluster++) {
		...
		for_each_cpu(i, &visit_cpus) {
			...
			if (prefer_cpu_cluster < 0 &&
			    cpumask_test_cpu(i, &prefer_cpus))
				prefer_cpu_cluster = i;
			/*
			...
			if (__cpu_overutilized(i, wake_cpu_util == cpu_util(i) ? task_util(p) : 0))
				continue;
			...
			if (available_idle_cpu(i)) {
				...
				this_sleep_time = walt_get_sleep_time(i);
				if (this_sleep_time < best_sleep_time)
					continue;

				new_util_cuml = cpu_util_cum(i);
				if (min_exit_latency == idle_exit_latency &&
				    this_sleep_time == best_sleep_time &&
				...
				if (cpumask_test_cpu(i, &prefer_cpus))
					prefer_cpu_cluster = i;
				continue;
			}
			...
			if (best_idle_cpu_cluster != -1 &&
			    best_idle_cpu_cluster == prefer_cpu_cluster)
				continue;
		...
		if (prefer_cpu_cluster != -1)
			cpumask_set_cpu(prefer_cpu_cluster, candidates);
		else if (best_idle_cpu_cluster != -1)
				...
				struct compute_energy_output *output, unsigned int x)
{
	...
	if (max_util >= SCHED_CAPACITY_SCALE)
		max_util = SCHED_CAPACITY_SCALE - 1;
...
static inline bool select_cpu_same_energy(int cpu, int best_cpu, int prev_cpu)
{
	...
	if (capacity_higher(cpu, best_cpu))
		return false;
	if (capacity_higher(best_cpu, cpu))
		return true;

	/* cpu and best cpu are in same cluster below */
				     ...
				     int sync, int sibling_count_hint)
{
	...
	if (sync && (need_idle || (is_rtg && curr_is_rtg) || !xr_is_task_prefer_cpu(p, cpu)))
		sync = 0;
	...
	xr_get_group_prefer_cpus(p, &prefer_cpus);
	if (cpumask_intersects(candidates, &prefer_cpus))
		cpumask_and(candidates, candidates, &prefer_cpus);
	...
	if (weight == 1 && (available_idle_cpu(first_cpu) || first_cpu == prev_cpu ||
				cpu_halted(prev_cpu))) {
		best_energy_cpu = first_cpu;
		goto unlock;
	}
	...
	if (need_idle) {
		idle_cpu = find_idle_cpu(candidates);
		if (idle_cpu >= 0) {
			best_energy_cpu = idle_cpu;
			goto unlock;
		}
	}
	...
	if (!(available_idle_cpu(best_energy_cpu) &&
	    ...
	    !capacity_higher(prev_cpu, start_cpu))
				...
				int sd_flag, int wake_flags, int *target_cpu)
{
	...
	vip_sched_select_task_rq_fair(p, prev_cpu, sd_flag, wake_flags, target_cpu);
	if (*target_cpu >= 0)
		return;
#endif
					  ...
					  int next_buddy_marked)
{
	...
	if (prior_check_preempt(wts_p))
		set_vip_prio_locked(p, VIP_PRIO_RESERVED, true);

#if IS_ENABLED(CONFIG_XRING_SCHED_VIP)
	vip_sched_check_preempt_wakeup(rq, p, preempt, nopreempt, wake_flags, se, pse,
			next_buddy_marked);
...
#if IS_ENABLED(CONFIG_XRING_SCHED_VIP)
	vip_sched_replace_next_task_fair(rq, p, se, repick, simple, prev);
	if (*repick)
		return;
#endif
}
