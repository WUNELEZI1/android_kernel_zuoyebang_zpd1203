...
static void long_running_rt_task_notifier(void *unused, struct rq *rq)
{
	...
	if (rq->clock_task -
		...
		printk_deferred("WALT-DEBUG: RT task %s (%d) runtime > %u now=%llu task arrival time=%llu runtime=%llu\n",
				curr->comm, curr->pid,
				sysctl_sched_long_running_rt_task_ms * MSEC_TO_NSEC,
				rq->clock_task,
				per_cpu(rt_task_arrival_time, cpu),
...
static bool is_prefer_cluster_task(struct task_struct *task)
{
	struct walt_task_struct *wts = (struct walt_task_struct *)task->group_leader->android_vendor_data1;

	if (wts->prefer_cluster_index == -1)
		return false;
	else
		return true;
}

static void is_prefer_cluster_boost(struct task_struct *task, int *order_index, int *end_index)
{
	struct walt_task_struct *wts;
	unsigned long tutil = task_util(task);
	bool need_boost = tutil > sysctl_sched_prefer_cluster_boost_thres;
	int8_t cluster_index = 0;

	if (!need_boost || !is_prefer_cluster_task(task))
		return;

	wts = (struct walt_task_struct *)task->group_leader->android_vendor_data1;
	cluster_index = wts->prefer_cluster_index;
	*order_index = cluster_index >> 4;
	*end_index = cluster_index & 0xF;
}
static bool skip_fast_path(struct task_struct *task)
{
	bool is_rtg = task_in_related_thread_group(task);
	unsigned long tutil = task_util(task);

	if (is_rtg || is_prefer_cluster_task(task))
		return tutil > sysctl_sched_prefer_cluster_boost_thres;
	else
		return false;
}
					  ...
					  int ret, int *best_cpu)
{
	...
	s64 best_sleep_time = S64_MIN;
	s64 this_sleep_time;
	bool boost_on_big = rt_boost_on_big();
	int cluster;
	int best_prefer_cpu = -1;
	// MIUI ADD: Performance_BoostFramework
	bool is_rtg = task_in_related_thread_group(task);
	// END Performance_BoostFramework
	...
	xr_get_group_prefer_cpus(task, &prefer_cpus);
	if (cpumask_full(&prefer_cpus))
		cpumask_clear(&prefer_cpus);

	// MIUI ADD: Performance_BoostFramework
	if (is_rtg && sched_boost_type == CONSERVATIVE_BOOST) {
		order_index = 2;
		end_index = 1;
	} else {
		is_prefer_cluster_boost(task, &order_index, &end_index);
	}
	// END Performance_BoostFramework
	...
	for (cluster = 0; cluster < num_sched_clusters; cluster++) {
		for_each_cpu_and(cpu, lowest_mask, &cpu_array[order_index][cluster]) {
			...
			if (best_prefer_cpu < 0 &&
			    cpumask_test_cpu(cpu, &prefer_cpus))
				best_prefer_cpu = cpu;
			...
			if (cpu != task_cpu(task) && best_cpu_util == util) {
				...
				if (best_sleep_time > this_sleep_time)
					continue;

				if (best_idle_exit_latency == cpu_idle_exit_latency &&
				    best_sleep_time == this_sleep_time &&
			...
			*best_cpu = cpu;
			best_cpu_lt = lt;
			if (cpumask_test_cpu(cpu, &prefer_cpus))
				best_prefer_cpu = cpu;
		}
		...
		if (*best_cpu != -1 &&
		    (*best_cpu == best_prefer_cpu || cpumask_empty(&prefer_cpus)))
			break;
	}

	rcu_read_unlock();
	if (best_prefer_cpu != -1)
		*best_cpu = best_prefer_cpu;
}
...
static inline bool walt_rt_task_fits_capacity(struct task_struct *p, int cpu)
{
	...
	if (!xr_is_task_prefer_cpu(p, cpu) &&
	    arch_scale_cpu_capacity(cpu) < xr_get_group_prefer_util(p))
		return false;
					...
					int sd_flag, int wake_flags, int *new_cpu)
{
	 ...
	 */
	// MIUI ADD: Performance_BoostFramework
	if (!skip_fast_path(task) && sysctl_sched_sync_hint_enable && cpu_active(this_cpu) && !cpu_halted(this_cpu) &&
	// END Performance_BoostFramework
		cpumask_test_cpu(this_cpu, task->cpus_ptr) &&
		xr_is_task_prefer_cpu(task, this_cpu) &&
	...
	*new_cpu = cpu; /* previous CPU as back up */
	 ...
	 */
	softirqs_busy = cpu_busy_with_softirqs(cpu);
	...
	if (packing_cpu >= 0 && !cpu_busy_with_softirqs(packing_cpu)) {
	 ...
	 */
	if (target != -1 &&
	    (softirqs_busy || task->prio < cpu_rq(target)->rt.highest_prio.curr))
		*new_cpu = target;
	...
	if (cpu_halted(*new_cpu)) {
		...
		xr_get_group_prefer_cpus(task, &non_halted);
		cpumask_andnot(&non_halted, &non_halted, cpu_halt_mask);
		target = cpumask_first_and(&non_halted, task->cpus_ptr);
...
unlock:
	rcu_read_unlock();
out:
	trace_sched_select_task_rt(task, wake_flags, fastpath, *new_cpu, lowest_mask);
}
...

{
	...
	packing_cpu = walt_find_and_choose_cluster_packing_cpu(0, task);
	// MIUI ADD: Performance_BoostFramework
	if (packing_cpu >= 0 && !cpu_busy_with_softirqs(packing_cpu) && !skip_fast_path(task)) {
	// END Performance_BoostFramework
...
int walt_rt_find_best_cpu(struct task_struct *task)
{
	int best_cpu = -1;
	struct cpumask *lowest_mask =
		this_cpu_cpumask_var_ptr(walt_local_cpu_mask);

	walt_rt_find_lowest_rq(NULL, task, lowest_mask, 0, &best_cpu);
	return best_cpu;
}
