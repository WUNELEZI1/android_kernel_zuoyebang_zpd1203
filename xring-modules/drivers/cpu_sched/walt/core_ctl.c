...
/* MAX_CCUR defines the maximum number of tasks that are allowed to
 * run in parallel on a single CPU.
 * XRing Enhancements to the Core Control Policy looks the maximum
 * number into the history to ignore the occasional bursts of tasks.
 * The following DECAY_RATE determines how fast the algorithm
 * forgets the number in history. The number itself contributes to
 * the strength of the memory. In gernal, the greater the number is,
 * the harder to forget the occasion.
 * HIST_WEIGHT determines the history influence on the current in
 * percentage, which should always be smaller than 100.
 * Note the maximum number is decayed once it is read.
 */
#define XRING_MAX_CCUR					4
#define XRING_MAX_NR_HIST_WEIGHT		75
#define XRING_MAX_NR_DECAY_RATE			1

/* HIGH_CCUR_PCT defines the percent of time a CPU can stand the parallel
 * execution of tasks. A value of 100 implies those tasks should consume
 * the full CPU time, and that shall never happen.
 */
#define XRING_HIGH_CCUR_PCT				80

/* FAST_EVAL_PCT defines the percent of CPU time tasks are running concurrently
 * and a fast evaluation should be taken. It is recommended to start the fast
 * evaluation only if a CPU is overloaded when concurrently percent greater
 * than 100. In this case, the CPU can not handle one or all of them within the
 * time slice depending on the scheduling policy. Tasks are delayed and will be
 * starved probably if no action is taken.
 */
#define	XRING_FAST_EVAL_PCT				150
...
struct cluster_data {
	...
	unsigned int		high_pct_allowed;
	unsigned int		fast_eval_thres;
	unsigned int		max_nr_hist;
	unsigned int		max_nr;
	unsigned int		nr_assist;
	unsigned int		nr_busy;
	unsigned int		nr_big;
	unsigned int		nr_high;
	unsigned int		nr_misfit;
	s64			need_ts;
					...
					const char *buf, size_t count)
{
	...
	if (ret == 1) {
		for (i = 0; i < state->num_cpus; i++) {
			if (val[0] < state->busy_down_thres[i])
				continue;
			state->busy_up_thres[i] = val[0];
		}
	} else {
		for (i = 0; i < state->num_cpus; i++) {
			if (val[i] < state->busy_down_thres[i])
				continue;
					...
					const char *buf, size_t count)
{
	...
	if (ret == 1) {
		for (i = 0; i < state->num_cpus; i++) {
			if (val[0] > state->busy_up_thres[i])
				continue;
			state->busy_down_thres[i] = val[0];
		}
	} else {
		for (i = 0; i < state->num_cpus; i++) {
			if (val[i] > state->busy_up_thres[i])
				continue;
				    ...
				    const char *buf, size_t count)
{
	...
	spin_lock_irqsave(&state_lock, flags);
	cpumask_and(&state->nrrun_cpu_mask, to_cpumask(temp_bitmap), cpu_possible_mask);
	spin_unlock_irqrestore(&state_lock, flags);
				    ...
				    const char *buf, size_t count)
{
	...
	spin_lock_irqsave(&state_lock, flags);
	cpumask_and(&state->nrrun_cpu_misfit_mask, to_cpumask(temp_bitmap), cpu_possible_mask);
	spin_unlock_irqrestore(&state_lock, flags);
				    ...
				    const char *buf, size_t count)
{
	...
	spin_lock_irqsave(&state_lock, flags);
	cpumask_and(&state->assist_cpu_mask, to_cpumask(temp_bitmap), cpu_possible_mask);
	spin_unlock_irqrestore(&state_lock, flags);
				    ...
				    const char *buf, size_t count)
{
	...
	spin_lock_irqsave(&state_lock, flags);
	cpumask_and(&state->assist_cpu_misfit_mask, to_cpumask(temp_bitmap), cpu_possible_mask);
	spin_unlock_irqrestore(&state_lock, flags);
...
#if IS_ENABLED(CONFIG_XRING_CORECTL)
static ssize_t show_max_concurrency(const struct cluster_data *state, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%u\n", state->max_concurrency);
}

static ssize_t store_max_concurrency(struct cluster_data *state,
				const char *buf, size_t count)
{
	unsigned int val;
	...
	state->max_concurrency = val;
	sysfs_param_changed(state);

	return count;
}

static ssize_t show_high_pct_allowed(const struct cluster_data *state, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%u\n", state->high_pct_allowed);
}

static ssize_t store_high_pct_allowed(struct cluster_data *state,
				const char *buf, size_t count)
{
	unsigned int val;
	...
	state->high_pct_allowed = val;
	sysfs_param_changed(state);

	return count;
}

static ssize_t show_fast_eval_thres(const struct cluster_data *state, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%u\n", state->fast_eval_thres);
}

static ssize_t store_fast_eval_thres(struct cluster_data *state,
				const char *buf, size_t count)
{
	unsigned int val;
...
#if IS_ENABLED(CONFIG_XRING_CORECTL)
core_ctl_attr_rw(max_concurrency);
core_ctl_attr_rw(high_pct_allowed);
core_ctl_attr_rw(fast_eval_thres);
#endif
...
#if IS_ENABLED(CONFIG_XRING_CORECTL)
	&max_concurrency.attr,
	&high_pct_allowed.attr,
	&fast_eval_thres.attr,
#endif
	NULL
};
...
/**
 * compute_cluster_nr_high
 * @index: cluster index
 *
 * For each cpu monitered by this cluster, check task
 * congestion level on the cpu, and return the worst
 * congestion condition seen on a single cpu.
 */
static int compute_cluster_nr_high(int index)
{
	int cpu = -1;
	...
	for_each_cpu(cpu, &cluster->nrrun_cpu_mask) {
		if (cpu_partial_halted(cpu))
			continue;
		nr_high = max(nr_high, nr_stats[cpu].nr_high);
	}
...
static void update_running_avg(u64 window_start, u32 wakeup_ctr_sum)
{
	...
	for_each_cluster(cluster, index) {
		...
		cluster->max_nr_hist += cluster->max_nr * 100;
		cluster->max_nr_hist = cluster->max_nr_hist >> XRING_MAX_NR_DECAY_RATE;
		cluster->nr_misfit = nr_misfit_need;
		cluster->nr_high = compute_cluster_nr_high(index);
...
#if IS_ENABLED(CONFIG_XRING_CORECTL)
	/* CPU can be over-crowded if
	 * 1. for too long tasks were running concurrently
	 * 2. too many tasks were running concurrently
	 */
	new_need += cluster->nr_high / cluster->high_pct_allowed;

	new_need += cluster->max_nr_hist / (cluster->max_concurrency * XRING_MAX_NR_HIST_WEIGHT);

	/* CPU should be ready for the misfit tasks */
	new_need = max(new_need, cluster->nr_misfit);
#else
				 ...
				 unsigned int need_cpus)
{
	...
	/* if all assist cpu offline, disable core ctl */
	if (!cpumask_intersects(&cluster->assist_cpu_mask, cpu_online_mask))
		return cluster->num_cpus;

	if (xr_is_group_prefer_cpus(&cluster->cpu_mask))
		need_cpus = cluster->num_cpus;
				 ...
				 struct cpumask *part_pause_cpus)
{
	...
	struct cpumask cluster_part_mask;
	int num_cpus;

	cpumask_and(&cluster_part_mask, &cluster->cpu_mask, &cpus_part_paused_by_us);
	num_cpus = cluster->min_partial_cpus - cpumask_weight(&cluster_part_mask);
...
again:
	...
	if (first_pass && active_cpus - nr_pending > need) {
		first_pass = false;
		goto again;
	}

	first_pass = cluster->nr_not_preferred_cpus;
			   ...
			   struct cpumask *part_unpause_cpus)
{
	...
	/* first resume the partial paused cpus */
	list_for_each_entry_safe(c, tmp, &cluster->lru, sib) {
		if (!num_cpus--)
			goto unlock;
		...
		pr_debug("Trying to partial resume CPU%u\n", c->cpu);

		cpumask_set_cpu(c->cpu, part_unpause_cpus);
		nr_pending++;
		move_cpu_lru(c);
	}

	/* then give the chance to the fully paused cpus */
	num_cpus = cluster->num_cpus;
	list_for_each_entry_safe(c, tmp, &cluster->lru, sib) {
		if (!num_cpus--)
			break;
...
#if IS_ENABLED(CONFIG_XRING_CORECTL)
int core_ctl_fast_eval(int cpu)
{
	struct cpu_data *c;
	struct cluster_data *cluster;
	bool control = false;
	unsigned int new_need, last_need;
	unsigned long flags;
	u64 nr_high;
	...
	nr_high = sched_get_high_load(cpu);
	if (nr_high <= cluster->fast_eval_thres)
		return 0;

	spin_lock_irqsave(&state_lock, flags);
	last_need = cluster->need_cpus;
	new_need = last_need + nr_high / cluster->high_pct_allowed;
	new_need = apply_limits(cluster, new_need);

	if ((new_need > last_need) &&
	    (new_need > get_active_cpu_count(cluster)) &&
	    cluster_paused_cpus(cluster)) {

		control = true;
		cluster->need_ts = ktime_to_ms(ktime_get());
		cluster->need_cpus = new_need;

		trace_core_ctl_fast_eval(cpu, nr_high, last_need, new_need, cluster->need_ts);
	}
	spin_unlock_irqrestore(&state_lock, flags);
...
EXPORT_SYMBOL(core_ctl_fast_eval);

int core_ctl_request_resume(int idx, int num)
{
	struct cluster_data *cluster;
	unsigned long flags;
	unsigned int new_need;
	...
	spin_lock_irqsave(&state_lock, flags);
	new_need = apply_limits(cluster, num);
	if (cluster->need_cpus >= new_need) {
		spin_unlock_irqrestore(&state_lock, flags);
		return 0;
	}
...
EXPORT_SYMBOL(core_ctl_request_resume);
#endif
...
static inline void core_ctl_xring(struct cluster_data *cluster)
{
	cluster->max_concurrency = XRING_MAX_CCUR;
	cluster->high_pct_allowed = XRING_HIGH_CCUR_PCT;
	cluster->fast_eval_thres = XRING_FAST_EVAL_PCT;
	cluster->max_nr_hist = 0;
...
static int cluster_init(const struct cpumask *mask)
{
	 ...
	 */
	cpumask_clear(&cluster->nrrun_cpu_mask);
	cpumask_clear(&cluster->nrrun_cpu_misfit_mask);
	cpumask_clear(&cluster->assist_cpu_mask);
	cpumask_clear(&cluster->assist_cpu_misfit_mask);
