...
TRACE_EVENT(sched_update_task_ravg,
	...
	TP_STRUCT__entry(
		...
		__field(unsigned int,		cur_scale)
	...
	TP_fast_assign(
		...
		__entry->cur_scale	= wrq->task_exec_scale;
		memcpy(__entry->comm, p->comm, TASK_COMM_LEN);
		__entry->pid		= p->pid;
	...
	TP_printk("wc %llu ws %llu delta %llu event %s cpu %d cur_scale %u cur_pid %d task %d (%s) ms %llu delta %llu demand %u coloc_demand: %u sum %u irqtime %llu pred_demand_scaled %u rq_cs %llu rq_ps %llu cur_window %u (%s) prev_window %u (%s) nt_cs %llu nt_ps %llu active_time %llu grp_cs %lld grp_ps %lld, grp_nt_cs %llu, grp_nt_ps: %llu curr_top %u prev_top %u global_ws %llu",
		__entry->wallclock, __entry->win_start, __entry->delta,
		task_event_names[__entry->evt], __entry->cpu,
		__entry->cur_scale, __entry->cur_pid,
...
TRACE_EVENT(core_ctl_fast_eval,

	TP_PROTO(unsigned int cpu, unsigned int nr_high,
		unsigned int last_need, unsigned int new_need, s64 need_ts),
	TP_ARGS(cpu, nr_high, last_need, new_need, need_ts),
	TP_STRUCT__entry(
		__field(u32, cpu)
		__field(u32, nr_high)
		__field(u32, last_need)
	...
	TP_fast_assign(
		__entry->cpu		= cpu;
		__entry->nr_high	= nr_high;
		__entry->last_need	= last_need;
		__entry->new_need	= new_need;
		__entry->need_ts	= need_ts;
	),
	TP_printk("cpu=%u nr_high=%u last_need=%u new_need=%u need_ts=%llu",
		  __entry->cpu,	__entry->nr_high,
		  __entry->last_need, __entry->new_need, __entry->need_ts)
);
...
TRACE_EVENT(core_ctl_update_nr_need,
	...
	TP_PROTO(int cpu, int nr_need, int nr_misfit_need, int nrrun,
		 int max_nr, int max_nr_hist, int strict_nrrun,
		 int nr_assist_need, int nr_misfit_assist_need, int nr_assist,
		 int nr_busy, int nr_high),

	TP_ARGS(cpu, nr_need, nr_misfit_need, nrrun,
		max_nr, max_nr_hist, strict_nrrun,
		nr_assist_need, nr_misfit_assist_need, nr_assist,
		nr_busy, nr_high),
	...
	TP_fast_assign(
		...
		__entry->max_nr_hist	= max_nr_hist;
	...
	TP_printk("cpu=%d nr_need=%d nr_misfit_need=%d nrrun=%d max_nr=%d max_nr_hist=%d strict_nrrun=%d nr_assist_need=%d nr_misfit_assist_need=%d nr_assist=%d nr_busy=%d nr_high=%d",
		__entry->cpu,
		__entry->nr_need, __entry->nr_misfit_need, __entry->nrrun,
		__entry->max_nr, __entry->max_nr_hist, __entry->strict_nrrun,
		__entry->nr_assist_need, __entry->nr_misfit_assist_need, __entry->nr_assist,
		__entry->nr_busy, __entry->nr_high)
);
...
TRACE_EVENT(sched_get_nr_running_avg,
	...
	TP_PROTO(int cpu, int nr, int nr_misfit, int nr_max, int nr_scaled, int nr_high),

	TP_ARGS(cpu, nr, nr_misfit, nr_max, nr_scaled, nr_high),
	...
	TP_printk("cpu=%d nr=%d nr_misfit=%d nr_max=%d nr_scaled=%d nr_high=%d",
		__entry->cpu, __entry->nr, __entry->nr_misfit, __entry->nr_max,
		__entry->nr_scaled, __entry->nr_high)
);
...
TRACE_EVENT(walt_active_load_balance,
	...
	TP_PROTO(struct task_struct *p, int prev_cpu, int new_cpu, struct walt_task_struct *wts, bool pp_lb),

	TP_ARGS(p, prev_cpu, new_cpu, wts, pp_lb),
	...
	TP_printk("pid=%d misfit=%d prev_cpu=%d new_cpu=%d pp_lb=%d\n",
			__entry->pid, __entry->misfit, __entry->prev_cpu,
			__entry->new_cpu, __entry->pp_lb)
);
...
TRACE_EVENT(walt_newidle_balance,
	...
	TP_PROTO(int this_cpu, int busy_cpu, int pulled, bool enough_idle,
		struct task_struct *p),

	TP_ARGS(this_cpu, busy_cpu, pulled, enough_idle, p),
	...
	TP_printk("cpu=%d busy_cpu=%d pulled=%d nr_running=%u rt_nr_running=%u nr_iowait=%d avg_idle=%llu enough_idle=%d overload=%d pid=%d",
			__entry->cpu, __entry->busy_cpu, __entry->pulled,
			__entry->nr_running, __entry->rt_nr_running,
			__entry->nr_iowait, __entry->avg_idle, __entry->enough_idle,
			__entry->overload, __entry->pid)
);

TRACE_EVENT(walt_lb_rotation,

	TP_PROTO(struct task_struct *src_task, int src_cpu,
			 struct task_struct *dst_task, int dst_cpu),

	TP_ARGS(src_task, src_cpu, dst_task, dst_cpu),
	...
	TP_fast_assign(
		memcpy(__entry->src_comm, src_task->comm, TASK_COMM_LEN);
		__entry->src_pid		= src_task->pid;
		__entry->src_cpu		= src_cpu;
		memcpy(__entry->dst_comm, dst_task->comm, TASK_COMM_LEN);
		__entry->dst_pid		= dst_task->pid;
		__entry->dst_cpu		= dst_cpu;
	),

	TP_printk("src: %d (%s) on cpu %d, dst: %d (%s) on cpu %d",
		__entry->src_pid, __entry->src_comm, __entry->src_cpu,
		__entry->dst_pid, __entry->dst_comm, __entry->dst_cpu)
);

TRACE_EVENT(check_for_rt_migration,

	TP_PROTO(struct task_struct *p, int misfit, int cpu, int new_cpu),

	TP_ARGS(p, misfit, cpu, new_cpu),
	...
	TP_printk("pid=%d(%s) misfit=%d cpu=%d new_cpu=%d",
		__entry->pid, __entry->comm, __entry->misfit,
		__entry->cpu, __entry->new_cpu)
);

TRACE_EVENT(balance_rt,

	TP_PROTO(bool pulled, struct task_struct *p, int this_cpu, int src_cpu,
		s64 delta, int nr_rt),

	TP_ARGS(pulled, p, this_cpu, src_cpu, delta, nr_rt),
	...
	TP_fast_assign(
		__entry->pulled		= pulled;
		memcpy(__entry->comm, pulled ? p->comm : "null", TASK_COMM_LEN);
		__entry->pid		= pulled ? p->pid : -1;
	...
	TP_printk("pulled=%d pid=%d(%s) this_cpu=%d src_cpu=%d delta=%lld nr_rt=%d",
		__entry->pulled, __entry->pid, __entry->comm, __entry->this_cpu,
		__entry->src_cpu, __entry->delta, __entry->nr_rt)
);
...
TRACE_EVENT(sched_cpu_util,
	...
	TP_fast_assign(
		...
		__entry->lowest_mask	= lowest_mask ? cpumask_bits(lowest_mask)[0] : 0;
...
TRACE_EVENT(sched_select_task_rt,
	...
	TP_PROTO(struct task_struct *p, int wake_flags, int fastpath,
		int new_cpu, struct cpumask *lowest_mask),

	TP_ARGS(p, wake_flags, fastpath, new_cpu, lowest_mask),
	...
	TP_printk("pid=%d comm=%s is_rtg=%d wake_flags=%d fastpath=%u best_cpu=%d lowest_mask=0x%lx",
		__entry->pid, __entry->comm, __entry->is_rtg, __entry->wake_flags,
		__entry->fastpath, __entry->new_cpu, __entry->lowest_mask)
);
...
TRACE_EVENT(sched_rt_find_lowest_rq,
	...
	TP_printk("pid=%d comm=%s is_rtg=%d fastpath=%u best_cpu=%d lowest_mask=0x%lx",
		__entry->pid, __entry->comm, __entry->is_rtg, __entry->fastpath,
		__entry->best_cpu, __entry->lowest_mask)
);
...
TRACE_EVENT(sched_task_util,
	...
	TP_printk("pid=%d comm=%s util=%lu prev_cpu=%d candidates=%lx best_energy_cpu=%d sync=%d need_idle=%d fastpath=%d placement_boost=%d latency=%llu stune_boosted=%d is_rtg=%d rtg_skip_min=%d start_cpu=%d unfilter=%u affinity=%lx task_boost=%d iowaited=%d load_boost=%d sync_state=%d",
		__entry->pid, __entry->comm, __entry->util, __entry->prev_cpu,
		...
		__entry->iowaited, __entry->load_boost, __entry->sync_state)
);
...
TRACE_EVENT(sched_enq_deq_task,
	...
	TP_printk("cpu=%d %s comm=%s pid=%d prio=%d nr_running=%u rt_nr_running=%u affine=%x demand=%u pred_demand_scaled=%u misfit=%d",
			__entry->cpu,
			...
			__entry->pred_demand_scaled, __entry->misfit)
);
...
TRACE_EVENT(rollover_cpu_cap,

	TP_PROTO(int cpu, u64 freq, u64 ratio, u64 cycles, u64 cst),

	TP_ARGS(cpu, freq, ratio, cycles, cst),
	...
	TP_printk("cpu=%d freq=%llu ratio=%llu cycles=%llu cst=%llu", __entry->cpu,
		__entry->freq, __entry->ratio, __entry->cycles, __entry->cst)
);
...
TRACE_EVENT(sched_task_handler,
	...
	TP_printk("comm=%s pid=%d param=%d val=%d callers=%lx <- %lx <- %lx <- %lx <- %lx <- %lx",
...
TRACE_EVENT(sched_update_cpu_migrate_values,

	TP_PROTO(int cluster),
	...
	TP_STRUCT__entry(
		__field(int, cluster)
		__field(unsigned int, cluster_val)
	),

	TP_fast_assign(
		__entry->cluster	= cluster;
		__entry->cluster_val	= sched_cpu_capacity_margin[cluster_first_cpu(
						sched_cluster[cluster])];
	),

	TP_printk("cluster=%d cluster_val=%u",
			__entry->cluster,
			__entry->cluster_val)
);
...
TRACE_EVENT(xr_get_cpu_cycle,

	TP_PROTO(int cpu, int evt, u64 val, u64 prev, u64 saved, u64 ret),

	TP_ARGS(cpu, evt, val, prev, saved, ret),
	...
	TP_printk("cpu=%d evt=%d val=%llu prev=%llu saved=%llu ret=%llu",
		  __entry->cpu, __entry->evt, __entry->val,
		  __entry->prev, __entry->saved, __entry->ret)
);

TRACE_EVENT(xr_top_task,

	TP_PROTO(int cpu, struct task_struct *p, int old_index, int new_index, u32 old_curr, u32 curr, u32 prev, u32 hist),

	TP_ARGS(cpu, p, old_index, new_index, old_curr, curr, prev, hist),

	TP_STRUCT__entry(
		__field(int, cpu)
		__array(char, comm, TASK_COMM_LEN)
		__field(pid_t, pid)
		__field(int, old_index)
		__field(int, new_index)
		__field(u32, old_curr)
		__field(u32, curr)
		__field(u32, prev)
		__field(u32, hist)
	),
	...
	TP_fast_assign(
		...
		memcpy(__entry->comm, p->comm, TASK_COMM_LEN);
		__entry->pid		= p->pid;
		__entry->old_index	= old_index;
		__entry->new_index	= new_index;
		__entry->old_curr	= old_curr;
		__entry->curr		= curr;
		__entry->prev		= prev;
		__entry->hist		= hist;
	),

	TP_printk("cpu=%d, pid=%d, comm=%s old_index=%d, index=%d, old_curr=%u, curr=%u, prev=%u, history=%u",
		  __entry->cpu,
		  __entry->pid,
		  __entry->comm,
		  __entry->old_index,
...
/*
 * for top/ed task debug
 * will be cover in governor
 */
TRACE_EVENT(xr_ed_top_task_boost,

	TP_PROTO(struct task_struct *p, int cpu, int type),
	...
	TP_printk("pid=%d, comm=%s, cpu=%d, type=%d", __entry->pid, __entry->comm, __entry->cpu, __entry->type)
);

TRACE_EVENT(xr_ed_wait_task,

	TP_PROTO(struct task_struct *p, int type, u64 runnable_sum, u64 thres,
		 u64 last_wake_ts, u64 last_enqueued_ts),

	TP_ARGS(p, type, runnable_sum, thres, last_wake_ts, last_enqueued_ts),
	...
	TP_STRUCT__entry(
		...
		__field(u32, thres)
		__field(u64, last_wake_ts)
		__field(u64, last_enqueued_ts)
	),
	...
	TP_fast_assign(
		...
		__entry->pid = p->pid;
		__entry->type = type;
		__entry->runnable_sum = runnable_sum;
		__entry->thres = thres;
		__entry->last_wake_ts = last_wake_ts;
		__entry->last_enqueued_ts = last_enqueued_ts;
	),

	TP_printk("pid=%d, comm=%s, type=%d, runnable_sum=%llu, thres=%u, wakets=%llu, eqts=%llu",
		 __entry->pid,
		 __entry->comm,
		 __entry->type,
		 __entry->runnable_sum,
		 __entry->thres,
		 __entry->last_wake_ts,
		 __entry->last_enqueued_ts)
);

TRACE_EVENT(xr_ed_run_task,

	TP_PROTO(struct task_struct *p, int type, u64 wallclock, u64 delta, u64 thres,
		 u64 last_wake_ts, u64 last_enqueued_ts),

	TP_ARGS(p, type, wallclock, delta, thres, last_wake_ts, last_enqueued_ts),
	...
	TP_printk("pid=%d, comm=%s, type=%d, wallclock=%llu, delta= %llu, thres = %u, wakets=%llu, eqts=%llu",
		 __entry->pid,
		 __entry->comm,
		 __entry->type,
		 __entry->wallclock,
