...
static bool g_read_b_buf;

static int log_read_line_internal(struct mitee_memlog_state *s, uint64_t put, uint64_t get,
					uint32_t rb_start, uint32_t rb_size)
{
	...
	max_to_read = min((size_t)(put - get), sizeof(s->line_buffer) - 1);
	offset = get % rb_size;
	end = (get + max_to_read) % rb_size;

	if (offset < end) {
		for (i = 0; i < max_to_read && c != '\n'; i++)
			s->line_buffer[i] = c = log->data[rb_start + offset + i];
	} else {
		for (i = 0; i < (rb_size - offset) && c != '\n'; i++)
			s->line_buffer[i] = c = log->data[rb_start + offset + i];
		if (c == '\n')
			goto done;
		for (j = 0; j < end && c != '\n'; j++)
			s->line_buffer[i + j] = c = log->data[rb_start + j];
	}
...
static int log_read_line(struct mitee_memlog_state *s, uint64_t put, uint64_t get)
{
	struct log_rb *log = s->log;

	return log_read_line_internal(s, put, get, 0, log->sz);
}

static int log_read_b_line(struct mitee_memlog_state *s, uint64_t put, uint64_t get)
{
	struct log_rb *log = s->log;

	return log_read_line_internal(s, put, get, log->sz, log->b_sz);
}

#ifdef ENABLE_DUMP_MITEE_LOG
static void mitee_dump_logs(struct mitee_memlog_state *s)
{
	struct log_rb *log = s->log;
	uint64_t get, put, alloc;
	int read_chars;
...
static int mitee_memlog_notify(struct notifier_block *nb,
				     unsigned long action, void *data)
{
	struct mitee_memlog_state *s;
				...
				size_t size)
{
	...
	if (!g_read_b_buf) {
		...
		/* check if the value of alloc(and put) is overflow, typically get value
		 * is a huge number, while alloc value is very small
		 */
		if (get > alloc) {
			pr_notice("mitee: bootup log overflow, alloc = %llx, get = %llx",
				alloc, get);
			get = 0;
		}

		if (alloc - get > log->b_sz) {
			pr_notice("mitee: bootup log overflow, lose some msg.");
			get = alloc - log->b_sz;
		}
		 ...
		 */
		if (get > alloc) {
			pr_notice("mitee: runtime log overflow, alloc = %llx, get = %llx",
				alloc, get);
			get = 0;
		}

		if (alloc - get > log->sz) {
			pr_notice("mitee: runtime log overflow, lose some msg.");
			get = alloc - log->sz;
		}
...
int mitee_memlog_probe(struct platform_device *pdev)
{
	...
	size_t size;
	void *va;
	union memlog_args args;
	int rc = 0;

	args.in.cmd = OPTEE_FFA_GET_MITEE_LOG_BUFFER;

	dev_dbg(&pdev->dev, "%s cmd = %llx\n", __func__, args.in.cmd);
	...
	s->panic_notifier.notifier_call = mitee_memlog_notify;
	result = atomic_notifier_chain_register(&panic_notifier_list,
						&s->panic_notifier);
	if (result < 0) {
		dev_err(&pdev->dev,
			"failed to register mitee notifier\n");
