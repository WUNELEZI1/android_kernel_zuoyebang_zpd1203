...
static const uuid_t tee_client_uuid_ns =
	UUID_INIT(0x58ac9ca0, 0x2086, 0x4683, 0xa1, 0xb8, 0xec, 0x4b, 0xc0,
		  0x8e, 0x01, 0xb6);
...
{
	struct tee_context *ctx =
		container_of(ref, struct tee_context, refcount);
	ctx->releasing = true;
		   ...
		   size_t size)
{
	...
	if (rc < 0) {
		pr_err("crypto_shash_init is failed\n");
		goto out_free_desc;
	}

	rc = crypto_shash_update(desc, (const u8 *)ns, sizeof(*ns));
	if (rc < 0) {
		pr_err("crypto_shash_update is failed\n");
		goto out_free_desc;
	}
	...
	if (rc < 0) {
		pr_err("crypto_shash_final is failed\n");
		goto out_free_desc;
	}
				 ...
				 const u8 connection_data[TEE_IOCTL_UUID_LEN])
{
	...
	default:
		pr_err("connection_method is error\n");
		rc = -EINVAL;
		goto out_free_name;
	}
			       ...
			       struct tee_ioctl_shm_alloc_data __user *udata)
{
	...
	if (copy_from_user(&data, udata, sizeof(data))) {
		pr_err("copy_from_user is failed\n");
		return -EFAULT;
	}

	/* Currently no input flags are supported */
	if (data.flags) {
		pr_err("flag is error\n");
		return -EINVAL;
	}
		       ...
		       struct tee_ioctl_shm_register_data __user *udata)
{
	...
	/* Currently no input flags are supported */
	if (data.flags) {
		pr_err("flags is error\n");
		return -EINVAL;
	}

	if (!access_ok((void __user *)(unsigned long)data.addr, data.length)) {
		pr_err("data is error\n");
		return -EFAULT;
	}
	...
	for (n = 0; n < num_params; n++) {
		...
		case TEE_IOCTL_PARAM_ATTR_TYPE_MEMREF_INOUT:
			...
			if (ip.c != TEE_MEMREF_NULL) {
				 ...
				 * resolved pointers.
				 */
				if (ip.c > INT_MAX_VALUE) {
					pr_err("ip.c is error\n");
					return -EINVAL;
				}
...
static int auth_ca_name(void)
{
	...
	ca_path = d_path(&(current->active_mm->exe_file->f_path), tpath, MAX_TPATH_LEN);
				  ...
				  struct tee_ioctl_buf_data __user *ubuf)
{
	...
	if (arg.num_params) {
		params = kcalloc(arg.num_params, sizeof(struct tee_param),
				 GFP_KERNEL);
		if (!params) {
			pr_err("params alloc is failed\n");
			return -ENOMEM;
		}
		uparams = uarg->params;
		rc = params_from_user(ctx, params, arg.num_params, uparams);
		if (rc) {
			pr_err("params_from_user is failed\n");
			goto out;
		}
	}
			    ...
			    struct tee_ioctl_buf_data __user *ubuf)
{
	...
	mitee_bench_timestamp(arg.session, "tee_ioctl_invoke enter");
...
out:
	...
	mitee_bench_timestamp(arg.session, "tee_ioctl_invoke exit");
	return rc;
}
			...
			struct tee_ioctl_close_session_arg __user *uarg)
{
	...
	if (!ctx->teedev->desc->ops->close_session) {
		pr_err("close_session is failed\n");
		return -EINVAL;
	}
			    ...
			    struct tee_ioctl_param __user *uparams)
{
	...
	for (n = 0; n < num_params; n++) {
		...
		/* All unused attribute bits has to be zero */
		if (ip.attr & ~TEE_IOCTL_PARAM_ATTR_MASK) {
			pr_err("ip.attr is failed 0x%llx n=%zu num=%zu\n", ip.attr, n, num_params);
			return -EINVAL;
		}
...
struct match_dev_data {
	struct tee_ioctl_version_data *vers;
	const void *data;
	int (*match)(struct tee_ioctl_version_data *para1, const void *para2);
};
...
int mitee_tee_init(void)
{
	...
	pr_info("mitee tee init success\n");
