...
struct xrp_hw_ops {
	 ...
	 * \param phdr: program header of a segment to be loaded
	 */
	long (*free_fw_segment)(void *hw_arg,
				const void *image,
				Elf32_Phdr *phdr);
...
int xrp_set_domain(struct device *dev, void *domain);
void *xrp_get_sysram_vaddr(void);
void xrp_dma_buf_end_cpu_access(void);
void xrp_dma_buf_begin_cpu_access(void);
uint32_t xrp_get_vdsp_case_num_tmp(void);
