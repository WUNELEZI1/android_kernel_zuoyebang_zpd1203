...
int getFWdata(const char *pathToFile, u8 **data, int *size)
{
	...
	logError(0, "%s getFWdata starting ...\n", tag);
...
#endif
	default:
		logError(0, "%s Read FW from BIN file %s !\n", tag, path);
		dev = getDev();
	...
	logError(0, "%s getFWdata Finished!\n", tag);
	return OK;
...
int parseBinFile(u8 *fw_data, int fw_size, Firmware *fwData, int keep_cx)
{
	...
	if (fw_size < FW_HEADER_SIZE + FW_BYTES_ALLIGN || fw_data == NULL) {
		...
		index += FW_BYTES_ALLIGN;
		u8ToU32(&fw_data[index], &temp);
		logError(0, "%s parseBinFile: FILE SVN REV = %08X\n", tag,
			 temp);

		index += FW_BYTES_ALLIGN;
		u8ToU32(&fw_data[index], &temp);
		fwData->fw_ver = temp;
		logError(0, "%s parseBinFile: FILE Fw Version = %04X\n", tag,
			 fwData->fw_ver);

		index += FW_BYTES_ALLIGN;
		u8ToU32(&fw_data[index], &temp);
		fwData->config_id = temp;
		logError(0, "%s parseBinFile: FILE Config Project ID = %08X\n",
			 tag, temp);

		index += FW_BYTES_ALLIGN;
		u8ToU32(&fw_data[index], &temp);
		logError(0, "%s parseBinFile: FILE Config Version = %08X\n",
			 tag, temp);

		index += FW_BYTES_ALLIGN * 2;

		index += FW_BYTES_ALLIGN;
		logError(0, "%s parseBinFile: File External Release =  ", tag);
		...
		logError(0, "%s parseBinFile:  sec0_size = %08X (%d bytes)\n",
			 tag, fwData->sec0_size, fwData->sec0_size);

		index += FW_BYTES_ALLIGN;
		u8ToU32(&fw_data[index], &temp);
		fwData->sec1_size = temp;
		logError(0, "%s parseBinFile:  sec1_size = %08X (%d bytes)\n",
			 tag, fwData->sec1_size, fwData->sec1_size);

		index += FW_BYTES_ALLIGN;
		u8ToU32(&fw_data[index], &temp);
		fwData->sec2_size = temp;
		logError(0, "%s parseBinFile:  sec2_size = %08X (%d bytes) \n",
			 tag, fwData->sec2_size, fwData->sec2_size);

		index += FW_BYTES_ALLIGN;
		u8ToU32(&fw_data[index], &temp);
		fwData->sec3_size = temp;
		logError(0, "%s parseBinFile:  sec3_size = %08X (%d bytes) \n",
		...
		logError(0, "%s parseBinFile: CX Version = %04X \n", tag,
			 fwData->cx_ver);
