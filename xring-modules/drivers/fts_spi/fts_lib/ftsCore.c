...
static bool is_irq_enabled = true;
...
int fts_disableInterrupt(void)
{
	...
	if (getClient() != NULL) {
		if (fts_info && fts_info->probe_ok) {
			desc = irq_to_desc(getClient()->irq);
			logError(0, "%s %s depth:%d\n", tag, __func__, desc->depth);
		}
		spin_lock_irq(&fts_int);
		logError(0, "%s disable irq, is_irq_enabled = %d \n", tag, is_irq_enabled);
		if (!is_irq_enabled) {
...
int fts_disableInterruptNoSync(void)
{
	...
	if (getClient() != NULL) {
		...
		spin_lock_irq(&fts_int);
		logError(0, "%s disable irq no sync, is_irq_enabled = %d \n", tag, is_irq_enabled);
		if (!is_irq_enabled) {
			spin_unlock_irq(&fts_int);
			return OK;
		}
...
int fts_enableInterrupt(void)
{
	...
	if (getClient() != NULL) {
		if (fts_info && fts_info->probe_ok) {
			desc = irq_to_desc(getClient()->irq);
			logError(1, "%s %s depth:%d\n", tag, __func__, desc->depth);
		}
		spin_lock_irq(&fts_int);
		logError(0, "%s enable irq, is_irq_enabled = %d \n", tag, is_irq_enabled);
		if (is_irq_enabled) {
			spin_unlock_irq(&fts_int);
			return OK;
		}
		...
		if (fts_info && fts_info->probe_ok) {
			while (desc->depth > 1) {
				logError(1, "%s %s depth unbalance, enable irq again\n", tag, __func__);
...
int fts_crc_check(void)
{
	...
	if (res >= OK) {
		...
		if (res < OK) {
			logError(0, "%s %s: No Config CRC Error Found! \n", tag,
				 __func__);
...
int writeLockDownInfo(u8 *data, int size, u8 lock_id)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret < OK) {
			...
			logError(0, "%s Echo FOUND... OK!\n", tag, ret);
			ret = pollForErrorType(error_to_search, 4);
			if (ret < OK) {
				logError(0, "%s %s: No Error Found! \n", tag,
					 __func__);
				ret = OK;
			} else {
				logError(1,
	...
	if (ret < OK)
		...
		logError(0, "%s %s end, write lockdown success\n", tag,
			 __func__, ret);
...
int readLockDownInfo(u8 *lockData, u8 lock_id, int size)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ret = fts_write_dma_safe(cmd_lockdown, 3);
		mdelay(10);
