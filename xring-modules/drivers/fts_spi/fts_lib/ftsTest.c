...
int production_test_ms_strength(const char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	memset(&msStrengthFrame, 0x00, sizeof(msStrengthFrame));
	/************** Mutual Sense Test *************/
	logError(0, "%s\n", tag);
	logError(0, "%s MS STRENGTH DATA TEST is starting...\n", tag);
	if (todo->MutualStrength == 1) {
		ret = setScanMode(SCAN_MODE_LOCKED, LOCKED_ACTIVE);
		msleep(300);
		...
		ret |= setScanMode(SCAN_MODE_ACTIVE, 0x00);
		msleep(300);
		logError(0, "%s MS STRENGTH MIN MAX TEST:\n", tag);
		...
		if (ret < OK || (trows != 1 || tcolumns != 2)) {
			logError(1,
				"%s production_test_data: parseProductionTestLimits MS_STRENGTH_MIN_MAX failed... ERROR %08X\n",
				tag, ERROR_PROD_TEST_DATA);
			ret |= ERROR_PROD_TEST_DATA;
			goto ERROR_LIMITS;
		}
		...
		if (ret != OK) {
			logError(1,
					"%s production_test_data: checkLimitsMinMax MS STRENGTH failed... ERROR COUNT = %d\n",
					tag, ret);
			logError(0,
			...
			logError(0,
					"%s MS STRENGTH MIN MAX TEST:.................OK\n",
					tag);
		kfree(thresholds);
		thresholds = NULL;

	} else
		logError(0, "%s MS STRENGTH DATA TEST:.................SKIPPED\n", tag);

ERROR:
	logError(0, "%s\n", tag);
	if (count_fail == 0) {
		print_frame_short("MS Strength frame =", array1dTo2d_short(
					  msStrengthFrame.node_data,
					  msStrengthFrame.node_data_size,
					  msStrengthFrame.header.sense_node),
				  msStrengthFrame.header.force_node,
				  msStrengthFrame.header.sense_node);
		...
		if (thresholds != NULL)
			kfree(thresholds);
		logError(0,
			 "%s MS STRENGTH DATA TEST:.................FAIL fails_count = %d\n\n",
			 tag, count_fail);

		logError(0,
			 "%s MS STRENGTH DATA TEST finished!.................OK\n",
			 tag);
		return OK;
	} else {
		print_frame_short("MS Strength frame =", array1dTo2d_short(
...
int production_test_ss_hover_raw(const char *path_limits, int stop_on_fail, TestToDo *todo)
{
	int ret;
	int rows, columns;
	int *thresholds = NULL;
	...
	memset(&ssHoverRawFrame, 0x00, sizeof(ssHoverRawFrame));
...
int production_test_data(const char *path_limits, int stop_on_fail, TestToDo *todo)
{
	...
	ret = production_test_ms_strength(path_limits, stop_on_fail, todo);
	res |= ret;

	if (ret < 0) {
		logError(1,
			 "%s production_test_data: production_test_ms_strength failed... ERROR = %08X\n",
			 tag, ret);
		if (stop_on_fail == 1)
			goto END;
	}
...
int getLimitsFile(const char *path, LimitFile *file)
{
	...
	if (strncmp(path, "NULL", 4) == 0) {
		...
		file->data =
			(char *)vmalloc((file->size) * sizeof(char));
