...
#define CMD_MSSTRENGTHTEST									0x58
				 ...
				 size_t count, loff_t *pos)
{
	...
	if (*pos == 0) {
		if (driver_test_buff == NULL) {
			...
			cnt = copy_to_user(buf, driver_test_buff, cnt);
			return 0;
		}
		chunk = CHUNK_PROC;
	}
				     ...
				     size_t count, loff_t *pos)
{
	...
	if (numberParam >= 1) {
		...
		case CMD_MSSTRENGTHTEST:
			if (numberParam == 2)
				res = production_test_ms_strength(LIMITS_FILE,
							     cmd[1], &tests);
			else {
				logError(1, "%s Wrong number of parameters!\n",
					 tag);
...
int fts_proc_init(void)
{
	...
	if (entry) {
		logError(0, "%s %s: proc entry CREATED! \n", tag, __func__);
	} else {
