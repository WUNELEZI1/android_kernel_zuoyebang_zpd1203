...
static int __tmc_etf_enable_hw(struct tmc_drvdata *drvdata)
{
	...
	CS_UNLOCK(drvdata->base);
	CS_UNLOCK(drvdata->tpiu_etf_base);
	...
	writel_relaxed(TMC_MODE_HARDWARE_FIFO, drvdata->tpiu_etf_base + TMC_MODE);


	writel_relaxed(TMC_FFCR_EN_FMT | TMC_FFCR_EN_TI,
		       drvdata->base + TMC_FFCR);
	writel_relaxed(TMC_FFCR_EN_FMT | TMC_FFCR_EN_TI,
		       drvdata->tpiu_etf_base + TMC_FFCR);

	writel_relaxed(0x0, drvdata->base + TMC_RRP);
	writel_relaxed(0x0, drvdata->tpiu_etf_base + TMC_RRP);
	writel_relaxed(0x0, drvdata->base + TMC_RWD);
	writel_relaxed(0x0, drvdata->tpiu_etf_base + TMC_RWD);

	tmc_enable_hw(drvdata);
	writel_relaxed(TMC_CTL_CAPT_EN, drvdata->tpiu_etf_base + TMC_CTL);

	CS_LOCK(drvdata->base);
	CS_LOCK(drvdata->tpiu_etf_base);
