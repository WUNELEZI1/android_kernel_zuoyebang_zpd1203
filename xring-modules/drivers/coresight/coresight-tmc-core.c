...
/*
 * set FFCR.StopOnFlush FFCR.FlushMan
 */
void tmc_flush_and_stop(struct tmc_drvdata *drvdata)
{
	...
	ffcr |= TMC_FFCR_STOP_ON_FLUSH;              // 由flush停止
	writel_relaxed(ffcr, drvdata->base + TMC_FFCR);
	ffcr |= BIT(TMC_FFCR_FLUSHMAN_BIT);          // 生成flush
	writel_relaxed(ffcr, drvdata->base + TMC_FFCR);
...
static struct attribute *coresight_tmc_mgmt_attrs[] = {
	coresight_simple_reg32(rsz, TMC_RSZ),                   // --> size of trace memory
	...
	coresight_simple_reg64(dba, TMC_DBALO, TMC_DBAHI),    //  --> DBAHI<<32 + DBALO
...
/*
 *	Whether the device tree contains child nodes: "arm,scatter-gather"
 */
...
/*
 * function:
 *	Gets the buffer-size from the device tree, or 1M if not set
 */
...
/*
 * function:
 *	Gets the max-burst-size from the device tree or 0xF if it is not set
 */
...
static int mainbus_etf_get_tpiuetf_area(struct device *dev, struct resource *res)
{
...
static int get_tpiu_etf_area(struct device *dev, struct resource *res)
{
	struct fwnode_handle *fwnode = dev_fwnode(dev);

	if (is_of_node(fwnode))
		return mainbus_etf_get_tpiuetf_area(dev, res);
...
static int tmc_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	struct tmc_drvdata *drvdata;    // specifics associated to an TMC component
	...
	case TMC_CONFIG_TYPE_ETR:
		...
		desc.ops = &tmc_etr_cs_ops;      // etr-->tmc_etr_sink_ops
		ret = tmc_etr_setup_caps(dev, devid,
					 coresight_get_uci_data(id));
		if (ret)
			goto out;
		idr_init(&drvdata->idr);                      // 初始化idr_init
	...
	case TMC_CONFIG_TYPE_ETF:
		...
		dev_list = &etf_devs;
		ret = get_tpiu_etf_area(dev, &ch_res);
		if (ret) {
			pr_err("[%s]: get_tpiu_etf_area fail,ret=%d", __func__, ret);
			goto out;
		}
...
/*
 *	The tmc-devices are registered as amba devices
 */
