...
function create_modules_order_lists() {
  ...
  for mod_order_file in ${!module_lists_arr[@]}; do
    ...
    if [[ -n "${mod_list_file}" ]]; then
      ...
      rm -f ${dest_file}
      if [[ ${mod_list_file} == *vendor_boot* ]]; then
        ko_list=$(cat ${modules_list_filter})

        for ko_file in ${ko_list[@]}; do
          grep -w ${ko_file} ${tmp_modules_order_file} >> ${dest_file}
        done
      else
...
function build_boot_images() {
  ...
  echo "xring's build_utils.sh"
  DTB_FILE_LIST=$(find ${DIST_DIR} -name O[[:digit:]]_basic.dtb)
  ...
  echo "VENDOR_FSTAB:${VENDOR_FSTAB}"
  if [ -f "${VENDOR_FSTAB}" ]; then
    ...
    echo "ERROR: VENDOR_FSTAB don't exist."
    exit 1
  fi
